...
#define If_LibBoxForEachBox(p, pBox, i)                                     \
    Vec_PtrForEachEntry(If_Box_t*, p->vBoxes, pBox, i) if (pBox == NULL) {} \
    else
...
If_Box_t* If_LibBoxFindBox(If_LibBox_t* p, char* pName) {
    ...
    if (p == NULL)
        return NULL;
    If_LibBoxForEachBox(p, pBox, i) if (!strcmp(pBox->pName, pName)) return pBox;
    return NULL;
}
            ...
            if (!strcmp(pToken, ".box")) {
                ...
                while (pToken) {
                    if (!strcmp(pToken, "seq"))
                        ...
                        assert(!strcmp(pToken, "comb") || !strcmp(pToken, "white") || !strcmp(pToken, "inner"));
                    pToken = strtok(NULL, " \n\r\t");
                }
                // create library
                if (p == NULL)
