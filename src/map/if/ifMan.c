...
static void If_ManCutSetRecycle(If_Man_t* p, If_Set_t* pSet) {
    pSet->pNext = p->pFreeList;
    p->pFreeList = pSet;
}
static If_Set_t* If_ManCutSetFetch(If_Man_t* p) {
    If_Set_t* pTemp = p->pFreeList;
    p->pFreeList = p->pFreeList->pNext;
    return pTemp;
}
...
If_Obj_t* If_ManCreateCo(If_Man_t* p, If_Obj_t* pDriver) {
    ...
    pObj->Type = IF_CO;
    pObj->fCompl0 = If_IsComplement(pDriver);
    pDriver = If_Regular(pDriver);
...
If_Obj_t* If_ManCreateAnd(If_Man_t* p, If_Obj_t* pFan0, If_Obj_t* pFan1) {
    ...
    pObj = If_ManSetupObj(p);
    pObj->Type = IF_AND;
    pObj->fCompl0 = If_IsComplement(pFan0);
    pFan0 = If_Regular(pFan0);
    pObj->fCompl1 = If_IsComplement(pFan1);
    pFan1 = If_Regular(pFan1);
    pObj->pFanin0 = pFan0;
    pFan0->nRefs++;
    pFan0->nVisits++;
    pFan0->nVisitsCopy++;
