...
int Ifn_ManStrCheck(char* pStr, int* pnInps, int* pnObjs) {
    ...
    for (i = 0; pStr[i]; i++) {
        if (Inf_ManOpenSymb(pStr + i))
            nNodes++;
        if (pStr[i] == ';' || pStr[i] == '(' || pStr[i] == ')' || pStr[i] == '[' || pStr[i] == ']' || pStr[i] == '<' || pStr[i] == '>' || pStr[i] == '{' || pStr[i] == '}')
            continue;
...
int Ifn_ManStrCheck2(char* pStr, int* pnInps, int* pnObjs) {
    ...
    for (i = 0; pStr[i]; i++) {
        if (pStr[i] == '=' || pStr[i] == ';' || pStr[i] == '(' || pStr[i] == ')' || pStr[i] == '[' || pStr[i] == ']' || pStr[i] == '<' || pStr[i] == '>' || pStr[i] == '{' || pStr[i] == '}')
            continue;
        if (pStr[i] >= 'A' && pStr[i] <= 'Z')
            continue;
        if (pStr[i] >= 'a' && pStr[i] <= 'z') {
...
Gia_Man_t* Ifn_ManStrFindCofactors(int nIns, Gia_Man_t* p) {
    ...
    Gia_ManForEachCi(p, pObj, i) if (i >= nIns)
        pObj->Value
...
int Ifn_ManSatCheckOne(sat_solver* pSat, Vec_Int_t* vPoVars, word* pTruth, int nVars, int* pPerm, int nInps, Vec_Int_t* vLits) {
    ...
    // find assumptions
    v = 0;
    Vec_IntForEachEntry(vLits, Value, m) if (Value >= 0)
