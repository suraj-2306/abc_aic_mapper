...
void If_CluHashTableCheck(If_Man_t* p) {
    ...
    for (i = 0; i < p->nTableSize[t]; i++) {
        for (pEntry = ((If_Hte_t**)p->pHashTable[t])[i]; pEntry; pEntry = pEntry->pNext) {
            ...
            if (RetValue != Status) {
                Kit_DsdPrintFromTruth((unsigned*)pEntry->pTruth, 13);
                printf("\n");
                RetValue = If_CutPerformCheck16(NULL, (unsigned*)pEntry->pTruth, 13, If_CluSupportSize(pEntry->pTruth, 13), "555");
...
void If_CluSemiCanonicizeVerify(word* pTruth, word* pTruth0, int nVars, int* pCanonPerm, unsigned uCanonPhase) {
    ...
    if (!If_CluEqual(pTruth0, pTruth, nVars)) {
        Kit_DsdPrintFromTruth((unsigned*)pTruth0, nVars);
        printf("\n");
        Kit_DsdPrintFromTruth((unsigned*)pGunc, nVars);
        printf("\n");
        Kit_DsdPrintFromTruth((unsigned*)pTruth, nVars);
        printf("\n");
...
void If_CluVerify(word* pF, int nVars, If_Grp_t* g, If_Grp_t* r, word BStruth, word* pFStruth) {
    ...
    if (!If_CluEqual(pTTRes, pF, nVars)) {
        ...
        Kit_DsdPrintFromTruth((unsigned*)pTTRes, nVars);
        printf("\n");
...
void If_CluVerify3(word* pF, int nVars, If_Grp_t* g, If_Grp_t* g2, If_Grp_t* r, word BStruth, word BStruth2, word FStruth) {
    ...
    if (!If_CluEqual(pTTRes, pF, nVars)) {
        ...
        Kit_DsdPrintFromTruth((unsigned*)&BStruth, g->nVars);
        printf("    ");
        If_CluPrintGroup(g); // printf( "\n" );
        Kit_DsdPrintFromTruth((unsigned*)&BStruth2, g2->nVars);
        printf("    ");
        If_CluPrintGroup(g2); // printf( "\n" );
        Kit_DsdPrintFromTruth((unsigned*)&FStruth, r->nVars);
        printf("    ");
        If_CluPrintGroup(r); // printf( "\n" );

        Kit_DsdPrintFromTruth((unsigned*)pTTWire, nVars);
        printf("\n");
        Kit_DsdPrintFromTruth((unsigned*)pTTWire2, nVars);
        printf("\n");
        Kit_DsdPrintFromTruth((unsigned*)pTTRes, nVars);
        printf("\n");
        Kit_DsdPrintFromTruth((unsigned*)pF, nVars);
...
*/
    ...
    // try second decomposition
    {
        int Test = 0;
        if (Test) {
            Kit_DsdPrintFromTruth((unsigned*)&pLeftOver, R2.nVars);
            printf("\n");
        }
    }
...
float If_CutDelayLutStruct(If_Man_t* p, If_Cut_t* pCut, char* pStr, float WireDelay) {
    ...
    assert(G1.nVars + G2.nVars + G3.nVars == nLeaves + (G1.nVars > 0) + (G2.nVars > 0) + (G1.nMyu > 2) + (G2.nMyu > 2));
...
*/
    ...
    Kit_DsdPrintFromTruth((unsigned*)&t, nVars);
    printf("\n");
