...
static inline void If_DsdObjSetTruth(If_DsdMan_t* p, If_DsdObj_t* pObj, int Id) {
    assert(pObj->Type == IF_DSD_PRIME && pObj->nFans > 2);
    Vec_IntWriteEntry(&p->vTruths, pObj->Id, Id);
}
...
static inline int If_DsdObjFaninC(If_DsdObj_t* pObj, int i) {
    assert(i < (int)pObj->nFans);
    return Abc_LitIsCompl(pObj->pFans[i]);
}
static inline int If_DsdObjFaninLit(If_DsdObj_t* pObj, int i) {
    assert(i < (int)pObj->nFans);
    return pObj->pFans[i];
}
...
static inline void If_DsdVecObjIncRef(Vec_Ptr_t* p, int iObj) {
    if (If_DsdVecObjRef(p, iObj) < 0x3FFFF) If_DsdVecObj(p, iObj)->Count++;
}
static inline If_DsdObj_t* If_DsdObjFanin(Vec_Ptr_t* p, If_DsdObj_t* pObj, int i) {
    assert(i < (int)pObj->nFans);
    return If_DsdVecObj(p, Abc_Lit2Var(pObj->pFans[i]));
}
...
int If_DsdManHasMarks(If_DsdMan_t* p) {
    If_DsdObj_t* pObj;
    int i;
    If_DsdVecForEachObj(&p->vObjs, pObj, i) if (pObj->fMark) return 1;
    return 0;
}
...
int If_DsdManCheckNonDec_rec(If_DsdMan_t* p, int Id) {
    ...
    if (If_DsdObjType(pObj) == IF_DSD_PRIME)
        return 1;
    If_DsdObjForEachFaninLit(&p->vObjs, pObj, iFanin, i) if (If_DsdManCheckNonDec_rec(p, Abc_Lit2Var(iFanin))) return 1;
    return 0;
}
...
unsigned* If_DsdObjHashLookup(If_DsdMan_t* p, int Type, int* pLits, int nLits, int truthId) {
    ...
    for (; *pSpot; pSpot = (unsigned*)Vec_IntEntryP(&p->vNexts, pObj->Id)) {
        pObj = If_DsdVecObj(&p->vObjs, *pSpot);
        if (If_DsdObjType(pObj) == Type && If_DsdObjFaninNum(pObj) == nLits && !memcmp(pObj->pFans, pLits, sizeof(int) * If_DsdObjFaninNum(pObj)) && truthId == If_DsdObjTruthId(p, pObj)) {
            p->nUniqueHits++;
            return pSpot;
        }
    }
...
void If_DsdManMerge(If_DsdMan_t* p, If_DsdMan_t* pNew) {
    ...
    If_DsdVecForEachNode(&pNew->vObjs, pObj, i) {
        If_DsdObjForEachFaninLit(&pNew->vObjs, pObj, iFanin, k)
            pFanins[k]
            = Abc_Lit2LitV(Vec_IntArray(vMap), iFanin);
...
If_DsdMan_t* If_DsdManFilter(If_DsdMan_t* p, int Limit) {
    ...
    If_DsdVecForEachNode(&p->vObjs, pObj, i) if ((int)pObj->Count >= Limit)
...
int If_DsdManCheckInv_rec(If_DsdMan_t* p, int iLit) {
    ...
    if (If_DsdObjType(pObj) == IF_DSD_XOR) {
        If_DsdObjForEachFaninLit(&p->vObjs, pObj, iFanin, i) if (If_DsdManCheckInv_rec(p, iFanin)) return 1;
        return 0;
    }
...
unsigned If_DsdSign_rec(If_DsdMan_t* p, If_DsdObj_t* pObj, int* pnSupp) {
    ...
    If_DsdObjForEachFanin(&p->vObjs, pObj, pFanin, i)
        uSign
        |= If_DsdSign_rec(p, pFanin, pnSupp);
    return uSign;
}
...
unsigned If_DsdManCheckAndXor(If_DsdMan_t* p, int iFirst, unsigned uMaskNot, If_DsdObj_t* pObj, int nSuppAll, int LutSize, int fDerive, int fVerbose) {
    ...
    for (i[0] = 0; i[0] < nFans; i[0]++)
        for (i[1] = i[0] + 1; i[1] < nFans; i[1]++) {
            ...
            uRes = If_DsdSign(p, pObj, i[0], iFirst + pFirsts[i[0]], 0) | If_DsdSign(p, pObj, i[1], iFirst + pFirsts[i[1]], 0);
            if (uRes & uMaskNot)
                continue;
            return uRes;
        }
        ...
        for (i[1] = i[0] + 1; i[1] < nFans; i[1]++)
            for (i[2] = i[1] + 1; i[2] < nFans; i[2]++) {
                ...
                if (!fDerive)
                    return ~0;
                If_DsdManComputeFirst(p, pObj, pFirsts);
                uRes = If_DsdSign(p, pObj, i[0], iFirst + pFirsts[i[0]], 0) | If_DsdSign(p, pObj, i[1], iFirst + pFirsts[i[1]], 0) | If_DsdSign(p, pObj, i[2], iFirst + pFirsts[i[2]], 0);
                if (uRes & uMaskNot)
                    continue;
                return uRes;
            }
            ...
            for (i[2] = i[1] + 1; i[2] < nFans; i[2]++)
                for (i[3] = i[2] + 1; i[3] < nFans; i[3]++) {
                    ...
                    if (!fDerive)
                        return ~0;
                    If_DsdManComputeFirst(p, pObj, pFirsts);
                    uRes = If_DsdSign(p, pObj, i[0], iFirst + pFirsts[i[0]], 0) | If_DsdSign(p, pObj, i[1], iFirst + pFirsts[i[1]], 0) | If_DsdSign(p, pObj, i[2], iFirst + pFirsts[i[2]], 0) | If_DsdSign(p, pObj, i[3], iFirst + pFirsts[i[3]], 0);
                    if (uRes & uMaskNot)
                        continue;
                    return uRes;
                }
...
unsigned If_DsdManCheckXY_int(If_DsdMan_t* p, int iDsd, int LutSize, int fDerive, unsigned uMaskNot, int fVerbose) {
    ...
    If_DsdVecForEachObjVec(p->vTemp1, &p->vObjs, pTemp, i) if (If_DsdObjSuppSize(pTemp) <= LutSize && If_DsdObjSuppSize(pObj) - If_DsdObjSuppSize(pTemp) <= LutSize - 1) {
        if (fVerbose)
    ...
    If_DsdVecForEachObjVec(p->vTemp1, &p->vObjs, pTemp, i) if ((If_DsdObjType(pTemp) == IF_DSD_AND || If_DsdObjType(pTemp) == IF_DSD_XOR) && If_DsdObjFaninNum(pTemp) > 2 && If_DsdObjSuppSize(pTemp) > LutSize) {
    ...
    If_DsdVecForEachObjVec(p->vTemp1, &p->vObjs, pTemp, i) if (If_DsdObjType(pTemp) == IF_DSD_MUX && If_DsdObjSuppSize(pTemp) > LutSize) {
    ...
    If_DsdVecForEachObjVec(p->vTemp1, &p->vObjs, pTemp, i) if (If_DsdObjType(pTemp) == IF_DSD_PRIME && If_DsdObjSuppSize(pTemp) > LutSize) {
...
int If_CutDsdBalancePinDelays_rec(If_DsdMan_t* p, int Id, int* pTimes, word* pRes, int* pnSupp, int nSuppAll, char* pPermLits) {
    ...
    if (If_DsdObjType(pObj) == IF_DSD_MUX) {
        ...
        If_DsdObjForEachFaninLit(&p->vObjs, pObj, iFanin, i)
            Delays[i]
            = If_CutDsdBalancePinDelays_rec(p, Abc_Lit2Var(iFanin), pTimes, pFaninRes + i, pnSupp, nSuppAll, pPermLits);
...
int If_CutDsdBalanceEval(If_Man_t* p, If_Cut_t* pCut, Vec_Int_t* vAig) {
    ...
    if (pCut->nLeaves == 1) // variable
    {
        ...
        if (fUseCofs) {
            ...
            if (nCountMax == 1 && pTimes[iMax] + 2 < Delay && If_DsdManCheckNonTriv(p->pIfDsdMan, Abc_Lit2Var(If_CutDsdLit(p, pCut)), If_CutLeaveNum(pCut), If_CutDsdPermLitMax(pPermLits, If_CutLeaveNum(pCut), iMax))) {
                //                fVerbose = 1;
                Delay = pTimes[iMax] + 2;
            }
        }
...
void Id_DsdManTuneStr(If_DsdMan_t* p, char* pStruct, int nConfls, int nProcs, int fVerbose) {
    ...
    // perform concurrent solving
    {
        ...
        for (i = 0; i < nProcs; i++) {
            ...
            status = pthread_create(WorkerThread + i, NULL, Ifn_WorkerThread, (void*)(ThData + i));
            assert(status == 0);
        }
        // run the threads
