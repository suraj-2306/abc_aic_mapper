...
static inline void If_CutSetup(If_Man_t* p, If_Cut_t* pCut) {
    memset(pCut, 0, (size_t)p->nCutBytes);
...
static inline unsigned If_ObjCutSignCompute(If_Cut_t* p) {
    unsigned s = 0;
    int i;
    for (i = 0; i < If_CutLeaveNum(p); i++)
        s |= If_ObjCutSign(p->pLeaves[i]);
    return s;
}
...
static inline word* If_CutTruthW(If_Man_t* p, If_Cut_t* pCut) {
    assert(pCut->iCutFunc >= 0);
    Abc_TtCopy(p->puTempW, If_CutTruthWR(p, pCut), p->nTruth6Words[pCut->nLeaves], If_CutTruthIsCompl(pCut));
    return p->puTempW;
}
...
#    define If_ManForEachNode(p, pObj, i)                         \
        If_ManForEachObj(p, pObj, i) if (pObj->Type != IF_AND) {} \
        else
...
extern int If_CluCheckExt3(void* p, word* pTruth, int nVars, int nLutLeaf, int nLutLeaf2, int nLutRoot, char* pLut0, char* pLut1, char* pLut2, word* pFunc0, word* pFunc1, word* pFunc2);
/*=== ifDelay.c =============================================================*/
