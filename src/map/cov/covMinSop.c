        ...
        *ppPrev = p->pBubble;
        ...
        if (v00 == 3 && v11 == 3) {
            ...
            if (fCont0) {
                ...
                if (p->nCubes < nCubesOld + 1) { // absorbed - add the second cube
                    Min_SopAddCube(p, pThis);
                } else { // remove this cube, and try another one
...
int Min_SopCheck(Min_Man_t* p) {
    ...
    for (i = 0; i <= p->nVars; i++)
        Min_CoverForEachCube(p->ppStore[i], pThis) if (pThis != p->pBubble && Min_CubeIsContained(pThis, pCube)) {
            Min_CubeRecycle(p, pCube);
            return 1;
        }
    Min_CubeRecycle(p, pCube);
