...
float Cm_FaBuildDepthOptimal(Cm_Obj_t** pNodes, Cm_Par_t* pPars) {
    ...
    int bestDepth = 0;
    int depth = fThreeInputGates ? Cm_Fa3BuildWithMaximumDepth(pNodes, pPars->nConeDepth) : Cm_FaBuildWithMaximumDepth(pNodes, pPars->nConeDepth);
...
float Cm_FaLatestMoInputArrival(Cm_Obj_t** pFa, int depth) {
    ...
    for (int i = 1; i < (1 << depth); i++)
        if (pFa[i] && !pFa[2 * i] && !pFa[2 * i + 1] && pFa[i]->BestCut.Arrival > latestInputArrival)
...
float Cm_Fa3LatestMoInputArrival(Cm_Obj_t** pFa, int depth) {
    ...
    for (int i = 1; i < Cm_Fa3LayerStart(depth); i++)
        if (pFa[i] && !pFa[3 * i - 1] && !pFa[3 * i] && !pFa[3 * i + 1] && pFa[i]->BestCut.Arrival > latestInputArrival)
            latestInputArrival = pFa[i]->BestCut.Arrival;
