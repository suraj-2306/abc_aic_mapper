...
int Mpm_ObjAddCutToStore(Mpm_Man_t* p, Mpm_Cut_t* pCut, int ArrTime) {
    ...
    if (fEnableContainment) {
        ...
        for (k = 0; k <= iPivot; k++) {
            pUnit = p->pCutStore[k];
            if (pUnitNew->pCut.nLeaves >= pUnit->pCut.nLeaves && (pUnitNew->uSign & pUnit->uSign) == pUnit->uSign && Mpm_CutIsContained(p, &pUnitNew->pCut, &pUnit->pCut)) {
#ifdef MIG_RUNTIME
    ...
    if (fEnableContainment) {
        ...
        for (k = last = iPivot + 1; k < p->nCutStore; k++) {
            pUnit = p->pCutStore[k];
            if (pUnitNew->pCut.nLeaves <= pUnit->pCut.nLeaves && (pUnitNew->uSign & pUnit->uSign) == pUnitNew->uSign && Mpm_CutIsContained(p, &pUnit->pCut, &pUnitNew->pCut)) {
...
static inline void Mpm_ObjDerefFaninCuts(Mpm_Man_t* p, Mig_Obj_t* pObj) {
    Mig_Obj_t* pFanin;
    int i;
    Mig_ObjForEachFanin(pObj, pFanin, i) if (Mig_ObjIsNode(pFanin) && Mig_ObjMigRefDec(p, pFanin) == 0)
...
static inline int Mpm_ManFindArrivalMax(Mpm_Man_t* p) {
    ...
    Mig_ManForEachCo(p->pMig, pObj, i)
        ArrMax
        = Abc_MaxInt(ArrMax, pmTimes[Mig_ObjFaninId0(pObj)]);
    return ArrMax;
}
