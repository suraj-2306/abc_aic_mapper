...
static inline Ifd_Obj_t* Ifd_ManObj(Ifd_Man_t* p, int i) {
    assert(i >= 0 && i < p->nObjs);
    return p->pObjs + i;
}
static inline Ifd_Obj_t* Ifd_ManObjFromLit(Ifd_Man_t* p, int iLit) { return Ifd_ManObj(p, Abc_Lit2Var(iLit)); }
static inline int Ifd_ObjId(Ifd_Man_t* p, Ifd_Obj_t* pObj) {
    assert(pObj - p->pObjs >= 0 && pObj - p->pObjs < p->nObjs);
    return pObj - p->pObjs;
}
...
*/
    ...
    Vec_IntForEachEntryStart(p->vSuper, iLit1, i, 1)
        iLit0
        = Abc_Var2Lit(Ifd_ManHashFindOrAdd(p, iLit0, iLit1, -1, Type), 0);
...
int Ifd_ManDsdTest() {
    ...
    Vec_WrdForEachEntry(vTruths, Truth, i) {
        ...
        Runner = 0;
        Vec_IntForEachEntry(vUniques, Uniq, k) if (Runner == Uniq) {
