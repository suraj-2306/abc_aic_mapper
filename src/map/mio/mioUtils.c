...
int Mio_CheckGates(Mio_Library_t* pLib) {
    ...
    Mio_LibraryForEachGate(pLib, pGate)
        Mio_GateForEachPin(pGate, pPin) if (Mio_CheckPins(pPin0, pPin))
            pPin0
        = pPin;
    else return 0;
    return 1;
...
void Mio_WriteLibrary(FILE* pFile, Mio_Library_t* pLib, int fPrintSops, int fShort, int fSelected) {
    ...
    Vec_PtrForEachEntry(Mio_Gate_t*, vGates, pGate, i) {
        ...
        Mio_GateForEachPin(pGate, pPin)
            NameLen
            = Abc_MaxInt(NameLen, strlen(pPin->pName));
    }
...
static inline float Mio_GateDelayAve(Mio_Gate_t* pGate) {
    ...
    Mio_GateForEachPin(pGate, pPin)
        GateDelay
        += (float)(0.5 * pPin->dDelayBlockRise + 0.5 * pPin->dDelayBlockFall);
    if (pGate->nInputs)
...
Mio_Cell2_t* Mio_CollectRootsNew2(Mio_Library_t* pLib, int nInputs, int* pnGates, int fVerbose) {
    ...
    Mio_LibraryForEachGate(pLib, pGate0) if (!(pGate0->nInputs > nInputs || pGate0->pTwin)) // skip large and multi-output
...
void Mio_LibraryWriteProfile(FILE* pFile, Mio_Library_t* pLib) {
    Mio_Gate_t* pGate;
    Mio_LibraryForEachGate(pLib, pGate) if (Mio_GateReadProfile(pGate) > 0)
        fprintf(pFile, "%-24s  %6d\n", Mio_GateReadName(pGate), Mio_GateReadProfile(pGate));
}

int Mio_LibraryHasProfile(Mio_Library_t* pLib) {
    Mio_Gate_t* pGate;
    Mio_LibraryForEachGate(pLib, pGate) if (Mio_GateReadProfile(pGate) > 0) return 1;
    return 0;
}
...
void Mio_LibraryTransferProfile(Mio_Library_t* pLibDst, Mio_Library_t* pLibSrc) {
    ...
    Mio_LibraryForEachGate(pLibSrc, pGateSrc) if (Mio_GateReadProfile(pGateSrc) > 0) {
        // find gate by name
        pGateDst = Mio_LibraryReadGateByName(pLibDst, Mio_GateReadName(pGateSrc), NULL);
        if (pGateDst == NULL) {
            // find gate by function
            Mio_LibraryForEachGate(pLibDst, pGateDst) if (pGateDst->uTruth == pGateSrc->uTruth) break;
            if (pGateDst == NULL) {
...
void Mio_LibraryTransferProfile2(Mio_Library_t* pLibDst, Mio_Library_t* pLibSrc) {
    ...
    Mio_LibraryForEachGate(pLibSrc, pGateSrc) if (Mio_GateReadProfile2(pGateSrc) > 0) {
        // find gate by name
        pGateDst = Mio_LibraryReadGateByName(pLibDst, Mio_GateReadName(pGateSrc), NULL);
        if (pGateDst == NULL) {
            // find gate by function
...
void Mio_LibraryHashGates(Mio_Library_t* pLib) {
    Mio_Gate_t* pGate;
    Mio_LibraryForEachGate(pLib, pGate) if (pGate->pTwin) {
