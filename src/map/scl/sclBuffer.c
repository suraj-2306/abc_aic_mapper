...
void Abc_SclReportDupFanins(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachNode(pNtk, pObj, i)
        Abc_ObjForEachFanin(pObj, pFanin, k)
            Abc_ObjForEachFanin(pObj, pFanin2, k2) if (k != k2 && pFanin == pFanin2)
...
Abc_Ntk_t* Abc_SclUnBufferPerform(Abc_Ntk_t* pNtk, int fVerbose) {
    ...
    Abc_NtkForEachNode(pNtk, pObj, i) if (Abc_SclIsInv(pObj) && !Abc_SclObjIsBufInv(Abc_ObjFanin0(pObj)))
...
int Abc_SclCountMaxPhases(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachNodeCo(pNtk, pObj, i) {
        ...
        Abc_ObjForEachFanin(pObj, pFanin, k)
            Max
            = Abc_MaxInt(Max, Vec_IntEntry(vPhLevel, Abc_ObjId(pFanin)) + Abc_ObjFaninPhase(pObj, k));
...
int Abc_SclCheckNtk(Abc_Ntk_t* p, int fVerbose) {
    ...
    Abc_NtkForEachNode(p, pObj, i) {
        Abc_ObjForEachFanin(pObj, pFanin, k) if (!Abc_NodeIsTravIdCurrent(pFanin))
            printf("obj %d and its fanin %d are not in the topo order\n", Abc_ObjId(pObj), Abc_ObjId(pFanin)),
            fFlag = 0;
        Abc_NodeSetTravIdCurrent(pObj);
...
int Abc_SclComputeReverseLevel(Abc_Obj_t* pObj) {
    ...
    Abc_ObjForEachFanout(pObj, pFanout, i)
        Level
        = Abc_MaxInt(Level, pFanout->Level);
    return Level + 1;
}
Abc_Obj_t* Abc_SclPerformBufferingOne(Abc_Obj_t* pObj, int Degree, int fUseInvs, int fVerbose) {
    ...
    if (Vec_PtrSize(vFanouts) < 2 * Degree) {
        ...
        if (Abc_ObjLevel(pFanThis) == Abc_ObjLevel(pFanLast) && Abc_ObjLevel(pFanPrev) < Abc_ObjLevel(pFanThis)) {
            // find the first one whose level is the same as last
            Vec_PtrForEachEntry(Abc_Obj_t*, vFanouts, pFanout, i) if (Abc_ObjLevel(pFanout) == Abc_ObjLevel(pFanLast)) break;
...
void Abc_BufUpdateGlobal(Buf_Man_t* p) {
    ...
    Abc_NtkForEachCo(p->pNtk, pObj, i)
        p->DelayMax
        = Abc_MaxInt(p->DelayMax, Abc_BufNodeArr(p, Abc_ObjFanin0(pObj)));
}
...
int Abc_BufComputeAverage(Buf_Man_t* p, int iPivot, Vec_Int_t* vOrder) {
    ...
    Abc_NtkForEachObjVec(vOrder, p->pNtk, pFanout, i)
        Average
        += Abc_BufEdgeSlack(p, pObj, pFanout);
...
int Abc_BufCountNonCritical(Buf_Man_t* p, Abc_Obj_t* pObj) {
    ...
    Abc_ObjForEachFanout(pObj, pFanout, i) if (Abc_BufEdgeSlack(p, pObj, pFanout) > 7 * BUF_SCALE / 2)
