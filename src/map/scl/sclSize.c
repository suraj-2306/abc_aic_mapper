...
Abc_Obj_t* Abc_SclFindMostCriticalFanin(SC_Man* p, int* pfRise, Abc_Obj_t* pNode) {
    ...
    *pfRise = 0;
    // find min-slack node
    Abc_ObjForEachFanin(pNode, pFanin, i) if (fMinSlack > Abc_SclObjGetSlack(p, pFanin, p->MaxDelay0)) {
...
void Abc_SclTimeNtkPrint(SC_Man* p, int fShowAll, int fPrintPath) {
    ...
    if (fShowAll) {
        ...
        Abc_NtkForEachNodeReverse(p->pNtk, pObj, i) if (Abc_ObjFaninNum(pObj) > 0)
            nLength
            = Abc_MaxInt(nLength, strlen(Abc_SclObjCell(pObj)->pName));
        // print timing
        Abc_NtkForEachNodeReverse(p->pNtk, pObj, i) if (Abc_ObjFaninNum(pObj) > 0)
    ...
    if (fPrintPath) {
        ...
        Abc_NtkForEachPi(p->pNtk, pTemp, iStart) if (pTemp == pPrev) break;
        Abc_NtkForEachPo(p->pNtk, pTemp, iEnd) if (pTemp == pPivot) break;
...
static inline void Abc_SclTimeIncAddFanouts(SC_Man* p, Abc_Obj_t* pObj) {
    Abc_Obj_t* pFanout;
    int i;
    Abc_ObjForEachFanout(pObj, pFanout, i) if (!pFanout->fMarkC && !Abc_ObjIsLatch(pFanout))
...
void Abc_SclTimeIncCheckLevel(Abc_Ntk_t* pNtk) {
    Abc_Obj_t* pObj;
    int i;
    Abc_NtkForEachObj(pNtk, pObj, i) if ((int)pObj->Level != Abc_ObjLevelNew(pObj))
...
int Abc_SclCheckCommonInputs(Abc_Obj_t* pObj, Abc_Obj_t* pFanin) {
    Abc_Obj_t* pTemp;
    int i;
    Abc_ObjForEachFanin(pObj, pTemp, i) if (Abc_NodeFindFanin(pFanin, pTemp) >= 0) {
...
void Abc_SclPrintFaninPairs(SC_Man* p, Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachNode(pNtk, pObj, i)
        Abc_ObjForEachFanin(pObj, pFanin, k) if (Abc_ObjIsNode(pFanin) && Abc_ObjFanoutNum(pFanin) == 1)
...
int Abc_SclHasBufferFanout(Abc_Obj_t* pObj) {
    Abc_Obj_t* pFanout;
    int i;
    Abc_ObjForEachFanout(pObj, pFanout, i) if (Abc_ObjIsBuffer(pFanout)) return 1;
    return 0;
}
int Abc_SclCountBufferFanoutsInt(Abc_Obj_t* pObj) {
    Abc_Obj_t* pFanout;
    int i, Counter = 0;
    Abc_ObjForEachFanout(pObj, pFanout, i) if (Abc_ObjIsBuffer(pFanout))
        Counter
        += Abc_SclCountBufferFanoutsInt(pFanout);
...
int Abc_SclCountNonBufferFanoutsInt(Abc_Obj_t* pObj) {
    ...
    Abc_ObjForEachFanout(pObj, pFanout, i)
        Counter
        += Abc_SclCountNonBufferFanoutsInt(pFanout);
    return Counter;
}
...
float Abc_SclCountNonBufferDelayInt(SC_Man* p, Abc_Obj_t* pObj) {
    ...
    Abc_ObjForEachFanout(pObj, pFanout, i)
        Delay
        += Abc_SclCountNonBufferDelayInt(p, pFanout);
    return Delay;
}
...
float Abc_SclCountNonBufferLoadInt(SC_Man* p, Abc_Obj_t* pObj) {
    ...
    Abc_ObjForEachFanout(pObj, pFanout, i)
        Load
        += Abc_SclCountNonBufferLoadInt(p, pFanout);
...
void Abc_SclPrintBufferTrees(SC_Man* p, Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachObj(pNtk, pObj, i) {
        if (!Abc_ObjIsBuffer(pObj) && Abc_SclCountBufferFanouts(pObj) > 3) {
            ...
            Abc_ObjForEachFanout(pObj, pFanout, k) if (Abc_ObjIsBuffer(pFanout))
