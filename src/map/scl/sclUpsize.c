...
Vec_Int_t* Abc_SclFindTFO(Abc_Ntk_t* p, Vec_Int_t* vPath) {
    ...
    Abc_NtkForEachObjVec(vPath, p, pObj, i)
        Abc_ObjForEachFanin(pObj, pFanin, k) if (Abc_ObjIsNode(pFanin))
...
Vec_Int_t* Abc_SclFindCriticalCoWindow(SC_Man* p, int Window) {
    ...
    Abc_NtkForEachCo(p->pNtk, pObj, i) if (Abc_SclObjTimeMax(p, pObj) >= fMaxArr)
...
void Abc_SclFindNodesToUpdate(Abc_Obj_t* pPivot, Vec_Int_t** pvNodes, Vec_Int_t** pvEvals, Abc_Obj_t* pExtra) {
    ...
    Abc_ObjForEachFanout(pPivot, pNext, i) if (Abc_ObjIsNode(pNext) && pNext->fMarkA) {
        Vec_IntPush(vNodes, Abc_ObjId(pNext));
        Abc_ObjForEachFanout(pNext, pNext2, k) if (Abc_ObjIsNode(pNext2) && pNext2->fMarkA)
    ...
    Vec_IntClear(vEvals);
    Abc_NtkForEachObjVec(vNodes, p, pObj, i)
        Abc_ObjForEachFanout(pObj, pNext, k) if (pNext->fMarkA && !pNext->fMarkB)
    //            if ( !pNext->fMarkB )
    {
        assert(pObj->fMarkB);
...
int Abc_SclObjCheckMarkedFanFans(Abc_Obj_t* pObj) {
    ...
    if (pObj->fMarkB)
        return 1;
    Abc_ObjForEachFanin(pObj, pNext, i) if (pNext->fMarkB) return 1;
    Abc_ObjForEachFanout(pObj, pNext, i) if (pNext->fMarkB) return 1;
    return 0;
}
...
void Abc_SclUpsizeRemoveDangling(SC_Man* p, Abc_Ntk_t* pNtk) {
    ...
    Abc_Obj_t* pObj;
    int i;
    Abc_NtkForEachNodeNotBarBuf(pNtk, pObj, i) if (Abc_ObjFanoutNum(pObj) == 0) {
