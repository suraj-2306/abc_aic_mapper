...
static inline SC_Cell* Abc_SclObjCell(Abc_Obj_t* p) {
    int c = Abc_SclObjCellId(p);
    return c == -1 ? NULL : SC_LibCell(Abc_SclObjLib(p), c);
}
...
static inline float Abc_SclGetAverageSize(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachNodeNotBarBuf1(pNtk, pObj, i)
        Count++,
        Total += 100.0 * Abc_SclObjCell(pObj)->Order / Abc_SclObjCell(pObj)->nGates;
...
static inline float Abc_SclGetMaxDelay(SC_Man* p) {
    ...
    Abc_NtkForEachCo(p->pNtk, pObj, i)
        fMaxArr
        = Abc_MaxFloat(fMaxArr, Abc_SclObjTimeMax(p, pObj));
    return fMaxArr;
}
