...
static inline void Bus_SclObjUpdateDept(Abc_Obj_t* p, float time) {
    float* q = Vec_FltEntryP(Bus_SclObjMan(p)->vDepts, Abc_ObjId(p));
    if (*q < time) *q = time;
}
...
void Abc_SclBufSize(Bus_Man_t* p, float Gain) {
    ...
    Abc_NtkForEachObjReverse(p->pNtk, pObj, i) {
        ...
        if (!p->pPars->fSizeOnly && (Abc_ObjFanoutNum(pObj) > p->pPars->nDegree || Load > GainGate * Cin)) {
            ...
            // update node fanouts
            Vec_PtrForEachEntry(Abc_Obj_t*, p->vFanouts, pFanout, k) if (Abc_ObjFaninNum(pFanout) == 0)
