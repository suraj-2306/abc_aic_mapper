...
void Abc_SclFindWindow(Abc_Obj_t* pPivot, Vec_Int_t** pvNodes, Vec_Int_t** pvEvals) {
    ...
    Abc_ObjForEachFanout(pPivot, pNext, i) if (Abc_ObjIsNode(pNext)) {
        Vec_IntPush(vNodes, Abc_ObjId(pNext));
        Abc_ObjForEachFanout(pNext, pNext2, k) if (Abc_ObjIsNode(pNext2))
    ...
    Vec_IntClear(vEvals);
    Abc_NtkForEachObjVec(vNodes, p, pObj, i)
        Abc_ObjForEachFanout(pObj, pNext, k) if (!pNext->fMarkB) {
        assert(pObj->fMarkB);
...
void Abc_NtkCollectNodesByArea(SC_Man* p, Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachNode(pNtk, pObj, i) if (Abc_ObjFaninNum(pObj) > 0) {
...
int Abc_SclCheckOverlap(Abc_Ntk_t* pNtk, Vec_Int_t* vNodes) {
    Abc_Obj_t* pObj;
    int i;
    Abc_NtkForEachObjVec(vNodes, pNtk, pObj, i) if (Abc_NodeIsTravIdCurrent(pObj)) return 1;
    return 0;
}
