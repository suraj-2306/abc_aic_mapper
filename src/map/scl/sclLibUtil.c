...
int Abc_SclClassCellNum(SC_Cell* pClass) {
    SC_Cell* pCell;
    int i, Count = 0;
    SC_RingForEachCell(pClass, pCell, i) if (!pCell->fSkip)
        Count++;
    return Count;
}
...
void Abc_SclShortFormula(SC_Cell* pCell, char* pForm, char* pBuffer) {
    ...
    for (pTemp = pForm; *pTemp;) {
        ...
        SC_CellForEachPinIn(pCell, pPin, i) if (Abc_SclAreEqual(pPin->pName, pTemp, pLimit)) {
            *pBuffer++ = 'a' + i;
            break;
        }
...
static inline void Abc_SclTimingUpdate(SC_Cell* pCell, SC_Timing* p, char* Buffer) {
    SC_Pin* pPin;
    int i;
    SC_CellForEachPinIn(pCell, pPin, i) if (p->related_pin && !strcmp(p->related_pin, pPin->pName)) {
...
static inline void Abc_SclPinUpdate(SC_Cell* pCell, SC_Pin* p, char* Buffer) {
    ...
    SC_PinForEachRTiming(p, pTemp, i) {
        ...
        SC_CellForEachPinIn(pCell, pPin, k) if (pTemp->pName && !strcmp(pTemp->pName, pPin->pName)) {
...
void Abc_SclLinkCells(SC_Lib* p) {
    ...
    SC_LibForEachCell(p, pCell, i) {
        ...
        SC_LibForEachCellClass(p, pRepr, k) if (pCell->n_inputs == pRepr->n_inputs && pCell->n_outputs == pRepr->n_outputs && Vec_WrdEqual(SC_CellFunc(pCell), SC_CellFunc(pRepr))) break;
        ...
        pRepr->pPrev->pNext = pCell;
        pCell->pNext = pRepr;
        pCell->pPrev = pRepr->pPrev;
        pRepr->pPrev = pCell;
    }
...
SC_Cell* Abc_SclFindInvertor(SC_Lib* p, int fFindBuff) {
    ...
    int k;
    SC_LibForEachCellClass(p, pCell, k) if (pCell->n_inputs == 1 && Vec_WrdEntry(&SC_CellPin(pCell, 1)->vFunc, 0) == Truth) break;
    // take representative
...
SC_Cell* Abc_SclFindSmallestGate(SC_Cell* p, float CinMin) {
    SC_Cell* pRes = NULL;
    int i;
    SC_RingForEachCell(p->pRepr, pRes, i) if (SC_CellPinCapAve(pRes) > CinMin) return pRes;
...
SC_WireLoad* Abc_SclFetchWireLoadModel(SC_Lib* p, char* pWLoadUsed) {
    ...
    SC_LibForEachWireLoad(p, pWL, i) if (!strcmp(pWL->pName, pWLoadUsed)) break;
...
SC_WireLoad* Abc_SclFindWireLoadModel(SC_Lib* p, float Area) {
    ...
    if (p->default_wire_load_sel && strlen(p->default_wire_load_sel)) {
        ...
        SC_LibForEachWireLoadSel(p, pWLS, i) if (!strcmp(pWLS->pName, p->default_wire_load_sel)) break;
...
void Abc_SclPrintCells(SC_Lib* p, float SlewInit, float Gain, int fInvOnly, int fShort) {
    ...
    SC_LibForEachCellClass(p, pRepr, k)
        SC_RingForEachCell(pRepr, pCell, i)
            nLength
        = Abc_MaxInt(nLength, strlen(pRepr->pName));
    // print cells
    SC_LibForEachCellClass(p, pRepr, k) {
...
Vec_Str_t* Abc_SclProduceGenlibStr(SC_Lib* p, float Slew, float Gain, int nGatesMin, int* pnCellCount) {
    ...
    SC_LibForEachCellClass(p, pRepr, i) if (pRepr->n_outputs == 1)
        nClassMax
        = Abc_MaxInt(nClassMax, Abc_SclClassCellNum(pRepr));
    // update the number
