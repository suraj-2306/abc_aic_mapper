...
static inline char* Amap_OuputStrsav(Aig_MmFlex_t* p, char* pStr) { return pStr ? strcpy(Aig_MmFlexEntryFetch(p, strlen(pStr) + 1), pStr) : NULL; }
...
Vec_Ptr_t* Amap_ManProduceMapped(Amap_Man_t* p) {
    ...
    Amap_ManForEachObj(p, pObj, i) {
        ...
        if (Amap_ObjIsPo(pObj)) {
            ...
            assert(Amap_ObjRefsTotal(pFanin));
            if (Amap_ObjIsConst1(pFanin)) { // create constant node
