    ...
    Vec_IntForEachEntry(vNodFanin, iNodFanin, k) {
        ...
        Kit_DsdObjForEachFaninReverse(pNtk, pDsdObj, iDsdFanin, i) {
            ...
            if (pDsdFanin->fMark == 1)
                continue;
            if (!((pDsdFanin->Type == KIT_DSD_AND && pNodFanin->Type == AMAP_OBJ_AND) || (pDsdFanin->Type == KIT_DSD_XOR && pNodFanin->Type == AMAP_OBJ_XOR) || (pDsdFanin->Type == KIT_DSD_PRIME && pNodFanin->Type == AMAP_OBJ_MUX)))
                continue;
...
int Amap_LibDeriveGatePerm(Amap_Lib_t* pLib, Amap_Gat_t* pGate, Kit_DsdNtk_t* pNtk, Amap_Nod_t* pNod, char* pArray) {
    ...
    Vec_IntForEachEntry(vPerm, Entry, i)
        Vec_IntForEachEntryStart(vPerm, Entry2, k, i + 1) if (Abc_Lit2Var(Entry) == Abc_Lit2Var(Entry2)) {
        Vec_IntFree(vPerm);
        return 0;
    }
