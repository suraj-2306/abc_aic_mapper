...
static inline void Amap_ObjSetChoice(Amap_Obj_t* pObj, Amap_Obj_t* pEqu) {
    assert(pObj->Equiv == 0);
    if (pObj->Id != pEqu->Id) pObj->Equiv = pEqu->Id;
}
...
static inline Amap_Cut_t* Amap_ManCutNext(Amap_Cut_t* pCut) { return (Amap_Cut_t*)(((int*)pCut) + pCut->nFans + 1); }
// returns pointer to the place of the next cut (temporary cuts only)
static inline Amap_Cut_t** Amap_ManCutNextP(Amap_Cut_t* pCut) { return (Amap_Cut_t**)(((int*)pCut) + pCut->nFans + 1); }
...
#    define Amap_ManForEachObj(p, pObj, i)                                         \
        Vec_PtrForEachEntry(Amap_Obj_t*, p->vObjs, pObj, i) if ((pObj) == NULL) {} \
        else
// iterator over all nodes
#    define Amap_ManForEachNode(p, pObj, i)                                                                 \
        Vec_PtrForEachEntry(Amap_Obj_t*, p->vObjs, pObj, i) if ((pObj) == NULL || !Amap_ObjIsNode(pObj)) {} \
        else
...
#    define Amap_MatchForEachFaninCompl(p, pM, pFanin, fCompl, i)                                                                                                                                                                                                             \
        for (i = 0; i < (int)(pM)->pCut->nFans && ((pFanin = Amap_ManObj((p), Abc_Lit2Var((pM)->pCut->Fans[Abc_Lit2Var((pM)->pSet->Ins[i])]))), 1) && ((fCompl = Abc_LitIsCompl((pM)->pSet->Ins[i]) ^ Abc_LitIsCompl((pM)->pCut->Fans[Abc_Lit2Var((pM)->pSet->Ins[i])])), 1); \
             i++)

// iterates through each fanin of the match
#    define Amap_MatchForEachFanin(p, pM, pFanin, i)                                                                                                \
        for (i = 0; i < (int)(pM)->pCut->nFans && ((pFanin = Amap_ManObj((p), Abc_Lit2Var((pM)->pCut->Fans[Abc_Lit2Var((pM)->pSet->Ins[i])]))), 1); \
             i++)
