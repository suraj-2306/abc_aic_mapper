...
int Amap_CreateCheckAllZero(Vec_Ptr_t* vVecNods) {
    Vec_Int_t* vNods;
    int i;
    Vec_PtrForEachEntryReverse(Vec_Int_t*, vVecNods, vNods, i) if (Vec_IntSize(vNods) != 1 || Vec_IntEntry(vNods, 0) != 0) return 0;
    return 1;
}
...
Vec_Int_t* Amap_CreateRulesFromDsd_rec(Amap_Lib_t* pLib, Kit_DsdNtk_t* p, int iLit) {
    ...
    Kit_DsdObjForEachFanin(p, pObj, iFanin, i) {
        ...
        if (Abc_LitIsCompl(iFanin)) {
            Vec_IntForEachEntry(vNodsFanin, iNod, k) if (iNod > 0)
...
void Amap_CreateCheckAsym_rec(Kit_DsdNtk_t* p, int iLit, Vec_Int_t** pvSyms) {
    ...
    for (i = 0; i < (int)pObj->nFans; i++)
        for (k = i + 1; k < (int)pObj->nFans; k++) {
            if (Abc_LitIsCompl(pObj->pFans[i]) != Abc_LitIsCompl(pObj->pFans[k]) && Kit_DsdNtkObj(p, Abc_Lit2Var(pObj->pFans[i])) == NULL && Kit_DsdNtkObj(p, Abc_Lit2Var(pObj->pFans[k])) == NULL) {
                if (*pvSyms == NULL)
...
void Amap_LibCreateRules(Amap_Lib_t* pLib, int fVeryVerbose) {
    ...
    Vec_VecFree((Vec_Vec_t*)pLib->vRules);
    pLib->vRules = NULL;
    Vec_VecFree((Vec_Vec_t*)pLib->vRulesX);
    pLib->vRulesX = NULL;
}
