...
float Amap_ManMaxDelay(Amap_Man_t* p) {
    ...
    Amap_ManForEachPo(p, pObj, i)
        Delay
        = Abc_MaxInt(Delay, Amap_ObjFanin0(p, pObj)->Best.Delay);
    return Delay;
}
...
float Amap_ManComputeMapping(Amap_Man_t* p) {
    ...
    Amap_ManForEachPo(p, pObj, i)
        Area
        += Amap_ManComputeMapping_rec(p, Amap_ObjFanin0(p, pObj), Amap_ObjFaninC0(pObj));
    return Area;
}
...
static inline float Amap_CutAreaDerefed2(Amap_Man_t* p, Amap_Obj_t* pNode, Amap_Mat_t* pM) {
    ...
    Vec_PtrForEachEntry(int*, p->vTempP, pInt, i)(*pInt)--;
...
void Amap_ManMatch(Amap_Man_t* p, int fFlow, int fRefs) {
    ...
    Amap_ManForEachNode(p, pObj, i) if (pObj->pData)
