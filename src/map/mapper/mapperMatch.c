...
int Map_MatchNodePhase(Map_Man_t* p, Map_Node_t* pNode, int fPhase) {
    ...
    if (p->fMappingMode == 2 || p->fMappingMode == 3) {
        ...
        if (pNode->nRefAct[fPhase] > 0 || (pNode->pCutBest[!fPhase] == NULL && pNode->nRefAct[!fPhase] > 0))
    ...
    if (p->fMappingMode >= 2 && (pNode->nRefAct[fPhase] > 0 || (pNode->pCutBest[!fPhase] == NULL && pNode->nRefAct[!fPhase] > 0))) {
        if (p->fMappingMode == 2 || p->fMappingMode == 3)
