...
void Wlc_BlastPrintMatrix(Gia_Man_t* p, Vec_Wec_t* vProds, int fVerbose) {
    ...
    Vec_WecForEachLevel(vProds, vLevel, i)
        Vec_IntForEachEntry(vLevel, iLit, k) if (Gia_ObjIsAnd(Gia_ManObj(p, Abc_Lit2Var(iLit))))
...
void Wlc_IntInsert2(Gia_Man_t* pNew, Vec_Int_t* vProd, int iLit) {
    ...
    Vec_IntForEachEntryReverse(vProd, Entry, i) if (Gia_ObjLevelId(pNew, Abc_Lit2Var(Entry)) >= Level) break;
...
Gia_Man_t* Wlc_NtkBitBlast(Wlc_Ntk_t* p, Wlc_BstPar_t* pParIn) {
    ...
    if (Vec_IntSize(&p->vFfs2) > 0) {
        ...
        Wlc_NtkForEachFf2(p, pObj, i) {
            ...
            Vec_IntForEachEntryDouble(vSignature, iClk0, iAsyn0, k) if (iClk == iClk0 && iAsyn == iAsyn0) {
                for (b = 0; b < nRange; b++)
    ...
    Wlc_NtkForEachObj(p, pObj, i) {
        ...
        } else if (Wlc_ObjIsCi(pObj) || pObj->Type == WLC_OBJ_FF) // assuming that FFs are ordered immediately after PIs
        {
            ...
            Wlc_ObjForEachFanin(pObj, iFanin, k) if (k > 0)
                fSigned
                &= Wlc_NtkObj(p, iFanin)->Signed;
            Vec_IntClear(vTemp1);
            if (pPar->fDecMuxes) {
        ...
        } else if (pObj->Type == WLC_OBJ_SHIFT_R || pObj->Type == WLC_OBJ_SHIFT_RA || pObj->Type == WLC_OBJ_SHIFT_L || pObj->Type == WLC_OBJ_SHIFT_LA) {
            ...
            Wlc_ObjForEachFanin(pObj, iFanin, k)
                nTotal
                += Wlc_ObjRange(Wlc_NtkObj(p, iFanin));
            ...
            for (k = 1; k < nRange; k++)
                Vec_IntPush(vRes, 0);
        } else if (pObj->Type == WLC_OBJ_COMP_LESS || pObj->Type == WLC_OBJ_COMP_MOREEQU || pObj->Type == WLC_OBJ_COMP_MORE || pObj->Type == WLC_OBJ_COMP_LESSEQU) {
            int nRangeMax = Abc_MaxInt(nRange0, nRange1);
            int fSigned = Wlc_ObjIsSignedFanin01(p, pObj);
            int* pArg0 = Wlc_VecLoadFanins(vTemp0, pFans0, nRange0, nRangeMax, fSigned);
            int* pArg1 = Wlc_VecLoadFanins(vTemp1, pFans1, nRange1, nRangeMax, fSigned);
            ...
            for (k = 1; k < nRange; k++)
                Vec_IntPush(vRes, 0);
        } else if (pObj->Type == WLC_OBJ_REDUCT_AND || pObj->Type == WLC_OBJ_REDUCT_OR || pObj->Type == WLC_OBJ_REDUCT_XOR || pObj->Type == WLC_OBJ_REDUCT_NAND || pObj->Type == WLC_OBJ_REDUCT_NOR || pObj->Type == WLC_OBJ_REDUCT_NXOR) {
        ...
        if (vAddOutputs && !Wlc_ObjIsCo(pObj) && ((pObj->Type >= WLC_OBJ_MUX && pObj->Type <= WLC_OBJ_ROTATE_L) || (pObj->Type >= WLC_OBJ_COMP_EQU && pObj->Type <= WLC_OBJ_COMP_MOREEQU) || (pObj->Type >= WLC_OBJ_ARI_ADD && pObj->Type <= WLC_OBJ_ARI_SQUARE))) {
    ...
    Wlc_NtkForEachCi(p, pObj, i) if (Wlc_ObjIsPi(pObj)) {
    ...
    Wlc_NtkForEachCo(p, pObj, i) if (Wlc_ObjIsPo(pObj)) {
        char* pName = Wlc_ObjName(p, Wlc_ObjId(p, pObj));
        nRange = Wlc_ObjRange(pObj);
        if (pPar->fCreateWordMiter) {
    ...
    if (pPar->vBoxIds) {
        Vec_PtrFreeFree(pNew->vNamesIn);
        pNew->vNamesIn = NULL;
        Vec_PtrFreeFree(pNew->vNamesOut);
        pNew->vNamesOut = NULL;
    }
...
Vec_Int_t* Wlc_ComputePerm(Wlc_Ntk_t* pNtk, int nPis) {
    ...
    for (k = 0; fChange; k++) {
        fChange = 0;
        Vec_IntForEachEntryTwo(vOffs, vSizes, First, Size, i) if (k < Size) {
