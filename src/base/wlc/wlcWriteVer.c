...
void Wlc_WriteTables(FILE* pFile, Wlc_Ntk_t* p) {
    ...
    Wlc_NtkForEachObj(p, pObj, i) if (pObj->Type == WLC_OBJ_TABLE)
...
void Wlc_WriteAddPos(Wlc_Ntk_t* p) {
    ...
    int i;
    Vec_IntClear(&p->vPos);
    Wlc_NtkForEachObj(p, pObj, i) if (pObj->Type != WLC_OBJ_PI && pObj->Type != WLC_OBJ_MUX) {
        pObj->fIsPo = 1;
...
void Wlc_WriteVerIntVec(FILE* pFile, Wlc_Ntk_t* p, Vec_Int_t* vVec, int Start) {
    ...
    Vec_IntForEachEntry(vVec, iObj, i) {
        ...
        if (NameCounter && LineLength + AddedLength + 3 > 70) { // write the line extender
            fprintf(pFile, "\n   ");
...
void Wlc_WriteVerInt(FILE* pFile, Wlc_Ntk_t* p, int fNoFlops) {
    ...
    Wlc_NtkForEachObj(p, pObj, i) if (pObj->Type == WLC_OBJ_ROTATE_R || pObj->Type == WLC_OBJ_ROTATE_L)
        Wlc_ObjFanin1(p, pObj)
            ->Mark
        = 1;
    ...
    Wlc_NtkForEachFf2(p, pObj, i) {
        ...
        Wlc_ObjForEachFanin(pObj, iFanin, k) if (iFanin) fprintf(pFile, " .%s(%s),", pInNames[k], Wlc_ObjName(p, iFanin));
