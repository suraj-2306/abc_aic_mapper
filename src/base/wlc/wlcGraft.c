...
int Wlc_NtkCollectObjs_rec(Wlc_Ntk_t* p, Wlc_Obj_t* pObj, Vec_Int_t* vObjs) {
    ...
    Wlc_ObjForEachFanin(pObj, iFanin, i)
        Count
        += Wlc_NtkCollectObjs_rec(p, Wlc_NtkObj(p, iFanin), vObjs);
...
Vec_Int_t* Wlc_NtkCollectObjs(Wlc_Ntk_t* p, int fEven, int* pCount) {
    ...
    int i, Count = 0;
    Wlc_NtkCleanMarks(p);
    Wlc_NtkForEachCo(p, pObj, i) if ((i & 1) == fEven)
        Count
        += Wlc_NtkCollectObjs_rec(p, pObj, vObjs);
    Wlc_NtkCleanMarks(p);
    if (pCount)
        *pCount = Count;
    return vObjs;
...
Wlc_Ntk_t* Wlc_NtkGraftMulti(Wlc_Ntk_t* p, int fVerbose) {
    ...
    Wlc_NtkForEachObjVec(vObjsLHS, p, pWlcObj, i) if (Wlc_ObjType(pWlcObj) == WLC_OBJ_ARI_MULTI) {
...
void Sbc_SimMult(word A[64], word B[64], word R[128], int nIns) {
    word a, b, r[2], Mask = Abc_Tt6Mask(nIns);
    int i, k;
    for (i = 0; i < 64; i++)
...
Vec_Int_t* Sbc_ManWlcNodes2(Wlc_Ntk_t* pNtk, Gia_Man_t* p, Vec_Int_t* vGiaLits) {
    ...
    Vec_IntForEachEntry(vGiaLits, iGiaLit, i) if (iGiaLit != -1)
    ...
    for (k = 0; k < nBits; k++) {
        ...
        // print simulation signature
        {
            ...
            Extra_PrintHex(stdout, (unsigned*)pInfoObj, 6);
            printf("\n");
        }
    }
    return vRes;
