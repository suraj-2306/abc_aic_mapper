...
int Wlc_NtkCreateLevels_(Wlc_Ntk_t* p) {
    ...
    Wlc_NtkForEachObj(p, pObj, i) {
        ...
        Wlc_ObjForEachFanin(pObj, iFanin, k)
            Level
            = Abc_MaxInt(Level, Wlc_ObjLevelId(p, iFanin) + 1);
...
void Wlc_NtkCreateLevels_rec(Wlc_Ntk_t* p, Wlc_Obj_t* pObj) {
    ...
    Wlc_ObjForEachFanin(pObj, iFanin, k) if (iFanin)
        Level
        = Abc_MaxInt(Level, Wlc_ObjLevelId(p, iFanin));
...
int Wlc_NtkCreateLevels(Wlc_Ntk_t* p) {
    ...
    Wlc_NtkForEachObj(p, pObj, i) if (!Wlc_ObjIsCi(pObj) && Wlc_ObjFaninNum(pObj))
...
int Wlc_NtkRemapLevels(Wlc_Ntk_t* p, Vec_Int_t* vObjs, int nLevels) {
    ...
    // create level map
    Vec_IntForEachEntry(vUsed, Entry, i) if (Entry)
...
void Wlc_NtkCollectStats(Wlc_Ntk_t* p, int nObjs[2][WLC_OBJ_NUMBER]) {
    ...
    for (n = 0; n < 2; n++) {
        ...
        Wlc_NtkForEachObj(p, pObj, i) if (pObj->Mark)
            nObjs[n][pObj->Type]++;
    }
    Wlc_NtkCleanMarks(p);
}
...
void Wlc_NtkPrintDistrib(Wlc_Ntk_t* p, int fTwoSides, int fVerbose) {
    ...
    Wlc_NtkForEachObj(p, pObj, i) {
        ...
        // 1-input types
        else if (pObj->Type == WLC_OBJ_BUF || pObj->Type == WLC_OBJ_BIT_SELECT || pObj->Type == WLC_OBJ_TABLE || pObj->Type == WLC_OBJ_BIT_ZEROPAD || pObj->Type == WLC_OBJ_BIT_SIGNEXT || pObj->Type == WLC_OBJ_BIT_NOT || pObj->Type == WLC_OBJ_LOGIC_NOT || pObj->Type == WLC_OBJ_ARI_MINUS)
...
Wlc_Ntk_t* Wlc_NtkDupDfs(Wlc_Ntk_t* p, int fMarked, int fSeq) {
    ...
    pNew->fMemPorts = p->fMemPorts;
    pNew->fEasyFfs = p->fEasyFfs;
    Wlc_NtkForEachCi(p, pObj, i) if (!fMarked || pObj->Mark) {
    ...
    Wlc_NtkForEachCo(p, pObj, i) if (!fMarked || pObj->Mark)
        Wlc_NtkDupDfs_rec(pNew, p, Wlc_ObjId(p, pObj), vFanins);
    Wlc_NtkForEachCo(p, pObj, i) if (!fMarked || pObj->Mark)
...
void Wlc_NtkMarkCone(Wlc_Ntk_t* p, int iCoId, int Range, int fSeq, int fAllPis) {
    ...
    Wlc_NtkForEachCo(p, pObj, i) if (iCoId == -1 || (i >= iCoId && i < iCoId + Range))
...
Wlc_Ntk_t* Wlc_NtkDupSingleNodes(Wlc_Ntk_t* p) {
    ...
    // count objects
    Wlc_NtkForEachObj(p, pObj, i) if (!Wlc_ObjIsCi(pObj))
        Count
        += 1 + Wlc_ObjFaninNum(pObj);
    // copy objects
    Wlc_NtkCleanCopy(p);
    vFanins = Vec_IntAlloc(100);
    pNew = Wlc_NtkAlloc(p->pName, p->nObjsAlloc);
