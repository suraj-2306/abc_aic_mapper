...
int Wlc_ObjCheckIsEmpty_rec(Wlc_Ntk_t* p, Wlc_Obj_t* pObj) {
    ...
    if (Wlc_ObjIsPi(pObj))
        ...
        return 0;
    Wlc_ObjForEachFanin(pObj, iFanin, k) if (!Wlc_ObjCheckIsEmpty_rec(p, Wlc_NtkObj(p, iFanin))) return 0;
    return 1;
}
...
Wlc_Ntk_t* Wlc_NtkAbstractMemory(Wlc_Ntk_t* p, Vec_Int_t* vMemObjs, Vec_Int_t* vMemFanins, int* piFirstMemPi, int* piFirstCi, int* piFirstMemCi, Vec_Wec_t* vConstrs, Vec_Int_t* vNodeFrames) {
    ...
    // duplicate PIs
    Wlc_NtkForEachPi(p, pObj, i) if (!pObj->Mark) {
        Wlc_ObjDup(pNew, p, Wlc_ObjId(p, pObj), vFanins);
    ...
    // create new PIs
    *piFirstMemPi = nBits;
    Wlc_NtkForEachObjVec(vMemObjs, p, pObj, i) if (Wlc_ObjType(pObj) == WLC_OBJ_READ) {
    ...
    *piFirstCi = nBits;
    Wlc_NtkForEachCi(p, pObj, i) if (!Wlc_ObjIsPi(pObj) && !pObj->Mark) {
        Wlc_ObjDup(pNew, p, Wlc_ObjId(p, pObj), vFanins);
    ...
    *piFirstMemCi = nBits;
    ...
    // duplicate objects
    Wlc_NtkForEachObj(p, pObj, i) if (!Wlc_ObjIsCi(pObj) && !pObj->Mark)
        Wlc_ObjDup(pNew, p, Wlc_ObjId(p, pObj), vFanins);
    ...
    if (Vec_IntSize(&p->vPoPairs)) {
        ...
        Wlc_NtkForEachCo(p, pObj, i) if (pObj->fIsFi && !pObj->Mark)
            Wlc_ObjSetCo(pNew, Wlc_ObjCopyObj(pNew, p, pObj), pObj->fIsFi);
        ...
        // duplicate POs
        Wlc_NtkForEachPo(p, pObj, i) if (!pObj->Mark)
            Wlc_ObjSetCo(pNew, Wlc_ObjCopyObj(pNew, p, pObj), pObj->fIsFi);
        // create constraint output
        if (vConstrs)
            Wlc_ObjSetCo(pNew, (pConstr = Wlc_NtkObj(pNew, Wlc_ObjAlloc(pNew, WLC_OBJ_BUF, 0, 0, 0))), 0);
        // duplicate FIs
        Wlc_NtkForEachCo(p, pObj, i) if (!Wlc_ObjIsPo(pObj) && !pObj->Mark)
            Wlc_ObjSetCo(pNew, Wlc_ObjCopyObj(pNew, p, pObj), pObj->fIsFi);
    }
...
Vec_Int_t* Wlc_NtkFindConflict(Wlc_Ntk_t* p, Vec_Int_t* vMemObjs, Vec_Wrd_t* vValues, int nFrames) {
    ...
    Vec_WecForEachLevel(vTraces, vTrace1, i)
        Vec_WecForEachLevelStop(vTraces, vTrace2, f, i) if (Vec_IntEntry(vTrace1, 0) == Vec_IntEntry(vTrace2, 0)) {
...
int Wlc_NtkExploreMem2_rec(Wlc_Ntk_t* p, Wlc_Obj_t* pObj, Vec_Int_t* vCollect, int nFrames) {
    ...
    Wlc_ObjForEachFanin(pObj, iFanin, k)
        nVisited
        += Wlc_NtkExploreMem2_rec(p, Wlc_NtkObj(p, iFanin), vCollect, nFrames);
    Vec_IntPushTwo(vCollect, Wlc_ObjId(p, pObj), nFrames);
    return nVisited + 1;
}
...
Vec_Int_t* Wlc_NtkCollectOneType(Wlc_Ntk_t* p, Vec_Int_t* vMemObjsClean, int Type1, int Type2) {
    ...
    int i;
    Vec_Int_t* vRes = Vec_IntAlloc(100);
    Wlc_NtkForEachObjVec(vMemObjsClean, p, pObj, i) if (Wlc_ObjType(pObj) == Type1 || Wlc_ObjType(pObj) == Type2)
