...
Wlc_Prs_t* Wlc_PrsStart(char* pFileName, char* pStr) {
    ...
    p->nFileSize = strlen(p->pBuffer);
    assert(p->nFileSize > 0);
...
int Wlc_PrsFindLine(Wlc_Prs_t* p, char* pCur) {
    int Entry, iLine = 0;
    Vec_IntForEachEntry(p->vLines, Entry, iLine) if (Entry > pCur - p->pBuffer) return iLine + 1;
    return -1;
}
...
static inline int Wlc_PrsIsChar(char* pStr) {
    return (pStr[0] >= 'a' && pStr[0] <= 'z') || (pStr[0] >= 'A' && pStr[0] <= 'Z') || (pStr[0] >= '0' && pStr[0] <= '9') || pStr[0] == '_' || pStr[0] == '$' || pStr[0] == '\\';
}
...
static inline int Wlc_PrsFindDefinition(Wlc_Prs_t* p, char* pStr, Vec_Int_t* vFanins, int* pXValue) {
    ...
    if (Wlc_PrsIsDigit(pStr)) {
    ...
    } else if (pStr[0] == '-' || pStr[0] == '&' || pStr[0] == '|' || pStr[0] == '^' || (pStr[0] == '~' && pStr[1] == '&') || (pStr[0] == '~' && pStr[1] == '|') || (pStr[0] == '~' && pStr[1] == '^')) {
        int shift = 1;
        if (pStr[0] == '-')
        ...
        if (pStr[0] == 0)
            ...
            if (pStr[0] == '>' && pStr[1] == '>' && pStr[2] != '>')
                pStr += 2, Type = fRotating ? WLC_OBJ_ROTATE_R : WLC_OBJ_SHIFT_R;
            else if (pStr[0] == '>' && pStr[1] == '>' && pStr[2] == '>')
                pStr += 3, Type = WLC_OBJ_SHIFT_RA;
            else if (pStr[0] == '<' && pStr[1] == '<' && pStr[2] != '<')
                pStr += 2, Type = fRotating ? WLC_OBJ_ROTATE_L : WLC_OBJ_SHIFT_L;
            else if (pStr[0] == '<' && pStr[1] == '<' && pStr[2] == '<')
                pStr += 3, Type = WLC_OBJ_SHIFT_LA;
            else if (pStr[0] == '&' && pStr[1] != '&')
                pStr += 1, Type = WLC_OBJ_BIT_AND;
            else if (pStr[0] == '|' && pStr[1] != '|')
                pStr += 1, Type = WLC_OBJ_BIT_OR;
            else if (pStr[0] == '^' && pStr[1] != '^')
                pStr += 1, Type = WLC_OBJ_BIT_XOR;
            else if (pStr[0] == '~' && pStr[1] == '&')
                pStr += 2, Type = WLC_OBJ_BIT_NAND;
            else if (pStr[0] == '~' && pStr[1] == '|')
                pStr += 2, Type = WLC_OBJ_BIT_NOR;
            else if (pStr[0] == '~' && pStr[1] == '^')
                pStr += 2, Type = WLC_OBJ_BIT_NXOR;
            else if (pStr[0] == '=' && pStr[1] == '>')
                pStr += 2, Type = WLC_OBJ_LOGIC_IMPL;
            else if (pStr[0] == '&' && pStr[1] == '&')
                pStr += 2, Type = WLC_OBJ_LOGIC_AND;
            else if (pStr[0] == '|' && pStr[1] == '|')
                pStr += 2, Type = WLC_OBJ_LOGIC_OR;
            else if (pStr[0] == '^' && pStr[1] == '^')
                pStr += 2, Type = WLC_OBJ_LOGIC_XOR;
            else if (pStr[0] == '=' && pStr[1] == '=')
                pStr += 2, Type = WLC_OBJ_COMP_EQU;
            else if (pStr[0] == '!' && pStr[1] == '=')
                pStr += 2, Type = WLC_OBJ_COMP_NOTEQU;
            else if (pStr[0] == '<' && pStr[1] != '=')
                pStr += 1, Type = WLC_OBJ_COMP_LESS;
            else if (pStr[0] == '>' && pStr[1] != '=')
                pStr += 1, Type = WLC_OBJ_COMP_MORE;
            else if (pStr[0] == '<' && pStr[1] == '=')
                pStr += 2, Type = WLC_OBJ_COMP_LESSEQU;
            else if (pStr[0] == '>' && pStr[1] == '=')
                pStr += 2, Type = WLC_OBJ_COMP_MOREEQU;
            else if (pStr[0] == '+')
                pStr += 1, Type = WLC_OBJ_ARI_ADD;
            else if (pStr[0] == '-')
                pStr += 1, Type = WLC_OBJ_ARI_SUB;
            else if (pStr[0] == '*' && pStr[1] != '*')
                pStr += 1, Type = WLC_OBJ_ARI_MULTI;
            else if (pStr[0] == '/')
                pStr += 1, Type = WLC_OBJ_ARI_DIVIDE;
            else if (pStr[0] == '%')
                pStr += 1, Type = WLC_OBJ_ARI_REM;
            else if (pStr[0] == '*' && pStr[1] == '*')
                pStr += 2, Type = WLC_OBJ_ARI_POWER;
            else
                return Wlc_PrsWriteErrorMessage(p, pStr, "Unsupported operation (%c).", pStr[0]);
            if (!(pStr = Wlc_PrsReadName(p, pStr + 1, vFanins)))
                return 0;
            pStr = Wlc_PrsSkipSpaces(pStr);
    ...
    startword:
        if (Wlc_PrsStrCmp(pStart, "module")) {
            ...
            if (Wlc_PrsStrCmp(pName, "table")) {
                ...
                Vec_IntForEachEntry(vValues, Value, v) for (b = 0; b < Width2; b++) if ((Value >> b) & 1)
            ...
            p->pNtk->pMemTable = p->pMemTable;
            p->pMemTable = NULL;
            p->pNtk->vTables = p->vTables;
            p->vTables = NULL;
            ...
            if (p->pNtk->vInits) {
                ...
                Wlc_NtkForEachFf(p->pNtk, pObj, i) if (i & 1)
                    Wlc_ObjSetCo(p->pNtk, pObj, 1);
                else Wlc_ObjSetCi(p->pNtk, pObj);
            ...
            if (p->vPoPairs) {
                ...
                while (*pName) {
                    Wlc_NtkForEachPo(p->pNtk, pObj, i) if (!strcmp(Wlc_ObjName(p->pNtk, Wlc_ObjId(p->pNtk, pObj)), pName)) {
...
Wlc_Ntk_t* Wlc_ReadVer(char* pFileName, char* pStr) {
    ...
    if (p->pNtk) {
        Wlc_Obj_t* pObj;
        int i;
        Wlc_NtkForEachObj(p->pNtk, pObj, i) if (pObj->Type == WLC_OBJ_FF)
