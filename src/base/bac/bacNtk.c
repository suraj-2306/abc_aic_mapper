...
void Bac_NtkDeriveFanout(Bac_Ntk_t* p) {
    ...
    Bac_NtkForEachCo(p, iCo) if (!Bac_ObjNextFanout(p, iCo))
...
int Bac_ManAssignCountNames(Bac_Ntk_t* p) {
    ...
    Bac_NtkForEachPiMain(p, iObj, i) if (!Bac_ObjNameInt(p, iObj))
        Count++;
    Bac_NtkForEachBox(p, iBox)
        Bac_BoxForEachBoMain(p, iBox, iObj, i) if (!Bac_ObjNameInt(p, iObj))
            Count++;
    return Count;
}
void Bac_ManAssignInternWordNamesNtk(Bac_Ntk_t* p, Vec_Int_t* vMap) {
    ...
    Bac_NtkForEachCi(p, iObj) if (Bac_ObjNameInt(p, iObj))
        Vec_IntWriteEntry(vMap, Bac_ObjNameId(p, iObj), 1);
    Bac_NtkForEachBox(p, iObj) if (Bac_ObjNameInt(p, iObj))
        Vec_IntWriteEntry(vMap, Bac_ObjNameId(p, iObj), 1);
    // assign CI names
    ...
    // unmark all names
    Bac_NtkForEachPi(p, iObj, k) if (Bac_ObjNameType(p, iObj) <= BAC_NAME_WORD)
        Vec_IntWriteEntry(vMap, Bac_ObjNameId(p, iObj), 0);
    Bac_NtkForEachBox(p, iObj) {
        Vec_IntWriteEntry(vMap, Bac_ObjNameId(p, iObj), 0);
        Bac_BoxForEachBo(p, iObj, iTerm, k) if (Bac_ObjNameType(p, iTerm) <= BAC_NAME_WORD)
...
int Bac_ManClpObjNum_rec(Bac_Ntk_t* p) {
    ...
    Bac_NtkForEachBox(p, i)
        Counter
        += Bac_ObjIsBoxUser(p, i) ? Bac_ManClpObjNum_rec(Bac_BoxNtk(p, i)) + 3 * Bac_BoxBoNum(p, i) : Bac_BoxSize(p, i);
...
int Bac_NtkDfsUserBoxes_rec(Bac_Ntk_t* p, int iObj, Vec_Int_t* vBoxes) {
    ...
    Bac_BoxForEachFanin(p, iObj, iFanin, k) if (Bac_ObjIsBo(p, iFanin) && Bac_ObjIsBoxUser(p, Bac_ObjFanin(p, iFanin))) if (!Bac_NtkDfsUserBoxes_rec(p, Bac_ObjFanin(p, iFanin), vBoxes)) return 0;
    Vec_IntPush(vBoxes, iObj);
    Bac_ObjSetCopy(p, iObj, 1);
    return 1;
}
int Bac_NtkDfsUserBoxes(Bac_Ntk_t* p) {
    ...
    Bac_NtkForEachBoxUser(p, iObj) if (!Bac_NtkDfsUserBoxes_rec(p, iObj, &p->vArray)) {
...
void Bac_NtkCollapse_rec(Bac_Ntk_t* pNew, Bac_Ntk_t* p, Vec_Int_t* vSigs) {
    ...
    Bac_NtkForEachBox(p, iObj) if (Bac_ObjIsBoxPrim(p, iObj))
...
Bac_Man_t* Bac_ManCollapse(Bac_Man_t* p) {
    ...
    if (Bac_NtkHasNames(pRoot)) {
        ...
        Bac_NtkForEachPoDriver(pRoot, iObj, i) if (!Bac_ObjIsPi(pRoot, iObj))
