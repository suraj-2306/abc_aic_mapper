...
int Bac_ManAddBarbuf(Gia_Man_t* pNew, int iRes, Bac_Man_t* p, int iLNtk, int iLObj, int iRNtk, int iRObj, Vec_Int_t* vMap) {
    ...
    if (iRes == 0 || iRes == 1)
        return iRes;
    assert(iRes > 0);
    if (vMap && Abc_Lit2Var(iRes) < Vec_IntSize(vMap) && (iIdLit = Vec_IntEntry(vMap, Abc_Lit2Var(iRes))) >= 0 && Vec_IntEntry(&p->vBuf2LeafNtk, Abc_Lit2Var(iIdLit)) == iLNtk && Vec_IntEntry(&p->vBuf2RootNtk, Abc_Lit2Var(iIdLit)) == iRNtk)
    ...
    if (Bac_ObjIsCo(p, i))
        ...
        } else // primitive
        {
            ...
            Bac_BoxForEachFanin(p, iBox, iFanin, nLits)
                pLits[nLits]
                = Bac_ManExtract_rec(pNew, p, iFanin, fBuffers, vMap);
            assert(nLits <= 16);
...
void Bac_NtkInsertGia(Bac_Man_t* p, Gia_Man_t* pGia) {
    ...
    Bac_ManForEachNtk(p, pNtk, i) {
        Bac_NtkForEachBox(pNtk, iBox)
            Bac_BoxForEachBi(pNtk, iBox, iTerm, j) if (Bac_ObjFanin(pNtk, iTerm) == -1)
                Bac_NtkCreateAndConnectBuffer(NULL, NULL, pNtk, iTerm);
        Bac_NtkForEachPo(pNtk, iTerm, k) if (pNtk != pRoot && Bac_ObjFanin(pNtk, iTerm) == -1)
            Bac_NtkCreateAndConnectBuffer(NULL, NULL, pNtk, iTerm);
    }
    // create node and connect POs
    Gia_ManForEachPo(pGia, pObj, i) if (Bac_ObjFanin(pRoot, Bac_NtkPo(pRoot, i)) == -1) // not a feedthrough
...
void Bac_NtkPrepareLibrary(Bac_Man_t* p, Mio_Library_t* pLib) {
    ...
    Mio_LibraryForEachGate(pLib, pGate) if (pGate != pGate0 && pGate != pGate1 && pGate != pGate2)
...
void Bac_NtkInsertNtk(Bac_Man_t* p, Abc_Ntk_t* pNtk) {
    ...
    Bac_ManForEachNtk(p, pCbaNtk, i) {
        Bac_NtkForEachBox(pCbaNtk, iBox)
            Bac_BoxForEachBi(pCbaNtk, iBox, iTerm, j) if (Bac_ObjFanin(pCbaNtk, iTerm) == -1)
                Bac_NtkCreateOrConnectFanin(NULL, pCbaNtk, iTerm);
        Bac_NtkForEachPo(pCbaNtk, iTerm, k) if (pCbaNtk != pRoot && Bac_ObjFanin(pCbaNtk, iTerm) == -1)
            Bac_NtkCreateOrConnectFanin(NULL, pCbaNtk, iTerm);
    }
    // create node and connect POs
    Abc_NtkForEachPo(pNtk, pObj, i) if (Bac_ObjFanin(pRoot, Bac_NtkPo(pRoot, i)) == -1) // not a feedthrough
