...
void Bac_PtrDumpBoxBlif(FILE* pFile, Vec_Ptr_t* vBox) {
    ...
    Vec_PtrForEachEntryStart(char*, vBox, pName, i, 2)
        fprintf(pFile, " %s=%s", pName, (char*)Vec_PtrEntry(vBox, i + 1)),
        i++;
    fprintf(pFile, "\n");
}
...
void Bac_PtrDumpBoxVerilog(FILE* pFile, Vec_Ptr_t* vBox) {
    ...
    Vec_PtrForEachEntryStart(char*, vBox, pName, i, 2)
        fprintf(pFile, ".%s(%s)%s", pName, (char*)Vec_PtrEntry(vBox, i + 1), i < Vec_PtrSize(vBox) - 2 ? ", " : ""),
        i++;
    fprintf(pFile, ");\n");
}
