...
DdNode* Abc_ConvertSopToBdd(DdManager* dd, char* pSop, DdNode** pbVars) {
    ...
    if (Abc_SopIsExorType(pSop)) {
        for (v = 0; v < nVars; v++) {
            bSum = Cudd_bddXor(dd, bTemp = bSum, pbVars ? pbVars[v] : Cudd_bddIthVar(dd, v));
            Cudd_Ref(bSum);
...
char* Abc_ConvertBddToSop(Mem_Flex_t* pMan, DdManager* dd, DdNode* bFuncOn, DdNode* bFuncOnDc, int nFanins, int fAllPrimes, Vec_Str_t* vCube, int fMode) {
    ...
    if (fMode == -1) { // try both phases
        ...
        if (nCubes1 <= nCubes0) { // use positive polarity
            nCubes = nCubes1;
            zCover = zCover1;
            Cudd_RecursiveDerefZdd(dd, zCover0);
            fPhase = 1;
        } else { // use negative polarity
            nCubes = nCubes0;
            zCover = zCover0;
    ...
    // verify
    if (fVerify) {
        bFuncNew = Abc_ConvertSopToBdd(dd, pSop, NULL);
        Cudd_Ref(bFuncNew);
...
int Abc_NtkBddToSop(Abc_Ntk_t* pNtk, int fMode, int nCubeLimit, int fCubeSort) {
    ...
    if (nCubeLimit < ABC_INFINITY) {
        ...
        Abc_NtkForEachNode(pNtk, pNode, i) if (!Abc_ObjIsBarBuf(pNode))
...
void Abc_NtkLogicMakeDirectSops(Abc_Ntk_t* pNtk) {
    ...
    fFound = 0;
    Abc_NtkForEachNode(pNtk, pNode, i) if (!Abc_ObjIsBarBuf(pNode) && Abc_SopIsComplement((char*)pNode->pData)) {
        fFound = 1;
        break;
    }
    if (!fFound)
    ...
    vCube = Vec_StrAlloc(100);
    Abc_NtkForEachNode(pNtk, pNode, i) if (!Abc_ObjIsBarBuf(pNode) && Abc_SopIsComplement((char*)pNode->pData)) {
        bFunc = Abc_ConvertSopToBdd(dd, (char*)pNode->pData, NULL);
        Cudd_Ref(bFunc);
...
DdNode* Abc_ConvertAigToBdd(DdManager* dd, Hop_Obj_t* pRoot) {
    ...
    bFunc = Cudd_NotCond(Hop_Regular(pRoot)->pData, Hop_IsComplement(pRoot));
    Cudd_Ref(bFunc);
    // dereference BDD
...
Gia_Man_t* Abc_NtkAigToGia(Abc_Ntk_t* p, int fGiaSimple) {
    ...
    Abc_NtkForEachNode(p, pNode, i)
        nObjs
        += Abc_ObjIsBarBuf(pNode) ? 1 : Hop_DagSize((Hop_Obj_t*)pNode->pData);
    if (!fGiaSimple)
