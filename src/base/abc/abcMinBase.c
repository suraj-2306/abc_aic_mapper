...
int Abc_NodeMinimumBase_buggy(Abc_Obj_t* pNode) {
    ...
    pNode->pData = Extra_bddRemapUp((DdManager*)pNode->pNtk->pManFunc, bTemp = (DdNode*)pNode->pData);
...
int Abc_NodeMinimumBase(Abc_Obj_t* pNode) {
    ...
    Vec_IntForEachEntry(&pNode->vFanins, iFanin, i) {
        ...
        Vec_IntForEachEntryStop(&pNode->vFanins, iFanin2, j, k) if (iFanin == iFanin2) break;
        fDupFanins |= (int)(j < k);
        if (j == k)
    ...
    Abc_ObjForEachFanin(pNode, pFanin2, i) {
        Abc_ObjForEachFanin(pNode, pFanin1, k) {
            ...
            if (pFanin1 == pFanin2) {
                ...
                DdNode *bTrans, *bTemp;
                bTrans = Cudd_bddXnor(dd, bVar1, bVar2);
                Cudd_Ref(bTrans);
                pNode->pData = Cudd_bddAndAbstract(dd, bTemp = (DdNode*)pNode->pData, bTrans, bVar2);
                Cudd_Ref((DdNode*)pNode->pData);
                Cudd_RecursiveDeref(dd, bTemp);
...
int Abc_NodeCheckDupFanin(Abc_Obj_t* pFanin, Abc_Obj_t* pFanout, int* piFanin) {
    Abc_Obj_t* pObj;
    int i, Counter = 0;
    Abc_ObjForEachFanin(pFanout, pObj, i) if (pObj == pFanin) {
        if (piFanin)
            *piFanin = i;
        Counter++;
    }
...
int Abc_ObjFaninNumberNew(Vec_Ptr_t* vFanins, Abc_Obj_t* pFanin) {
    Abc_Obj_t* pObj;
    int i;
    Vec_PtrForEachEntry(Abc_Obj_t*, vFanins, pObj, i) if (pObj == pFanin) return i;
    return -1;
}
...
DdNode* Abc_NodeCollapseFunc(Abc_Obj_t* pFanin, Abc_Obj_t* pFanout, Vec_Ptr_t* vFanins, int* pPermFanin, int* pPermFanout) {
    ...
    bFunc0 = Cudd_Cofactor(dd, (DdNode*)pFanout->pData, Cudd_Not(bVar));
    Cudd_Ref(bFunc0);
    bFunc1 = Cudd_Cofactor(dd, (DdNode*)pFanout->pData, bVar);
    Cudd_Ref(bFunc1);
    // find the permutation after collapsing
    bFunc0 = Cudd_bddPermute(dd, bTemp = bFunc0, pPermFanout);
    Cudd_Ref(bFunc0);
    Cudd_RecursiveDeref(dd, bTemp);
    bFunc1 = Cudd_bddPermute(dd, bTemp = bFunc1, pPermFanout);
    Cudd_Ref(bFunc1);
    Cudd_RecursiveDeref(dd, bTemp);
    bFanin = Cudd_bddPermute(dd, (DdNode*)pFanin->pData, pPermFanin);
    Cudd_Ref(bFanin);
    // create the new function
    bFanout = Cudd_bddIte(dd, bFanin, bFunc1, bFunc0);
    Cudd_Ref(bFanout);
...
int Abc_NtkEliminate(Abc_Ntk_t* pNtk, int nMaxSize, int fReverse, int fVerbose) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pNode, i) {
        ...
        if (Abc_ObjFaninNum(pNode) > nMaxSize)
            continue;
        Abc_ObjForEachFanout(pNode, pFanout, k) if (Abc_NodeCollapseSuppSize(pNode, pFanout, vFanins) > nMaxSize) break;
...
int Abc_NodeCountAppearancesAll(Abc_Obj_t* pNode) {
    ...
    Abc_ObjForEachFanout(pNode, pFanout, i)
        Count
        += Abc_NodeCountAppearances(pNode, pFanout);
    return Count;
}
...
int Abc_NodeIsExor(Abc_Obj_t* pNode) {
    ...
    if (Truth == 0x6666666666666666 || Truth == 0x9999999999999999 || Truth == 0x9696969696969696 || Truth == 0x6969696969696969 || Truth == 0x6996699669966996 || Truth == 0x9669966996699669 || Truth == 0x9669699696696996 || Truth == 0x6996966969969669 || Truth == 0x6996966996696996 || Truth == 0x9669699669969669)
        return 1;
    return 0;
}
