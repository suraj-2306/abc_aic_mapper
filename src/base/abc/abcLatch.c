...
void Abc_NtkInsertLatchValues(Abc_Ntk_t* pNtk, Vec_Int_t* vValues) {
    ...
    Abc_NtkForEachLatch(pNtk, pLatch, i)
        pLatch->pData
        = (void*)(ABC_PTRINT_T)(vValues ? (Vec_IntEntry(vValues, i) ? ABC_INIT_ONE : ABC_INIT_ZERO) : ABC_INIT_DC);
}
...
void Abc_NtkConvertDcLatches(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachLatch(pNtk, pLatch, i) if (Abc_LatchIsInitDc(pLatch)) {
        fFound = 1;
        break;
    }
    if (!fFound)
...
int Abc_NtkVerifyCex(Abc_Ntk_t* pNtk, Abc_Cex_t* p) {
    ...
    for (i = 0; i <= p->iFrame; i++) {
        ...
        Abc_NtkForEachNode(pNtk, pObj, k)
            pObj->fMarkC
            = (Abc_ObjFanin0(pObj)->fMarkC ^ Abc_ObjFaninC0(pObj)) & (Abc_ObjFanin1(pObj)->fMarkC ^ Abc_ObjFaninC1(pObj));
        Abc_NtkForEachCo(pNtk, pObj, k)
            pObj->fMarkC
            = Abc_ObjFanin0(pObj)->fMarkC ^ Abc_ObjFaninC0(pObj);
        Abc_NtkForEachLatch(pNtk, pObj, k)
            Abc_ObjFanout0(pObj)
                ->fMarkC
            = Abc_ObjFanin0(pObj)->fMarkC;
    }
    assert(iBit == p->nBits);
