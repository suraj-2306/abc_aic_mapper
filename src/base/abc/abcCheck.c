...
int Abc_NtkDoCheck(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachObj(pNtk, pObj, i) if (!Abc_NtkCheckObj(pNtk, pObj)) return 0;
    ...
    if (Abc_NtkIsNetlist(pNtk)) {
        ...
        // check the nets
        Abc_NtkForEachNet(pNtk, pNet, i) if (!Abc_NtkCheckNet(pNtk, pNet)) return 0;
    } else {
        if (Abc_NtkNetNum(pNtk) != 0) {
    ...
    if (Abc_NtkIsStrash(pNtk)) {
        ...
        Abc_NtkForEachNode(pNtk, pNode, i) if (!Abc_NtkCheckNode(pNtk, pNode)) return 0;
    }

    // check the latches
    Abc_NtkForEachLatch(pNtk, pNode, i) if (!Abc_NtkCheckLatch(pNtk, pNode)) return 0;
