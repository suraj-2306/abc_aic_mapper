...
Abc_Ntk_t* Abc_NtkStrashBlifMv(Abc_Ntk_t* pNtk) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pObj, i) if (!Abc_NodeStrashBlifMv(pNtkNew, pObj)) {
    ...
    if (Abc_NtkLatchNum(pNtk)) {
        ...
        Abc_NtkForEachPi(pNtkNew, pObj, i) if (strncmp(Abc_ObjName(pObj), "free_var_", 9) == 0)
        ...
        Abc_NtkForEachCi(pNtkNew, pObj, i) if (strncmp(Abc_ObjName(pObj), "free_var_", 9) == 0)
            Vec_PtrPush(vTemp, pObj);
        Abc_NtkForEachCi(pNtkNew, pObj, i) if (strncmp(Abc_ObjName(pObj), "free_var_", 9) != 0)
            Vec_PtrPush(vTemp, pObj);
    ...
    // cleanup
    ABC_FREE(pBits);
    Abc_NtkForEachObj(pNtk, pObj, i) if (pObj->pCopy)
        ABC_FREE(pObj->pCopy);
...
Abc_Ntk_t* Abc_NtkSkeletonBlifMv(Abc_Ntk_t* pNtk) {
    ...
    if (Abc_NtkHasBlifMv(pNtk) && Abc_NtkMvVar(pNtk)) {
        ...
        Abc_NtkForEachNet(pNtk, pObj, i) if (pObj->pCopy)
