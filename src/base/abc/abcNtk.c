...
Abc_Ntk_t* Abc_NtkStartFrom(Abc_Ntk_t* pNtk, Abc_NtkType_t Type, Abc_NtkFunc_t Func) {
    ...
    if (pNtk->AndGateDelay != 0.0 && pNtk->pManTime != NULL && pNtk->ntkType != ABC_NTK_STRASH && Type == ABC_NTK_STRASH) {
        Abc_NtkForEachCi(pNtk, pObj, i)
            pObj->pCopy->Level
            = (int)(Abc_MaxFloat(0, Abc_NodeReadArrivalWorst(pObj)) / pNtk->AndGateDelay);
    }
...
Abc_Ntk_t* Abc_NtkDup(Abc_Ntk_t* pNtk) {
    ...
    if (Abc_NtkIsStrash(pNtk)) {
        ...
        Abc_AigForEachAnd(pNtk, pObj, i)
            pObj->pCopy
            = Abc_AigAnd((Abc_Aig_t*)pNtkNew->pManFunc, Abc_ObjChild0Copy(pObj), Abc_ObjChild1Copy(pObj));
        // relink the choice nodes
        Abc_AigForEachAnd(pNtk, pObj, i) if (pObj->pData)
            pObj->pCopy->pData
            = ((Abc_Obj_t*)pObj->pData)->pCopy;
        // relink the CO nodes
        Abc_NtkForEachCo(pNtk, pObj, i)
...
Abc_Ntk_t* Abc_NtkDupDfsNoBarBufs(Abc_Ntk_t* pNtk) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pObj, i) if (Abc_ObjIsBarBuf(pObj))
        pObj->pCopy
        = Abc_ObjFanin0(pObj)->pCopy;
    else Abc_NtkDupObj(pNtkNew, pObj, 0);
    Vec_PtrFree(vNodes);
    // reconnect all objects (no need to transfer attributes on edges)
    Abc_NtkForEachObj(pNtk, pObj, i) if (!Abc_ObjIsBox(pObj) && !Abc_ObjIsBo(pObj) && !Abc_ObjIsBarBuf(pObj))
        Abc_ObjForEachFanin(pObj, pFanin, k) if (pObj->pCopy && pFanin->pCopy)
            Abc_ObjAddFanin(pObj->pCopy, pFanin->pCopy);
    // duplicate the EXDC Ntk
    if (pNtk->pExdc)
...
Abc_Ntk_t* Abc_NtkRestrashWithLatches(Abc_Ntk_t* pNtk, int nLatches) {
    ...
    Abc_NtkForEachNode(pNtk, pObj, i)
        pObj->pCopy
        = Abc_AigAnd((Abc_Aig_t*)pNtkAig->pManFunc, Abc_ObjChild0Copy(pObj), Abc_ObjChild1Copy(pObj));
...
Abc_Ntk_t* Abc_NtkBottom(Abc_Ntk_t* pNtk, int Level) {
    ...
    // copy the internal nodes
    // duplicate the nets and nodes (CIs/COs/latches already dupped)
    Abc_NtkForEachObj(pNtk, pObj, i) if (pObj->pCopy == NULL && Abc_ObjIsNode(pObj) && Abc_ObjLevel(pObj) <= Level)
        Abc_NtkDupObj(pNtkNew, pObj, 0);
    // reconnect all objects (no need to transfer attributes on edges)
    Abc_NtkForEachObj(pNtk, pObj, i)
        Abc_ObjForEachFanin(pObj, pFanin, k) if (pObj->pCopy && pFanin->pCopy)
    ...
    Abc_NtkForEachObj(pNtk, pObj, i)
        Abc_ObjForEachFanin(pObj, pFanin, k) if (!pObj->pCopy && pFanin->pCopy && Abc_ObjIsNode(pFanin)) {
...
void Abc_NtkDelete(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachObj(pNtk, pObj, i) {
        ...
        if (!fWarning && (pObj->fMarkA || pObj->fMarkB || pObj->fMarkC)) { printf("Flags A, B, or C are not zero.\n"), fWarning = 1; }
    }
    // free the nodes
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, pNtk->vAttrs, pAttrMan, i) if (pAttrMan)
...
void Abc_NtkMakeComb(Abc_Ntk_t* pNtk, int fRemoveLatches) {
    ...
    if (fRemoveLatches) {
        ...
        Abc_NtkForEachCo(pNtk, pObj, i) if (Abc_ObjIsBi(pObj))
            Vec_PtrPush(vBos, pObj);
        else Vec_PtrPush(pNtk->vPos, pObj);
        // remove COs
        Vec_PtrFree(pNtk->vCos);
        pNtk->vCos = NULL;
        // remove the BOs
...
Abc_Ntk_t* Abc_NtkMakeOnePo(Abc_Ntk_t* pNtkInit, int Output, int nRange) {
    ...
    Abc_NtkForEachPo(pNtk, pNodePo, i) if (i < Output || i >= Output + nRange)
        Abc_NtkDeleteObjPo(pNodePo);
    else Vec_PtrPush(vPosLeft, pNodePo);
    // filter COs
...
Abc_Ntk_t* Abc_NtkNodeDup(Abc_Ntk_t* pNtkInit, int nLimit, int fVerbose) {
    ...
    do {
        Vec_PtrClear(vNodes);
        Abc_NtkForEachNode(pNtk, pObj, i) if (Abc_ObjFanoutNum(pObj) >= nLimit)
