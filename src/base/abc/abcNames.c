...
void Abc_NtkTrasferNamesNoLatches(Abc_Ntk_t* pNtk, Abc_Ntk_t* pNtkNew) {
    ...
    Abc_NtkForEachCi(pNtk, pObj, i) if (Abc_ObjFaninNum(pObj) == 0 || !Abc_ObjIsLatch(Abc_ObjFanin0(pObj)))
        Abc_ObjAssignName(pObj->pCopy, Abc_ObjName(Abc_ObjFanout0Ntk(pObj)), NULL);
    Abc_NtkForEachCo(pNtk, pObj, i) if (Abc_ObjFanoutNum(pObj) == 0 || !Abc_ObjIsLatch(Abc_ObjFanout0(pObj)))
        Abc_ObjAssignName(pObj->pCopy, Abc_ObjName(Abc_ObjFanin0Ntk(pObj)), NULL);
    Abc_NtkForEachBox(pNtk, pObj, i) if (!Abc_ObjIsLatch(pObj))
        Abc_ObjAssignName(pObj->pCopy, Abc_ObjName(pObj), NULL);
}
...
void Abc_NtkOrderObjsByName(Abc_Ntk_t* pNtk, int fComb) {
    ...
    Abc_NtkForEachPi(pNtk, pObj, i)
        pObj->pCopy
        = (Abc_Obj_t*)Abc_ObjName(pObj);
    Abc_NtkForEachPo(pNtk, pObj, i)
        pObj->pCopy
        = (Abc_Obj_t*)Abc_ObjName(pObj);
    Abc_NtkForEachBox(pNtk, pObj, i)
        pObj->pCopy
        = (Abc_Obj_t*)Abc_ObjName(Abc_ObjFanout0(pObj));
...
void Abc_NtkStartNameIds(Abc_Ntk_t* p) {
    ...
    Abc_NtkForEachCi(p, pObj, i)
        ...
        Vec_IntWriteEntry(p->vNameIds, Abc_ObjId(pObj), 2 * Counter++);
    // add outputs
    Abc_NtkForEachCo(p, pObj, i) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pObj, i) if (!Vec_IntEntry(p->vNameIds, Abc_ObjId(pObj)))
        fprintf(pFile, "%s            \n", Abc_ObjName(Abc_ObjFanout0(pObj))),
        Vec_IntWriteEntry(p->vNameIds, Abc_ObjId(pObj), 2 * Counter++);
    Vec_PtrFree(vNodes);
    fclose(pFile);
...
void Abc_NtkTransferNameIds(Abc_Ntk_t* p, Abc_Ntk_t* pNew) {
    ...
    //    printf( "\n" );
    Abc_NtkForEachObj(p, pObj, i) if (pObj->pCopy && i < Vec_IntSize(p->vNameIds) && Vec_IntEntry(p->vNameIds, i)) {
