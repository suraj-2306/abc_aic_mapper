...
int Abc_AigCleanup(Abc_Aig_t* pMan) {
    ...
    for (i = 0; i < pMan->nBins; i++)
        Abc_AigBinForEachEntry(pMan->pBins[i], pAnd) if (Abc_ObjFanoutNum(pAnd) == 0)
...
int Abc_AigCheck(Abc_Aig_t* pMan) {
    ...
    Abc_NtkForEachNode(pMan->pNtkAig, pObj, i) if (Abc_AigNodeIsChoice(pObj)) for (pAnd = (Abc_Obj_t*)pObj->pData; pAnd; pAnd = (Abc_Obj_t*)pAnd->pData) if (Abc_ObjFanoutNum(pAnd) > 0) {
...
int Abc_AigLevel(Abc_Ntk_t* pNtk) {
    ...
    LevelsMax = 0;
    Abc_NtkForEachCo(pNtk, pNode, i) if (LevelsMax < (int)Abc_ObjFanin0(pNode)->Level)
        LevelsMax
...
*/
    ...
    Abc_AigBinForEachEntry(pMan->pBins[Key], pAnd) if (p0 == Abc_ObjChild0(pAnd) && p1 == Abc_ObjChild1(pAnd)) {
...
Abc_Obj_t* Abc_AigXorLookup(Abc_Aig_t* pMan, Abc_Obj_t* p0, Abc_Obj_t* p1, int* pType) {
    ...
    if ((pNode1 = Abc_AigAndLookup(pMan, Abc_ObjNot(p0), Abc_ObjNot(p1))) && (pNode2 = Abc_AigAndLookup(pMan, p0, p1))) {
        pNode = Abc_AigAndLookup(pMan, Abc_ObjNot(pNode1), Abc_ObjNot(pNode2));
        if (pNode && pType) *pType = 1;
        return pNode;
    }
    // check the case of XOR(a,b) = OR(a'b, ab')
    if ((pNode1 = Abc_AigAndLookup(pMan, p0, Abc_ObjNot(p1))) && (pNode2 = Abc_AigAndLookup(pMan, Abc_ObjNot(p0), p1))) {
        pNode = Abc_AigAndLookup(pMan, Abc_ObjNot(pNode1), Abc_ObjNot(pNode2));
...
Abc_Obj_t* Abc_AigMuxLookup(Abc_Aig_t* pMan, Abc_Obj_t* pC, Abc_Obj_t* pT, Abc_Obj_t* pE, int* pType) {
    ...
    if ((pNode1 = Abc_AigAndLookup(pMan, pC, Abc_ObjNot(pT))) && (pNode2 = Abc_AigAndLookup(pMan, Abc_ObjNot(pC), Abc_ObjNot(pE)))) {
        pNode = Abc_AigAndLookup(pMan, Abc_ObjNot(pNode1), Abc_ObjNot(pNode2));
        if (pNode && pType) *pType = 1;
        return pNode;
    }
    // check the case of MUX(c,t,e) = OR(ct, c'e)
    if ((pNode1 = Abc_AigAndLookup(pMan, pC, pT)) && (pNode2 = Abc_AigAndLookup(pMan, Abc_ObjNot(pC), pE))) {
        pNode = Abc_AigAndLookup(pMan, Abc_ObjNot(pNode1), Abc_ObjNot(pNode2));
        return pNode ? Abc_ObjNot(pNode) : NULL;
    }
    return NULL;
...
void Abc_AigReplace_int(Abc_Aig_t* pMan, Abc_Obj_t* pOld, Abc_Obj_t* pNew, int fUpdateLevel) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, pMan->vNodes, pFanout, k) {
        ...
        if ((pFanoutNew = Abc_AigAndLookup(pMan, pFanin1, pFanin2))) { // such node exists (it may be a constant)
        ...
        Abc_ObjForEachFanout(pFanout, pFanoutFanout, v) if (Abc_AigNodeIsAnd(pFanoutFanout))
            pFanoutFanout->fExor
            = Abc_NodeIsExorType(pFanoutFanout);
    }
...
void Abc_AigDeleteNode(Abc_Aig_t* pMan, Abc_Obj_t* pNode) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, pMan->vStackReplaceOld, pTemp, i) if (pNode == pTemp) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, pMan->vStackReplaceNew, pTemp, i) if (pNode == Abc_ObjRegular(pTemp)) return;
    ...
    Vec_VecForEachLevel(pMan->vLevelsR, vVec, i) {
        ...
        Vec_PtrForEachEntry(Abc_Obj_t*, vVec, pNode, k) {
            ...
            Abc_ObjForEachFanin(pNode, pFanin, v) {
                ...
                LevelNew = 0;
                Abc_ObjForEachFanout(pFanin, pFanout, j) if (LevelNew < Abc_ObjReverseLevel(pFanout))
                    LevelNew
                    = Abc_ObjReverseLevel(pFanout);
                LevelNew += 1;
                assert(LevelNew > i);
...
void Abc_AigSetNodePhases(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachLatchOutput(pNtk, pObj, i)
        pObj->fPhase
        = Abc_LatchIsInit1(pObj);
    Abc_AigForEachAnd(pNtk, pObj, i)
        pObj->fPhase
        = (Abc_ObjFanin0(pObj)->fPhase ^ Abc_ObjFaninC0(pObj)) & (Abc_ObjFanin1(pObj)->fPhase ^ Abc_ObjFaninC1(pObj));
    Abc_NtkForEachPo(pNtk, pObj, i)
        pObj->fPhase
        = (Abc_ObjFanin0(pObj)->fPhase ^ Abc_ObjFaninC0(pObj));
