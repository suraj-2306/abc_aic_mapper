...
Vec_Ptr_t* Abc_NtkDfs(Abc_Ntk_t* pNtk, int fCollectAll) {
    ...
    if (fCollectAll) {
        Abc_NtkForEachNode(pNtk, pObj, i) if (!Abc_NodeIsTravIdCurrent(pObj))
...
Vec_Ptr_t* Abc_NtkDfsReverse(Abc_Ntk_t* pNtk) {
    ...
    if (!Abc_NtkIsStrash(pNtk)) {
        Abc_NtkForEachNode(pNtk, pObj, i) if (Abc_NodeIsConst(pObj))
            Vec_PtrPush(vNodes, pObj);
    }
    return vNodes;
}
...
Vec_Ptr_t* Abc_NtkDfsIterNodes(Abc_Ntk_t* pNtk, Vec_Ptr_t* vRoots) {
    ...
    vNodes = Vec_PtrAlloc(1000);
    vStack = Vec_PtrAlloc(1000);
    Vec_PtrForEachEntry(Abc_Obj_t*, vRoots, pObj, i) if (!Abc_NodeIsTravIdCurrent(Abc_ObjRegular(pObj)))
...
Vec_Ptr_t* Abc_NtkDfsHie(Abc_Ntk_t* pNtk, int fCollectAll) {
    ...
    // collect dangling nodes if asked to
    if (fCollectAll) {
        Abc_NtkForEachObj(pNtk, pObj, i) if (!Abc_NodeIsTravIdCurrent(pObj))
            Abc_NtkDfs_rec(pObj, vNodes);
    }
    return vNodes;
}
...
Vec_Ptr_t* Abc_NtkSupport(Abc_Ntk_t* pNtk) {
    ...
    // add unused CIs
    Abc_NtkForEachCi(pNtk, pNode, i) if (!Abc_NodeIsTravIdCurrent(pNode))
        Vec_PtrPush(vNodes, pNode);
...
int Abc_NtkLevel(Abc_Ntk_t* pNtk) {
    ...
    if (pNtk->pManTime == NULL || pNtk->AndGateDelay <= 0)
        ...
        Abc_NtkForEachCi(pNtk, pNode, i)
            pNode->Level
            = (int)(Abc_MaxFloat(0, Abc_NodeReadArrivalWorst(pNode)) / pNtk->AndGateDelay);
...
Vec_Ptr_t* Abc_AigGetLevelizedOrder(Abc_Ntk_t* pNtk, int fCollectCis) {
    ...
    Vec_PtrForEachEntryStart(Abc_Obj_t*, vLevels, pNode, i, !fCollectCis) for (; pNode; pNode = pNode->pCopy)
        Vec_PtrPush(vNodes, pNode);
    Vec_PtrFree(vLevels);
    return vNodes;
}
...
int Abc_ObjSugraphSize(Abc_Obj_t* pObj) {
    ...
    if (Abc_ObjFanoutNum(pObj) > 1)
        return 0;
    return 1 + Abc_ObjSugraphSize(Abc_ObjFanin0(pObj)) + Abc_ObjSugraphSize(Abc_ObjFanin1(pObj));
}
...
int Abc_NtkPrintSubraphSizes(Abc_Ntk_t* pNtk) {
    ...
    int i;
    assert(Abc_NtkIsStrash(pNtk));
    Abc_NtkForEachNode(pNtk, pObj, i) if (Abc_ObjFanoutNum(pObj) > 1 && !Abc_NodeIsExorType(pObj))
        printf("%d(%d) ", 1 + Abc_ObjSugraphSize(Abc_ObjFanin0(pObj)) + Abc_ObjSugraphSize(Abc_ObjFanin1(pObj)), Abc_ObjFanoutNum(pObj));
    printf("\n");
    return 1;
}
