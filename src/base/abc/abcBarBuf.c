...
int Abc_NtkCollectPiPos_rec(Abc_Obj_t* pNet, Vec_Ptr_t* vLiMaps, Vec_Ptr_t* vLoMaps) {
    ...
    if (Abc_ObjIsNode(pObj))
        Abc_ObjForEachFanin(pObj, pFanin, i)
            Counter
            += Abc_NtkCollectPiPos_rec(pFanin, vLiMaps, vLoMaps);
    if (Abc_ObjIsNode(pObj))
        return Counter;
    if (Abc_ObjIsBo(pObj))
    ...
    Abc_ObjForEachFanin(pObj, pFanin, i)
        Counter
        += Abc_NtkCollectPiPos_rec(Abc_ObjFanin0(pFanin), vLiMaps, vLoMaps);
...
Abc_Ntk_t* Abc_NtkFromBarBufs(Abc_Ntk_t* pNtkBase, Abc_Ntk_t* pNtk) {
    ...
    Vec_PtrForEachEntry(Abc_Ntk_t*, pNtkBase->pDesign->vModules, pTemp, i)
        pTemp->pCopy
        = Abc_NtkStartFrom(pTemp, pNtk->ntkType, pNtk->ntkFunc);
    Vec_PtrForEachEntry(Abc_Ntk_t*, pNtkBase->pDesign->vModules, pTemp, i)
