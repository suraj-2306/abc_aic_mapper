...
void Abc_DesCleanManPointer(Abc_Des_t* p, void* pMan) {
    ...
    if (p->pManFunc == pMan)
        p->pManFunc = NULL;
    Vec_PtrForEachEntry(Abc_Ntk_t*, p->vModules, pTemp, i) if (pTemp->pManFunc == pMan)
        pTemp->pManFunc
        = NULL;
}
...
Abc_Des_t* Abc_DesDup(Abc_Des_t* p) {
    ...
    Vec_PtrForEachEntry(Abc_Ntk_t*, p->vModules, pTemp, i)
        pTemp->pCopy->pAltView
        = pTemp->pAltView ? pTemp->pAltView->pCopy : NULL;
    // update box models
    Vec_PtrForEachEntry(Abc_Ntk_t*, p->vModules, pTemp, i)
        Abc_NtkForEachBox(pTemp, pObj, k) if (Abc_ObjIsWhitebox(pObj) || Abc_ObjIsBlackbox(pObj))
            pObj->pCopy->pData
        = Abc_ObjModel(pObj)->pCopy;
    return pNew;
}
...
Abc_Des_t* Abc_DesDupBlackboxes(Abc_Des_t* p, Abc_Ntk_t* pNtkSave) {
    ...
    Vec_PtrForEachEntry(Abc_Ntk_t*, p->vModules, pNtkTemp, i) if (Abc_NtkHasBlackbox(pNtkTemp))
...
Abc_Ntk_t* Abc_DesDeriveRoot(Abc_Des_t* p) {
    ...
    pNtk = (Abc_Ntk_t*)Vec_PtrEntry(p->vModules, 0);
    Vec_PtrClear(p->vModules);
    pNtk->pManFunc = p->pManFunc;
    p->pManFunc = NULL;
    return pNtk;
}
