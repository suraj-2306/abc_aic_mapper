...
void Abc_NtkFlattenHierarchyGia2_rec(Gia_Man_t* pNew, Abc_Ntk_t* pNtk, int* pCounter, Vec_Int_t* vBufs) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vDfs, pObj, i) {
        if (Abc_ObjIsNode(pObj))
            ...
            if (vBufs)
                Abc_ObjForEachFanin(pObj, pTerm, k)
                    Abc_ObjFanout0(Abc_NtkPi(pModel, k))
                        ->iTemp
                    = Gia_ManAppendBuf(pNew, Abc_ObjFanout0(Abc_NtkPi(pModel, k))->iTemp);
            Abc_NtkFlattenHierarchyGia2_rec(pNew, pModel, pCounter, vBufs);
            if (vBufs)
                Abc_ObjForEachFanout(pObj, pTerm, k)
                    Abc_ObjFanin0(Abc_NtkPo(pModel, k))
                        ->iTemp
                    = Gia_ManAppendBuf(pNew, Abc_ObjFanin0(Abc_NtkPo(pModel, k))->iTemp);
            Abc_ObjForEachFanout(pObj, pTerm, k) {
...
Gia_Man_t* Abc_NtkFlattenHierarchyGia2(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachPi(pNtk, pTerm, i)
        Abc_ObjFanout0(pTerm)
            ->iTemp
        = fUseInter ? Gia_ManAppendBuf(pNew, pTerm->iTemp) : pTerm->iTemp;
    ...
    Abc_NtkForEachPo(pNtk, pTerm, i)
        pTerm->iTemp
        = fUseInter ? Gia_ManAppendBuf(pNew, Abc_ObjFanin0(pTerm)->iTemp) : Abc_ObjFanin0(pTerm)->iTemp;
    Abc_NtkForEachPo(pNtk, pTerm, i)
...
void Gia_ManInsertOne(Abc_Ntk_t* pNtk, Abc_Ntk_t* pNew) {
    ...
    Abc_NtkForEachBox(pNtk, pBox, i)
        Abc_ObjForEachFanin(pBox, pObj, k) if (Abc_ObjFaninNum(pObj) == 0)
    ...
    Abc_NtkForEachPo(pNtk, pObj, i) if (Abc_ObjFaninNum(pObj) == 0)
...
void Abc_NtkInsertHierarchyGia(Abc_Ntk_t* pNtk, Abc_Ntk_t* pNew, int fVerbose) {
    ...
    Abc_NtkForEachPo(pNew, pObj, i)
        Abc_NtkPo(pNtk, i)
            ->pCopy
        = pObj;
    Abc_NtkForEachBarBuf(pNew, pObj, i)((Abc_Obj_t*)Vec_PtrEntry(vBuffers, k++))->pCopy = pObj;
    ...
    if (pNtk->pDesign)
        Vec_PtrForEachEntry(Abc_Ntk_t*, pNtk->pDesign->vModules, pModel, i) if (pModel != pNtk)
