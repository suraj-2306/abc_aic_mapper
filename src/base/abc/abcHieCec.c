...
*/
        ...
        // assign PIs
        Abc_ObjForEachFanin(pObj, pTerm, k)
            Abc_ObjFanout0(Abc_NtkPi(pNtkModel, k))
                ->iTemp
            = Abc_ObjFanin0(pTerm)->iTemp;
        // call recursively
        ...
        Abc_ObjForEachFanout(pObj, pTerm, k)
            Abc_ObjFanout0(pTerm)
                ->iTemp
            = Abc_ObjFanin0(Abc_NtkPo(pNtkModel, k))->iTemp;
    }
...
*/
        ...
        Abc_ObjForEachFanout(pObj, pTerm, k)
            Abc_ObjFanout0(pTerm)
                ->iTemp
            = Gia_ObjFanin0Copy(Gia_ManPo(pGiaBox, k));
    }
    // create POs
    Abc_NtkForEachPo(pNtk, pTerm, i)
        Gia_ManAppendCo(pGia, Abc_ObjFanin0(pTerm)->iTemp);
...
void Abc_NtkCollectHie_rec(Abc_Ntk_t* pNtk, Vec_Ptr_t* vModels) {
    ...
    if (pNtk->iStep >= 0)
        return;
    vOrder = Abc_NtkDfsBoxes(pNtk);
    Vec_PtrForEachEntry(Abc_Obj_t*, vOrder, pObj, i) if (Abc_ObjIsBox(pObj) && (Abc_Ntk_t*)pObj->pData != pNtk)
...
int Abc_NtkCountInst_rec(Abc_Ntk_t* pNtk) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vOrder, pObj, i) if (Abc_ObjIsBox(pObj) && (Abc_Ntk_t*)pObj->pData != pNtk)
        Counter
        += Abc_NtkCountInst_rec((Abc_Ntk_t*)pObj->pData);
    Vec_PtrFree(vOrder);
...
double Abc_NtkCountNodes_rec(Abc_Ntk_t* pNtk) {
    ...
    if (pNtk->dTemp >= 0)
        return pNtk->dTemp;
    vOrder = Abc_NtkDfsBoxes(pNtk);
    Vec_PtrForEachEntry(Abc_Obj_t*, vOrder, pObj, i) if (Abc_ObjIsNode(pObj))
        Counter++;
    else if (Abc_ObjIsBox(pObj) && (Abc_Ntk_t*)pObj->pData != pNtk)
        Counter
        += Abc_NtkCountNodes_rec((Abc_Ntk_t*)pObj->pData);
    Vec_PtrFree(vOrder);
...
int Abc_NtkCheckRecursive(Abc_Ntk_t* pNtk) {
    ...
    vMods = pNtk->pDesign->vModules;
    Vec_PtrForEachEntry(Abc_Ntk_t*, vMods, pModel, i) {
        Abc_NtkForEachObj(pModel, pObj, k) if (Abc_ObjIsBox(pObj) && pObj->pData == (void*)pModel) {
