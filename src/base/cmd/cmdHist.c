...
void Cmd_HistoryAddCommand(Abc_Frame_t* p, const char* command) {
    ...
    if (Len > 0 && Buffer[Len - 1] == '\n')
        Buffer[Len - 1] = 0;
    if (strlen(Buffer) > 3 && strncmp(Buffer, "set", 3) && strncmp(Buffer, "unset", 5) && strncmp(Buffer, "time", 4) && strncmp(Buffer, "quit", 4) && strncmp(Buffer, "alias", 5) && strncmp(Buffer, "source abc.rc", 13) && strncmp(Buffer, "source ..\\abc.rc", 16) && strncmp(Buffer, "history", 7) && strncmp(Buffer, "hi ", 3) && strcmp(Buffer, "hi") && Buffer[strlen(Buffer) - 1] != '?') {
        char* pStr = NULL;
        ...
        Vec_PtrForEachEntryStart(char*, p->aHistory, pStr, i, Start) if (!strcmp(pStr, Buffer)) break;
        if (i == Vec_PtrSize(p->aHistory)) { // add new entry
