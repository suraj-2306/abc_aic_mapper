...
Abc_Ntk_t* Abc_NtkDeriveFromBdd(void* dd0, void* bFunc, char* pNamePo, Vec_Ptr_t* vNamesPi) {
    ...
    bSupp = Cudd_Support(dd, (DdNode*)bFunc);
    Cudd_Ref(bSupp);
    ...
    pNode->pData = (DdNode*)Cudd_bddTransfer(dd, (DdManager*)pNtk->pManFunc, (DdNode*)bFunc);
...
int Abc_NtkBddToMuxesPerformGlo(Abc_Ntk_t* pNtk, Abc_Ntk_t* pNtkNew, int Limit, int fReorder, int fUseAdd) {
    ...
    Abc_NtkForEachCo(pNtk, pObj, i) {
        ...
        if (fUseAdd) {
            DdNode* aFunc = Cudd_BddToAdd(dd, bFunc);
            Cudd_Ref(aFunc);
...
void* Abc_NtkBuildGlobalBdds(Abc_Ntk_t* pNtk, int nBddSizeMax, int fDropInternal, int fReorder, int fReverse, int fVerbose) {
    ...
    Abc_NtkForEachCi(pNtk, pObj, i) if (Abc_ObjFanoutNum(pObj) > 0) {
    ...
    Abc_NtkForEachCo(pNtk, pObj, i) {
        ...
        if (bFunc == NULL) {
            ...
            // reset references
            Abc_NtkForEachObj(pNtk, pObj, i) if (!Abc_ObjIsBox(pObj) && !Abc_ObjIsBi(pObj))
                pObj->vFanouts.nSize
                = 0;
...
DdNode* Abc_NodeGlobalBdds_rec(DdManager* dd, Abc_Obj_t* pNode, int nBddSizeMax, int fDropInternal, ProgressBar* pProgress, int* pCounter, int fVerbose) {
    ...
    if (Abc_ObjGlobalBdd(pNode) == NULL) {
        ...
        if (fDetectMuxes && Abc_ObjGlobalBdd(pNode0) == NULL && Abc_ObjGlobalBdd(pNode1) == NULL && Abc_ObjIsNode(pNode0) && Abc_ObjFanoutNum(pNode0) == 1 && Abc_ObjIsNode(pNode1) && Abc_ObjFanoutNum(pNode1) == 1 && Abc_NodeIsMuxType(pNode)) {
            // deref the fanins
            ...
            bFunc = Cudd_bddIte(dd, bFuncC, bFunc1, bFunc0);
            Cudd_Ref(bFunc);
...
void Abc_NtkBddImplicationTest() {
    ...
    for (i = 0; i < nImps; i++) {
        printf(".");
        bImp = Cudd_bddAnd(dd, dd->vars[rand() % nVars], dd->vars[rand() % nVars]);
        Cudd_Ref(bImp);
        bSum = Cudd_bddOr(dd, bTemp = bSum, bImp);
        Cudd_Ref(bSum);
