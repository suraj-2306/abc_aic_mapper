...
DdNode* Abc_ResBuildBdd(Abc_Ntk_t* pNtk, DdManager* dd) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pObj, i) {
        if (Abc_ObjFaninNum(pObj) == 0) {
            bFunc = Cudd_NotCond(Cudd_ReadOne(dd), Abc_SopIsConst0((char*)pObj->pData));
            Cudd_Ref(bFunc);
        ...
        bFunc = Abc_ConvertSopToBdd(dd, (char*)pObj->pData, (DdNode**)Vec_PtrArray(vLocals));
        Cudd_Ref(bFunc);
        Vec_PtrWriteEntry(vBdds, Abc_ObjId(pObj), bFunc);
    }
    Vec_PtrFree(vLocals);
    ...
    Abc_NtkForEachCo(pNtk, pObj, i) {
        ...
        bPart = Cudd_bddXnor(dd, bTemp, bVar);
        Cudd_Ref(bPart);
        bFunc = Cudd_bddAnd(dd, bTemp = bFunc, bPart);
        Cudd_Ref(bFunc);
...
int Abc_ResCofCount(DdManager* dd, DdNode* bFunc, unsigned uMask, int* pCheck) {
    ...
    for (i = 0; i < (1 << nVars); i++) {
        ...
        Cudd_Ref(bCube);
        bCof = Cudd_Cofactor(dd, bFunc, bCube);
        Cudd_Ref(bCof);
        Cudd_RecursiveDeref(dd, bCube);
        Vec_PtrForEachEntry(DdNode*, vCofs, bTemp, k) if (bTemp == bCof) break;
...
void Abc_NtkExploreCofs(DdManager* dd, DdNode* bFunc, DdNode** pbVars, int nIns, int nLutSize) {
    ...
    bFuncNew = Cudd_bddTransfer(dd, ddNew, bFunc);
    Cudd_Ref(bFuncNew);
...
DdNode* Abc_NtkBddToAdd_rec(DdManager* dd, DdNode* bFunc, int nOuts, stmm_table* tTable, int fCompl) {
    ...
    if ((int)bFunc->index >= Cudd_ReadSize(dd) - nOuts) {
        ...
        aFunc = Abc_NtkBddFindAddConst(dd, Cudd_NotCond(bFunc, fCompl), nOuts);
        Cudd_Ref(aFunc);
    } else {
        ...
        aFunc = Cudd_addIte(dd, Cudd_addIthVar(dd, bFunc->index), aFunc1, aFunc0);
        Cudd_Ref(aFunc);
    }
    return (*ppSlot = aFunc);
}
...
DdNode* Abc_NtkAddToBdd_rec(DdManager* dd, DdNode* aFunc, int nIns, int nOuts, stmm_table* tTable) {
    ...
    if (Cudd_IsConstant(aFunc)) {
        ...
        bFunc = Extra_bddBitsToCube(dd, (int)Cudd_V(aFunc), nOuts, dd->vars + nIns, 1);
        Cudd_Ref(bFunc);
    } else {
        ...
        bFunc = Cudd_bddIte(dd, Cudd_bddIthVar(dd, aFunc->index), bFunc1, bFunc0);
        Cudd_Ref(bFunc);
    }
    return (*ppSlot = bFunc);
}
...
DdNode* Abc_NtkBddDecCharFunc(DdManager* dd, DdNode** pFuncs, int nOuts, int Mask, int nBits) {
    ...
    for (i = 0; i < nOuts; i++) {
        ...
        bExor = Cudd_bddXor(dd, pFuncs[i], bVar);
        Cudd_Ref(bExor);
        bFunc = Cudd_bddAnd(dd, bTemp = bFunc, Cudd_Not(bExor));
        Cudd_Ref(bFunc);
        Cudd_RecursiveDeref(dd, bTemp);
...
DdNode* Abc_NtkBddDecTry(reo_man* pReo, DdManager* dd, DdNode** pFuncs, int nIns, int nOuts, int Mask, int nBits) {
    ...
    bFunc = Abc_NtkBddDecCharFunc(dd, pFuncs, nOuts, Mask, nBits);
    Cudd_Ref(bFunc);
    /*
    // transfer to ADD
...
Abc_Ntk_t* Abc_NtkCreateFromCharFunc(Abc_Ntk_t* pNtk, DdManager* dd, DdNode* bFunc) {
    ...
    // transfer BDD
    pNodeNew->pData = Extra_TransferLevelByLevel(dd, (DdManager*)pNtkNew->pManFunc, bFunc);
...
Abc_Ntk_t* Abc_NtkBddDec(Abc_Ntk_t* pNtk, int fVerbose) {
    ...
    bFunc = Abc_NtkBddDecInt(pReo, dd, pFuncs, Abc_NtkCiNum(pNtk), Abc_NtkCoNum(pNtk));
    Cudd_Ref(bFunc);
