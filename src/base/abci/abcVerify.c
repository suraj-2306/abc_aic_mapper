...
*/
    ...
    Abc_NtkForEachCi(pNtk, pNode, i)
        pNode->pCopy
        = (Abc_Obj_t*)(ABC_PTRINT_T)pModel[i];
    ...
    Abc_NtkForEachCo(pNtk, pNode, i)
        pValues[i]
        = ((int)(ABC_PTRINT_T)Abc_ObjFanin0(pNode)->pCopy) ^ (int)Abc_ObjFaninC0(pNode);
    if (fStrashed)
        Abc_NtkDelete(pNtk);
    return pValues;
}
...
void Abc_NtkGetSeqPoSupp(Abc_Ntk_t* pNtk, int iFrame, int iNumPo) {
    ...
    Abc_NtkForEachCi(pNtk, pObj, i)
        pObj->pCopy
        = NULL;
    Abc_NtkForEachLatch(pNtk, pObj, i) if (Abc_NtkBox(pFrames, i)->pCopy)
        pObj->pCopy
        = (Abc_Obj_t*)1;
    Abc_NtkForEachPi(pNtk, pObj, i) for (k = 0; k <= iFrame; k++) if (Abc_NtkPi(pFrames, k * Abc_NtkPiNum(pNtk) + i)->pCopy)
        pObj->pCopy
        = (Abc_Obj_t*)1;
    // free stuff
    Vec_PtrFree(vSupp);
...
void Abc_NtkVerifyReportErrorSeq(Abc_Ntk_t* pNtk1, Abc_Ntk_t* pNtk2, int* pModel, int nFrames) {
    ...
    printf("PIs: ");
    Abc_NtkForEachPi(pNtk1, pObj, i) if (pObj->pCopy)
        printf("%s ", Abc_ObjName(pObj));
    printf("\n");
    printf("Latches: ");
    Abc_NtkForEachLatch(pNtk1, pObj, i) if (pObj->pCopy)
        printf("%s ", Abc_ObjName(pObj));
    printf("\n");

    printf("The cone of influence of output %s in Network2:\n", Abc_ObjName(pObjError));
    printf("PIs: ");
    Abc_NtkForEachPi(pNtk2, pObj, i) if (pObj->pCopy)
        printf("%s ", Abc_ObjName(pObj));
    printf("\n");
    printf("Latches: ");
    Abc_NtkForEachLatch(pNtk2, pObj, i) if (pObj->pCopy)
        printf("%s ", Abc_ObjName(pObj));
    printf("\n");
    ...
    for (i = 0; i <= iFrameError; i++) {
        ...
        printf("PI(1):");
        Abc_NtkForEachPi(pNtk1, pObj, k) if (pObj->pCopy)
            printf("%d", Sim_SimInfoGet(vInfo1, pObj)[i] > 0);
        printf(" ");
        printf("L(1):");
        Abc_NtkForEachLatch(pNtk1, pObj, k) if (pObj->pCopy)
            printf("%d", Sim_SimInfoGet(vInfo1, pObj)[i] > 0);
        printf(" ");
        ...
        printf("PI(2):");
        Abc_NtkForEachPi(pNtk2, pObj, k) if (pObj->pCopy)
            printf("%d", Sim_SimInfoGet(vInfo2, pObj)[i] > 0);
        printf(" ");
        printf("L(2):");
        Abc_NtkForEachLatch(pNtk2, pObj, k) if (pObj->pCopy)
            printf("%d", Sim_SimInfoGet(vInfo2, pObj)[i] > 0);
        printf(" ");
...
void Abc_NtkSimulteBuggyMiter(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachLatch(pNtk, pObj, i)
        pModel1[Abc_NtkPiNum(pNtk) + i]
        = ((int)(ABC_PTRINT_T)pObj->pData) - 1;
    ...
    Abc_NtkForEachPi(pNtk, pObj, i)
        ...
        = vPiValues2[i] - '0';
    Abc_NtkForEachLatch(pNtk, pObj, i)
        pModel2[Abc_NtkPiNum(pNtk) + i]
        = pResult1[Abc_NtkPoNum(pNtk) + i];
