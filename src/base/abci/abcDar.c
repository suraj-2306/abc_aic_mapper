...
Aig_Man_t* Abc_NtkToDarBmc(Abc_Ntk_t* pNtk, Vec_Int_t** pvMap) {
    ...
    Abc_NtkForEachCi(pNtk, pObj, i)
        pObj->pCopy
        = (Abc_Obj_t*)Aig_ObjCreateCi(pMan);
    // create flops
    Abc_NtkForEachLatch(pNtk, pObj, i)
        Abc_ObjFanout0(pObj)
            ->pCopy
        = Abc_ObjNotCond(Abc_ObjFanout0(pObj)->pCopy, Abc_LatchIsInit1(pObj));
    // copy internal nodes
    Abc_NtkForEachNode(pNtk, pObj, i)
        pObj->pCopy
        = (Abc_Obj_t*)Aig_And(pMan, (Aig_Obj_t*)Abc_ObjChild0Copy(pObj), (Aig_Obj_t*)Abc_ObjChild1Copy(pObj));
    // create the POs
...
Abc_Ntk_t* Abc_NtkFromDar(Abc_Ntk_t* pNtkOld, Aig_Man_t* pMan) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vNodes, pObj, i) if (Aig_ObjIsBuf(pObj))
        pObj->pData
        = (Abc_Obj_t*)Aig_ObjChild0Copy(pObj);
    else pObj->pData = Abc_AigAnd((Abc_Aig_t*)pNtkNew->pManFunc, (Abc_Obj_t*)Aig_ObjChild0Copy(pObj), (Abc_Obj_t*)Aig_ObjChild1Copy(pObj));
    Vec_PtrFree(vNodes);
...
Abc_Ntk_t* Abc_NtkFromMappedGia(Gia_Man_t* p, int fFindEnables, int fUseBuffs) {
    ...
    Gia_ManForEachPi(p, pObj, i)
        pObj->Value
        = Abc_ObjId(Abc_NtkCreatePi(pNtkNew));
    // create POs
    Gia_ManForEachPo(p, pObj, i)
        pObj->Value
        = Abc_ObjId(Abc_NtkCreatePo(pNtkNew));
    // create as many latches as there are registers in the manager
...
static inline Abc_Obj_t* Abc_NtkFromCellRead(Abc_Ntk_t* p, Vec_Int_t* vCopyLits, int i, int c) {
    ...
    if (i == 0)
        ...
        iObjNew = Vec_IntEntry(vCopyLits, Abc_Var2Lit(i, !c));
        assert(iObjNew >= 0);
...
Abc_Ntk_t* Abc_NtkFromCellMappedGia(Gia_Man_t* p, int fUseBuffs) {
    ...
    Gia_ManForEachCo(p, pObj, i) if (Gia_ObjFaninId0p(p, pObj) == 0)
        fNeedConst[Gia_ObjFaninC0(pObj)]
        = 1;
    Gia_ManForEachBuf(p, pObj, i) if (Gia_ObjFaninId0p(p, pObj) == 0)
        fNeedConst[Gia_ObjFaninC0(pObj)]
        = 1;
    if (fNeedConst[0])
...
Abc_Ntk_t* Abc_NtkFromDarChoices(Abc_Ntk_t* pNtkOld, Aig_Man_t* pMan) {
    ...
    if (0) {
        Abc_Obj_t* pNode;
        Abc_NtkForEachNode(pNtkNew, pNode, i) if (Abc_AigNodeIsChoice(pNode)) {
            int Counter = 0;
        ...
        Saig_ManForEachPo(pMan, pObjPo, i) {
            ...
            if (pTemp->pSeqModel) {
                if (pPars->fDropSatOuts) {
                    ...
                    Abc_Cex_t* pCex;
                    pCex = pMan->pSeqModel = pTemp->pSeqModel;
                    pTemp->pSeqModel = NULL;
                    pCex->iPo = i;
                    Aig_ManStop(pTemp);
                    break;
...
Abc_Ntk_t* Amap_ManProduceNetwork(Abc_Ntk_t* pNtk, Vec_Ptr_t* vMapping) {
    ...
    Vec_PtrForEachEntry(Amap_Out_t*, vMapping, pRes, i) if (pRes->pName && Mio_LibraryReadGateByName(pLib, pRes->pName, NULL) == NULL) {
