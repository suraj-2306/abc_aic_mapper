...
static inline void Abc_ObjSetIvy2Abc(Ivy_Man_t* p, int IvyId, Abc_Obj_t* pObjAbc) {
    assert(Vec_PtrEntry((Vec_Ptr_t*)p->pCopy, IvyId) == NULL);
    assert(!Abc_ObjIsComplement(pObjAbc));
    Vec_PtrWriteEntry((Vec_Ptr_t*)p->pCopy, IvyId, pObjAbc);
}
