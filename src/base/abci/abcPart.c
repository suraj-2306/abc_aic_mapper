...
Vec_Ptr_t* Abc_NtkDfsNatural(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachNode(pNtk, pObj, i) {
        ...
        Abc_ObjForEachFanin(pObj, pNext, k) if (Abc_ObjIsCi(pNext) && !Abc_NodeIsTravIdCurrent(pNext)) {
        ...
        Abc_ObjForEachFanout(pObj, pNext, k) if (Abc_ObjIsCo(pNext) && !Abc_NodeIsTravIdCurrent(pNext)) {
            Abc_NodeSetTravIdCurrent(pNext);
            Vec_PtrPush(vNodes, pNext);
        }
    }
...
Hop_Man_t* Abc_NtkPartStartHop(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachCi(pNtk, pObj, i)
        pObj->pNext
        = (Abc_Obj_t*)Hop_ObjCreatePi(pMan);
