...
void Abc_NtkGenFaultList(Abc_Ntk_t* pNtk, char* pFileName, int fStuckAt) {
    ...
    Abc_NtkForEachNode(pNtk, pObj, i) {
        ...
        Mio_LibraryForEachGate(pLib, pGate) if (pGate != pGateObj && Mio_GateReadPinNum(pGate) == nInputs)
            fprintf(pFile, "%d %s %s\n", Count, Abc_ObjName(pObj), Mio_GateReadName(pGate)),
            Count++;
    }
...
static inline void Abc_NtkFinSimOneWord(Abc_Obj_t* pObj, int Type, Vec_Wrd_t* vSims, int nWords) {
    if (Abc_NtkIsMappedLogic(pObj->pNtk)) {
        ...
        if (Type == ABC_FIN_SA0)
            for (w = 0; w < nWords; w++)
                pSim[w] = 0;
        else if (Type == ABC_FIN_SA1)
            for (w = 0; w < nWords; w++)
                pSim[w] = ~((word)0);
        else if (Type == ABC_FIN_RDOB_BUFF)
            for (w = 0; w < nWords; w++)
                pSim[w] = pSim0[w];
        else if (Type == ABC_FIN_RDOB_NOT)
            for (w = 0; w < nWords; w++)
                pSim[w] = ~pSim0[w];
        else if (Type == ABC_FIN_RDOB_AND)
            for (w = 0; w < nWords; w++)
                pSim[w] = pSim0[w] & pSim1[w];
        else if (Type == ABC_FIN_RDOB_OR)
            for (w = 0; w < nWords; w++)
                pSim[w] = pSim0[w] | pSim1[w];
        else if (Type == ABC_FIN_RDOB_XOR)
            for (w = 0; w < nWords; w++)
                pSim[w] = pSim0[w] ^ pSim1[w];
        else if (Type == ABC_FIN_RDOB_NAND)
            for (w = 0; w < nWords; w++)
                pSim[w] = ~(pSim0[w] & pSim1[w]);
        else if (Type == ABC_FIN_RDOB_NOR)
            for (w = 0; w < nWords; w++)
                pSim[w] = ~(pSim0[w] | pSim1[w]);
        else if (Type == ABC_FIN_RDOB_NXOR)
            for (w = 0; w < nWords; w++)
                pSim[w] = ~(pSim0[w] ^ pSim1[w]);
        else
            assert(0);
...
int Abc_NtkFinCheckTypesOk(Abc_Ntk_t* pNtk) {
    Abc_Obj_t* pObj;
    int i;
    Abc_NtkForEachNode(pNtk, pObj, i) if (Abc_ObjFinGateType(pObj) == ABC_FIN_NONE) return i;
    return 0;
}
