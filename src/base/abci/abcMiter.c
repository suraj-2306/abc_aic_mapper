...
void Abc_NtkMiterAddCone(Abc_Ntk_t* pNtk, Abc_Ntk_t* pNtkMiter, Abc_Obj_t* pRoot) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pNode, i) if (Abc_AigNodeIsAnd(pNode))
        pNode->pCopy
        = Abc_AigAnd((Abc_Aig_t*)pNtkMiter->pManFunc, Abc_ObjChild0Copy(pNode), Abc_ObjChild1Copy(pNode));
    Vec_PtrFree(vNodes);
}
...
void Abc_NtkAddFrame(Abc_Ntk_t* pNtkFrames, Abc_Ntk_t* pNtk, int iFrame) {
    ...
    Abc_AigForEachAnd(pNtk, pNode, i)
        pNode->pCopy
        = Abc_AigAnd((Abc_Aig_t*)pNtkFrames->pManFunc, Abc_ObjChild0Copy(pNode), Abc_ObjChild1Copy(pNode));
    // add the new POs
...
int Abc_NtkCombinePos(Abc_Ntk_t* pNtk, int fAnd, int fXor) {
    ...
    Abc_NtkForEachPo(pNtk, pNode, i) if (fAnd)
        pMiter
        = Abc_AigAnd((Abc_Aig_t*)pNtk->pManFunc, pMiter, Abc_ObjChild0(pNode));
    else if (fXor)
        pMiter
        = Abc_AigXor((Abc_Aig_t*)pNtk->pManFunc, pMiter, Abc_ObjChild0(pNode));
    else pMiter = Abc_AigOr((Abc_Aig_t*)pNtk->pManFunc, pMiter, Abc_ObjChild0(pNode));
...
Abc_Ntk_t* Abc_NtkSpecialMiter(Abc_Ntk_t* pNtk, Vec_Ptr_t* vNodes) {
    Abc_Ntk_t* pNtkNew;
    Abc_Obj_t *pObj, *pFanin, *pObjNew, *pPoNew;
    char* pName;
