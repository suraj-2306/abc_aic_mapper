...
Vec_Wrd_t* Abc_NtkShareSuperXor(Abc_Obj_t* pObj, int* pfCompl, int* pCounter) {
    ...
    while (Vec_WrdSize(vSuper) > 0) {
        ...
        fCompl ^= Abc_ObjIsComplement(pObjC);
        pObjC = Abc_ObjRegular(pObjC);
        fCompl ^= Abc_ObjIsComplement(pObj0);
...
void Abc_NtkTraverseSupers(Abc_ShaMan_t* p, int fAnd) {
    ...
    if (fAnd) {
        Abc_NtkForEachCo(p->pNtk, pObj, i) if (Abc_ObjIsNode(Abc_ObjFanin0(pObj)))
    ...
    nOnesMax = 0;
    Vec_PtrForEachEntry(Vec_Int_t*, vInputs, vInput, i) if (vInput)
        nOnesMax
        = Abc_MaxInt(nOnesMax, Vec_IntSize(vInput) - SHARE_NUM);
...
void Abc_NtkShareOptimize(Abc_ShaMan_t* p, int fAnd) {
    ...
    for (i = 0;; i++) {
        ...
        // save new node
        vOld1 = Vec_IntAlloc(16);
        Vec_IntPush(vOld1, Vec_IntEntry(vInput, 0));
        Vec_IntPush(vOld1, Vec_IntEntry(vInput, 1));
        vOld2 = Vec_IntAlloc(16);
        Vec_IntPush(vOld2, Vec_IntEntry(vInput2, 0));
        Vec_IntPush(vOld2, Vec_IntEntry(vInput2, 1));
        vNew = Vec_IntAlloc(16);
        Vec_IntPush(vNew, Abc_ObjToLit(pObj));
        Vec_IntPush(vNew, Abc_ObjLevel(Abc_ObjRegular(pObj)));
