...
void Abc_ManRewritePrintDivs(Vec_Ptr_t* vDivs, int nLeaves) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vDivs, pNode, i) {
        ...
        Vec_PtrForEachEntry(Abc_Obj_t*, vDivs, pFanin, k) if (Abc_ObjFanin0(pNode) == pFanin) break;
        if (k < nLeaves)
            printf("%c", 'a' + k);
        else
            printf("%d", k);
        ...
        Vec_PtrForEachEntry(Abc_Obj_t*, vDivs, pFanin, k) if (Abc_ObjFanin1(pNode) == pFanin) break;
        if (k < nLeaves)
            printf("%c", 'a' + k);
        else
            printf("%d", k);
