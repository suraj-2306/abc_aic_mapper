...
Abc_Ntk_t* Abc_NtkFromGlobalBdds(Abc_Ntk_t* pNtk, int fReverse) {
    ...
    if (pNtk->pExdc) {
        ...
        bBddDc = Cudd_bddTransfer(ddExdc, dd, bBddDc);
        Cudd_Ref(bBddDc);
        ...
        Abc_NtkForEachCo(pNtk, pNode, i) {
            ...
            bBddL = Cudd_bddAnd(dd, bBddMin, Cudd_Not(bBddDc));
            Cudd_Ref(bBddL);
            bBddU = Cudd_bddAnd(dd, Cudd_Not(bBddMin), Cudd_Not(bBddDc));
            Cudd_Ref(bBddU);
            Cudd_RecursiveDeref(dd, bBddMin);
            // compute new one
            bBddMin = Cudd_bddIsop(dd, bBddL, Cudd_Not(bBddU));
            Cudd_Ref(bBddMin);
...
int Abc_NtkCollapseReduce(Vec_Str_t* vSop, Vec_Int_t* vSupp, Vec_Int_t* vClass, Vec_Wec_t* vSupps) {
    ...
    Abc_NtkSopForEachCube(pSop, nVars, pCube) for (k = 0; k < nVars; k++) if (pCube[k] != '-')
    ...
    // reduce cubes
    Abc_NtkSopForEachCube(pSop, nVars, pCube) for (k = 0; k < nVars + 3; k++) if (k >= nVars || Vec_IntEntry(vPres, k))
    ...
    Vec_IntForEachEntry(vClass, iCo, i) {
        ...
        Vec_IntForEachEntry(vSupp, iVar, k) if (Vec_IntEntry(vPres, k))
...
sat_solver* Abc_NtkClpDeriveSatSolver(Cnf_Dat_t* pCnf, int iCoObjId, Vec_Int_t* vSupp, Vec_Int_t* vAnds, Vec_Int_t* vMap, sat_solver** ppSat1, sat_solver** ppSat2, sat_solver** ppSat3) {
    ...
    Vec_IntForEachEntry(vAnds, iObj, k) if (pCnf->pObj2Clause[iObj] != -1)
        Vec_IntWriteEntry(vMap, iObj, nVars++);
    //    Vec_IntForEachEntry( vSupp, iObj, k )
    //        Vec_IntWriteEntry( vMap, iObj, nVars++ );
