...
float Abc_NtkDelayTraceLut(Abc_Ntk_t* pNtk, int fUseLutLib) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pNode, i) {
        ...
        if (pLutLib == NULL) {
            Abc_ObjForEachFanin(pNode, pFanin, k) if (tArrival < Abc_ObjArrival(pFanin) + 1.0)
                tArrival
                = Abc_ObjArrival(pFanin) + 1.0;
        } else if (!pLutLib->fVarPinDelays) {
            pDelays = pLutLib->pLutDelays[Abc_ObjFaninNum(pNode)];
            Abc_ObjForEachFanin(pNode, pFanin, k) if (tArrival < Abc_ObjArrival(pFanin) + pDelays[0])
                tArrival
                = Abc_ObjArrival(pFanin) + pDelays[0];
        } else {
            pDelays = pLutLib->pLutDelays[Abc_ObjFaninNum(pNode)];
            if (fUseSorting) {
                Abc_NtkDelayTraceSortPins(pNode, pPinPerm, pPinDelays);
                Abc_ObjForEachFanin(pNode, pFanin, k) if (tArrival < Abc_ObjArrival(Abc_ObjFanin(pNode, pPinPerm[k])) + pDelays[k])
                    tArrival
                    = Abc_ObjArrival(Abc_ObjFanin(pNode, pPinPerm[k])) + pDelays[k];
            } else {
                Abc_ObjForEachFanin(pNode, pFanin, k) if (tArrival < Abc_ObjArrival(pFanin) + pDelays[k])
                    tArrival
                    = Abc_ObjArrival(pFanin) + pDelays[k];
            }
        }
    ...
    tArrival = -ABC_INFINITY;
    Abc_NtkForEachCo(pNtk, pNode, i) if (tArrival < Abc_ObjArrival(Abc_ObjFanin0(pNode)))
        tArrival
        = Abc_ObjArrival(Abc_ObjFanin0(pNode));

    // initialize the required times
    Abc_NtkForEachCo(pNtk, pNode, i) if (Abc_ObjRequired(Abc_ObjFanin0(pNode)) > tArrival)
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pNode, i) {
        if (pLutLib == NULL) {
            ...
            Abc_ObjForEachFanin(pNode, pFanin, k) if (Abc_ObjRequired(pFanin) > tRequired)
...
void Abc_NtkSpeedupNode(Abc_Ntk_t* pNtk, Abc_Ntk_t* pAig, Abc_Obj_t* pNode, Vec_Ptr_t* vLeaves, Vec_Ptr_t* vTimes) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vLeaves, pObj, i)
        Vec_PtrForEachEntry(Abc_Obj_t*, vLeaves, pObj2, k) if (i != k && Abc_ObjRegular(pObj->pCopy) == Abc_ObjRegular(pObj2->pCopy)) {
    ...
    for (i = 0; i < nCofs; i++) {
        ...
        Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pObj, k)
            pObj->pCopy
            = Abc_AigAnd((Abc_Aig_t*)pAig->pManFunc, Abc_ObjChild0Copy(pObj), Abc_ObjChild1Copy(pObj));
        // save the result
        pAnd = pNode->pCopy;
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vTimes, pObj, k) for (nSkip = (1 << k), i = 0; i < nCofs; i += 2 * nSkip) {
        pAnd = pObj->pCopy;
...
unsigned Abc_NtkDelayTraceTCEdges(Abc_Ntk_t* pNtk, Abc_Obj_t* pNode, float tDelta, int fUseLutLib) {
    ...
    if (pLutLib == NULL) {
        Abc_ObjForEachFanin(pNode, pFanin, k) if (tRequired < Abc_ObjArrival(pFanin) + 1.0 + tDelta)
            uResult
            |= (1 << k);
    } else if (!pLutLib->fVarPinDelays) {
        pDelays = pLutLib->pLutDelays[Abc_ObjFaninNum(pNode)];
        Abc_ObjForEachFanin(pNode, pFanin, k) if (tRequired < Abc_ObjArrival(pFanin) + pDelays[0] + tDelta)
            uResult
            |= (1 << k);
    } else {
        pDelays = pLutLib->pLutDelays[Abc_ObjFaninNum(pNode)];
        Abc_NtkDelayTraceSortPins(pNode, pPinPerm, pPinDelays);
        Abc_ObjForEachFanin(pNode, pFanin, k) if (tRequired < Abc_ObjArrival(Abc_ObjFanin(pNode, pPinPerm[k])) + pDelays[k] + tDelta)
            uResult
            |= (1 << pPinPerm[k]);
    }
    return uResult;
...
Abc_Ntk_t* Abc_NtkSpeedup(Abc_Ntk_t* pNtk, int fUseLutLib, int Percentage, int Degree, int fVerbose, int fVeryVerbose) {
    ...
    if (fVerbose) {
        ...
        Abc_NtkForEachNode(pNtk, pNode, i) {
            Abc_ObjForEachFanin(pNode, pFanin, k) if (!Abc_ObjIsCi(pFanin) && Abc_ObjSlack(pFanin) < tDelta)
                Counter++;
    ...
    Abc_NtkForEachNode(pNtk, pNode, i) {
        ...
        nTimeCris = 0;
        Abc_ObjForEachFanin(pNode, pFanin, k) if (!Abc_ObjIsCi(pFanin) && (puTCEdges[pNode->Id] & (1 << k)))
            nTimeCris++;
        ...
        if (nTimeCris) {
            Abc_ObjForEachFanin(pNode, pFanin, k) if (!Abc_ObjIsCi(pFanin) && (puTCEdges[pNode->Id] & (1 << k)))
                Abc_ObjForEachFanin(pFanin, pFanin2, k2) if (puTCEdges[pFanin->Id] & (1 << k2))
    ...
    Abc_AigForEachAnd(pNtkNew, pNode, i) if (pNode->pData) {
...
Vec_Int_t* Abc_NtkPowerEstimate(Abc_Ntk_t* pNtk, int fProbOne) {
    ...
    Abc_NtkForEachObj(pNtk, pObjAbc, i) if (Abc_ObjRegular((Abc_Obj_t*)pObjAbc->pTemp)->Type == ABC_FUNC_NONE)
        pObjAbc->pTemp
        = NULL;
...
unsigned Abc_NtkPowerCriticalEdges(Abc_Ntk_t* pNtk, Abc_Obj_t* pNode, float Limit, Vec_Int_t* vProbs) {
    ...
    unsigned uResult = 0;
    int k;
    Abc_ObjForEachFanin(pNode, pFanin, k) if (pProb[pFanin->Id] >= Limit)
        uResult
        |= (1 << k);
    return uResult;
}
...
*/
    ...
    Abc_NtkForEachNode(pNtk, pNode, i) {
        ...
        nTimeCris = 0;
        Abc_ObjForEachFanin(pNode, pFanin, k) if (!Abc_ObjIsCi(pFanin) && (puPCEdges[pNode->Id] & (1 << k)))
            nTimeCris++;
        if (!fVeryVerbose && nTimeCris == 0)
            continue;
        Counter++;
        ...
        if (nTimeCris) {
            Abc_ObjForEachFanin(pNode, pFanin, k) if (!Abc_ObjIsCi(pFanin) && (puPCEdges[pNode->Id] & (1 << k)))
                Abc_ObjForEachFanin(pFanin, pFanin2, k2) if (puPCEdges[pFanin->Id] & (1 << k2))
                    Vec_PtrPushUnique(vTimeCries, pFanin2);
        }
        //        if ( !fVeryVerbose && (Vec_PtrSize(vTimeCries) == 0 || Vec_PtrSize(vTimeCries) > Degree) )
        if ((Vec_PtrSize(vTimeCries) == 0 || Vec_PtrSize(vTimeCries) > Degree))
