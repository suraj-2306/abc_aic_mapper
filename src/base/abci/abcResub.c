...
void Abc_ManResubPrint(Abc_ManRes_t* p) {
    printf("Used constants    = %6d.             ", p->nUsedNodeC);
    ABC_PRT("Cuts  ", p->timeCut);
    printf("Used replacements = %6d.             ", p->nUsedNode0);
    ABC_PRT("Resub ", p->timeRes);
    printf("Used single ORs   = %6d.             ", p->nUsedNode1Or);
    ABC_PRT(" Div  ", p->timeDiv);
    printf("Used single ANDs  = %6d.             ", p->nUsedNode1And);
    ABC_PRT(" Mffc ", p->timeMffc);
    printf("Used double ORs   = %6d.             ", p->nUsedNode2Or);
    ABC_PRT(" Sim  ", p->timeSim);
    printf("Used double ANDs  = %6d.             ", p->nUsedNode2And);
    ABC_PRT(" 1    ", p->timeRes1);
    printf("Used OR-AND       = %6d.             ", p->nUsedNode2OrAnd);
    ABC_PRT(" D    ", p->timeResD);
    printf("Used AND-OR       = %6d.             ", p->nUsedNode2AndOr);
    ABC_PRT(" 2    ", p->timeRes2);
    printf("Used OR-2ANDs     = %6d.             ", p->nUsedNode3OrAnd);
    ABC_PRT("Truth ", p->timeTruth); //ABC_PRT( " 3    ", p->timeRes3 );
    printf("Used AND-2ORs     = %6d.             ", p->nUsedNode3AndOr);
    ABC_PRT("AIG   ", p->timeNtk);
    printf("TOTAL             = %6d.             ", p->nUsedNodeC + p->nUsedNode0 + p->nUsedNode1Or + p->nUsedNode1And + p->nUsedNode2Or + p->nUsedNode2And + p->nUsedNode2OrAnd + p->nUsedNode2AndOr + p->nUsedNode3OrAnd + p->nUsedNode3AndOr);
...
void Abc_ManResubPrintDivs(Abc_ManRes_t* p, Abc_Obj_t* pRoot, Vec_Ptr_t* vLeaves) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, p->vDivs, pNode, i) {
        ...
        Vec_PtrForEachEntry(Abc_Obj_t*, p->vDivs, pFanin, k) if (Abc_ObjFanin0(pNode) == pFanin) break;
        ...
        Vec_PtrForEachEntry(Abc_Obj_t*, p->vDivs, pFanin, k) if (Abc_ObjFanin1(pNode) == pFanin) break;
        if (k < Vec_PtrSize(vLeaves))
            printf("%c", 'a' + k);
        else
            printf("%d", k);
...
void Abc_ManResubSimulate(Vec_Ptr_t* vDivs, int nLeaves, Vec_Ptr_t* vSims, int nLeavesMax, int nWords) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vDivs, pObj, i) {
        if (i < nLeaves) { // initialize the leaf
...
int Abc_CutVolumeCheck_rec(Abc_Obj_t* pObj) {
    ...
    return 1 + Abc_CutVolumeCheck_rec(Abc_ObjFanin0(pObj)) + Abc_CutVolumeCheck_rec(Abc_ObjFanin1(pObj));
}
...
Vec_Ptr_t* Abc_CutFactorLarge(Abc_Obj_t* pNode, int nLeavesMax) {
    ...
    Vec_PtrForEachEntry(Vec_Ptr_t*, vFactors, vFact, i) if (vFact) Vec_PtrFree(vFact);
