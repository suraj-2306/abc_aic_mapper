...
int Abc_NodeGetLevel(Abc_Obj_t* pObj) {
    ...
    Abc_ObjForEachFanin(pObj, pFanin, i)
        Level
        = Abc_MaxInt(Level, (int)pFanin->Level);
    return Level + 1;
}
...
Abc_Obj_t* Abc_NodeSuperChoiceLut(Abc_ManScl_t* p, Abc_Obj_t* pObj) {
    ...
    while (Vec_PtrSize(p->vLeaves) > p->nLutSize)
        if (!Abc_NodeDecomposeStep(p)) {
            Vec_PtrForEachEntry(Abc_Obj_t*, p->vLeaves, pFanin, i) if (Abc_ObjIsNode(pFanin) && Abc_ObjFanoutNum(pFanin) == 0)
...
*/
    ...
    assert(((Abc_Obj_t*)Vec_PtrEntry(p->vLeaves, p->pBSet[0]))->Level <= ((Abc_Obj_t*)Vec_PtrEntry(p->vLeaves, p->pBSet[1]))->Level);
...
Abc_Ntk_t* Abc_NtkSpecialMapping(Abc_Ntk_t* pNtk, int fVerbose) {
    ...
    Abc_NtkForEachCo(pNtk, pObj, i) if (Abc_ObjFaninNum(Abc_ObjFanin0(pObj)) == 0)
        Abc_ObjFanin0(pObj)
            ->pCopy
        = Abc_NodeIsConst0(Abc_ObjFanin0(pObj)) ? Abc_NtkCreateNodeConst0(pNtkNew) : Abc_NtkCreateNodeConst1(pNtkNew);
    else Abc_NtkSpecialMap_rec(pNtkNew, Abc_ObjFanin0(pObj), vSupps, vCover);
