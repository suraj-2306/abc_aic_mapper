...
void* Abc_NtkToFraig(Abc_Ntk_t* pNtk, void* pParams, int fAllNodes, int fExdc) {
    ...
    Abc_NtkForEachCi(pNtk, pNode, i)
        pNode->pCopy
        = (Abc_Obj_t*)Fraig_ManReadIthVar(pMan, i);
...
void Abc_NtkFraigRemapUsingExdc(Fraig_Man_t* pMan, Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachNode(pNtk, pNode, i) if (pNode->pCopy) {
    ...
    Abc_NtkForEachNode(pNtk, pNode, i)
        pNode->pNext
        = (Abc_Obj_t*)Vec_PtrEntry(vNexts, pNode->Id);
    Vec_PtrFree(vNexts);
}
...
Abc_Ntk_t* Abc_NtkFromFraig2(Fraig_Man_t* pMan, Abc_Ntk_t* pNtk) {
    ...
    if (!stmm_find_or_add(tTable, (char*)Fraig_Regular(pNode->pCopy), (char***)&ppSlot))
        *ppSlot = pNode;
    Abc_NtkForEachCi(pNtk, pNode, i) if (!stmm_find_or_add(tTable, (char*)Fraig_Regular(pNode->pCopy), (char***)&ppSlot))* ppSlot = pNode;
    Abc_NtkForEachNode(pNtk, pNode, i) if (pNode->pCopy) {
        if (!stmm_find_or_add(tTable, (char*)Fraig_Regular(pNode->pCopy), (char***)&ppSlot))
            *ppSlot = pNode;
