...
int Abc_NtkExtractSequentialDcs(Abc_Ntk_t* pNtk, int fVerbose) {
    ...
    bRelation = Abc_NtkTransitionRelation(dd, pNtk, fVerbose);
    Cudd_Ref(bRelation);
    // create the initial state and the variable map
    bInitial = Abc_NtkInitStateAndVarMap(dd, pNtk, fVerbose);
    Cudd_Ref(bInitial);
    // compute the unreachable states
    bUnreach = Abc_NtkComputeUnreachable(dd, pNtk, bRelation, bInitial, fVerbose);
    Cudd_Ref(bUnreach);
...
DdNode* Abc_NtkTransitionRelation(DdManager* dd, Abc_Ntk_t* pNtk, int fVerbose) {
    ...
    Abc_NtkForEachLatch(pNtk, pNode, i) {
        ...
        bProd = Cudd_bddXnor(dd, bVar, (DdNode*)Abc_ObjGlobalBdd(Abc_ObjFanin0(pNode)));
        Cudd_Ref(bProd);
        bRel = Cudd_bddAnd(dd, bTemp = bRel, bProd);
        Cudd_Ref(bRel);
    ...
    bInputs = Extra_bddComputeRangeCube(dd, 0, Abc_NtkPiNum(pNtk));
    Cudd_Ref(bInputs);
    bRel = Cudd_bddExistAbstract(dd, bTemp = bRel, bInputs);
    Cudd_Ref(bRel);
    Cudd_RecursiveDeref(dd, bTemp);
...
DdNode* Abc_NtkInitStateAndVarMap(DdManager* dd, Abc_Ntk_t* pNtk, int fVerbose) {
    ...
    Abc_NtkForEachLatch(pNtk, pLatch, i) {
        ...
        bProd = Cudd_bddAnd(dd, bTemp = bProd, bVar);
        Cudd_Ref(bProd);
        Cudd_RecursiveDeref(dd, bTemp);
    }
...
DdNode* Abc_NtkComputeUnreachable(DdManager* dd, Abc_Ntk_t* pNtk, DdNode* bTrans, DdNode* bInitial, int fVerbose) {
    ...
    bRelation = bTrans;
    Cudd_Ref(bRelation);
    bCubeCs = Extra_bddComputeRangeCube(dd, Abc_NtkPiNum(pNtk), Abc_NtkCiNum(pNtk));
    Cudd_Ref(bCubeCs);
    for (nIters = 1;; nIters++) {
        // compute the next states
        bNext = Cudd_bddAndAbstract(dd, bRelation, bCurrent, bCubeCs);
        Cudd_Ref(bNext);
...
Abc_Ntk_t* Abc_NtkConstructExdc(DdManager* dd, Abc_Ntk_t* pNtk, DdNode* bUnreach) {
    ...
    for (i = 0; i < dd->size; i++)
        pPermute[i] = -1;
    Abc_NtkForEachLatch(pNtk, pNode, i)
        pPermute[Abc_NtkPiNum(pNtk) + i]
        = i;
    // remap the functions
    pNodeNew->pData = Extra_TransferPermute(dd, (DdManager*)pNtkNew->pManFunc, bUnreach, pPermute);
    ...
    Abc_NtkForEachPo(pNtk, pNode, i) if (!Abc_ObjIsCi(Abc_ObjFanin0(pNode)))
