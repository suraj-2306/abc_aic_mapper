...
static inline Abc_IffObj_t* Abc_IffObj(Abc_IffMan_t* p, int i) {
...
void Abc_NtkPerformIfif(Abc_Ntk_t* pNtk, Ifif_Par_t* pPars) {
    ...
    Abc_NtkForEachCo(pNtk, pObj, i)
        Delay
        = Abc_MaxFloat(Delay, Abc_IffDelay(p, Abc_ObjFanin0(pObj), DegreeFinal));
    Delay10 = 0.9 * Delay;

    // consider delay at the outputs
    Count10 = 0;
    Abc_NtkForEachCo(pNtk, pObj, i) if (Abc_IffDelay(p, Abc_ObjFanin0(pObj), DegreeFinal) >= Delay10)
        Count10++;
