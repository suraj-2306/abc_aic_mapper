...
int Abc_NtkCompareAndSaveBest(Abc_Ntk_t* pNtk) {
    ...
    if (ParsBest.pName == NULL || strcmp(ParsBest.pName, pNtk->pName) || ParsBest.Depth > ParsNew.Depth || (ParsBest.Depth == ParsNew.Depth && ParsBest.Flops > ParsNew.Flops) || (ParsBest.Depth == ParsNew.Depth && ParsBest.Flops == ParsNew.Flops && ParsBest.Edges > ParsNew.Edges)) {
        ABC_FREE(ParsBest.pName);
...
double Abc_NtkMemory(Abc_Ntk_t* p) {
    ...
    Abc_NtkForEachObj(p, pObj, i)
        Memory
        += sizeof(int) * (Vec_IntCap(&pObj->vFanins) + Vec_IntCap(&pObj->vFanouts));
    return Memory;
}
...
float Abc_NtkMfsTotalSwitching(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachObj(pNtk, pObjAbc, i) if ((pObjAbc->pTemp && Abc_ObjRegular((Abc_Obj_t*)pObjAbc->pTemp)->Type == ABC_FUNC_NONE) || (!Abc_ObjIsCi(pObjAbc) && !Abc_ObjIsNode(pObjAbc)))
        pObjAbc->pTemp
        = NULL;
...
void Abc_NtkPrintLevel(FILE* pFile, Abc_Ntk_t* pNtk, int fProfile, int fListNodes, int fVerbose) {
    ...
    if (fProfile && Abc_NtkHasMapping(pNtk)) {
        ...
        LevelMax = 0;
        Abc_NtkForEachCo(pNtk, pNode, i) if (LevelMax < (int)Abc_ObjFanin0(pNode)->Level)
            LevelMax
            = Abc_ObjFanin0(pNode)->Level;
    ...
    if (fVerbose) {
        ...
        Length = 0;
        Abc_NtkForEachCo(pNtk, pNode, i) if (Length < (int)strlen(Abc_ObjName(pNode)))
            Length
            = strlen(Abc_ObjName(pNode));
        if (Length < 5)
            Length = 5;
