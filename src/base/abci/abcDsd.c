...
Abc_Obj_t* Abc_NtkDsdConstructNode(Dsd_Manager_t* pManDsd, Dsd_Node_t* pNodeDsd, Abc_Ntk_t* pNtkNew, int* pCounters) {
    ...
    switch (Type) {
        ...
        case DSD_NODE_OR: {
            bLocal = Cudd_Not(ddNew->one);
            Cudd_Ref(bLocal);
            for (i = 0; i < nDecs; i++) {
                pFaninDsd = Dsd_NodeReadDec(pNodeDsd, i);
                bVar = Cudd_NotCond(ddNew->vars[i], Dsd_IsComplement(pFaninDsd));
                bLocal = Cudd_bddOr(ddNew, bTemp = bLocal, bVar);
                Cudd_Ref(bLocal);
        ...
        case DSD_NODE_EXOR: {
            ...
            Cudd_Ref(bLocal);
            for (i = 0; i < nDecs; i++) {
                bLocal = Cudd_bddXor(ddNew, bTemp = bLocal, ddNew->vars[i]);
                Cudd_Ref(bLocal);
                Cudd_RecursiveDeref(ddNew, bTemp);
            }
            break;
        ...
        case DSD_NODE_PRIME: {
            ...
            bLocal = Dsd_TreeGetPrimeFunction(ddDsd, pNodeDsd);
            Cudd_Ref(bLocal);
            bLocal = Extra_TransferLevelByLevel(ddDsd, ddNew, bTemp = bLocal);
            Cudd_Ref(bLocal);
            /*
if ( nDecs == 3 )
{
...
void Abc_NodeDecompDsdAndMux(Abc_Obj_t* pNode, Vec_Ptr_t* vNodes, Dsd_Manager_t* pManDsd, int fRecursive, int* pCounters) {
    ...
    if (!fRecursive || Dsd_NodeReadDecsNum(pNodeDsd) != Abc_ObjFaninNum(pNode)) { // perform DSD
        ...
        pNode->pData = Cudd_NotCond((DdNode*)dd->vars[0], fCompl);
    ...
    } else // perform MUX-decomposition
    {
        ...
        pNode1->pData = Cudd_Cofactor(dd, (DdNode*)pNode->pData, Cudd_Not(dd->vars[iVar]));
        Cudd_Ref((DdNode*)pNode1->pData);
        ...
        pNode2->pData = Cudd_Cofactor(dd, (DdNode*)pNode->pData, dd->vars[iVar]);
        Cudd_Ref((DdNode*)pNode2->pData);
        ...
        Cudd_RecursiveDeref(dd, (DdNode*)pNode->pData);
        pNode->pData = Cudd_bddIte(dd, dd->vars[0], dd->vars[1], dd->vars[2]);
        Cudd_Ref((DdNode*)pNode->pData);
    }
}
...
int Abc_NodeFindMuxVar(DdManager* dd, DdNode* bFunc, int nVars) {
    ...
    for (i = 0; i < nVars; i++) {
        ...
        bCof0 = Cudd_Cofactor(dd, bFunc, Cudd_Not(bVar));
        Cudd_Ref(bCof0);
        bCof1 = Cudd_Cofactor(dd, bFunc, bVar);
        Cudd_Ref(bCof1);
...
DdNode* Extra_bddComputeSum(DdManager* dd, DdNode** pbCubes, int nCubes) {
    ...
    Cudd_Ref(bRes);
    for (i = 0; i < nCubes; i++) {
        bRes = Cudd_bddOr(dd, bTemp = bRes, pbCubes[i]);
        Cudd_Ref(bRes);
...
DdNode* Abc_NtkSparsifyInternalOne(DdManager* ddNew, DdNode* bFunc, int nFanins, int nPerc) {
    ...
    for (i = 0; i < nMints; i++)
        Cudd_Ref(pbMints[i]);
    bRes = Extra_bddComputeSum(ddNew, pbMints, nMints);
    Cudd_Ref(bRes);
    for (i = 0; i < nMints; i++)
...
Abc_Ntk_t* Abc_NtkSparsifyInternal(Abc_Ntk_t* pNtk, int nPerc, int fVerbose) {
    ...
    Abc_NtkForEachCo(pNtk, pObj, i) {
        ...
        for (c = 0; c < 2; c++) {
            ...
            bFunc = Abc_NtkSparsifyInternalOne(ddNew, bFuncOld, Abc_ObjFaninNum(pDriver), nPerc);
            Cudd_Ref(bFunc);
