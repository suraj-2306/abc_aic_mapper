...
***********************************************************************/
static inline int Lms_DelayGet(word D, int v) {
    assert(v >= 0 && v < LMS_VAR_MAX);
    return (int)((D >> (v << 2)) & 0xF);
}
static inline void Lms_DelaySet(word* pD, int v, int d) {
    assert(v >= 0 && v < LMS_VAR_MAX);
    assert(d >= 0 && d < LMS_VAR_MAX);
    *pD |= ((word)d << (v << 2));
}
static inline word Lms_DelayInit(int v) {
    assert(v >= 0 && v < LMS_VAR_MAX);
    return (word)1 << (v << 2);
}
...
void Lms_ManPrepare(Lms_Man_t* p) {
    ...
    Vec_IntForEachEntry(p->vTruthIds, Entry, i) if (Vec_IntEntry(p->vTruthPo, Entry) == -1)
    ...
    Gia_ManForEachCo(pLib, pObjPo, k) {
        ...
        if (fCheck) {
            ...
            if (memcmp(p->pTemp1, pTruth, p->nWords * sizeof(word)) != 0) {
                Kit_DsdPrintFromTruth((unsigned*)pTruth, nLeaves);
                printf("\n");
                Kit_DsdPrintFromTruth((unsigned*)p->pTemp1, nLeaves);
                printf("\n");
...
Vec_Int_t* Lms_GiaCountTruths(Lms_Man_t* p) {
    ...
    int i, Entry;
    Vec_IntForEachEntry(p->vTruthIds, Entry, i) if (Entry >= 0)
...
Vec_Int_t* Lms_GiaFindNonRedundantCos(Lms_Man_t* p) {
    ...
    Vec_IntForEachEntry(vUseful, EntryI, i) if (EntryI >= 0)
