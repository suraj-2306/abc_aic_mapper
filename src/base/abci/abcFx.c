...
void Abc_NtkFxInsert(Abc_Ntk_t* pNtk, Vec_Wec_t* vCubes) {
    ...
    Vec_WecForEachLevel(vCubes, vCube, i)
        iNodeMax
        = Abc_MaxInt(iNodeMax, Vec_IntEntry(vCube, 0));
...
int Abc_NtkFxCheck(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachNode(pNtk, pNode, i) if (!Vec_IntCheckUniqueSmall(&pNode->vFanins)) return 0;
    return 1;
}
...
static inline int Fx_ManComputeLevelDiv(Fx_Man_t* p, Vec_Int_t* vCubeFree) {
    ...
    Vec_IntForEachEntry(vCubeFree, Lit, i)
        Level
        = Abc_MaxInt(Level, Vec_IntEntry(p->vLevels, Abc_Lit2Var(Abc_Lit2Var(Lit))));
...
static inline int Fx_ManComputeLevelCube(Fx_Man_t* p, Vec_Int_t* vCube) {
    ...
    Vec_IntForEachEntryStart(vCube, Lit, k, 1)
        Level
        = Abc_MaxInt(Level, Vec_IntEntry(p->vLevels, Abc_Lit2Var(Lit)));
    return Level;
}
...
static inline void Fx_PrintDivArray(Vec_Int_t* vDiv) {
    int i, Lit;
    Vec_IntForEachEntry(vDiv, Lit, i) if (!Abc_LitIsCompl(Lit))
...
static inline void Fx_ManDivAddLits(Vec_Int_t* vCube, Vec_Int_t* vCube2, Vec_Int_t* vDiv) {
    ...
    else Vec_IntPush(vCube, Abc_Lit2Var(Lit));
...
void Fx_ManCreateLiterals(Fx_Man_t* p, int nVars) {
    ...
    Vec_WecForEachLevel(p->vCubes, vCube, i) {
        ...
        Vec_IntForEachEntryStart(vCube, Lit, k, 1)
            p->nVars
            = Abc_MaxInt(p->nVars, Abc_Lit2Var(Lit));
    }
    //    p->nVars++;
    ...
    Vec_WecForEachLevel(p->vCubes, vCube, i) if (Vec_IntEntry(p->vVarCube, Vec_IntEntry(vCube, 0)) == -1)
...
void Fx_ManCreateDivisors(Fx_Man_t* p) {
    ...
    Vec_FltForEachEntry(p->vWeights, Weight, i) if (Weight > 0.0)
...
static inline void Fx_ManCompressCubes(Vec_Wec_t* vCubes, Vec_Int_t* vLit2Cube) {
    int i, CubeId, k = 0;
    Vec_IntForEachEntry(vLit2Cube, CubeId, i) if (Vec_IntSize(Vec_WecEntry(vCubes, CubeId)) > 0)
