...
void Abc_NtkTimeInitialize(Abc_Ntk_t* pNtk, Abc_Ntk_t* pNtkOld) {
    ...
    Abc_NtkForEachCi(pNtk, pObj, i)
        * ppTimes[pObj->Id]
        = pNtkOld ? *Abc_NodeReadArrival(Abc_NtkCi(pNtkOld, i)) : pNtk->pManTime->tArrDef;
    ppTimes = (Abc_Time_t**)pNtk->pManTime->vReqs->pArray;
    Abc_NtkForEachCo(pNtk, pObj, i)
        * ppTimes[pObj->Id]
        = pNtkOld ? *Abc_NodeReadRequired(Abc_NtkCo(pNtkOld, i)) : pNtk->pManTime->tReqDef;
}
...
Abc_ManTime_t* Abc_ManTimeStart(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachCi(pNtk, pObj, i) {
        ...
        if (pTime && (!(p->tArrDef.Fall == -ABC_INFINITY || p->tArrDef.Rise != -ABC_INFINITY))) {
...
float* Abc_NtkGetCiArrivalFloats(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachCi(pNtk, pNode, i)
        p[i]
        = Abc_NodeReadArrivalWorst(pNode);
    return p;
}
float* Abc_NtkGetCoRequiredFloats(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachCo(pNtk, pNode, i)
        p[i]
        = Abc_NodeReadRequiredWorst(pNode);
    return p;
}
...
float Abc_NtkDelayTrace(Abc_Ntk_t* pNtk, Abc_Obj_t* pOut, Abc_Obj_t* pIn, int fPrint) {
    ...
    if (fPrint) {
        ...
        if (!Abc_NtkDelayTraceCritPath_rec(vSlacks, Abc_ObjFanin0(pOut), pIn, vBest)) {
            ...
            Vec_PtrForEachEntry(Abc_Obj_t*, vPath, pNode, i) if (Abc_ObjIsNode(pNode))
                Length
                = Abc_MaxInt(Length, strlen(Mio_GateReadName((Mio_Gate_t*)pNode->pData)));
            // print critical path
            Abc_NtkLevel(pNtk);
...
int Abc_ObjLevelNew(Abc_Obj_t* pObj) {
    ...
    Abc_ObjForEachFanin(pObj, pFanin, i)
        Level
        = Abc_MaxFloat(Level, Abc_ObjLevel(pFanin));
