...
Dec_Graph_t* Abc_NodeRestructureCut(Abc_ManRst_t* p, Abc_Obj_t* pRoot, Cut_Cut_t* pCut) {
    ...
    bFunc = Abc_NodeConeBdd(p->dd, p->dd->vars, pRoot, p->vLeaves, p->vVisited);
    Cudd_Ref(bFunc);
...
Dec_Edge_t Abc_NodeEvaluateDsd_rec(Dec_Graph_t* pGraph, Abc_ManRst_t* pManRst, Dsd_Node_t* pNodeDsd, int Required, int nNodesSaved, int* pnNodesAdded) {
    ...
    if (DecType == DSD_NODE_PRIME) {
        ...
        bLocal = Dsd_TreeGetPrimeFunction(pManRst->dd, pNodeDsd);
        Cudd_Ref(bLocal);
        //Extra_bddPrint( pManRst->dd, bLocal );

        bVar = pManRst->dd->vars[0];
        bCofE = Cudd_Cofactor(pManRst->dd, bLocal, Cudd_Not(bVar));
        Cudd_Ref(bCofE);
        bCofT = Cudd_Cofactor(pManRst->dd, bLocal, bVar);
        Cudd_Ref(bCofT);
...
int Abc_NodeResubMffc_rec(Abc_Obj_t* pNode) {
    if (Abc_NodeIsTravIdCurrent(pNode))
        return 0;
    Abc_NodeSetTravIdCurrent(pNode);
    return 1 + Abc_NodeResubMffc_rec(Abc_ObjFanin0(pNode)) + Abc_NodeResubMffc_rec(Abc_ObjFanin1(pNode));
}
...
int Abc_NodeResubMffc(Abc_ManRst_t* p, Vec_Ptr_t* vDecs, int nLeaves, Abc_Obj_t* pRoot) {
    ...
    k = 0;
    Vec_PtrForEachEntryStart(Abc_Obj_t*, vDecs, pObj, i, nLeaves) if (Abc_NodeIsTravIdCurrent(pObj))
        Vec_PtrPush(p->vTemp, pObj);
    else Vec_PtrWriteEntry(vDecs, k++, pObj);
