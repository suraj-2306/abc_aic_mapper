...
void Abc_NtkMultiInt(Abc_Ntk_t* pNtk, Abc_Ntk_t* pNtkNew) {
    ...
    if (Abc_ObjFanoutNum(pConst1) > 0) {
        ...
        pNodeNew->pData = Cudd_ReadOne((DdManager*)pNtkNew->pManFunc);
...
DdNode* Abc_NtkMultiDeriveBdd(DdManager* dd, Abc_Obj_t* pNodeOld, Vec_Ptr_t* vFaninsOld) {
    ...
    for (i = 0; i < vFaninsOld->nSize; i++) {
        ...
        pFaninOld->pData = Cudd_bddIthVar(dd, i);
        Cudd_Ref((DdNode*)pFaninOld->pData);
        pFaninOld->fMarkC = 1;
    }
    // call the recursive BDD computation
    bFunc = Abc_NtkMultiDeriveBdd_rec(dd, pNodeOld, vFaninsOld);
    Cudd_Ref(bFunc);
...
DdNode* Abc_NtkMultiDeriveBdd_rec(DdManager* dd, Abc_Obj_t* pNode, Vec_Ptr_t* vFanins) {
    ...
    bFunc0 = Abc_NtkMultiDeriveBdd_rec(dd, Abc_ObjFanin(pNode, 0), vFanins);
    Cudd_Ref(bFunc0);
    bFunc1 = Abc_NtkMultiDeriveBdd_rec(dd, Abc_ObjFanin(pNode, 1), vFanins);
    Cudd_Ref(bFunc1);
...
int Abc_NtkMultiLimit_rec(Abc_Obj_t* pNode, Vec_Ptr_t* vCone, int nFaninMax, int fCanStop, int fFirst) {
    ...
    if (nNodes0 >= nNodes1) { // the left branch is larger - cut it
        assert(Abc_ObjFanin(pNode, 0)->fMarkA == 0);
        Abc_ObjFanin(pNode, 0)->fMarkA = 1;
    } else { // the right branch is larger - cut it
...
void Abc_NtkMultiSetBoundsCnf(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachNode(pNtk, pNode, i) {
        ...
        //        if ( Abc_NodeIsConst(pNode) )
        //            continue;
        if (Abc_NodeIsMuxType(pNode) && Abc_ObjFanin0(pNode)->fMarkA == 0 && Abc_ObjFanin1(pNode)->fMarkA == 0)
            nMuxes++;
    }
...
void Abc_NtkMultiSetBoundsFactor(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachNode(pNtk, pNode, i)
        pNode->fMarkA
        = (pNode->vFanouts.nSize > 1 && !Abc_NodeIsMuxControlType(pNode));
    // mark the PO drivers
    Abc_NtkForEachCo(pNtk, pNode, i)
        Abc_ObjFanin0(pNode)
            ->fMarkA
