...
static inline Odc_Lit_t Odc_Var(Odc_Man_t* p, int i) {
    assert(i >= 0 && i < p->nPis);
    return (i + 1) << 1;
}
...
static inline Odc_Obj_t* Odc_ObjNew(Odc_Man_t* p) {
    assert(p->nObjs < p->nObjsAlloc);
    return p->pObjs + p->nObjs++;
}
static inline Odc_Lit_t Odc_Obj2Lit(Odc_Man_t* p, Odc_Obj_t* pObj) {
    assert(pObj);
    return (pObj - p->pObjs) << 1;
}
static inline Odc_Obj_t* Odc_Lit2Obj(Odc_Man_t* p, Odc_Lit_t Lit) {
    assert(!(Lit & 1) && (int)(Lit >> 1) < p->nObjs);
    return p->pObjs + (Lit >> 1);
}
...
// truth tables
static inline unsigned* Odc_ObjTruth(Odc_Man_t* p, Odc_Lit_t Lit) {
    assert(!(Lit & 1));
    return (unsigned*)Vec_PtrEntry(p->vTruths, Lit >> 1);
}
...
void Abc_NtkDontCareWinCollectRoots_rec(Abc_Obj_t* pObj, Vec_Ptr_t* vRoots) {
    ...
    Abc_ObjForEachFanout(pObj, pFanout, i) if (!Abc_NodeIsTravIdCurrent(pFanout)) break;
...
int Abc_NtkDontCareWinAddMissing_rec(Odc_Man_t* p, Abc_Obj_t* pObj) {
    ...
    Abc_ObjForEachFanin(pObj, pFanin, i) if (!Abc_NtkDontCareWinAddMissing_rec(p, pFanin)) return 0;
    return 1;
}
...
int Abc_NtkDontCareWinAddMissing(Odc_Man_t* p) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, p->vRoots, pObj, i) if (!Abc_NtkDontCareWinAddMissing_rec(p, pObj)) return 0;
    return 1;
}
