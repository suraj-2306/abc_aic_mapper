...
Vec_Ptr_t* Abc_NtkAssignIDs(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachCo(pNtk, pObj, i)
        pObj->iTemp
        = Abc_NtkCiNum(pNtk) + Vec_PtrSize(vNodes) + i;
    return vNodes;
}
...
Sfm_Ntk_t* Abc_NtkExtractMfs2(Abc_Ntk_t* pNtk, int iPivot) {
    ...
    Abc_NtkForEachNode(pNtk, pObj, i) if (i >= iPivot)
...
void Abc_NtkInsertMfs(Abc_Ntk_t* pNtk, Sfm_Ntk_t* p) {
    ...
    Abc_NtkForEachNode(pNtk, pNode, i) if (pNode->iTemp > 0)
        Vec_IntWriteEntry(vMap, pNode->iTemp, Abc_ObjId(pNode));
    // remove old fanins
    Abc_NtkForEachNode(pNtk, pNode, i) if (!Sfm_NodeReadFixed(p, pNode->iTemp))
