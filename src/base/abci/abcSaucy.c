...
print_partition(struct coloring* left, struct coloring* right, int n, Abc_Ntk_t* pNtk, int fNames) {
    ...
    for (i = 0; i < n; i += (left->clen[i] + 1)) {
        for (j = 0; j < (left->clen[i] + 1); j++) {
            if (fNames)
                printf("%s ", getVertexName(pNtk, left->lab[i + j]));
            else
                printf("%d ", left->lab[i + j]);
        }
    ...
    for (i = 0; i < n; i += (right->clen[i] + 1)) {
        for (j = 0; j < (right->clen[i] + 1); j++) {
            if (fNames)
                printf("%s ", getVertexName(pNtk, right->lab[i + j]));
            else
                printf("%d ", right->lab[i + j]);
        }
...
static int
refine_cell(struct saucy* s, struct coloring* c, int (*refine)(struct saucy*, struct coloring*, int)) {
    int i, cf, ret = 1;
...
static int
ref_singleton(struct saucy* s, struct coloring* c, const int* adj, const int* edg, int cf) {
    int i, k = c->lab[cf];
...
static int
ref_nonsingle(struct saucy* s, struct coloring* c, const int* adj, const int* edg, int cf) {
    int i, j, k, ret;
...
void saucy_search(
    ...
    if (SELECT_DYNAMICALLY)
        s->select_decomposition = select_dynamically;
    else
