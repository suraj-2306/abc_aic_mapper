...
void Abc_NtkFraigMergeClassMapped(Abc_Ntk_t* pNtk, Abc_Obj_t* pChain, int fUseInv, int fVerbose) {
    ...
    for (pNode = pListDir; pNode; pNode = pNode->pNext) {
        ...
        if (Arrival1 > Arrival2 || (Arrival1 == Arrival2 && pNodeMin->Level > pNode->Level) || (Arrival1 == Arrival2 && pNodeMin->Level == pNode->Level && Abc_NodeDroppingCost(pNodeMin) < Abc_NodeDroppingCost(pNode)))
            pNodeMin = pNode;
    }
...
void Abc_NtkFraigMergeClass(Abc_Ntk_t* pNtk, Abc_Obj_t* pChain, int fUseInv, int fVerbose) {
    ...
    for (pNode = pChain->pNext; pNode; pNode = pNode->pNext)
        if (pNodeMin->Level > pNode->Level || (pNodeMin->Level == pNode->Level && Abc_NodeDroppingCost(pNodeMin) < Abc_NodeDroppingCost(pNode)))
            pNodeMin = pNode;
...
int Abc_NtkCleanupNodes(Abc_Ntk_t* pNtk, Vec_Ptr_t* vRoots, int fVerbose) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vRoots, pObj, i) if (pObj)
        Vec_PtrPush(vStarts, pObj);
    // mark the nodes reachable from the POs
...
int Abc_NtkReduceNodes(Abc_Ntk_t* pNtk, Vec_Ptr_t* vNodes) {
    ...
    Counter = 0;
    Abc_NtkForEachNode(pNtk, pNode, i) if (pNode->fMarkA == 0) {
...
void Abc_NodeConstantInput(Abc_Obj_t* pNode, Abc_Obj_t* pFanin, int fConst0) {
    ...
    pNode->pData = Cudd_Cofactor(dd, bTemp = (DdNode*)pNode->pData, bVar);
...
int Abc_NtkSweep(Abc_Ntk_t* pNtk, int fVerbose) {
    ...
    Abc_NtkForEachNode(pNtk, pNode, i) if (Abc_ObjFaninNum(pNode) < 2)
...
int Abc_NtkLatchSweep(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachLatch(pNtk, pLatch, i) {
        ...
        if ((RetValue == 1 && Abc_LatchIsInit0(pLatch)) || (RetValue == 0 && Abc_LatchIsInit1(pLatch))) {
...
int Abc_NtkReplaceAutonomousLogic(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachObj(pNtk, pNode, i) {
        ...
        Abc_ObjForEachFanin(pNode, pFanin, k) {
            ...
            if (Abc_NtkCheckConstant_rec(pFanin) != -1 || (Abc_ObjIsBo(pFanin) && Abc_NtkCheckConstant_rec(Abc_ObjFanin0(Abc_ObjFanin0(pFanin))) != -1)) {
