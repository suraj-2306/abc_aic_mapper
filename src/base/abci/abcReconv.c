...
***********************************************************************/
static inline int Abc_NodeGetLeafCostTwo(Abc_Obj_t* pNode, int nFaninLimit, Abc_Obj_t** ppLeafToAdd, Abc_Obj_t** pNodeToMark1, Abc_Obj_t** pNodeToMark2) {
...
int Abc_NodeBuildCutLevelOne_int(Vec_Ptr_t* vVisited, Vec_Ptr_t* vLeaves, int nSizeLimit, int nFaninLimit) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vLeaves, pNode, i) {
        ...
        if (CostBest > CostCur || (CostBest == CostCur && pNode->Level > pFaninBest->Level)) {
            CostBest = CostCur;
            pFaninBest = pNode;
        }
        if (CostBest == 0)
...
DdNode* Abc_NodeConeDcs(DdManager* dd, DdNode** pbVarsX, DdNode** pbVarsY, Vec_Ptr_t* vLeaves, Vec_Ptr_t* vRoots, Vec_Ptr_t* vVisited) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vRoots, pNode, i) {
        bFunc = Cudd_bddXnor(dd, (DdNode*)pNode->pCopy, pbVarsY[i]);
        Cudd_Ref(bFunc);
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vVisited, pNode, i)
        Cudd_RecursiveDeref(dd, (DdNode*)pNode->pCopy);
    // compute don't-cares
    bCube = Extra_bddComputeRangeCube(dd, vRoots->nSize, vRoots->nSize + vLeaves->nSize);
    Cudd_Ref(bCube);
...
Vec_Ptr_t* Abc_NodeCollectTfoCands(Abc_ManCut_t* p, Abc_Obj_t* pRoot, Vec_Ptr_t* vLeaves, int LevelMax) {
    ...
    Vec_VecForEachEntryStart(Abc_Obj_t*, p->vLevels, pNode, i, k, LevelMin) {
        ...
        if (!Abc_NodeIsTravIdCurrent(pNode)) {
            ...
            if (!Abc_NodeIsTravIdCurrent(Abc_ObjFanin0(pNode)) || !Abc_NodeIsTravIdCurrent(Abc_ObjFanin1(pNode)))
                continue;
