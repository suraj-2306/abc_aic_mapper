...
#define Wln_RetForEachFanin(p, iObj, iFanin, pLink, i)                                                                                                            \
    for (i = 0; (i < Wln_ObjFaninNum(p->pNtk, iObj)) && (((iFanin) = Wln_RetFanins(p, iObj)[2 * i]), 1) && ((pLink) = (Wln_RetFanins(p, iObj) + 2 * i + 1)); i++) \
        if (!iFanin || (!Wln_ObjFaninNum(p->pNtk, iFanin) && !Wln_ObjIsCi(p->pNtk, iFanin))) {                                                                    \
        } else

#define Wln_RetForEachFanout(p, iObj, iFanout, pLink, i)                                                                                                                                  \
    for (i = 0; (i < Wln_ObjRefs(p->pNtk, iObj)) && (((iFanout) = Wln_RetFanouts(p, iObj)[2 * i]), 1) && ((pLink) = Vec_IntEntryP(&p->vFanins, Wln_RetFanouts(p, iObj)[2 * i + 1])); i++) \
        if (!iFanout) {                                                                                                                                                                   \
        } else
...
void Wln_RetPrint(Wln_Ret_t* p, int fVerbose) {
    int iObj, nCount = 0;
    Wln_NtkForEachObj(p->pNtk, iObj) if (Wln_ObjInstId(p->pNtk, iObj) > 1)
        nCount++;
...
Wln_Ret_t* Wln_RetAlloc(Wln_Ntk_t* pNtk) {
    ...
    Wln_NtkForEachObj(pNtk, iObj) if (Wln_ObjRefs(pNtk, iObj) == 0 && !Wln_ObjIsCio(pNtk, iObj)) {
        if (fFirst) {
            fFirst = 0;
...
void Wln_RetMarkChanges_rec(Wln_Ret_t* p, int iObj) {
    ...
    if (*pDelay < 0)
        return;
    *pDelay = -1;
    Wln_RetForEachFanout(p, iObj, iFanout, pLink, k) if (!pLink[0])
...
int Wln_RetPropDelay(Wln_Ret_t* p) {
    ...
    Wln_NtkForEachObj(p->pNtk, iObj) if (!Wln_ObjIsCio(p->pNtk, iObj)) {
...
void Wln_RetFindSources_rec(Wln_Ret_t* p, int iObj) {
    ...
    Wln_RetForEachFanin(p, iObj, iFanin, pLink, k) if (!pLink[0] && Vec_IntEntry(&p->vPathDelays, iFanin) == FaninDelay)
...
int Wln_RetMarkPaths_rec(Wln_Ntk_t* p, int iObj, int fVerbose) {
    ...
    Wln_ObjForEachFanin(p, iObj, iFanin, k)
        fPrev
        &= Wln_RetMarkPaths_rec(p, iFanin, fVerbose);
    if (fPrev) {
...
int Wln_RetCheckForward(Wln_Ret_t* p, Vec_Int_t* vSet) {
    int i, iObj;
    Vec_IntForEachEntry(vSet, iObj, i) if (!Wln_RetCheckForwardOne(p, iObj)) return 0;
    return 1;
}
...
int Wln_RetCheckBackward(Wln_Ret_t* p, Vec_Int_t* vSet) {
    int i, iObj;
    Vec_IntForEachEntry(vSet, iObj, i) if (!Wln_RetCheckBackwardOne(p, iObj)) return 0;
    return 1;
}
...
void Wln_NtkRetimeCreateDelayInfo(Wln_Ntk_t* pNtk) {
    ...
    if (Wln_NtkHasInstId(pNtk)) {
        ...
        Wln_NtkForEachObj(pNtk, iObj) if (!Wln_ObjIsCio(pNtk, iObj) && !Wln_ObjIsConst(pNtk, iObj) && Wln_ObjInstId(pNtk, iObj) == 0)
