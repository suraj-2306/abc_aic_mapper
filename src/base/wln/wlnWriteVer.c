...
void Wln_WriteTables(FILE* pFile, Wln_Ntk_t* p) {
    ...
    Wln_NtkForEachObj(p, iObj) if (Wln_ObjType(p, iObj) == ABC_OPER_TABLE)
...
void Wln_WriteAddPos(Wln_Ntk_t* p) {
    int iObj;
    Wln_NtkForEachObj(p, iObj) if (!Wln_ObjIsCio(p, iObj))
...
void Wln_WriteVerInt(FILE* pFile, Wln_Ntk_t* p) {
    ...
    Wln_NtkForEachFf(p, iObj, j) {
        ...
        Wln_ObjForEachFanin(p, iObj, iFanin, k) if (iFanin) fprintf(pFile, " .%s(%s),", pInNames[k], Wln_ObjName(p, iFanin));
