...
*/
        if (Type == ABC_OPER_COMP_EQU || Type == ABC_OPER_COMP_NOTEQU) {
            ...
            Vec_IntForEachEntryTwo(vArg0, vArg1, iLit0, iLit1, k)
                iLit
                = Gia_ManHashOr(pNew, iLit, Gia_ManHashXor(pNew, iLit0, iLit1));
            Vec_IntFill(vRes, 1, Abc_LitNotCond(iLit, Type == ABC_OPER_COMP_EQU));
            for (k = 1; k < nRange; k++)
                Vec_IntPush(vRes, 0);
            return;
        }
        if (Type == ABC_OPER_COMP_LESS || Type == ABC_OPER_COMP_LESSEQU || Type == ABC_OPER_COMP_MORE || Type == ABC_OPER_COMP_MOREEQU) {
...
*/
        if (Type == ABC_OPER_SHIFT_R || Type == ABC_OPER_SHIFT_RA || Type == ABC_OPER_SHIFT_L || Type == ABC_OPER_SHIFT_LA) {
