...
int Wln_NtkIsAcyclic(Wln_Ntk_t* p) {
    ...
    Wln_NtkForEachObj(p, iObj)
        nUnvisited
        += !Wln_ObjIsTravIdPrevious(p, iObj) && !Wln_ObjIsCi(p, iObj);
    if (nUnvisited) {
        int nSinks = 0;
        Wln_NtkCreateRefs(p);
        ...
        Wln_NtkForEachObj(p, iObj) if (!Wln_ObjRefs(p, iObj) && !Wln_ObjIsCi(p, iObj) && !Wln_ObjIsCo(p, iObj) && !Wln_ObjIsFf(p, iObj))
            nSinks++;
        if (nSinks) {
            int nPrinted = 0;
        ...
        Wln_NtkForEachObj(p, iObj) if (/*!Wln_ObjRefs(p, iObj) &&*/ !Wln_ObjIsTravIdPrevious(p, iObj) && !Wln_ObjIsCi(p, iObj)) {
            // traverse the output logic cone
            if ((fAcyclic = Wln_NtkIsAcyclic_rec(p, iObj)))
                continue;
            // stop as soon as the first loop is detected
...
void Wln_NtkTransferNames(Wln_Ntk_t* pNew, Wln_Ntk_t* p) {
    ...
    if (Wln_NtkHasNameId(p)) {
        ...
        Wln_NtkForEachObj(p, i) if (Wln_ObjCopy(p, i) && i < Vec_IntSize(&p->vNameIds) && Wln_ObjNameId(p, i))
    ...
    if (Wln_NtkHasInstId(p)) {
        ...
        Wln_NtkForEachObj(p, i) if (Wln_ObjCopy(p, i) && i < Vec_IntSize(&p->vInstIds) && Wln_ObjInstId(p, i))
