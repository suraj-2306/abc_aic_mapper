...
void Wln_NtkToNdrTest(Wln_Ntk_t* p) {
    ...
    // collect names
    char** ppNames = ABC_ALLOC(char*, Wln_NtkObjNum(p) + 1);
    int i;
    Wln_NtkForEachObj(p, i)
        ppNames[i]
        = Abc_UtilStrsav(Wln_ObjName(p, i));
...
Wln_Ntk_t* Wln_NtkFromNdr(void* pData, int fDump) {
    ...
    for (i = 1; i <= NameIdMax; i++) {
        char pName[20];
        sprintf(pName, "s%0*d", nDigits, i);
        NameId = Abc_NamStrFindOrAdd(pNtk->pManName, pName, &fFound);
        assert(!fFound && i == NameId);
    }
    // add const strings
    i = 0;
    Wln_NtkForEachObj(pNtk, iObj) if (Wln_ObjIsConst(pNtk, iObj))
