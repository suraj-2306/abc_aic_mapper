...
#define Rtl_CellForEachInput(p, pCell, Par, Val, i)                                           \
    Rtl_CellForEachConnect(p, pCell, Par, Val, i) if (i >= Rtl_CellInputNum(pCell)) continue; \
    else
...
void Rtl_NtkPrintOpers(Rtl_Ntk_t* p) {
    ...
    if (Rtl_NtkCellNum(p) == 0)
        return;
    Rtl_NtkForEachCell(p, pCell, i) if (Rtl_CellModule(pCell) < ABC_OPER_LAST)
...
void Rtl_NtkPrintHieStats(Rtl_Ntk_t* p, int nOffset) {
    ...
    if (nOffset)
        printf("|--> ");
    printf("%s\n", Rtl_NtkName(p));
    Rtl_NtkForEachCell(p, pCell, i) if (Rtl_CellModule(pCell) >= ABC_INFINITY) {
...
int Rtl_LibFindModule(Rtl_Lib_t* p, int NameId) {
    Rtl_Ntk_t* pNtk;
    int i;
    Vec_PtrForEachEntry(Rtl_Ntk_t*, p->vNtks, pNtk, i) if (pNtk->NameId == NameId) return i;
    return -1;
}
int Rtl_LibFindModule2(Rtl_Lib_t* p, int NameId, int iNtk0) {
    ...
    int i;
    int Counts0[4] = {0};
    Rtl_NtkCountPio(pNtk0, Counts0);
    Vec_PtrForEachEntry(Rtl_Ntk_t*, p->vNtks, pNtk, i) if (strstr(Rtl_NtkName(pNtk), pName + 1)) {
        int Counts[4] = {0};
        Rtl_NtkCountPio(pNtk, Counts);
...
void Rtl_LibPrintStats(Rtl_Lib_t* p) {
    ...
    Vec_PtrForEachEntry(Rtl_Ntk_t*, p->vNtks, pNtk, i)
        nSymbs
        = Abc_MaxInt(nSymbs, strlen(Rtl_NtkName(pNtk)));
    Vec_PtrForEachEntry(Rtl_Ntk_t*, p->vNtks, pNtk, i)
...
int Rtl_NtkReviewCells(Rtl_Ntk_t* p) {
    ...
    Rtl_NtkForEachCell(p, pCell, i) {
        if (pCell[7])
            continue;
        Rtl_CellForEachInput(p, pCell, Par, Val, k) if (!Rtl_NtkCheckSignalRange(p, Val)) break;
...
void Rtl_NtkPrintCell(Rtl_Ntk_t* p, int* pCell) {
    ...
    Rtl_CellForEachParam(p, pCell, Par, Val, i)
        fprintf(Rtl_NtkFile(p), "    parameter"),
        Rtl_NtkPrintSig(p, Par), Rtl_NtkPrintSig(p, Val), printf("\n");
...
int Rtl_NtkReadConcat(Rtl_Ntk_t* p, int* pPos) {
    ...
    int RetVal = Vec_IntSize(vConcat);
    char* pTok;
...
int Wln_ReadMatchEnd(Rtl_Ntk_t* p, int Mod) {
    int i, Entry, Count = 0;
    Vec_IntForEachEntryStart(p->pLib->vTokens, Entry, i, Mod) if (Rtl_NtkTokCheck(p, Entry, RTL_CELL))
        Count++;
...
void Rtl_NtkReportUndefs(Rtl_Ntk_t* p) {
    ...
    Rtl_NtkForEachCell(p, pCell, i) if (Rtl_CellModule(pCell) == ABC_INFINITY - 1) {
...
void Rtl_LibReorderModules(Rtl_Lib_t* p) {
    ...
    Vec_PtrForEachEntry(Rtl_Ntk_t*, p->vNtks, pNtk, i)
        pNtk->iCopy
        = -1;
    Vec_PtrForEachEntry(Rtl_Ntk_t*, p->vNtks, pNtk, i) if (pNtk->iCopy == -1)
...
Rtl_Lib_t* Rtl_LibReadFile(char* pFileName, char* pFileSpec) {
    ...
    Vec_IntForEachEntry(p->vTokens, Entry, i) if (Entry == p->pMap[RTL_MODULE])
        i
        = Rtl_NtkReadNtk(p, i + 1);
    else if (Entry == p->pMap[RTL_ATTRIBUTE])
        i
        = Rtl_NtkReadAttribute2(p, i + 1);
    Rtl_LibSetParents(p);
...
Vec_Int_t* Rtl_NtkRevPermInput(Rtl_Ntk_t* p) {
    Vec_Int_t* vNew = Vec_IntAlloc(100);
    int b, i, Count = 0;
    for (i = 0; i < p->nInputs; i++) {
        int Width = Rtl_WireWidth(p, i);
        for (b = 0; b < Width; b++)
...
void Rtl_NtkBlastHierarchy(Gia_Man_t* pNew, Rtl_Ntk_t* p, int* pCell) {
    ...
    Rtl_CellForEachOutput(p, pCell, Par, Val, k)
        nBits
        += Rtl_NtkInsertSignalRange(p, Val, Vec_IntArray(&p->vBitTemp) + nBits, Vec_IntSize(&p->vBitTemp) - nBits);
    assert(nBits == Vec_IntSize(&p->vBitTemp));
}
...
int Rtl_NtkCellParamValue(Rtl_Ntk_t* p, int* pCell, char* pParam) {
    ...
    Rtl_CellForEachParam(p, pCell, Par, Val, i) if ((Par >> 2) == ParamId) {
...
void Rtl_NtkBlastOperator(Gia_Man_t* pNew, Rtl_Ntk_t* p, int* pCell) {
    ...
    Rtl_CellForEachOutput(p, pCell, Par, ValOut, i)
        nRange
        = Rtl_NtkCountSignalRange(p, ValOut);
    assert(nRange > 0);
    for (i = 0; i < TEMP_NUM; i++)
...
void Rtl_LibBlast(Rtl_Lib_t* pLib) {
    Rtl_Ntk_t* p;
    int i;
    Vec_PtrForEachEntry(Rtl_Ntk_t*, pLib->vNtks, p, i) if (p->pGia == NULL)
        p->pGia
        = Rtl_NtkBlast(p);
}
...
void Rtl_NtkBlastMap(Rtl_Ntk_t* p, int nBits) {
    ...
    Rtl_NtkForEachCell(p, pCell, i) {
        ...
        Rtl_CellForEachOutput(p, pCell, Par, Val, k)
            iBit
            += Rtl_NtkMapSignalRange(p, Val, i, iBit);
    }
    for (i = 0; i < 100; i++)
...
void Rtl_NtkBlastPrepareInputs(Rtl_Ntk_t* p, int* pCell) {
    ...
    Rtl_CellForEachInput(p, pCell, Par, Val, i) {
        ...
        Rtl_NtkCollectSignalInfo(p, Val);
        vTemp = Vec_IntDup(&p->vBitTemp);
        Vec_IntForEachEntry(vTemp, iBit, k) if (iBit >= 0)
...
void Rtl_LibBlast2(Rtl_Lib_t* pLib, Vec_Int_t* vRoots, int fInv) {
    ...
    Vec_PtrForEachEntry(Rtl_Ntk_t*, pLib->vNtks, pNtk, i) if (pNtk->iCopy == -1 && pNtk->pGia == NULL)
        pNtk->pGia
        = Rtl_NtkBlast2(pNtk);
    //    Vec_PtrForEachEntry( Rtl_Ntk_t *, pLib->vNtks, pNtk, i )
    //        if ( pNtk->iCopy == -2 )
...
void Rtl_LibPreprocess(Rtl_Lib_t* pLib) {
    ...
    Vec_PtrForEachEntry(Rtl_Ntk_t*, pLib->vNtks, p1, i)
        ...
        if (i >= k)
            continue;
        if (Gia_ManCiNum(p1->pGia) != Gia_ManCiNum(p2->pGia) || Gia_ManCoNum(p1->pGia) != Gia_ManCoNum(p2->pGia))
            continue;
        // two similar modules
...
finish:
    ...
    // blast AIGs again
    Vec_PtrForEachEntry(Rtl_Ntk_t*, pLib->vNtks, p, i) if (p != p1 && p != p2)
        Gia_ManStopP(&p->pGia);
    //Rtl_LibBlast( pLib );
...
void Wln_SolveEqual(Rtl_Lib_t* p, int iNtk1, int iNtk2) {
    ...
    if (Gia_ManCiNum(pNtk1->pGia) != Gia_ManCiNum(pNtk2->pGia) || Gia_ManCoNum(pNtk1->pGia) != Gia_ManCoNum(pNtk2->pGia)) {
...
void Wln_SolveWithGuidance(char* pFileName, Rtl_Lib_t* p) {
    ...
    Vec_Int_t *vRoots, *vLevel;
    int i, iNtk1, iNtk2, fInv = 0;
    Vec_WecForEachLevel(vGuide, vLevel, i) if (Vec_IntEntry(vLevel, 1) == Rtl_LibStrId(p, "inverse"))
        fInv
        = 1;
...
Gia_Man_t* Rtl_ReduceInverse(Rtl_Lib_t* pLib, Gia_Man_t* p) {
    ...
    Gia_ManForEachAnd(p, pObj, i) if (Gia_ObjIsBuf(pObj)) {
        Vec_WecPush(vBufs, Vec_IntEntry(vTypes, k), i);
        Vec_IntWriteEntry(vMap, i, Vec_IntEntry(vTypes, k++));
    }
    assert(k == p->nBufs);
    Gia_ManForEachAnd(p, pObj, i) if (Gia_ObjIsBuf(pObj) && Gia_ObjIsBuf(Gia_ObjFanin0(pObj)))
    ...
    while (fChange) {
        int Entry1, Entry2, j;
        fChange = 0;
        Vec_IntForEachEntryDouble(vPairs, Entry1, Entry2, j) if ((Entry1 & 0xFFFF) + 1 == (Entry2 >> 16)) {
...
Gia_Man_t* Gia_ManDupPermIO(Gia_Man_t* p, Vec_Int_t* vPermI, Vec_Int_t* vPermO) {
    ...
    Gia_ManForEachCi(p, pObj, i)
        Gia_ManCi(p, Vec_IntEntry(vPermI, i))
            ->Value
