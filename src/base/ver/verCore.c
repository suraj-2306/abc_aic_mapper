...
static inline int Ver_NtkIsDefined(Abc_Ntk_t* pNtkBox) {
    assert(pNtkBox->pName);
    return Abc_NtkPiNum(pNtkBox) || Abc_NtkPoNum(pNtkBox);
}
static inline int Ver_ObjIsConnected(Abc_Obj_t* pObj) {
...
*/
    ...
    Vec_IntForEachEntry(pMan->vPerm, Input, i) if (Input != i) break;
            ...
            while (1) {
                ...
                if (pWord[0] >= '1' && pWord[0] <= '9') {
                    ...
                    if (nMsb == -1 && nLsb == -1) {
                        ...
                        pNetActual = Ver_ParseFindNet(pNtk, pWord);
                        if (pNetActual == NULL) {
                            if (!strncmp(pWord, "Open_", 5) || !strncmp(pWord, "dct_unconnected", 15))
...
*/
    ...
    Vec_PtrForEachEntry(Ver_Bundle_t*, vBundles, pBundle, k) if (pBundle->pNameFormal == NULL) break;
    if (k < Vec_PtrSize(vBundles)) {
        ...
        Vec_PtrForEachEntry(Ver_Bundle_t*, vBundles, pBundle, j)
            iBundle
            += Vec_PtrSize(pBundle->vNetsActual);
    ...
    Abc_NtkForEachPi(pNtkBox, pTerm, i) {
        ...
        pBundle = NULL;
        Vec_PtrForEachEntry(Ver_Bundle_t*, vBundles, pBundle, k) if (!strcmp(pBundle->pNameFormal, pNameFormal)) break;
        assert(pBundle != NULL);
        ...
        if (k == Vec_PtrSize(vBundles)) {
            ...
            if (pNameFormal[Length - 1] == ']') {
                ...
                if (Length > 0) {
                    Vec_PtrForEachEntry(Ver_Bundle_t*, vBundles, pBundle, j) if (!strncmp(pBundle->pNameFormal, pNameFormal, Length) && (int)strlen(pBundle->pNameFormal) == Length) break;
...
void Ver_ParseReportUndefBoxes(Ver_Man_t* pMan) {
    ...
    // count
    Vec_PtrForEachEntry(Abc_Ntk_t*, pMan->pDesign->vModules, pNtk, i)
        Abc_NtkForEachBlackbox(pNtk, pBox, k) if (pBox->pData && !Ver_NtkIsDefined((Abc_Ntk_t*)pBox->pData))((Abc_Ntk_t*)pBox->pData)
            ->fHiePath++;
    // print the stats
    printf("Warning: The design contains %d undefined object types interpreted as blackboxes:\n", nBoxes);
    Vec_PtrForEachEntry(Abc_Ntk_t*, pMan->pDesign->vModules, pNtk, i) if (!Ver_NtkIsDefined(pNtk))
...
int Ver_ParseFormalNetsAreDriven(Abc_Ntk_t* pNtk, char* pNameFormal) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, (Vec_Ptr_t*)pNtk->pData, pBox, k) {
        ...
        Vec_PtrForEachEntryReverse(Ver_Bundle_t*, (Vec_Ptr_t*)pBox->pCopy, pBundle, j) if (pBundle && !strcmp(pBundle->pNameFormal, pNameFormal)) break;
        ...
        Vec_PtrForEachEntry(Abc_Obj_t*, pBundle->vNetsActual, pNet, m) if (Abc_ObjFaninNum(pNet) > 0) return 1;
    }
    return 0;
}
...
Ver_Bundle_t* Ver_ParseGetNondrivenBundle(Abc_Ntk_t* pNtk, int Counter) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, (Vec_Ptr_t*)pNtk->pData, pBox, k) {
        ...
        if (pBundle == NULL)
            continue;
        // go through the actual nets of this bundle
        Vec_PtrForEachEntry(Abc_Obj_t*, pBundle->vNetsActual, pNet, m) if (!Abc_ObjFaninNum(pNet) && !Ver_ParseFormalNetsAreDriven(pNtk, pBundle->pNameFormal)) // non-driven
            return pBundle;
    }
    return NULL;
}
...
int Ver_ParseDriveFormal(Ver_Man_t* pMan, Abc_Ntk_t* pNtk, Ver_Bundle_t* pBundle0) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, (Vec_Ptr_t*)pNtk->pData, pBox, k) {
        // find a bundle with the given name in this instance
        Vec_PtrForEachEntryReverse(Ver_Bundle_t*, (Vec_Ptr_t*)pBox->pCopy, pBundle, j) if (pBundle && !strcmp(pBundle->pNameFormal, pName)) break;
        // skip non-driven bundles
        if (j == Vec_PtrSize((Vec_Ptr_t*)pBox->pCopy))
            continue;
        // check if any nets are driven in this bundle
        assert(pBundle); // Verify pBundle was assigned to.
        Vec_PtrForEachEntry(Abc_Obj_t*, pBundle->vNetsActual, pNetAct, m) if (Abc_ObjFaninNum(pNetAct) > 0) {
...
int Ver_ParseMaxBoxSize(Vec_Ptr_t* vUndefs) {
    ...
    Vec_PtrForEachEntry(Abc_Ntk_t*, vUndefs, pNtk, i)
        ...
        if (nMaxSize < Vec_PtrSize((Vec_Ptr_t*)pBox->pCopy))
            nMaxSize
        = Vec_PtrSize((Vec_Ptr_t*)pBox->pCopy);
    return nMaxSize;
}
...
int Ver_ParseAttachBoxes(Ver_Man_t* pMan) {
    ...
    while (Ver_ParseCheckNondrivenNets(vUndefs) && Counter < nMaxBoxSize) {
        // go through undef box types
        pBundle = NULL;
        Vec_PtrForEachEntry(Abc_Ntk_t*, vUndefs, pNtk, i) if ((pBundle = Ver_ParseGetNondrivenBundle(pNtk, Counter))) break;
        if (pBundle == NULL) {
            Counter++;
            continue;
        }
