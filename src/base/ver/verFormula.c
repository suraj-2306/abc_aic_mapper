...
*/
            ...
            case VER_PARSE_SYM_NEGBEF2:
                if (Flag == VER_PARSE_FLAG_VAR) { // if NEGBEF follows a variable, AND is assumed
        ...
        if (Flag == VER_PARSE_FLAG_ERROR)
            ...
            while (1) {                       // execute all the operations in the OpStack
                ...
                if (!Vec_IntSize(vStackOp)) { // if it is the only operation, push it back
                    Vec_IntPush(vStackOp, Oper1);
                    break;
                }
                Oper2 = Vec_IntPop(vStackOp);                                             // the operation before the last one
                if (Oper2 >= Oper1 && !(Oper1 == Oper2 && Oper1 == VER_PARSE_OPER_MUX)) { // if Oper2 precedence is higher or equal, execute it
...
int Ver_FormulaParserFindVar(char* pString, Vec_Ptr_t* vNames) {
    ...
    if (*pTemp == '\\') {
        ...
        while (*pTemp && *pTemp != ' ' && *pTemp != '\t' && *pTemp != '\r' && *pTemp != '\n' && *pTemp != ',' && *pTemp != '}' && *pTemp != VER_PARSE_SYM_OPEN && *pTemp != VER_PARSE_SYM_CLOSE && *pTemp != VER_PARSE_SYM_NEGBEF1 && *pTemp != VER_PARSE_SYM_NEGBEF2 && *pTemp != VER_PARSE_SYM_AND && *pTemp != VER_PARSE_SYM_OR && *pTemp != VER_PARSE_SYM_XOR && *pTemp != VER_PARSE_SYM_MUX1 && *pTemp != VER_PARSE_SYM_MUX2)
            pTemp++;
    }
