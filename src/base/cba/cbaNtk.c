...
static inline void Cba_NtkPrintDistribAddOne(Vec_Ptr_t* vTypes, Vec_Ptr_t* vOccurs, int Type, word Sign) {
    ...
    word Entry;
    int i;
    Vec_WrdForEachEntry(vType, Entry, i) if (Entry == Sign) {
...
void Cba_NtkCollectDistrib(Cba_Ntk_t* p, int* pCounts, int* pUserCounts) {
    int i;
    Cba_NtkForEachBox(p, i) if (Cba_ObjIsBoxUser(p, i))
        pUserCounts[Cba_ObjNtkId(p, i)]++;
    else pCounts[Cba_ObjType(p, i)]++;
}
...
void Cba_NtkPrintDistribStat(Cba_Ntk_t* p, int* pCounts, int* pUserCounts) {
    ...
    Cba_ManForEachNtk(p->pDesign, pNtk, i) if (pUserCounts[i])
...
void Cba_ManPrintDistribStat(Cba_Man_t* p, int* pCounts, int* pUserCounts) {
    ...
    Cba_ManForEachNtk(p, pNtk, i) if (pUserCounts[i])
        printf("%-20s = %5d\n", Cba_NtkName(pNtk), pUserCounts[i]);
}
...
int Cba_NtkIsTopoOrder(Cba_Ntk_t* p) {
    ...
    Cba_NtkForEachBox(p, iObj) if (!Cba_ObjIsSeq(p, iObj)) {
        Cba_ObjForEachFinFaninReal(p, iObj, iFin, iFanin, k) if (!Vec_BitEntry(vVisited, iFanin))
            fTopo
            = 0;
        if (!fTopo)
            break;
...
int Cba_ManIsTopoOrder(Cba_Man_t* p) {
    Cba_Ntk_t* pNtk;
    int i;
    Cba_ManForEachNtk(p, pNtk, i) if (!Cba_NtkIsTopoOrder(pNtk)) return 0;
    return 1;
}
...
int Cba_NtkCheckComboLoop_rec(Cba_Ntk_t* p, int iObj) {
    ...
    Cba_ObjForEachFinFaninReal(p, iObj, iFin, iFanin, k) if (!Cba_ObjIsSeq(p, iFanin)) if (!Cba_NtkCheckComboLoop_rec(p, iFanin)) return 0;
...
int Cba_NtkCheckComboLoop(Cba_Ntk_t* p) {
    ...
    Cba_NtkForEachBox(p, iObj) if (!Cba_ObjIsSeq(p, iObj)) if (!Cba_NtkCheckComboLoop_rec(p, iObj)) {
...
int Cba_NtkFindMissing(Vec_Int_t* vObjs, int nObjs) {
    ...
    Vec_IntForEachEntryStart(vMap, i, iObj, 1) if (i == -1) {
        Vec_IntFree(vMap);
        return iObj;
    }
    Vec_IntFree(vMap);
...
void Cba_ManGetClpStats_rec(Cba_Ntk_t* p, int* pCountN, int* pCountI, int* pCountO) {
    ...
    Cba_NtkForEachObj(p, iObj) if (Cba_ObjIsBoxPrim(p, iObj)) {
        pCountN[Id] += 1;
...
Vec_Int_t* Cba_NtkCollectInFons(Cba_Ntk_t* p, Vec_Int_t* vObjs) {
    ...
    // mark fanin fons
    Vec_IntForEachEntry(vObjs, iObj, i)
        Cba_ObjForEachFinFon(p, iObj, iFin, iFon, k) if (iFon > 0)
    ...
    // collect fons
    Vec_BitForEachEntry(vVisFons, Entry, iFon) if (Entry)
...
Vec_Int_t* Cba_NtkCollectOutFons(Cba_Ntk_t* p, Vec_Int_t* vObjs) {
    ...
    Cba_NtkForEachObj(p, iObj) if (!Vec_BitEntry(vMapObjs, iObj))
        Cba_ObjForEachFinFon(p, iObj, iFin, iFon, k) if (iFon > 0)
            Vec_BitWriteEntry(vVisFons, iFon, 1);
    // collect pointed fons among those in objects
    Vec_IntForEachEntry(vObjs, iObj, i)
        Cba_ObjForEachFon(p, iObj, iFon, k) if (Vec_BitEntry(vVisFons, iFon))
            Vec_IntPush(vFons, iFon);
...
void Cba_NtkInsertGroup(Cba_Ntk_t* p, Vec_Int_t* vObjs, Cba_Ntk_t* pSyn) {
    ...
    Cba_NtkForEachFinFon(p, iFon, iFin) if (Cba_FonIsReal(iFon) && Cba_FonCopy(p, iFon))
