...
Gia_Man_t* Cba_NtkBlast(Cba_Ntk_t* p, int fSeq) {
    ...
    Cba_NtkForEachObj(p, i) {
        ...
        if (Type == CBA_BOX_SLICE) {
            ...
            Cba_ObjForEachFinFon(p, i, iFinT, iFonT, k) if (k > 0)
                fSigned
                &= Cba_FonSigned(p, iFonT);
            for (b = 0; b < nRange; b++) {
                Vec_IntClear(vTemp0);
                Cba_ObjForEachFinFon(p, i, iFinT, iFonT, k) if (k > 0) {
        ...
        } else if (Type == CBA_BOX_SHIR || Type == CBA_BOX_SHIRA || Type == CBA_BOX_SHIL || Type == CBA_BOX_SHILA) {
            ...
            for (k = 1; k < nRange; k++)
                Vec_IntPush(vRes, 0);
        } else if (Type == CBA_BOX_LTHAN || Type == CBA_BOX_METHAN || Type == CBA_BOX_MTHAN || Type == CBA_BOX_LETHAN) {
