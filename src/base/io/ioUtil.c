...
Abc_Ntk_t* Io_ReadNetlist(char* pFileName, Io_FileType_t FileType, int fCheck) {
    ...
    if (fCheck && (Abc_NtkBlackboxNum(pNtk) || Abc_NtkWhiteboxNum(pNtk))) {
        ...
        Vec_PtrForEachEntry(Abc_Ntk_t*, pNtk->pDesign->vModules, pModel, i) if (!Abc_NtkIsAcyclicWithBoxes(pModel))
            fCycle
            = 1;
        if (fCycle) {
            Abc_NtkDelete(pNtk);
...
void Io_WriteHie(Abc_Ntk_t* pNtk, char* pBaseName, char* pFileName) {
    ...
    if (Io_ReadFileType(pFileName) == IO_FILE_BLIF) {
        if (pNtkResult->pDesign) {
            Vec_PtrForEachEntry(Abc_Ntk_t*, pNtkResult->pDesign->vModules, pNtkTemp, i) if (!Abc_NtkHasSop(pNtkTemp) && !Abc_NtkHasMapping(pNtkTemp) && !Abc_NtkHasMappingMO(pNtkTemp))
                Abc_NtkToSop(pNtkTemp, -1, ABC_INFINITY);
        } else {
        ...
        if (pNtkResult->pDesign) {
            Vec_PtrForEachEntry(Abc_Ntk_t*, pNtkResult->pDesign->vModules, pNtkTemp, i) if (!Abc_NtkHasAig(pNtkTemp) && !Abc_NtkHasMapping(pNtkTemp) && !Abc_NtkHasMappingMO(pNtkTemp))
                Abc_NtkToAig(pNtkTemp);
        } else {
...
Abc_Obj_t* Io_ReadCreateInv(Abc_Ntk_t* pNtk, char* pNameIn, char* pNameOut) {
    Abc_Obj_t *pNet, *pNode;
    pNet = Abc_NtkFindNet(pNtk, pNameIn);
    assert(pNet);
    pNode = Abc_NtkCreateNodeInv(pNtk, pNet);
    pNet = Abc_NtkFindNet(pNtk, pNameOut);
    assert(pNet);
    Abc_ObjAddFanin(pNet, pNode);
    return pNode;
}
