...
Vec_Ptr_t* Bbl_ManDfs(Bbl_Man_t* p) {
    ...
    Bbl_ManForEachObj(p, pObj) if (Bbl_ObjIsLut(pObj))
...
Abc_Ntk_t* Bbl_ManToAig(Bbl_Man_t* p) {
    ...
    Bbl_ManForEachObj(p, pObj) if (pFForms[Bbl_ObjFncHandle(pObj)] == NULL)
        pFForms[Bbl_ObjFncHandle(pObj)]
        = Dec_Factor(Bbl_ObjSop(p, pObj));
    if (fVerbose)
