...
static void Io_NtkWriteMiMoLibBlackboxes(FILE *pFile, MiMo_Library_t *pLib,
                                         int fWriteSpecialGates);
...
***********************************************************************/
void Io_NtkWrite(FILE *pFile, Abc_Ntk_t *pNtk, int fWriteLatches, int fBb2Wb,
                 int fSeq) {
  Abc_Ntk_t *pExdc;
  assert(Abc_NtkIsNetlist(pNtk));
...
***********************************************************************/
void Io_NtkWriteOne(FILE *pFile, Abc_Ntk_t *pNtk, int fWriteLatches, int fBb2Wb,
                    int fSeq) {
  ...
  if (fWriteLatches && !Abc_NtkIsComb(pNtk)) {
    fprintf(pFile, "\n");
    Abc_NtkForEachLatch(pNtk, pLatch, i) Io_NtkWriteLatch(pFile, pLatch);
    fprintf(pFile, "\n");
  }
  ...
  if (Abc_NtkBlackboxNum(pNtk) > 0 || Abc_NtkWhiteboxNum(pNtk) > 0) {
    fprintf(pFile, "\n");
    Abc_NtkForEachBlackbox(pNtk, pNode, i) Io_NtkWriteSubckt(pFile, pNode);
    fprintf(pFile, "\n");
    Abc_NtkForEachWhitebox(pNtk, pNode, i) Io_NtkWriteSubckt(pFile, pNode);
    fprintf(pFile, "\n");
  }

  // write each internal node
  Length = Abc_NtkHasMapping(pNtk)
               ? Mio_LibraryReadGateNameMax((Mio_Library_t *)pNtk->pManFunc)
               : 0;
...
void Io_NtkWritePis(FILE *pFile, Abc_Ntk_t *pNtk, int fWriteLatches) {
  ...
  if (fWriteLatches) {
    Abc_NtkForEachPi(pNtk, pTerm, i) {
      ...
      if (NameCounter && LineLength + AddedLength + 3 >
                             IO_WRITE_LINE_LENGTH) { // write the line extender
        fprintf(pFile, " \\\n");
...
void Io_NtkWriteNodeFanins(FILE *pFile, Abc_Obj_t *pNode) {
  ...
  AddedLength = strlen(pName) + 1;
  if (NameCounter && LineLength + AddedLength > 75) { // write the line extender
    fprintf(pFile, " \\\n");
    // reset the line length
    LineLength = 0;
    NameCounter = 0;
...
int Io_NtkWriteNodeGate(FILE *pFile, Abc_Obj_t *pNode, int Length) {
  ...
  for (pGatePin = Mio_GateReadPins(pGate), i = 0; pGatePin;
       pGatePin = Mio_PinReadNext(pGatePin), i++)
    fprintf(pFile, "%s=%s ", Mio_PinReadName(pGatePin),
            Abc_ObjName(Abc_ObjFanin(pNode, i)));
  assert(i == Abc_ObjFaninNum(pNode));
  fprintf(pFile, "%s=%s", Mio_GateReadOutName(pGate),
  ...
  fprintf(pFile, " %s=%s", Mio_GateReadOutName((Mio_Gate_t *)pNode2->pData),
...
void Io_NtkWriteNodeMappingMO(FILE *pFile, Abc_Obj_t *pNode) {
  ...
  while (pCellPinOut) {
    ...
    int expressionLength =
        sprintf(pPrintStr, " %s=%s", MiMo_GateOutRenamer(pCellPinOut->pPinOut),
                Abc_ObjName(pFanout));
    if (expressionLength + addedLength + 3 > IO_WRITE_LINE_LENGTH) {
      fprintf(pFile, " \\\n");
      addedLength = 0;
  ...
  if (pCell->pGate->Type == MIMO_NNC3) {
    for (int layer = 1; layer <= pCell->pGate->Depth + 1; layer++) {
      fprintf(pFile, "#");
      for (int k = Cm_Fa3LayerStart(layer - 1); k < Cm_Fa3LayerStart(layer);
           k++)
...
int Io_NtkWriteNode(FILE *pFile, Abc_Obj_t *pNode, int Length) {
  ...
  if (Abc_NtkHasMapping(pNode->pNtk)) {
    ...
    if (Abc_ObjIsBarBuf(pNode)) {
      ...
      fprintf(pFile, "%s %s", Abc_ObjName(Abc_ObjFanin0(pNode)),
              Abc_ObjName(Abc_ObjFanout0(pNode)));
      fprintf(pFile, "\n");
    } else {
      fprintf(pFile, ".gate");
...
***********************************************************************/
void Io_NtkWriteMiMoLibBlackboxes(FILE *pFile, MiMo_Library_t *pLib,
                                  int fWriteSpecialGates) {
  int i, j;
  char pPrintStr[256];
  int addedLength = 0;
  ...
  MiMo_LibForEachGate(pLib, pGate, i) {
    ...
    MiMo_GateForEachPinIn(pGate, pPinIn, j) {
      int expressionLength =
          sprintf(pPrintStr, " %s", MiMo_GateInRenamer(pPinIn));
    ...
    MiMo_GateForEachPinOut(pGate, pPinOut, j) {
      int expressionLength =
          sprintf(pPrintStr, " %s", MiMo_GateOutRenamer(pPinOut));
      if (expressionLength + addedLength + 2 > IO_WRITE_LINE_LENGTH) {
        fprintf(pFile, " \\\n");
        addedLength = 0;
      }
...
***********************************************************************/
void Abc_NtkConvertBb2Wb(char *pFileNameIn, char *pFileNameOut, int fSeq,
                         int fVerbose) {
  FILE *pFile;
  Abc_Ntk_t *pNetlist;
  // check the files
...
***********************************************************************/
char *Io_NtkDeriveSop(Mem_Flex_t *pMem, word uTruth, int nVars,
                      Vec_Int_t *vCover) {
  char *pSop;
  ...
  if (Vec_IntSize(vCover) == 0 ||
      (Vec_IntSize(vCover) == 1 && Vec_IntEntry(vCover, 0) == 0)) {
    char *pStr0 = " 0\n", *pStr1 = " 1\n";
    assert(RetValue == 0);
...
void Io_NtkWriteNodeInt(FILE *pFile, Abc_Obj_t *pNode, Vec_Int_t *vCover) {
  ...
  if (nVars <= 4) {
    ...
    extern word If_Dec6MinimumBase(word uTruth, int *pSupp, int nVarsAll,
                                   int *pnVars);
    extern void If_Dec7MinimumBase(word uTruth[2], int *pSupp, int nVarsAll,
                                   int *pnVars);
    ...
    // collect variables
    Abc_ObjForEachFanin(pNode, pNet, i) pVars[0][i] = pVars[1][i] = i;
    ...
    if (iVar >= 0) {
      ...
      for (c = 0; c < 2; c++) {
        pSop = Io_NtkDeriveSop((Mem_Flex_t *)Abc_ObjNtk(pNode)->pManFunc,
                               (word)(nVars == 7 ? Cofs7[c][0] : Cofs6[c]),
                               nVarsMin[c], vCover);
        fprintf(pFile, ".names");
...
***********************************************************************/
void Io_NtkWriteNodeIntStruct(FILE *pFile, Abc_Obj_t *pNode, Vec_Int_t *vCover,
                              char *pStr) {
  Abc_Obj_t *pNet;
  ...
  if (nLeaves <= Abc_MaxInt(nLutLeaf2, Abc_MaxInt(nLutLeaf, nLutRoot))) {
    ...
    extern int If_CluMinimumBase(word * t, int *pSupp, int nVarsAll,
                                 int *pnVars);
    ...
    // derive truth table
    Abc_SopToTruthBig((char *)Abc_ObjData(pNode), nLeaves, pTruths, pCube,
                      pRes);
    if (Kit_TruthIsConst0((unsigned *)pRes, nLeaves) ||
        Kit_TruthIsConst1((unsigned *)pRes, nLeaves)) {
      fprintf(pFile, ".names %s\n %d\n", Abc_ObjName(Abc_ObjFanout0(pNode)),
              Kit_TruthIsConst1((unsigned *)pRes, nLeaves));
      return;
    }
    ...
    if (Length == 2) {
      if (!If_CluCheckExt(NULL, pRes, nLeaves, nLutLeaf, nLutRoot, pLut0, pLut1,
                          &Func0, &Func1)) {
        Extra_PrintHex(stdout, (unsigned *)pRes, nLeaves);
        printf("    ");
        Kit_DsdPrintFromTruth((unsigned *)pRes, nLeaves);
        printf("\n");
        printf("Node \"%s\" is not decomposable. Writing BLIF has failed.\n",
               Abc_ObjName(Abc_ObjFanout0(pNode)));
        return;
      ...
      if (!If_CluCheckExt3(NULL, pRes, nLeaves, nLutLeaf, nLutLeaf2, nLutRoot,
                           pLut0, pLut1, pLut2, &Func0, &Func1, &Func2)) {
        Extra_PrintHex(stdout, (unsigned *)pRes, nLeaves);
        printf("    ");
        Kit_DsdPrintFromTruth((unsigned *)pRes, nLeaves);
        printf("\n");
    ...
    // write SOP
    pSop = Io_NtkDeriveSop((Mem_Flex_t *)Abc_ObjNtk(pNode)->pManFunc, Func1,
                           pLut1[0], vCover);
    fprintf(pFile, "%s", pSop);
    ...
    if (Length == 3 && pLut2[0] > 0) {
      ...
      // write SOP
      pSop = Io_NtkDeriveSop((Mem_Flex_t *)Abc_ObjNtk(pNode)->pManFunc, Func2,
                             pLut2[0], vCover);
      fprintf(pFile, "%s", pSop);
    }
    ...
    fprintf(pFile, " %s\n", Abc_ObjName(Abc_ObjFanout0(pNode)));
    // write SOP
    pSop = Io_NtkDeriveSop((Mem_Flex_t *)Abc_ObjNtk(pNode)->pManFunc, Func0,
                           pLut0[0], vCover);
    fprintf(pFile, "%s", pSop);
  }
}
...
***********************************************************************/
void Io_NtkWriteModelIntStruct(FILE *pFile, Abc_Obj_t *pNode, Vec_Int_t *vCover,
                               char *pStr) {
  Abc_Obj_t *pNet;
  int nLeaves = Abc_ObjFaninNum(pNode);
  int i, nLutLeaf, nLutLeaf2, nLutRoot, Length;
  ...
  if (nLeaves <= Abc_MaxInt(nLutLeaf2, Abc_MaxInt(nLutLeaf, nLutRoot))) {
    // write the .names line
    fprintf(pFile, ".names");
    Abc_ObjForEachFanin(pNode, pNet, i) fprintf(pFile, " %c", 'a' + i);
    // get the output name
    fprintf(pFile, " %s\n", "o");
    // write the cubes
    fprintf(pFile, "%s", (char *)Abc_ObjData(pNode));
...
void Io_WriteBlifInt(Abc_Ntk_t *pNtk, char *FileName, char *pLutStruct,
  ...
  if (fUseHie) {
    // write each internal node
    fprintf(pFile, "\n");
    Abc_NtkForEachNode(pNtk, pNode, i) Io_NtkWriteNodeSubckt(pFile, pNode, 0);
    fprintf(pFile, ".end\n\n");
    // write models
    Abc_NtkForEachNode(pNtk, pNode, i)
