...
void Io_WriteVerilogLatches(FILE* pFile, Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachLatch(pNtk, pLatch, i) if (Abc_LatchInit(pLatch) == ABC_INIT_ZERO || Abc_LatchInit(pLatch) == ABC_INIT_ONE) break;
        ...
        Abc_NtkForEachNode(pNtk, pObj, i) {
            ...
            Abc_ObjForEachFanin(pObj, pFanin, k)
                Hop_IthVar((Hop_Man_t*)pNtk->pManFunc, k)
                    ->pData
                = Extra_UtilStrsav(Io_WriteVerilogGetName(Abc_ObjName(pFanin)));
            // write the formula
...
void Io_WriteVerilogObjectsLut(FILE* pFile, Abc_Ntk_t* pNtk, int nLutSize, int fFixed) {
    ...
    Abc_NtkForEachNode(pNtk, pObj, i) {
        ...
        Abc_ObjForEachFanin(pObj, pTerm, k)
            Length
            = Abc_MaxInt(Length, strlen(Io_WriteVerilogGetName(Abc_ObjName(pTerm))));
    }
...
void Io_WriteVerilogLut(Abc_Ntk_t* pNtk, char* pFileName, int nLutSize, int fFixed, int fNoModules) {
    ...
    Abc_Obj_t* pObj;
    int i, Counter = 0;
    Abc_NtkForEachNode(pNtk, pObj, i) if (Abc_ObjFaninNum(pObj) > nLutSize) {
        if (Counter < 3)
