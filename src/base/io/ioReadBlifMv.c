...
Abc_Ntk_t* Io_ReadBlifMv(char* pFileName, int fBlifMv, int fCheck) {
    ...
    if (Vec_PtrSize(pDesign->vModules) > 1) {
        ...
        Vec_PtrForEachEntryStart(Abc_Ntk_t*, pDesign->vModules, pExdc, i, 1) if (!strcmp(pExdc->pName, "EXDC")) {
...
static int Io_MvGetLine(Io_MvMan_t* p, char* pToken) {
    char* pLine;
    int i;
    Vec_PtrForEachEntry(char*, p->vLines, pLine, i) if (pToken < pLine) return i;
    return -1;
}
...
static int Io_MvReadInterfaces(Io_MvMan_t* p) {
    ...
    Vec_PtrForEachEntry(Io_MvMod_t*, p->vModels, pMod, i) {
        ...
        // parse the inputs
        Vec_PtrForEachEntry(char*, pMod->vInputs, pLine, k) if (!Io_MvParseLineInputs(pMod, pLine)) return 0;
        // parse the outputs
        Vec_PtrForEachEntry(char*, pMod->vOutputs, pLine, k) if (!Io_MvParseLineOutputs(pMod, pLine)) return 0;
        // parse the constraints
        nOutsOld = Abc_NtkPoNum(pMod->pNtk);
        Vec_PtrForEachEntry(char*, pMod->vConstrs, pLine, k) if (!Io_MvParseLineConstrs(pMod, pLine)) return 0;
        pMod->pNtk->nConstrs = Abc_NtkPoNum(pMod->pNtk) - nOutsOld;
        Vec_PtrForEachEntry(char*, pMod->vLtlProperties, pLine, k) if (!Io_MvParseLineLtlProperty(pMod, pLine)) return 0;
        // report the results
...
static Abc_Des_t* Io_MvParse(Io_MvMan_t* p) {
    ...
    Vec_PtrForEachEntry(Io_MvMod_t*, p->vModels, pMod, i) {
        ...
        // parse the mv lines
        Vec_PtrForEachEntry(char*, pMod->vMvs, pLine, k) if (!Io_MvParseLineMv(pMod, pLine)) return NULL;
        ...
        // parse the flops
        Vec_PtrForEachEntry(char*, pMod->vFlops, pLine, k) if (!Io_MvParseLineFlop(pMod, pLine)) return NULL;
        // parse the latches
        Vec_PtrForEachEntry(char*, pMod->vLatches, pLine, k) if (!Io_MvParseLineLatch(pMod, pLine)) return NULL;
        // parse the reset lines
        if (p->fUseReset)
            Vec_PtrForEachEntry(char*, pMod->vResets, pLine, k) if (!Io_MvParseLineNamesMv(pMod, pLine, 1)) return NULL;
        // parse the nodes
        if (p->fBlifMv) {
            Vec_PtrForEachEntry(char*, pMod->vNames, pLine, k) if (!Io_MvParseLineNamesMv(pMod, pLine, 0)) return NULL;
        } else {
            Vec_PtrForEachEntry(char*, pMod->vNames, pLine, k) if (!Io_MvParseLineNamesBlif(pMod, pLine)) return NULL;
            Vec_PtrForEachEntry(char*, pMod->vShorts, pLine, k) if (!Io_MvParseLineShortBlif(pMod, pLine)) return NULL;
        }
        // parse the subcircuits
        Vec_PtrForEachEntry(char*, pMod->vSubckts, pLine, k) if (!Io_MvParseLineSubckt(pMod, pLine)) return NULL;
...
static int Io_MvParseLineFlop(Io_MvMod_t* p, char* pLine) {
    ...
    // get flop output
    Vec_PtrForEachEntry(char*, vTokens, pToken, i) if (pToken[0] == 'Q' && pToken[1] == '=') break;
    ...
    pOutput = pToken + 2;
    // get flop input
    Vec_PtrForEachEntry(char*, vTokens, pToken, i) if (pToken[0] == 'D' && pToken[1] == '=') break;
    if (i == Vec_PtrSize(vTokens)) {
...
static int Io_MvParseLiteralMv(Io_MvMod_t* p, Abc_Obj_t* pNode, char* pToken, Vec_Str_t* vFunc, int iLit) {
    ...
    if (pToken[0] == '=') {
        // find the fanins
        Abc_ObjForEachFanin(pNode, pFanin, i) if (!strcmp(Abc_ObjName(pFanin), pToken + 1)) break;
