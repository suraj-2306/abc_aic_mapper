...
Vec_Int_t* Acb_NtkDivisors(Acb_Ntk_t* p, int Pivot, int nTfiLevMin, int fDelay) {
    ...
    if (0) // delay-oriented
    {
        ...
        Acb_ObjForEachFaninFast(p, Pivot, pFanins, iFanin, k) if (Acb_ObjIsDelayCriticalFanin(p, Pivot, iFanin))
            Acb_NtkDivisors_rec(p, iFanin, nTfiLevMin, vDivs);
        // add non-critical fanins
        Acb_ObjForEachFaninFast(p, Pivot, pFanins, iFanin, k) if (!Acb_ObjIsDelayCriticalFanin(p, Pivot, iFanin)) if (!Acb_ObjSetTravIdCur(p, iFanin))
        ...
        Acb_ObjForEachFaninFast(p, Pivot, pFanins, iFanin, k) if (!Acb_ObjSetTravIdCur(p, iFanin))
            Vec_IntPush(vDivs, iFanin);
        /*
        // start from critical fanins
        assert( Acb_ObjLevelD( p, Pivot ) > 1 );
...
int Acb_ObjLabelTfo_rec(Acb_Ntk_t* p, int iObj, int nTfoLevMax, int nFanMax, int fFirst) {
    ...
    Acb_ObjForEachFanout(p, iObj, iFanout, i) if (!fFirst || Acb_ObjIsDelayCriticalFanin(p, iFanout, iObj))
        fHasNone
        |= 2 == Acb_ObjLabelTfo_rec(p, iFanout, nTfoLevMax, nFanMax, 0);
...
Vec_Int_t* Acb_NtkCollectTfoSideInputs(Acb_Ntk_t* p, int Pivot, Vec_Int_t* vTfo) {
    ...
    Vec_IntForEachEntry(vTfo, Node, i)
        Acb_ObjSetTravIdCur(p, Node);
    Vec_IntForEachEntry(vTfo, Node, i)
        Acb_ObjForEachFaninFast(p, Node, pFanins, iFanin, k) if (!Acb_ObjSetTravIdCur(p, iFanin) && iFanin != Pivot)
...
Vec_Int_t* Acb_NtkCollectWindow(Acb_Ntk_t* p, int Pivot, Vec_Int_t* vTfi, Vec_Int_t* vTfo, Vec_Int_t* vRoots) {
    ...
    Vec_IntForEachEntry(vTfi, iObj, i) {
        int fIsTfiInput = 0;
        Acb_ObjForEachFaninFast(p, iObj, pFanins, iFanin, k) if (!Acb_ObjIsTravIdCur(p, iFanin)) // fanin is not in TFI
            fIsTfiInput
            = 1; // mark as leaf
...
int Acb_NtkFindSupp2(Acb_Ntk_t* p, int Pivot, sat_solver* pSat, int nVars, int nDivs, Vec_Int_t* vWin, Vec_Int_t* vSupp, int nLutSize, int fDelay) {
    ...
    Acb_ObjForEachFaninFast(p, Pivot, pFanins, iFanin, k) {
        ...
        Vec_IntClear(vSupp);
        Acb_ObjForEachFaninFast(p, Pivot, pFanins2, iFanin2, k2) if (iFanin != iFanin2)
...
int Acb_NtkFindSupp3(Acb_Ntk_t* p, int Pivot, sat_solver* pSat, int nVars, int nDivs, Vec_Int_t* vWin, Vec_Int_t* vSupp, int nLutSize, int fDelay) {
    ...
    Acb_ObjForEachFaninFast(p, Pivot, pFanins, iFanin, k) {
        ...
        Acb_ObjForEachFaninFast(p, Pivot, pFanins2, iFanin2, k2) {
            ...
            // collect fanins of the root node
            Vec_IntClear(vSupp);
            Acb_ObjForEachFaninFast(p, Pivot, pFanins3, iFanin3, k3) if (iFanin3 != iFanin && iFanin3 != iFanin2) {
            ...
            NodeMark = 0;
            Vec_IntForEachEntry(vSupp, iFanin3, k3) if (iFanin3 >= nDivs)
    ...
    Acb_ObjForEachFaninFast(p, Pivot, pFanins, iFanin, k) {
        ...
        Acb_ObjForEachFaninFast(p, iFanin, pFanins2, iFanin2, k2) {
            ...
            // collect fanins of the second node
            Acb_ObjForEachFaninFast(p, iFanin, pFanins3, iFanin3, k3) if (iFanin3 != iFanin2)
                Vec_IntPush(vSupp, Abc_Var2Lit(Acb_ObjFunc(p, iFanin3) + 6 * nVars + nDivs, 0));
            // collect fanins of the third node
            Acb_ObjForEachFaninFast(p, iFanin2, pFanins3, iFanin3, k3) {
                assert(Acb_ObjFunc(p, iFanin3) >= 0);
...
static inline int Acb_NtkObjMffcEstimate(Acb_Ntk_t* pNtk, int iObj) {
    ...
    Acb_ObjForEachFaninFast(pNtk, iObj, pFanins, iFanin, k) if (Acb_ObjIsAreaCritical(pNtk, iFanin))
        iFaninCrit
        = iFanin,
        Count++;
    if (Count != 1)
        return Count;
    Acb_ObjForEachFaninFast(pNtk, iFaninCrit, pFanins, iFanin, k) if (Acb_ObjIsAreaCritical(pNtk, iFanin))
        Count++;
    return Count;
}
        ...
        if (Result) {
            ...
            Vec_IntForEachEntryStart(p->vSupp, Var, i, Vec_IntSize(p->vSupp) - Result) // H variables
            {
                ...
                if (!sat_solver_addclause(p->pSat[1], &Lit, &Lit + 1)) {
                    if (fVerbose || p->pPars->fVerbose) printf("Error: SAT solver became UNSAT at a wrong place (place 2).  ");
                    fBecameUnsat = 1;
                }
            }
            ...
            Vec_IntForEachEntryStop(p->vSupp, Var, i, Vec_IntSize(p->vSupp) - Result) // G variables
            {
                ...
                if (!sat_solver_addclause(p->pSat[1], &Lit, &Lit + 1)) {
                    if (fVerbose || p->pPars->fVerbose) printf("Error: SAT solver became UNSAT at a wrong place (place 1).  ");
                    fBecameUnsat = 1;
                }
            }
            if (fBecameUnsat) {
...
void Acb_NtkOpt(Acb_Ntk_t* pNtk, Acb_Par_t* pPars) {
    ...
    if (pMan->pPars->fArea) {
        ...
        for (n = 2; n >= 0; n--)
            Acb_NtkForEachNode(pNtk, iObj) if (iObj < nNodes && !Vec_BitEntry(vVisited, iObj) && Acb_NtkObjMffcEstimate(pNtk, iObj) >= n) {
                pMan->nNodes++;
                //if ( iObj != 103 )
                //    continue;
