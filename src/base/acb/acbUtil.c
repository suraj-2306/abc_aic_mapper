...
int Acb_ObjComputeLevelD(Acb_Ntk_t* p, int iObj) {
    ...
    Acb_ObjForEachFaninFast(p, iObj, pFanins, iFanin, k)
        Level
        = Abc_MaxInt(Level, Acb_ObjLevelD(p, iFanin));
...
int Acb_NtkComputeLevelD(Acb_Ntk_t* p, Vec_Int_t* vTfo) {
    ...
    Acb_NtkForEachCo(p, iObj, i)
        Level
        = Abc_MaxInt(Level, Acb_ObjLevelD(p, iObj));
    p->LevelMax = Level;
    return Level;
}
...
int Acb_ObjComputeLevelR(Acb_Ntk_t* p, int iObj) {
    ...
    Acb_ObjForEachFanout(p, iObj, iFanout, k)
        Level
        = Abc_MaxInt(Level, Acb_ObjLevelR(p, iFanout));
...
int Acb_NtkComputeLevelR(Acb_Ntk_t* p, Vec_Int_t* vTfi) {
    ...
    Acb_NtkForEachCi(p, iObj, i)
        Level
        = Abc_MaxInt(Level, Acb_ObjLevelR(p, iObj));
...
int Acb_ObjComputePathD(Acb_Ntk_t* p, int iObj) {
    ...
    Acb_ObjForEachFaninFast(p, iObj, pFanins, iFanin, k) if (!Acb_ObjSlack(p, iFanin))
        Path
        += Acb_ObjPathD(p, iFanin);
...
int Acb_ObjComputePathR(Acb_Ntk_t* p, int iObj) {
    ...
    Acb_ObjForEachFanout(p, iObj, iFanout, k) if (!Acb_ObjSlack(p, iFanout))
        Path
        += Acb_ObjPathR(p, iFanout);
...
void Acb_NtkResetNode(Acb_Ntk_t* p, int Pivot, word uTruth, Vec_Int_t* vSupp) {
    ...
    Vec_IntForEachEntry(vFanins, iFanin, k) if (!Acb_ObjIsCio(p, iFanin) && Acb_ObjFanoutNum(p, iFanin) == 0)
...
int Acb_ObjToGia2(Gia_Man_t* pNew, int fUseBuf, Acb_Ntk_t* p, int iObj, Vec_Int_t* vTemp, int fUseXors) {
    ...
    if (Type == ABC_OPER_BIT_AND || Type == ABC_OPER_BIT_NAND) {
        ...
        Vec_IntForEachEntry(vTemp, iFanin, k)
            Res
            = Gia_ManAppendAnd2(pNew, Res, iFanin);
    ...
    if (Type == ABC_OPER_BIT_OR || Type == ABC_OPER_BIT_NOR) {
        ...
        Vec_IntForEachEntry(vTemp, iFanin, k)
            Res
            = Gia_ManAppendOr2(pNew, Res, iFanin);
    ...
    if (Type == ABC_OPER_BIT_XOR || Type == ABC_OPER_BIT_NXOR) {
        ...
        Vec_IntForEachEntry(vTemp, iFanin, k)
            Res
            = fUseXors ? Gia_ManAppendXorReal2(pNew, Res, iFanin) : Gia_ManAppendXor2(pNew, Res, iFanin);
...
Vec_Int_t* Acb_NtkCollectCopies(Acb_Ntk_t* p, Gia_Man_t* pGia, Vec_Ptr_t** pvNodesR, Vec_Bit_t** pvPolar) {
    ...
    Gia_ManForEachCiId(pGia, iObj, i) if (i < Gia_ManCiNum(pGia) - nTargets)
        Vec_PtrWriteEntry(vNodesR, iObj, Abc_UtilStrsav(Acb_ObjNameStr(p, Acb_NtkCi(p, i))));
    else Vec_PtrWriteEntry(vNodesR, iObj, Abc_UtilStrsav(Acb_ObjNameStr(p, Vec_IntEntry(&p->vTargets, i - (Gia_ManCiNum(pGia) - nTargets)))));
    ...
    Vec_IntForEachEntry(vObjs, iObj, i) if ((iLit = Acb_ObjCopy(p, iObj)) >= 0 && Gia_ObjIsAnd(Gia_ManObj(pGia, Abc_Lit2Var(iLit)))) {
...
Vec_Int_t* Acb_NtkCollectUser(Acb_Ntk_t* p, Vec_Ptr_t* vUser) {
    ...
    Acb_NtkForEachNode(p, i) if (Acb_ObjName(p, i) > 0)
        Vec_IntWriteEntry(vMap, Acb_ObjName(p, i), Acb_ObjCopy(p, i));
    Vec_PtrForEachEntry(char*, vUser, pTemp, i) if (Acb_NtkStrId(p, pTemp) < Vec_IntSize(vMap)) {
...
Vec_Int_t* Abc_NtkCollectCopies(Abc_Ntk_t* p, Gia_Man_t* pGia, Vec_Ptr_t** pvNodesR, Vec_Bit_t** pvPolar) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vObjs, pObj, i) if ((iLit = pObj->iTemp) >= 0 && Gia_ObjIsAnd(Gia_ManObj(pGia, Abc_Lit2Var(iLit)))) {
        if (!Vec_BitEntry(vDriver, Abc_Lit2Var(iLit)) && Vec_PtrEntry(vNodesR, Abc_Lit2Var(iLit)) == NULL) {
...
Gia_Man_t* Abc_NtkToGia2(Abc_Ntk_t* p, int fUseXors) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pObj, i)
        pObj->iTemp
        = Abc_ObjToGia2(pNew, p, pObj, vFanins, fUseXors);
    Vec_PtrFree(vNodes);
    Vec_IntFree(vFanins);
    ...
    for (pTemp = pBuffer; *pTemp; pTemp++) {
        ...
        if (*pTemp == '\n')
            ...
            if (First)
                ...
                char* pName;
                int i;
                Vec_PtrForEachEntry(char*, vNames, pName, i) if (!strcmp(pName, pToken))
...
void Acb_Ntk4DumpWeights(char* pFileNameIn, Vec_Ptr_t* vObjNames, char* pFileName) {
    ...
    Vec_PtrForEachEntry(char*, vObjNames, pName, i) {
        Acb_NtkForEachObj(pNtkF, iObj) if (!strcmp(Acb_ObjNameStr(pNtkF, iObj), pName))
