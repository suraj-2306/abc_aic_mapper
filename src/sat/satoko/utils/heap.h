...
static inline void heap_percolate_down(heap_t* h, unsigned i) {
    ...
    while (left(i) < vec_uint_size(h->data)) {
        unsigned child = right(i) < vec_uint_size(h->data) && compare(h, vec_uint_at(h->data, right(i)), vec_uint_at(h->data, left(i)))
                             ? right(i)
                             : left(i);
...
static inline int heap_in_heap(heap_t* p, unsigned entry) {
    return (entry < vec_int_size(p->indices)) && (vec_int_at(p->indices, entry) >= 0);
}
