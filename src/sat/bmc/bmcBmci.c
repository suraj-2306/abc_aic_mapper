...
void Bmc_BmciUnfold(Gia_Man_t* pNew, Gia_Man_t* p, Vec_Int_t* vFFLits, int fPiReuse) {
    ...
    Gia_ManForEachRo(p, pObj, i)
        pObj->Value
        = Vec_IntEntry(vFFLits, i);
    Gia_ManForEachPi(p, pObj, i)
        pObj->Value
        = fPiReuse ? Gia_ObjToLit(pNew, Gia_ManPi(pNew, Gia_ManPiNum(pNew) - Gia_ManPiNum(p) + i)) : Gia_ManAppendCi(pNew);
...
int Bmc_BmciPerform(Gia_Man_t* p, Vec_Int_t* vInit0, Vec_Int_t* vInit1, int nFrames, int nWords, int nTimeOut, int fVerbose) {
    ...
    for (f = 0; f < nFrames; f++) {
        ...
        Vec_IntForEachEntryTwo(vLits0, vLits1, iLit0, iLit1, i) if ((iLit0 >= 2 || iLit1 >= 2) && iLit0 != iLit1)
            Vec_IntPush(vMiters, Gia_ManHashXor(pNew, iLit0, iLit1)),
            nMiters++;
        else Vec_IntPush(vMiters, -1);
        ...
        // transfer variables
        Gia_ManForEachCand(pPart, pObj, i) if (pCnf->pVarNums[i] >= 0) {
