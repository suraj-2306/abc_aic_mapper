...
int Saig_ManBmcTerSimCount01(Aig_Man_t* p, unsigned* pInfo) {
    ...
    Saig_ManForEachLi(p, pObj, i) if (!Aig_ObjIsConst1(Aig_ObjFanin0(pObj)))
        Counter
        += (Saig_ManBmcSimInfoGet(pInfo, pObj) != SAIG_TER_UND);
    return Counter;
}
...
Gia_ManBmc_t* Saig_Bmc3ManStart(Aig_Man_t* pAig, int nTimeOutOne, int nConfLimit, int fUseSatoko, int fUseGlucose) {
    ...
    Aig_ManForEachNode(pAig, pObj, i) if (Vec_IntEntry(p->vMapping, Aig_ObjId(pObj)) > 0)
        Vec_IntWriteEntry(p->vId2Num, Aig_ObjId(pObj), p->nObjNums++);
...
void Saig_Bmc3ManStop(Gia_ManBmc_t* p) {
    if (p->pPars->fVerbose) {
        ...
        Abc_Print(1, "LStart(P) = %d  LDelta(Q) = %d  LRatio(R) = %d  ReduceDB = %d  Vars = %d  Used = %d (%.2f %%)\n",
                  ...
                  p->pSat ? sat_solver_nvars(p->pSat) : p->pSat3 ? bmcg_sat_solver_varnum(p->pSat3)
                                                                 : satoko_varnum(p->pSat2),
                  nUsedVars,
                  100.0 * nUsedVars / (p->pSat ? sat_solver_nvars(p->pSat) : p->pSat3 ? bmcg_sat_solver_varnum(p->pSat3)
                                                                                      : satoko_varnum(p->pSat2)));
    ...
    for (f = 0; f < pPars->nFramesMax; f++) {
        ...
        if (!pPars->nFramesJump && Aig_ManRegNum(pAig) < 30 && f == (1 << Aig_ManRegNum(pAig))) {
            ...
            if (p->pPars->fUseBridge)
                Saig_ManForEachPo(pAig, pObj, i) if (!(p->vCexes && Vec_PtrEntry(p->vCexes, i)) && !(p->pTime4Outs && p->pTime4Outs[i] == 0)) // not SAT and not timed out
...
*/
            ...
            if (status == l_False) {
                ...
                if (!pPars->fSolveAll) {
                    if (pPars->fVerbose) {
                        ...
                        Abc_Print(1, "Cla =%9.0f. ", (double)(p->pSat ? p->pSat->stats.clauses : p->pSat3 ? bmcg_sat_solver_clausenum(p->pSat3)
                                                                                                          : satoko_clausenum(p->pSat2)));
                        Abc_Print(1, "Conf =%7.0f. ", (double)(p->pSat ? p->pSat->stats.conflicts : p->pSat3 ? bmcg_sat_solver_conflictnum(p->pSat3)
                                                                                                             : satoko_conflictnum(p->pSat2)));
                        ...
                        Abc_Print(1, "Learn =%7.0f. ", (double)(p->pSat ? p->pSat->stats.learnts : p->pSat3 ? bmcg_sat_solver_learntnum(p->pSat3)
                                                                                                            : satoko_learntnum(p->pSat2)));
        ...
        if (pPars->fVerbose) {
            if (fFirst == 1 && f > 0 && (p->pSat ? p->pSat->stats.conflicts : p->pSat3 ? bmcg_sat_solver_conflictnum(p->pSat3)
                                                                                       : satoko_conflictnum(p->pSat2))
                                            > 1) {
                fFirst = 0;
