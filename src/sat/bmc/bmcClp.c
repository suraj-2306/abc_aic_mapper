...
void Bmc_ComputeSimTest(Gia_Man_t* p) {
    ...
    for (i = 0; i < nPats; i++) {
        ...
        Vec_IntForEachEntry(vPat2, Bit, v) {
            if (Bit)
                DiffCount[v]++;
            Vec_IntForEachEntryStart(vPat2, Bit2, w, v + 1) if (Bit && Bit2)
                Count[Res][v][w]++;
        }
    }
    Vec_IntFree(vPat);
...
int Bmc_CollapseIrredundant(Vec_Str_t* vSop, int nCubes, int nVars) {
    ...
    {
        int j = 0;
        Vec_PtrForEachEntry(char*, vCubes, pCube, i) if (pCube != NULL) for (k = 0; k < nVars + 3; k++)
...
int Bmc_CollapseExpandRound2(sat_solver* pSat, Vec_Int_t* vLits, Vec_Int_t* vTemp, int nBTLimit, int fOnOffSetLit) {
    ...
    int i, iLit, nLits;
    Vec_IntClear(vTemp);
    Vec_IntForEachEntry(vLits, iLit, i) if (iLit != -1)
    ...
    Vec_IntForEachEntry(vLits, iLit, i) if (iLit != -1 && Vec_IntFind(vTemp, iLit) == -1)
...
int Bmc_CollapseExpandRound(sat_solver* pSat, sat_solver* pSatOn, Vec_Int_t* vLits, Vec_Int_t* vNums, Vec_Int_t* vTemp, int nBTLimit, int fCanon, int fOnOffSetLit) {
    ...
    for (k = Vec_IntSize(vLits) - 1; k >= 0; k--) {
        ...
        if (pSatOn) {
            ...
            // put into new array
            Vec_IntClear(vTemp);
            Vec_IntForEachEntry(vLits, iLit, n) if (iLit != -1)
...
int Bmc_CollapseExpand2(sat_solver* pSat, sat_solver* pSatOn, Vec_Int_t* vLits, Vec_Int_t* vNums, Vec_Int_t* vTemp, int nBTLimit, int fCanon, int fOnOffSetLit) {
    ...
    if (!fCanon) {
        ...
        for (i = 0; i < Vec_IntSize(vNums); i++) {
            ...
            if (fOnOffSetLit >= 0)
                Vec_IntPush(vTemp, fOnOffSetLit);
            Vec_IntForEachEntry(vNums, iNum, k) if (k != i)
...
cleanup:
    ...
    {
        vRes = vSop[fComplete[1]];
        vSop[fComplete[1]] = NULL;
        if (iCube > 1)
    ...
    if (fVeryVerbose) {
        ...
        if (fProfile) {
            Abc_PrintTime(1, "Expand check1 ", clkCheck1);
            clkCheck1 = 0;
            Abc_PrintTime(1, "Expand check2 ", clkCheck2);
            clkCheck2 = 0;
            Abc_PrintTime(1, "Expand sat    ", clkCheckS);
            clkCheckS = 0;
            Abc_PrintTime(1, "Expand unsat  ", clkCheckU);
            clkCheckU = 0;
        }
    }
    Vec_StrFreeP(&vSop[0]);
