    ...
    for (i = p->pPars->nVars + 1; i < p->nObjs; i++) {
        ...
        if (k == -1) // fanins are equal
        {
            ...
            for (k = p->LutMask; k >= 0; k--)
                if (bmcg_sat_solver_read_cex_varvalue(p->pSat, Zyx_FuncVar(p, i - 1, k)) != bmcg_sat_solver_read_cex_varvalue(p->pSat, Zyx_FuncVar(p, i, k)))
                    break;
            if (k == -1) // truth tables cannot be equal
                continue;
            // rule out these truth tables
            if (bmcg_sat_solver_read_cex_varvalue(p->pSat, Zyx_FuncVar(p, i - 1, k)) == 0 && bmcg_sat_solver_read_cex_varvalue(p->pSat, Zyx_FuncVar(p, i, k)) == 1) {
                continue;
            }
            nLazy++;
...
static inline int Zyx_ManEval(Zyx_Man_t* p) {
    ...
    if (p->pPars->fMajority) {
        Vec_IntForEachEntry(p->vMidMints, iMint, i) if (Abc_TtGetBit(pSpec, iMint) != Abc_TtGetBit(Zyx_ManTruth(p, p->nObjs - 1), iMint)) return iMint;
        return -1;
    } else {
...
void Zyx_ManExactSynthesis(Bmc_EsPar_t* pPars) {
    ...
    abctime clkTotal = Abc_Clock(), clk = Abc_Clock();
    Zyx_Man_t* p;
    word pTruth[16];
    if (!pPars->fMajority) {
        ...
        if (pTruth[0] & 1) {
            fCompl = 1;
            Abc_TtNot(pTruth, Abc_TtWordNum(pPars->nVars));
        }
    }
...
void Zyx_TestExact(char* pFileName) {
    ...
    for (iObj = nVars; fgets(Line, 1000, pFile) != NULL; iObj++) {
        ...
        if (Abc_TtEqual(pImpl, pSpec, Abc_TtWordNum(nVars)))
            ...
            Dau_DsdPrintFromTruth(pImpl, nVars);
            printf("Specification:  ");
            Dau_DsdPrintFromTruth(pSpec, nVars);
        }
        iObj = nVars - 1;
    }
    Vec_WrdFree(vInfo);
