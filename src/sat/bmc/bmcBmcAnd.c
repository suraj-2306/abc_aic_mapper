    ...
    *iFirst = -1;
    ...
    for (f = 0;; f++) {
        ...
        Gia_ManForEachAnd(p, pObj, i)
            pObj->Value
            = Gia_XsimAndCond(Gia_ObjFanin0(pObj)->Value, Gia_ObjFaninC0(pObj), Gia_ObjFanin1(pObj)->Value, Gia_ObjFaninC1(pObj));
...
void Bmc_MnaSelect_rec(Gia_Man_t* p, Gia_Obj_t* pObj, Vec_Int_t* vLeaves) {
    ...
    if (Gia_ObjIsAnd(pObj)) {
        if (Gia_ObjFanin0(pObj)->Value == GIA_UND)
            ...
            assert(Gia_ObjFanin0(pObj)->Value + Gia_ObjFaninC0(pObj) == GIA_ONE);
        if (Gia_ObjFanin1(pObj)->Value == GIA_UND)
            Bmc_MnaSelect_rec(p, Gia_ObjFanin1(pObj), vLeaves);
        else
            assert(Gia_ObjFanin1(pObj)->Value + Gia_ObjFaninC1(pObj) == GIA_ONE);
    } else if (Gia_ObjIsRo(p, pObj))
...
Gia_Man_t* Gia_ManBmcUnroll(Gia_Man_t* pGia, int nFramesMax, int nFramesAdd, int fVerbose, Vec_Int_t** pvPiMap) {
    ...
    *pvPiMap = Vec_IntAlloc(1000);
    ...
    for (f = Vec_PtrSize(vStates) - 1; f >= 0; f--) {
        ...
        Gia_ManForEachPo(pGia, pObj, i) if (Gia_ManTerSimInfoGet(pStateF, Gia_ObjCioId(pObj)) == GIA_UND)
    ...
    for (f = 0; f < Vec_PtrSize(vStates); f++) {
        ...
        // set a new map
        Gia_ManForEachObjVec(vRoots, pGia, pObj, i) if (Gia_ObjIsRi(pGia, pObj))
...
Gia_Man_t* Gia_ManBmcDupCone(Gia_Man_t* p, Vec_Int_t* vIns, Vec_Int_t* vNodes, Vec_Int_t* vOuts) {
    ...
    Gia_ManForEachObjVec(vIns, p, pObj, i)
        pObj->Value
        = Vec_IntEntry(vTempIn, i);
    // reset values
    Gia_ManForEachObjVec(vNodes, p, pObj, i)
        pObj->Value
        = Vec_IntEntry(vTempNode, i);
    // reset values
...
int Gia_ManBmcAssignVarIds(Bmc_Mna_t* p, Vec_Int_t* vIns, Vec_Int_t* vUsed, Vec_Int_t* vOuts) {
    ...
    Vec_IntForEachEntry(vIns, iObj, i) if (Vec_IntEntry(p->vId2Var, iObj) == 0)
...
void Gia_ManBmcAddCnf(Bmc_Mna_t* p, Gia_Man_t* pGia, Vec_Int_t* vIns, Vec_Int_t* vNodes, Vec_Int_t* vOuts) {
    ...
    Gia_ManForEachAnd(pNew, pObj, i) if (pCnf->pVarNums[i] >= 0)
...
int Gia_ManBmcFindFirst(Gia_Man_t* pFrames) {
    Gia_Obj_t* pObj;
    int i;
    Gia_ManForEachPo(pFrames, pObj, i) if (Gia_ObjChild0(pObj) != Gia_ManConst0(pFrames)) return i;
    return -1;
}
...
int Gia_ManBmcPerformInt(Gia_Man_t* pGia, Bmc_AndPar_t* pPars) {
    ...
    if (pPars->fUseSynth) {
        p->pFrames = Gia_ManAigSyn2(pTemp = p->pFrames, 1, 0, 0, 0, 0, pPars->fVerbose, 0);
        Gia_ManStop(pTemp);
    } else if (pPars->fVerbose)
        Gia_ManPrintStats(p->pFrames, NULL);
    if (pPars->fDumpFrames) {
