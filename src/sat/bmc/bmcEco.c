...
Gia_Man_t* Bmc_EcoMiter(Gia_Man_t* pGold, Gia_Man_t* pOld, Vec_Int_t* vFans) {
    ...
    Gia_ManForEachCi(pOld, pObj, i)
        pObj->Value
        = Gia_ManCi(pGold, i)->Value;
    Gia_ManForEachAnd(pOld, pObj, i) if (pObj == pRoot)
        pObj->Value
        = NewPi;
    ...
    Gia_ManForEachCo(pGold, pObj, i)
        Miter
        = Gia_ManHashOr(pNew, Miter, Gia_ManHashXor(pNew, pObj->Value, Gia_ManCo(pOld, i)->Value));
...
int Bmc_EcoPatch(Gia_Man_t* p, int nIns, int nOuts) {
    ...
    Gia_ManForEachPo(p, pObj, i) if (i >= nOuts + 1)
