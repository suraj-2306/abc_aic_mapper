...
int Inta_ManProofTraceOne(Inta_Man_t* p, Sto_Cls_t* pConflict, Sto_Cls_t* pFinal) {
    ...
    for (i = p->nTrailSize - 1; i >= 0; i--) {
        ...
        if (p->fProofVerif) {
            ...
            Vec_IntForEachEntry(p->vResLits, Entry, v1) if (lit_var(Entry) == Var) break;
            ...
            for (v2 = 1; v2 < (int)pReason->nLits; v2++) {
                Vec_IntForEachEntry(p->vResLits, Entry, v1) if (lit_var(Entry) == lit_var(pReason->pLits[v2])) break;
    ...
    if (p->fProofVerif) {
        ...
        if (Vec_IntSize(p->vResLits) != (int)pFinal->nLits) {
            for (v1 = 0; v1 < (int)pFinal->nLits; v1++) {
                Vec_IntForEachEntry(p->vResLits, Entry, v2) if (pFinal->pLits[v1] == Entry) break;
