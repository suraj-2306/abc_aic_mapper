...
static inline satset* Proof_NodeRead(Vec_Set_t* p, int h) {
    assert(h > 0);
    return (satset*)Vec_SetEntry(p, h);
}
static inline int Proof_NodeWordNum(int nEnts) {
    assert(nEnts > 0);
    return 1 + ((nEnts + 1) >> 1);
}
...
void Proof_CollectUsed_iter(Vec_Set_t* vProof, int hNode, Vec_Int_t* vUsed, Vec_Int_t* vStack) {
    ...
    while (Vec_IntSize(vStack)) {
        ...
        Proof_NodeForeachFanin(vProof, pNode, pNext, i) if (pNext && !pNext->Id) {
            pNext->Id = 1;
...
Vec_Int_t* Proof_CollectUsedIter(Vec_Set_t* vProof, Vec_Int_t* vRoots, int fSort) {
    ...
    Vec_IntForEachEntry(vRoots, Entry, i) if (Entry >= 0)
...
int Proof_MarkUsed_rec(Vec_Set_t* vProof, int hNode) {
    ...
    Proof_NodeForeachFanin(vProof, pNode, pNext, i) if (pNext && !pNext->Id)
        Counter
        += Proof_MarkUsed_rec(vProof, pNode->pEnts[i] >> 2);
    return Counter;
}
int Proof_MarkUsedRec(Vec_Set_t* vProof, Vec_Int_t* vRoots) {
    ...
    Vec_IntForEachEntry(vRoots, Entry, i) if (Entry >= 0)
        Counter
        += Proof_MarkUsed_rec(vProof, Entry);
    return Counter;
}
...
int Sat_ProofReduce(Vec_Set_t* vProof, void* pRoots, int hProofPivot) {
    ...
    Vec_SetForEachEntry(satset*, vProof, nSize, pNode, i, j) {
        ...
        // update fanins
        Proof_NodeForeachFanin(vProof, pNode, pFanin, k) if ((pNode->pEnts[k] & 1) == 0) // proof node
        {
            assert(pFanin->Id > 0);
            pNode->pEnts[k] = (pFanin->Id << 2) | (pNode->pEnts[k] & 2);
        }
...
Vec_Int_t* Sat_ProofCollectCore(Vec_Set_t* vProof, Vec_Int_t* vUsed) {
    ...
    Proof_ForeachNodeVec(vUsed, vProof, pNode, i)
        Proof_NodeForeachFanin(vProof, pNode, pFanin, k) if (pFanin == NULL)
            MaxCla
