...
int Msat_OrderCheck(Msat_Order_t* p) {
    ...
    for (i = 0; i < nVars; i++) {
        ...
        if (Msat_OrderVarIsAssigned(p, pVars[i]) || Msat_OrderVarIsInBoundary(p, pVars[i]))
            continue;
...
void Msat_OrderVarUnassigned(Msat_Order_t* p, int Var) {
    ...
    for (i = 0; i < vRound->nSize; i++)
        if (Msat_OrderVarIsInBoundary(p, vRound->pArray[i])) { // the neighbor is in the J-boundary (and unassigned)
