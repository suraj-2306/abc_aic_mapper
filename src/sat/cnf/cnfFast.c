...
word Cnf_CutDeriveTruth(Aig_Man_t* p, Vec_Ptr_t* vLeaves, Vec_Ptr_t* vNodes) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vNodes, pObj, i) {
        ...
        S[pObj->iData] = (S[Aig_ObjFanin0(pObj)->iData] ^ C[Aig_ObjFaninC0(pObj)]) & (S[Aig_ObjFanin1(pObj)->iData] ^ C[Aig_ObjFaninC1(pObj)]);
    }
    return S[pObj->iData];
}
...
Cnf_Dat_t* Cnf_DeriveFastClauses(Aig_Man_t* p, int nOutputs) {
    ...
    Aig_ManForEachNodeReverse(p, pObj, i) if (pObj->fMarkA)
        Vec_IntWriteEntry(vMap, Aig_ObjId(pObj), nVars++);
