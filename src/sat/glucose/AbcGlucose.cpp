...
int glucose_solver_solve(Gluco::SimpSolver* S, int* plits, int nlits) {
    ...
    return (res == l_True ? 1 : res == l_False ? -1
                                               : 0);
}
...
void Glucose_SolveCnf(char* pFileName, Glucose_Pars* pPars, int fDumpCnf) {
    ...
    printf(ret == l_True ? "SATISFIABLE" : ret == l_False ? "UNSATISFIABLE"
                                                          : "INDETERMINATE");
    Abc_PrintTime(1, "      Time", Abc_Clock() - clk);
}
...
void bmcg_sat_generate_dvars(Vec_Int_t* vCiVars, Vec_Str_t* vSop, Vec_Int_t* vDLits) {
    ...
    Vec_IntForEachEntry(vCounts, Counter, Lit) if (Counter == nCubes)
...
int bmcg_sat_solver_quantify2(Gia_Man_t* p, int iLit, int fHash, int (*pFuncCiToKeep)(void*, int), void* pData, Vec_Int_t* vDLits) {
    ...
    Vec_IntForEachEntry(vCisUsed, CiId, i) if (!pFuncCiToKeep(pData, CiId)) {
...
void Gia_ManQuantLoadCnf(Gia_Man_t* p, Vec_Int_t* vObjsUsed, bmcg_sat_solver* pSats[]) {
    ...
    Gia_ManForEachObjVec(vObjsUsed, p, pObj, i) if (Gia_ObjIsAnd(pObj)) {
...
int Gia_ManFactorSop(Gia_Man_t* p, Vec_Int_t* vCiObjIds, Vec_Str_t* vSop, int fHash) {
    ...
    Gia_ManForEachPi(pMan, pObj, i)
        pObj->Value
        = Abc_Var2Lit(Vec_IntEntry(vCiObjIds, i), 0);
    Gia_ManForEachAnd(pMan, pObj, i) if (fHash)
        pObj->Value
        = Gia_ManHashAnd(p, Gia_ObjFanin0Copy(pObj), Gia_ObjFanin1Copy(pObj));
    else pObj->Value = Gia_ManAppendAnd(p, Gia_ObjFanin0Copy(pObj), Gia_ObjFanin1Copy(pObj));
