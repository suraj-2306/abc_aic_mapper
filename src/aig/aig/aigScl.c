...
Aig_Man_t* Aig_ManRemap(Aig_Man_t* p, Vec_Ptr_t* vMap) {
    ...
    if (p->vFlopReprs) {
        Aig_ManForEachLoSeq(p, pObj, i)
            pObj->pNext
            = (Aig_Obj_t*)(long)Vec_IntEntry(p->vFlopNums, i - nTruePis);
    }
    Aig_ManForEachCi(p, pObj, i) {
...
int Aig_ManSeqCleanup(Aig_Man_t* p) {
    ...
    if (Vec_PtrSize(vNodes) < Aig_ManCoNum(p)) {
        if (p->vFlopNums) {
            ...
            int iNum, k = 0;
            Aig_ManForEachCo(p, pObj, i) if (i >= nTruePos && Aig_ObjIsTravIdCurrent(p, pObj)) {
        ...
        Aig_ManForEachCo(p, pObj, i) if (Aig_ObjIsTravIdCurrent(p, pObj))
