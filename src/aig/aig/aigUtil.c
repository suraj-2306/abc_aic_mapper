...
int Aig_ManLevels(Aig_Man_t* p) {
    ...
    Aig_ManForEachCo(p, pObj, i)
        LevelMax
        = Abc_MaxInt(LevelMax, (int)Aig_ObjFanin0(pObj)->Level);
    return LevelMax;
}
...
int Aig_ObjIsMuxType(Aig_Obj_t* pNode) {
    ...
    return (Aig_ObjFanin0(pNode0) == Aig_ObjFanin0(pNode1) && (Aig_ObjFaninC0(pNode0) ^ Aig_ObjFaninC0(pNode1))) || (Aig_ObjFanin0(pNode0) == Aig_ObjFanin1(pNode1) && (Aig_ObjFaninC0(pNode0) ^ Aig_ObjFaninC1(pNode1))) || (Aig_ObjFanin1(pNode0) == Aig_ObjFanin0(pNode1) && (Aig_ObjFaninC1(pNode0) ^ Aig_ObjFaninC0(pNode1))) || (Aig_ObjFanin1(pNode0) == Aig_ObjFanin1(pNode1) && (Aig_ObjFaninC1(pNode0) ^ Aig_ObjFaninC1(pNode1)));
}
...
Aig_Obj_t* Aig_ObjRecognizeMux(Aig_Obj_t* pNode, Aig_Obj_t** ppNodeT, Aig_Obj_t** ppNodeE) {
    ...
    if (Aig_ObjFanin1(pNode0) == Aig_ObjFanin1(pNode1) && (Aig_ObjFaninC1(pNode0) ^ Aig_ObjFaninC1(pNode1))) {
        ...
        if (Aig_ObjFaninC1(pNode0)) {                  // pNode2->p2 is positive phase of C
        ...
        if (Aig_ObjFaninC0(pNode0)) {                  // pNode2->p1 is positive phase of C
        ...
        //        if ( Fraig_IsComplement(pNode1->p1) )
        if (Aig_ObjFaninC0(pNode0)) {                  // pNode2->p2 is positive phase of C
            *ppNodeT = Aig_Not(Aig_ObjChild0(pNode1)); //pNode2->p1);
            *ppNodeE = Aig_Not(Aig_ObjChild1(pNode0)); //pNode1->p2);
            return Aig_ObjChild1(pNode1);              //pNode2->p2;
        } else {                                       // pNode1->p1 is positive phase of C
        ...
        //        if ( Fraig_IsComplement(pNode1->p2) )
        if (Aig_ObjFaninC1(pNode0)) {                  // pNode2->p1 is positive phase of C
            *ppNodeT = Aig_Not(Aig_ObjChild1(pNode1)); //pNode2->p2);
            *ppNodeE = Aig_Not(Aig_ObjChild0(pNode0)); //pNode1->p1);
            return Aig_ObjChild0(pNode1);              //pNode2->p1;
        } else {                                       // pNode1->p2 is positive phase of C
...
void Aig_ObjPrintVerboseCone(Aig_Man_t* p, Aig_Obj_t* pRoot, int fHaig) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vNodes, pObj, i)
        Aig_ObjPrintVerbose(pObj, fHaig),
        printf("\n");
    printf("\n");
    Vec_PtrFree(vNodes);
}
...
void Aig_ManDumpBlif(Aig_Man_t* p, char* pFileName, Vec_Ptr_t* vPiNames, Vec_Ptr_t* vPoNames) {
    ...
    Aig_ManForEachCo(p, pObj, i) if (Aig_ObjIsConst1(Aig_ObjFanin0(pObj)))
        pConst1
        = Aig_ManConst1(p);
    ...
    Aig_ManForEachPiSeq(p, pObj, i) if (vPiNames)
        fprintf(pFile, " %s", (char*)Vec_PtrEntry(vPiNames, i));
    else fprintf(pFile, " n%0*d", nDigits, pObj->iData);
    fprintf(pFile, "\n");
    // write POs
    fprintf(pFile, ".outputs");
    Aig_ManForEachPoSeq(p, pObj, i) if (vPoNames)
...
void Aig_ManCounterExampleValueStart(Aig_Man_t* pAig, Abc_Cex_t* pCex) {
    ...
    for (i = 0; i <= pCex->iFrame; i++) {
        ...
        Saig_ManForEachPi(pAig, pObj, k) if (Abc_InfoHasBit(pCex->pData, iBit++))
        ...
        Saig_ManForEachLiLo(pAig, pObjRi, pObjRo, k) if (Abc_InfoHasBit((unsigned*)pAig->pData2, nObjs * i + Aig_ObjId(pObjRi)))
...
Vec_Ptr_t* Aig_ManMuxesCollect(Aig_Man_t* pAig) {
    ...
    Aig_ManForEachNode(pAig, pObj, i) if (Aig_ObjIsMuxType(pObj))
