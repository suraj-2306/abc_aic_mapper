...
Aig_Man_t* Aig_ManDupSimple(Aig_Man_t* p) {
    ...
    Aig_ManForEachObj(p, pObj, i) if (Aig_ObjIsBuf(pObj)) {
...
Aig_Man_t* Aig_ManDupSimpleDfs(Aig_Man_t* p) {
    ...
    // duplicate internal nodes
    Aig_ManForEachObj(p, pObj, i) if (!Aig_ObjIsCo(pObj)) {
...
Aig_Man_t* Aig_ManDupUnsolvedOutputs(Aig_Man_t* p, int fAddRegs) {
    ...
    Aig_ManForEachPoSeq(p, pObj, i)
        nOuts
        += (Aig_ObjFanin0(pObj) != Aig_ManConst1(p));
    // set registers
    pNew->nRegs = fAddRegs ? p->nRegs : 0;
    pNew->nTruePis = fAddRegs ? p->nTruePis : p->nTruePis + p->nRegs;
    pNew->nTruePos = nOuts;
    ...
    Aig_ManForEachNode(p, pObj, i)
        ...
        = Aig_And(pNew, Aig_ObjChild0Copy(pObj), Aig_ObjChild1Copy(pObj));
    // create the PO
    Aig_ManForEachPoSeq(p, pObj, i) if (Aig_ObjFanin0(pObj) != Aig_ManConst1(p))
        Aig_ObjCreateCo(pNew, Aig_ObjChild0Copy(pObj));
    // create register inputs with MUXes
    if (fAddRegs) {
        Aig_ManForEachLiSeq(p, pObj, i)
...
Aig_Man_t* Aig_ManDupNodes(Aig_Man_t* pMan, Vec_Ptr_t* vArray) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vObjs, pObj, i) if (Aig_ObjIsCi(pObj))
        pObj->pData
        = Aig_ObjCreateCi(pNew);
    Vec_PtrForEachEntry(Aig_Obj_t*, vObjs, pObj, i) if (Aig_ObjIsNode(pObj))
        pObj->pData
        = Aig_And(pNew, Aig_ObjChild0Copy(pObj), Aig_ObjChild1Copy(pObj));
