...
Aig_Man_t* Aig_ManRegCreatePart(Aig_Man_t* pAig, Vec_Int_t* vPart, int* pnCountPis, int* pnCountRegs, int** ppMapBack) {
    ...
    Aig_ManForEachPiSeq(pAig, pObj, i)
        nCountPis
        += Aig_ObjIsTravIdCurrent(pAig, pObj);
    ...
    // create the PIs
    Aig_ManForEachCi(pAig, pObj, i) if (Aig_ObjIsTravIdCurrent(pAig, pObj))
        pObj->pData
    ...
    // create the nodes
    Vec_PtrForEachEntry(Aig_Obj_t*, vNodes, pObj, i) if (Aig_ObjIsNode(pObj))
        pObj->pData
