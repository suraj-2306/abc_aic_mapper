...
static inline int Ndr_DataType(Ndr_Data_t* p, int i) {
    assert(p->pHead[i]);
    return (int)p->pHead[i];
}
...
static inline void Ndr_DataAddTo(Ndr_Data_t* p, int i, int Add) {
    assert(Ndr_DataType(p, i) <= NDR_OBJECT);
    p->pBody[i] += Add;
}
static inline void Ndr_DataPush(Ndr_Data_t* p, int Type, int Entry) {
    p->pHead[p->nSize] = Type;
    p->pBody[p->nSize++] = Entry;
}
...
#    define Ndr_DesForEachMod(p, Mod)                                         \
        for (Mod = 1; Mod < Ndr_DataEntry(p, 0); Mod += Ndr_DataSize(p, Mod)) \
            if (Ndr_DataType(p, Mod) != NDR_MODULE) {                         \
            } else

// iterates over objects in a module
#    define Ndr_ModForEachObj(p, Mod, Obj)                                          \
        for (Obj = Mod + 1; Obj < Ndr_DataEnd(p, Mod); Obj += Ndr_DataSize(p, Obj)) \
            if (Ndr_DataType(p, Obj) != NDR_OBJECT) {                               \
            } else
...
#    define Ndr_ModForEachPi(p, Mod, Obj)                                          \
        Ndr_ModForEachObj(p, Mod, Obj) if (!Ndr_ObjIsType(p, Obj, ABC_OPER_CI)) {} \
        else

// iteraots over primary outputs of a module
#    define Ndr_ModForEachPo(p, Mod, Obj)                                          \
        Ndr_ModForEachObj(p, Mod, Obj) if (!Ndr_ObjIsType(p, Obj, ABC_OPER_CO)) {} \
        else

// iterates over internal nodes of a module
#    define Ndr_ModForEachNode(p, Mod, Obj)                                                                             \
        Ndr_ModForEachObj(p, Mod, Obj) if (Ndr_ObjIsType(p, Obj, ABC_OPER_CI) || Ndr_ObjIsType(p, Obj, ABC_OPER_CO)) {} \
        else

// iterates over target signals of a module
#    define Ndr_ModForEachTarget(p, Mod, Obj)                                       \
        for (Obj = Mod + 1; Obj < Ndr_DataEnd(p, Mod); Obj += Ndr_DataSize(p, Obj)) \
            if (Ndr_DataType(p, Obj) != NDR_TARGET) {                               \
            } else
...
static inline int Ndr_ObjReadEntry(Ndr_Data_t* p, int Obj, int Type) {
    int Ent;
    Ndr_ObjForEachEntry(p, Obj, Ent) if (Ndr_DataType(p, Ent) == Type) return Ndr_DataEntry(p, Ent);
    return -1;
}
static inline int Ndr_ObjReadArray(Ndr_Data_t* p, int Obj, int Type, int** ppStart) {
    int Ent, Counter = 0;
    *ppStart = NULL;
    Ndr_ObjForEachEntry(p, Obj, Ent) if (Ndr_DataType(p, Ent) == Type) {
        Counter++;
        if (*ppStart == NULL)
            *ppStart = (int*)p->pBody + Ent;
    }
    else if (*ppStart) return Counter;
    return Counter;
}
static inline int Ndr_ObjIsType(Ndr_Data_t* p, int Obj, int Type) {
    int Ent;
    Ndr_ObjForEachEntry(p, Obj, Ent) if (Ndr_DataType(p, Ent) == NDR_OPERTYPE) return (int)(Ndr_DataEntry(p, Ent) == Type);
    return -1;
}
...
static inline int* Ndr_ObjReadBodyP(Ndr_Data_t* p, int Obj, int Type) {
    int Ent;
    Ndr_ObjForEachEntry(p, Obj, Ent) if (Ndr_DataType(p, Ent) == Type) return Ndr_DataEntryP(p, Ent);
    return NULL;
}
...
static inline void Ndr_WriteVerilogModule(FILE* pFile, void* pDesign, int Mod, char** pNames, int fSimple) {
    ...
    Ndr_ModForEachPo(p, Mod, Obj)
        fprintf(pFile, "%s%s", fFirst ? "" : ", ", Ndr_ObjReadInName(p, Obj, pNames)),
        fFirst = 0;
...
static inline void Ndr_AddObject(void* pDesign, int ModuleId, int ObjType, int InstName, int RangeLeft, int RangeRight, int fSignedness, int nInputs, int* pInputs, int nOutputs, int* pOutputs, char* pFunction) {
    Ndr_Data_t* p = (Ndr_Data_t*)pDesign;
