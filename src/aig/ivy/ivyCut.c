...
int Ivy_ManFindBoolCut(Ivy_Man_t* p, Ivy_Obj_t* pRoot, Vec_Ptr_t* vFront, Vec_Ptr_t* vVolume, Vec_Ptr_t* vLeaves) {
    ...
    for (Lev = Ivy_ObjLevel(pRoot) - 1; Lev >= LevelLimit; Lev--) {
        while (1) {
            ...
            Vec_PtrForEachEntry(Ivy_Obj_t*, vFront, pObj, k) if ((int)pObj->Level == Lev) break;
    ...
    while (1) {
        Vec_PtrForEachEntry(Ivy_Obj_t*, vLeaves, pObj, k) if (Ivy_ManFindBoolCutCost(pObj) < 2) break;
