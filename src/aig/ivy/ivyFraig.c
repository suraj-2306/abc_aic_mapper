...
int Ivy_FraigProve(Ivy_Man_t** ppManAig, void* pPars) {
    ...
    if (!pParams->fUseRewriting && !pParams->fUseFraiging) {
        ...
        pManAig = Ivy_FraigMiter(pManTemp = pManAig, pIvyParams);
        Ivy_ManStop(pManTemp);
    ...
    for (nIter = 0; nIter < pParams->nItersMax; nIter++) {
        ...
        // try rewriting
        if (pParams->fUseRewriting) { // bug in Ivy_NodeFindCutsAll() when leaves are identical!
                                      /*
            clk = Abc_Clock();
...
*/
        ...
        if (pParams->fUseFraiging) {
            ...
            pManAig = Ivy_FraigPerform_int(pManTemp = pManAig, pIvyParams, pParams->nTotalBacktrackLimit, pParams->nTotalInspectLimit, &nSatConfs, &nSatInspects);
            Ivy_ManStop(pManTemp);
            RetValue = Ivy_FraigMiterStatus(pManAig);
        ...
        if ((pParams->nTotalBacktrackLimit && pParams->nTotalBacktracksMade >= pParams->nTotalBacktrackLimit) || (pParams->nTotalInspectLimit && pParams->nTotalInspectsMade >= pParams->nTotalInspectLimit)) {
...
Ivy_Man_t* Ivy_FraigMiter(Ivy_Man_t* pManAig, Ivy_FraigParams_t* pParams) {
    ...
    Ivy_ManForEachNode(p->pManAig, pObj, i)
        pObj->pEquiv
        = Ivy_And(p->pManFraig, Ivy_ObjChild0Equiv(pObj), Ivy_ObjChild1Equiv(pObj));
...
void Ivy_FraigCreateClasses(Ivy_FraigMan_t* p) {
    ...
    Ivy_ManForEachObj(p->pManAig, pObj, i) {
        ...
        Ivy_FraigForEachBinNode(pBin, pEntry) if (Ivy_NodeCompareSims(p, pEntry, pObj)) {
...
int Ivy_FraigRefineClass_rec(Ivy_FraigMan_t* p, Ivy_Obj_t* pClass) {
    ...
    Ivy_FraigForEachClassNode(Ivy_ObjClassNodeNext(pClassNew), pNode) if (Ivy_NodeCompareSims(p, pClass, pNode)) {
...
int* Ivy_FraigCreateModel(Ivy_FraigMan_t* p) {
    ...
    Ivy_ManForEachPi(p->pManFraig, pObj, i)
        ...
        pModel[i]
        = (p->pSat->model[Ivy_ObjSatNum(pObj)] == l_True);
    return pModel;
}
...
void Ivy_FraigSweep(Ivy_FraigMan_t* p) {
    ...
    Ivy_ManForEachObj(p->pManAig, pObj, i)
        pObj->pFanout
        = pObj->pNextFan0 = pObj->pNextFan1 = pObj->pPrevFan0 = pObj->pPrevFan1 = NULL;
    // clean the new manager
    Ivy_ManForEachObj(p->pManFraig, pObj, i) {
        if (Ivy_ObjFaninVec(pObj))
            Vec_PtrFree(Ivy_ObjFaninVec(pObj));
...
void Ivy_FraigAddClausesSuper(Ivy_FraigMan_t* p, Ivy_Obj_t* pNode, Vec_Ptr_t* vSuper) {
    ...
    Vec_PtrForEachEntry(Ivy_Obj_t*, vSuper, pFanin, i)
        pLits[i]
        = toLitCond(Ivy_ObjSatNum(Ivy_Regular(pFanin)), !Ivy_IsComplement(pFanin));
...
void Ivy_FraigCollectSuper_rec(Ivy_Obj_t* pObj, Vec_Ptr_t* vSuper, int fFirst, int fUseMuxes) {
    ...
    if (Ivy_IsComplement(pObj) || Ivy_ObjIsPi(pObj) || (!fFirst && Ivy_ObjRefs(pObj) > 1) || (fUseMuxes && Ivy_ObjIsMuxType(pObj))) {
...
int Ivy_FraigSetActivityFactors_rec(Ivy_FraigMan_t* p, Ivy_Obj_t* pObj, int LevelMin, int LevelMax) {
    ...
    Vec_PtrForEachEntry(Ivy_Obj_t*, vFanins, pFanin, i)
        Counter
        += Ivy_FraigSetActivityFactors_rec(p, Ivy_Regular(pFanin), LevelMin, LevelMax);
    return 1 + Counter;
}
...
DdNode* Ivy_FraigNodesAreEquivBdd_int(DdManager* dd, DdNode* bFunc, Vec_Ptr_t* vFront, int Level) {
    ...
    // create new
    bFuncNew = Cudd_bddVectorCompose(dd, bFunc, pFuncs);
    Cudd_Ref(bFuncNew);
    // clean trav Id
...
int Ivy_FraigNodesAreEquivBdd(Ivy_Obj_t* pObj1, Ivy_Obj_t* pObj2) {
    ...
    // get the function
    bFunc = Cudd_bddXor(dd, Cudd_bddIthVar(dd, 0), Cudd_bddIthVar(dd, 1));
    Cudd_Ref(bFunc);
    ...
    for (Iter = 0;; Iter++) {
        // find max level
        Level = 0;
        Vec_PtrForEachEntry(Ivy_Obj_t*, vFront, pObj, i) if (Level < (int)pObj->Level)
            Level
            = (int)pObj->Level;
        if (Level == 0)
            break;
        bFunc = Ivy_FraigNodesAreEquivBdd_int(dd, bTemp = bFunc, vFront, Level);
        Cudd_Ref(bFunc);
...
int Ivy_FraigCheckCone(Ivy_FraigMan_t* pGlo, Ivy_Man_t* p, Ivy_Obj_t* pObj1, Ivy_Obj_t* pObj2, int nConfLimit) {
    ...
    if (RetValue == 0) {
        ...
        Aig_ManForEachCi(pMan, pObj, i) if (((int*)pMan->pData)[i]) {
