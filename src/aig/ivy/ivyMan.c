...
Ivy_Man_t* Ivy_ManDup(Ivy_Man_t* p) {
    ...
    Ivy_ManForEachNodeVec(p, vNodes, pObj, i) if (Ivy_ObjIsBuf(pObj))
        pObj->pEquiv
        = Ivy_ObjChild0Equiv(pObj);
    else pObj->pEquiv = Ivy_And(pNew, Ivy_ObjChild0Equiv(pObj), Ivy_ObjChild1Equiv(pObj));
    // add the POs
...
int Ivy_ManCleanup(Ivy_Man_t* p) {
    ...
    Ivy_ManForEachObj(p, pNode, i) if (Ivy_ObjIsNode(pNode) || Ivy_ObjIsLatch(pNode) || Ivy_ObjIsBuf(pNode)) if (Ivy_ObjRefs(pNode) == 0)
