...
static inline void Ivy_ObjRefsDec(Ivy_Obj_t* pObj) {
    assert(pObj->nRefs > 0);
    pObj->nRefs--;
}
...
static inline Ivy_Obj_t* Ivy_ObjChild0Equiv(Ivy_Obj_t* pObj) {
    assert(!Ivy_IsComplement(pObj));
    return Ivy_ObjFanin0(pObj) ? Ivy_NotCond(Ivy_ObjFanin0(pObj)->pEquiv, Ivy_ObjFaninC0(pObj)) : NULL;
}
static inline Ivy_Obj_t* Ivy_ObjChild1Equiv(Ivy_Obj_t* pObj) {
    assert(!Ivy_IsComplement(pObj));
...
#    define Ivy_ManForEachObj(p, pObj, i)                                         \
        Vec_PtrForEachEntry(Ivy_Obj_t*, p->vObjs, pObj, i) if ((pObj) == NULL) {} \
        else
// iterator over the combinational inputs
#    define Ivy_ManForEachCi(p, pObj, i)                         \
        Ivy_ManForEachObj(p, pObj, i) if (!Ivy_ObjIsCi(pObj)) {} \
        else
// iterator over the combinational outputs
#    define Ivy_ManForEachCo(p, pObj, i)                         \
        Ivy_ManForEachObj(p, pObj, i) if (!Ivy_ObjIsCo(pObj)) {} \
        else
...
#    define Ivy_ManForEachLatch(p, pObj, i)                         \
        Ivy_ManForEachObj(p, pObj, i) if (!Ivy_ObjIsLatch(pObj)) {} \
        else
