...
Aig_Man_t* Saig_ManUnrollTwo(Aig_Man_t* pBot, Aig_Man_t* pTop, int nFrames) {
    ...
    for (f = 0; f < nFrames; f++) {
        ...
        Aig_ManForEachNode(pAig, pObj, i)
            pObj->pData
            = Aig_And(p, Aig_ObjChild0Copy(pObj), Aig_ObjChild1Copy(pObj));
        if (f == nFrames - 1) {
...
Aig_Man_t* Aig_ManDupNodesHalf(Aig_Man_t* p, Vec_Ptr_t* vSet, int iPart) {
    ...
    Saig_ManForEachPi(p, pObj, i)
        ...
        = Aig_ObjCreateCi(pNew);
    if (iPart == 0) {
        Saig_ManForEachLo(p, pObj, i) if (i < Saig_ManRegNum(p) / 2)
            pObj->pData
            = Aig_ObjCreateCi(pNew);
    } else {
        Saig_ManForEachLo(p, pObj, i) if (i >= Saig_ManRegNum(p) / 2)
    ...
    Aig_ManForEachNode(p, pObj, i) if (Aig_ObjFanin0(pObj)->pData && Aig_ObjFanin1(pObj)->pData)
        pObj->pData
        = Aig_And(pNew, Aig_ObjChild0Copy(pObj), Aig_ObjChild1Copy(pObj));
    //    Saig_ManForEachPo( p, pObj, i )
    //        pObj->pData = Aig_ObjCreateCo( pNew, Aig_ObjChild0Copy(pObj) );
    ...
    if (iPart == 0) {
        Saig_ManForEachLi(p, pObj, i) if (i < Saig_ManRegNum(p) / 2)
            pObj->pData
            = Aig_ObjCreateCo(pNew, Aig_ObjChild0Copy(pObj));
    } else {
        Saig_ManForEachLi(p, pObj, i) if (i >= Saig_ManRegNum(p) / 2)
...
int Saig_ManDemiterCheckPo(Aig_Man_t* p, Aig_Obj_t* pObj, Aig_Obj_t** ppPo0, Aig_Obj_t** ppPo1) {
    ...
    if ((pObjR0->fMarkA && pObjR0->fMarkB) || (pObjR1->fMarkA && pObjR1->fMarkB) || (pObjR0->fMarkA && pObjR1->fMarkA) || (pObjR0->fMarkB && pObjR1->fMarkB))
        return 0;
...
int Saig_ManDemiter(Aig_Man_t* p, Aig_Man_t** ppAig0, Aig_Man_t** ppAig1) {
    ...
    for (i = 2; i < Vec_PtrSize(vPairs); i += 2) {
        ...
        if ((pFlop0->fMarkA && pFlop0->fMarkB) || (pFlop1->fMarkA && pFlop1->fMarkB) || (pFlop0->fMarkA && pFlop1->fMarkA) || (pFlop0->fMarkB && pFlop1->fMarkB))
    ...
    Counter = 0;
    Saig_ManForEachLo(p, pObj, i) if (pObj->fMarkA && pObj->fMarkB)
        Counter++;
    if (Counter > 0)
...
int Ssw_SecCexResimulate(Aig_Man_t* p, int* pModel, int* pnOutputs) {
    ...
    *pnOutputs = 0;
    ...
    Aig_ManForEachCo(p, pObj, i) if (pObj->fMarkA) {
        if (RetValue == -1)
            RetValue = i;
        (*pnOutputs)++;
    }
    ...
    Saig_ManForEachPo(pMan, pObj, i) {
        ...
        if (Vec_PtrSize(vSuper) == 2) {
            if (Aig_ObjRecognizeExor(Aig_ObjFanin0(pObj), &pFan0, &pFan1)) {
                ...
                Vec_PtrForEachEntry(Aig_Obj_t*, vSupp0, pTemp, k) if (Saig_ObjIsLo(pMan, pTemp))
                    printf(" %d", Aig_ObjCioId(pTemp));
                printf("\n");
                Vec_PtrFree(vSupp0);

                vSupp1 = Aig_Support(pMan, Aig_Regular(pFan1));
                Vec_PtrForEachEntry(Aig_Obj_t*, vSupp1, pTemp, k) if (Saig_ObjIsLo(pMan, pTemp))
                    printf(" %d", Aig_ObjCioId(pTemp));
                printf("\n");
                Vec_PtrFree(vSupp1);
            } else
