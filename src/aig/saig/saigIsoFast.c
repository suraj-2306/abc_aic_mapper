...
Vec_Int_t* Iso_StoCollectInfo(Iso_Sto_t* p, Aig_Obj_t* pPo) {
    ...
    // mark internal nodes
    Vec_PtrForEachEntry(Aig_Obj_t*, p->vRoots, pObj, i) if (!Aig_ObjIsConst1(Aig_ObjFanin0(pObj)))
...
Vec_Vec_t* Saig_IsoDetectFast(Aig_Man_t* pAig) {
    ...
    Vec_PtrForEachEntry(Vec_Int_t*, vClasses, vLevel, i) if (Vec_IntSize(vLevel) > 1)
