...
void Saig_MvManPostProcess(Saig_MvMan_t* p, int iState) {
    ...
    Vec_PtrForEachEntry(Saig_MvObj_t*, p->vFlops, pEntry, i) if (p->pRegsUndef[i] == 0)
        nTotal++;
    ...
    Vec_PtrForEachEntry(Saig_MvObj_t*, p->vFlops, pEntry, i) {
        ...
        Vec_IntForEachEntry(vUniques, iFlop, k) {
            Vec_PtrForEachEntryStart(unsigned*, p->vStates, pState, j, 1) if (pState[iFlop + 1] != pState[i + 1]) break;
...
int Saig_MvManCheckOscilator(Saig_MvMan_t* p, int iFlop) {
    ...
    for (Per = 0; Per < Vec_IntSize(vValues) / 2; Per++) {
        ...
        Vec_IntForEachEntryStart(vValues, Entry, Per, Per) if (!Saig_MvIsConst0(Entry)) break;
...
Vec_Int_t* Saig_MvManFindOscilators(Saig_MvMan_t* p, Vec_Int_t** pvConst0) {
    ...
    *pvConst0 = Saig_MvManFindConstBinaryFlops(p, &vBinary);
    ...
    Vec_IntForEachEntry(vBinary, Entry, i) if (Saig_MvManCheckOscilator(p, Entry))
...
Vec_Ptr_t* Saig_MvManDeriveMap(Saig_MvMan_t* p, int fVerbose) {
    ...
    Vec_IntForEachEntry(vBinValued, FlopK, k) if (FlopK >= 0)
        Vec_IntForEachEntryStart(vBinValued, FlopJ, j, k + 1) if (FlopJ >= 0) {
        // check if they are equal
        Vec_PtrForEachEntryStart(unsigned*, p->vStates, pState, i, 1) if (pState[FlopK + 1] != pState[FlopJ + 1]) break;
        if (i < Vec_PtrSize(p->vStates))
            continue;
        // set the equivalence
...
Vec_Ptr_t* Saig_MvManSimulate(Aig_Man_t* pAig, int nFramesSymb, int nFramesSatur, int fVerbose, int fVeryVerbose) {
    ...
    for (f = 0;; f++) {
        ...
        // retire some flops
        if (p->vXFlops) {
            Vec_PtrForEachEntry(Saig_MvObj_t*, p->vFlops, pEntry, i) if (Vec_IntEntry(p->vXFlops, i))
                pEntry->Value
                = SAIG_UNDEF_VALUE;
        }
        // simulate timeframe
