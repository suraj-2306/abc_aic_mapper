...
static inline Iso_Obj_t* Iso_ManObj(Iso_Man_t* p, int i) {
    assert(i >= 0 && i < p->nObjs);
    return i ? p->pObjs + i : NULL;
}
static inline int Iso_ObjId(Iso_Man_t* p, Iso_Obj_t* pObj) {
...
#define Iso_ManForEachObj(p, pObj, i)                               \
    for (i = 1; (i < p->nObjs) && ((pObj) = Iso_ManObj(p, i)); i++) \
        if (pIso->Level == -1) {                                    \
        } else
...
void Iso_ManCollectClasses(Iso_Man_t* p) {
    ...
    Vec_PtrForEachEntry(Iso_Obj_t*, p->vSingles, pIso, i) if (pIso->Id == 0)
        pIso->Id
        = p->nObjIds++;
}
