...
int Saig_ManRetimeUnsatCore(Aig_Man_t* p, int fVerbose) {
    ...
    // collect the nodes
    if (fVeryVerbose) {
        Aig_ManForEachObj(p, pObj, i) if (pCnf->pVarNums[pObj->Id] >= 0 && pVars[pCnf->pVarNums[pObj->Id]] == 1) {
            Aig_ObjPrint(p, pObj);
            printf("\n");
        }
    }
    ...
    nPos = 0;
    iBadPo = -1;
    Aig_ManForEachCo(p, pObj, i) if (pCnf->pVarNums[pObj->Id] >= 0 && pVars[pCnf->pVarNums[pObj->Id]] == 1) {
        if (iBadPo == -1)
            iBadPo = i;
        nPos++;
    }
...
Aig_Man_t* Saig_ManRetimeDupForward(Aig_Man_t* p, Vec_Ptr_t* vCut) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vCut, pObj, i)
        pObj->pData
        = Aig_NotCond(Aig_ObjCreateCi(pNew), pObj->fPhase);
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vCut, pObj, i) if (Aig_ObjIsNode(pObj))
        pObj->pData
        = NULL;
...
Aig_Man_t* Saig_ManRetimeDupBackward(Aig_Man_t* p, Vec_Ptr_t* vCut, Vec_Int_t* vInit) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vCut, pObj, i)
        pObj->pData
        = Aig_NotCond(Aig_ObjCreateCi(pNew), vInit ? Vec_IntEntry(vInit, i) : 0);
