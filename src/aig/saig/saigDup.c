...
Aig_Man_t* Saig_ManTrimPis(Aig_Man_t* p) {
    ...
    fAllPisHaveNoRefs = 1;
    Saig_ManForEachPi(p, pObj, i) if (pObj->nRefs)
        fAllPisHaveNoRefs
        = 0;
    // start the new manager
    ...
    Aig_ManForEachCi(p, pObj, i) if (fAllPisHaveNoRefs || pObj->nRefs || Saig_ObjIsLo(p, pObj)) {
...
Aig_Man_t* Saig_ManDupAbstraction(Aig_Man_t* p, Vec_Int_t* vFlops) {
    ...
    pNew->vCiNumsOrig = Vec_IntAlloc(Aig_ManCiNum(p));
    Aig_ManForEachCi(p, pObj, i) if (!pObj->fMarkA) {
        pObj->pData = Aig_ObjCreateCi(pNew);
...
Abc_Cex_t* Saig_ManExtendCex(Aig_Man_t* pAig, Abc_Cex_t* p) {
    ...
    for (i = 0; i <= p->iFrame; i++) {
        Saig_ManForEachPi(pAig, pObj, k)
            ...
            = Abc_InfoHasBit(p->pData, iBit++);
        ///////// write PI+LO values ////////////
        Aig_ManForEachCi(pAig, pObj, k) if (pObj->fMarkB)
...
int Saig_ManFindFailedPoCex(Aig_Man_t* pAig, Abc_Cex_t* p) {
    ...
    RetValue = -1;
    Saig_ManForEachPo(pAig, pObj, i) if (pObj->fMarkB) {
        RetValue = i;
        break;
    }
    Aig_ManCleanMarkB(pAig);
...
Aig_Man_t* Saig_ManDupWithPhase(Aig_Man_t* pAig, Vec_Int_t* vInit) {
    ...
    Saig_ManForEachLo(pAig, pObj, i)
        pObj->pData
        = Aig_NotCond((Aig_Obj_t*)pObj->pData, Vec_IntEntry(vInit, i));
    // add internal nodes of this frame
    Aig_ManForEachNode(pAig, pObj, i)
        pObj->pData
        = Aig_And(pAigNew, Aig_ObjChild0Copy(pObj), Aig_ObjChild1Copy(pObj));
