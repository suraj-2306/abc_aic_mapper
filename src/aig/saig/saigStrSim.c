...
void Saig_StrSimulateRound(Aig_Man_t* p0, Aig_Man_t* p1) {
    ...
    for (f = 0; f < SAIG_WORDS; f++) {
        ...
        Aig_ManForEachNode(p0, pObj0, i) if (Aig_ObjRepr(p0, pObj0) == NULL)
        ...
        Aig_ManForEachNode(p1, pObj1, i) if (Aig_ObjRepr(p1, pObj1) == NULL)
...
int Saig_StrSimCountMatchedFlops(Aig_Man_t* p) {
    Aig_Obj_t* pObj;
    int i, Counter = 0;
    Saig_ManForEachLo(p, pObj, i) if (Aig_ObjRepr(p, pObj))
        Counter++;
    return Counter;
}
...
int Saig_StrSimCountMatchedNodes(Aig_Man_t* p) {
    Aig_Obj_t* pObj;
    int i, Counter = 0;
    Aig_ManForEachNode(p, pObj, i) if (Aig_ObjRepr(p, pObj))
        Counter++;
    return Counter;
}
...
void Saig_StrSimPrepareAig(Aig_Man_t* p) {
    ...
    Aig_ManForEachObj(p, pObj, i)
        pObj->pData
        = Vec_PtrEntry((Vec_Ptr_t*)p->pData2, i);
