...
Vec_Ptr_t* Saig_ManDetectConstrCheckCont(Vec_Ptr_t* vSuper, Vec_Ptr_t* vSuper2) {
    ...
    int i;
    Vec_PtrForEachEntry(Aig_Obj_t*, vSuper2, pObj2, i) if (Vec_PtrFind(vSuper, pObj2) == -1) return 0;
    vUnique = Vec_PtrAlloc(100);
    Vec_PtrForEachEntry(Aig_Obj_t*, vSuper, pObj, i) if (Vec_PtrFind(vSuper2, pObj) == -1)
...
int Saig_ManDetectConstr(Aig_Man_t* p, int iOut, Vec_Ptr_t** pvOuts, Vec_Ptr_t** pvCons) {
    ...
    *pvCons = NULL;
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vSuper, pObj, i)
        nFlops
        += Saig_ObjIsLo(p, Aig_Regular(pObj));
    if (nFlops == 0) {
...
Aig_Man_t* Saig_ManDupUnfoldConstrs(Aig_Man_t* pAig) {
    ...
    Vec_PtrForEachEntry(Vec_Ptr_t*, vConsAll, vCons, i) if (Vec_PtrSize(vCons))
        vCons0
        = vCons;
