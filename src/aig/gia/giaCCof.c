...
static inline int Gia_Obj0Copy(Vec_Int_t* vCopies, int Fan0, int fCompl0) { return Abc_LitNotCond(Vec_IntEntry(vCopies, Fan0), fCompl0); }

static inline int Gia_Obj1Copy(Vec_Int_t* vCopies, int Fan1, int fCompl1) { return Abc_LitNotCond(Vec_IntEntry(vCopies, Fan1), fCompl1); }
...
Gia_Man_t* Gia_ManCofTest(Gia_Man_t* pGia, int nFrameMax, int nConfMax, int nTimeMax, int fVerbose) {
    ...
    for (f = 0; f < nFrameMax; f++) {
        ...
        Gia_ManForEachPo(p->pFrames, pObj, i) if (pObj->fPhase) {
