...
static inline Gia_PtrAre_t Gia_Int2Ptr(unsigned n) {
    Gia_PtrAreInt_t g;
    g.iInt = n;
    return g.iGia;
}
static inline unsigned Gia_Ptr2Int(Gia_PtrAre_t n) {
    Gia_PtrAreInt_t g;
    g.iGia = n;
    return g.iInt & 0x7fffffff;
}
...
int Gia_ManCountMinterms(Gia_ManAre_t* p) {
    ...
    Gia_ManAreForEachCubeStore(p, pCube, i) if (Gia_StaIsUsed(pCube))
...
int Gia_ManAreListCountUsed_rec(Gia_ManAre_t* p, Gia_PtrAre_t Root, int fTree) {
    ...
    return Gia_ManAreListCountUsed_rec(p, pObj->F[0], Gia_ObjHasBranch0(pObj)) + Gia_ManAreListCountUsed_rec(p, pObj->F[1], Gia_ObjHasBranch1(pObj)) + Gia_ManAreListCountUsed_rec(p, pObj->F[2], Gia_ObjHasBranch2(pObj));
}
...
static inline int Gia_ManArePrintListUsed(Gia_ManAre_t* p, Gia_PtrAre_t Root) {
    Gia_StaAre_t* pCube;
    Gia_ManAreForEachCubeList(p, Gia_ManAreSta(p, Root), pCube) if (Gia_StaIsUsed(pCube))
...
int Gia_ManArePrintUsed_rec(Gia_ManAre_t* p, Gia_PtrAre_t Root, int fTree) {
    ...
    pObj = Gia_ManAreObj(p, Root);
    return Gia_ManArePrintUsed_rec(p, pObj->F[0], Gia_ObjHasBranch0(pObj)) + Gia_ManArePrintUsed_rec(p, pObj->F[1], Gia_ObjHasBranch1(pObj)) + Gia_ManArePrintUsed_rec(p, pObj->F[2], Gia_ObjHasBranch2(pObj));
}
...
static inline Gia_Obj_t* Gia_ManAreMostUsedPi(Gia_ManAre_t* p) {
    ...
    Gia_ManForEachCi(p->pNew, pObj, i) if (pObjMax == NULL || pObjMax->Value < pObj->Value)
        pObjMax
        = pObj;
    // return the result
...
int Gia_ManCheckPOs_rec(Gia_Man_t* p, Gia_Obj_t* pObj) {
    ...
    if (Gia_ObjIsCi(pObj))
        return 1;
    assert(Gia_ObjIsAnd(pObj));
    return Gia_ManCheckPOs_rec(p, Gia_ObjFanin0(pObj)) + Gia_ManCheckPOs_rec(p, Gia_ObjFanin1(pObj));
}
...
int Gia_ManAreDeriveNexts(Gia_ManAre_t* p, Gia_PtrAre_t Sta) {
    ...
    Gia_ManForEachAnd(p->pAig, pObj, i)
        pObj->Value
        = Gia_ManHashAnd(p->pNew, Gia_ObjFanin0Copy(pObj), Gia_ObjFanin1Copy(pObj));
...
void Gia_ManArePrintReport(Gia_ManAre_t* p, abctime Time, int fFinal) {
    printf("States =%10d. Reached =%10d. R = %5.3f. Depth =%6d. Mem =%9.2f MB.  ",
           ...
           (sizeof(Gia_ManAre_t) + 4.0 * Gia_ManRegNum(p->pAig) + 8.0 * MAX_PAGE_NUM + 4.0 * p->nStaPages * p->nSize * MAX_ITEM_NUM + 16.0 * p->nObjPages * MAX_ITEM_NUM) / (1 << 20));
    if (fFinal) {
