...
static inline void Gia_ManSwiSimInfoRandom(Gia_ManSwi_t* p, unsigned* pInfo, int nProbNum) {
    unsigned Mask = 0;
    int w, i;
    if (nProbNum == -1) { // 3/8 = 1/4 + 1/8
        Mask = (Gia_ManRandom(0) & Gia_ManRandom(0)) | (Gia_ManRandom(0) & Gia_ManRandom(0) & Gia_ManRandom(0));
...
Vec_Int_t* Gia_ManSwiSimulate(Gia_Man_t* pAig, Gia_ParSwi_t* pPars) {
    ...
    if (pPars->fProbOne) {
        Gia_ManForEachObj(pAig, pObj, i)
            pSwitching[i]
            = Gia_ManSwiComputeProbOne(p->pData1[i], pPars->nWords * (pPars->nIters - pPars->nPref));
        ...
        Gia_ManForEachObj(pAig, pObj, i)
            pSwitching[i]
            = Gia_ManSwiComputeSwitching(p->pData1[i], pPars->nWords * (pPars->nIters - pPars->nPref));
    }
    /*
    printf( "PI: " );
...
float Gia_ManEvaluateSwitching(Gia_Man_t* p) {
    ...
    Gia_ManForEachObj(p, pObj, i)
        SwitchTotal
        += (float)Gia_ObjRefNum(p, pObj) * p->pSwitching[i] / 255;
    return SwitchTotal;
}
...
float Gia_ManComputeSwitching(Gia_Man_t* p, int nFrames, int nPref, int fProbOne) {
    ...
    if (Gia_ManHasMapping(p)) {
        ...
        Gia_ManForEachAnd(p, pObj, i)
            SwiTotal
            += pSwi[Gia_ObjFaninId0(pObj, i)] + pSwi[Gia_ObjFaninId1(pObj, i)];
    }
    Vec_IntFree(vSwitching);
    return SwiTotal;
}
