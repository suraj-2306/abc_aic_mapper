...
void Gia_QbfDumpFile(Gia_Man_t* pGia, int nPars) {
    ...
    Gia_ManForEachCi(pGia, pObj, i) if (i < nPars)
    ...
    Vec_IntForEachEntry(vVarMap, Entry, i) if (Entry)
        Vec_IntPush(vForAlls, i);
    else Vec_IntPush(vExists, i);
    // generate CNF
...
Gia_Man_t* Gia_QbfCofactor(Gia_Man_t* p, int nPars, Vec_Int_t* vValues, Vec_Int_t* vParMap) {
    ...
    Gia_ManHashAlloc(pNew);
    Gia_ManConst0(p)->Value = 0;
    Gia_ManForEachPi(p, pObj, i) if (i < nPars) {
        pObj->Value = Gia_ManAppendCi(pNew);
        if (Vec_IntEntry(vParMap, i) != -1)
            pObj->Value = Vec_IntEntry(vParMap, i);
    }
    else pObj->Value = Vec_IntEntry(vValues, i - nPars);
    Gia_ManForEachAnd(p, pObj, i)
        pObj->Value
        = Gia_ManHashAnd(pNew, Gia_ObjFanin0Copy(pObj), Gia_ObjFanin1Copy(pObj));
    Gia_ManForEachCo(p, pObj, i)
...
int Gia_QbfSolve(Gia_Man_t* pGia, int nPars, int nIterLimit, int nConfLimit, int nTimeOut, int nEncVars, int fGlucose, int fVerbose) {
    ...
    for (i = 0; Gia_QbfVerify(p, p->vValues); i++) {
        ...
        if (nIterLimit && i + 1 == nIterLimit) {
            RetValue = -1;
            break;
        }
        if (nTimeOut && (Abc_Clock() - p->clkStart) / CLOCKS_PER_SEC >= nTimeOut) {
            RetValue = -1;
            break;
        }
    }
