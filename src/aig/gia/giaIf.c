...
int Gia_ManLutSizeMax(Gia_Man_t* p) {
    ...
    Gia_ManForEachLut(p, i)
        nSizeMax
        = Abc_MaxInt(nSizeMax, Gia_ObjLutSize(p, i));
    return nSizeMax;
}
...
int Gia_ManLutLevel(Gia_Man_t* p, int** ppLevels) {
    ...
    Gia_ManForEachLut(p, i) {
        Level = 0;
        Gia_LutForEachFanin(p, i, iFan, k) if (Level < pLevels[iFan])
            Level
            = pLevels[iFan];
        pLevels[i] = Level + 1;
    }
...
int Gia_ManComputeOverlap2(Gia_Man_t* p) {
    ...
    Gia_ManForEachLut(p, i)
        Count
        += Gia_ManComputeOverlap2One(p, i, vLabel, vVisit);
    Vec_StrFree(vLabel);
    Vec_IntFree(vVisit);
    return Count;
}
...
int Gia_ManCountDupLut(Gia_Man_t* p) {
    ...
    Gia_ManForEachLut(p, i) if (Gia_ObjLutIsMux(p, i)) {
        pObj = Gia_ManObj(p, i);
        pObj->fMark1 = 1;
        if (Gia_ObjLutSize(p, i) == 3) {
...
void Gia_ManPrintMappingStats(Gia_Man_t* p, char* pDumpFile) {
    ...
    Gia_ManForEachCo(p, pObj, i)
        Ave
        += pLevels[Gia_ObjFaninId0p(p, pObj)];
    ABC_FREE(pLevels);
...
void If_ManCheckChoices(If_Man_t* pIfMan) {
    ...
    If_ManForEachCo(pIfMan, pIfObj, i)
        If_ManCheckChoices_rec(pIfMan, If_ObjFanin0(pIfObj));
    If_ManForEachNode(pIfMan, pIfObj, i) if (!pIfObj->fDriver)
...
int Gia_ManFromIfLogicCreateLut(Gia_Man_t* pNew, word* pRes, Vec_Int_t* vLeaves, Vec_Int_t* vCover, Vec_Int_t* vMapping, Vec_Int_t* vMapping2) {
    ...
    if (Abc_Lit2Var(iObjLit1) == 0)
        return iObjLit1;
    Vec_IntForEachEntry(vLeaves, iLit, i) if (Abc_Lit2Var(iObjLit1) == Abc_Lit2Var(iLit)) return iObjLit1;
    // write mapping
...
*/
    ...
    Vec_IntForEachEntry(vMapping, Entry, i) if (Entry > 0)
...
int Gia_ManMappingVerify_rec(Gia_Man_t* p, Gia_Obj_t* pObj) {
    ...
    Id = Gia_ObjId(p, pObj);
    Gia_LutForEachFanin(p, Id, iFan, k) if (Result)
        Result
        &= Gia_ManMappingVerify_rec(p, Gia_ManObj(p, iFan));
    return Result;
}
...
void Gia_ManTransferTiming(Gia_Man_t* p, Gia_Man_t* pGia) {
    if (pGia->vCiArrs || pGia->vCoReqs || pGia->vCoArrs || pGia->vCoAttrs) {
        ...
        pGia->vCiArrs = NULL;
        p->vCoReqs = pGia->vCoReqs;
        pGia->vCoReqs = NULL;
        p->vCoArrs = pGia->vCoArrs;
        pGia->vCoArrs = NULL;
        p->vCoAttrs = pGia->vCoAttrs;
        pGia->vCoAttrs = NULL;
    ...
    if (pGia->vNamesIn || pGia->vNamesOut || pGia->vNamesNode) {
        ...
        pGia->vNamesOut = NULL;
        p->vNamesNode = pGia->vNamesNode;
        pGia->vNamesNode = NULL;
    }
...
*/
    ...
    if (p->vCoAttrs) {
        ...
        Vec_IntForEachEntry(p->vCoAttrs, Entry, i)
            If_ObjFanin0(If_ManCo(pIfMan, i))
                ->fSpec
            = (Entry != 0);
    }
...
void Gia_ManTestStruct(Gia_Man_t* p) {
    ...
    Gia_ManForEachLut(p, i) {
        ...
        if (LutNDecomp[nFanins] > 10)
            continue;
        Kit_DsdPrintFromTruth((unsigned*)pTruth, nFanins);
        printf("\n");
    }
