...
void Gia_ManDetectSeqSignals(Gia_Man_t* p, int fSetReset, int fVerbose) {
    ...
    Gia_ManForEachRi(p, pFlop, i) {
        ...
        // detect sets/resets
        Gia_CollectSuper(p, pNode, vSuper);
        Vec_IntForEachEntry(vSuper, Ent, k) if (pSets[Ent] > 1 || pResets[Ent] > 1) {
            nHaveSetReset++;
            break;
        }
    }
...
Gia_Man_t* Gia_ManRemoveEnables2(Gia_Man_t* p) {
    ...
    Gia_ManForEachAnd(p, pThis, i)
        pThis->Value
        = Gia_ManHashAnd(pNew, Gia_ObjFanin0Copy(pThis), Gia_ObjFanin1Copy(pThis));
    Gia_ManForEachPo(p, pThis, i)
        pThis->Value
        = Gia_ObjFanin0Copy(pThis);
...
Gia_Man_t* Gia_ManRemoveEnables(Gia_Man_t* p) {
    ...
    Gia_ManForEachRi(p, pFlopIn, i) {
        ...
        if (Gia_ObjChild0(pFan0) != pFlopOut && Gia_ObjChild1(pFan0) != pFlopOut && Gia_ObjChild0(pFan1) != pFlopOut && Gia_ObjChild1(pFan1) != pFlopOut) {
