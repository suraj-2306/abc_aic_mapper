...
void Hcd_ManSimulateSimple(Hcd_Man_t* p) {
    ...
    Gia_ManForEachAnd(p->pGia, pObj, i) {
        ...
        Hcd_ObjSetSim(p, i, (Gia_ObjFaninC0(pObj) ? ~Res0 : Res0) & (Gia_ObjFaninC1(pObj) ? ~Res1 : Res1));
    }
}
...
unsigned Gia_Resimulate_rec(Hcd_Man_t* p, int iObj) {
    ...
    return Hcd_ObjSetSim(p, iObj, (Gia_ObjFaninC0(pObj) ? ~Res0 : Res0) & (Gia_ObjFaninC1(pObj) ? ~Res1 : Res1));
}
...
int Gia_ComputeEquivalencesLevel(Hcd_Man_t* p, Gia_Man_t* pGiaLev, Vec_Ptr_t* vOldRoots, int Level, int fUseMiniSat) {
    ...
    for (nIter = 0; Vec_PtrSize(vOldRoots) > 0; nIter++) {
        ...
        Vec_PtrForEachEntry(Gia_Obj_t*, vOldRoots, pRoot, i) {
            ...
            if (fOneFailed) {
                nClassRefs++;
                Vec_PtrForEachEntry(Gia_Obj_t*, vMembers, pMember, k) if (pMember != pTempRepr && !Gia_ObjFailed(p->pGia, Gia_ObjId(p->pGia, pMember)))
