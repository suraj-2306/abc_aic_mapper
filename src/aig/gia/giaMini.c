...
Mini_Aig_t* Gia_ManToMiniAig(Gia_Man_t* pGia) {
    ...
    Gia_ManForEachAnd(pGia, pObj, i)
        pObj->Value
        = Mini_AigAnd(p, Gia_ObjFanin0Copy(pObj), Gia_ObjFanin1Copy(pObj));
    // create primary outputs
    Gia_ManForEachCo(pGia, pObj, i)
        pObj->Value
        = Mini_AigCreatePo(p, Gia_ObjFanin0Copy(pObj));
    // set registers
...
Vec_Bit_t* Gia_ManFindComplLuts(Gia_Man_t* pGia) {
    ...
    Gia_ManForEachCo(pGia, pObj, i) if (Gia_ObjIsAnd(Gia_ObjFanin0(pObj)) && Gia_ObjFaninC0(pObj))
...
Mini_Lut_t* Gia_ManToMiniLut(Gia_Man_t* pGia) {
    ...
    Gia_ManForEachLut(pGia, i) {
        ...
        Vec_IntForEachEntry(vLeaves, iFanin, k) if (Vec_BitEntry(vMarks, iFanin))
...
char* Gia_ManToMiniLutAttr(Gia_Man_t* pGia, void* pMiniLut) {
    Mini_Lut_t* p = (Mini_Lut_t*)pMiniLut;
    int i;
    char* pAttrs = ABC_CALLOC(char, Mini_LutNodeNum(p));
    Gia_ManForEachLut(pGia, i) if (Gia_ObjLutIsMux(pGia, i))
        pAttrs[Gia_ManObj(pGia, i)->Value]
        = 1;
    return pAttrs;
}
...
int* Abc_FrameReadMiniLutSwitching(Abc_Frame_t* pAbc) {
    ...
    Vec_IntForEachEntry(pAbc->vCopyMiniLut, iObj, i) if (iObj >= 0)
        pRes[i]
        = (int)(10000 * Vec_FltEntry((Vec_Flt_t*)vSwitching, Abc_Lit2Var(iObj)));
...
int* Abc_FrameReadMiniLutSwitchingPo(Abc_Frame_t* pAbc) {
    ...
    Gia_ManForEachCoDriverId(pAbc->pGiaMiniAig, iObj, i)
        pRes[i]
        = (int)(10000 * Vec_FltEntry((Vec_Flt_t*)vSwitching, iObj));
    Vec_IntFree(vSwitching);
    return pRes;
}
...
void Gia_MiniAigGenerateFromFile() {
    ...
    Vec_IntForEachEntryTwo(vMapOut, vMapIn, ObjOut, ObjIn, i) if (!ObjOut && ObjIn)
        Vec_IntPush(vPis, i);
