...
void Gia_ManCounterExampleValueStart(Gia_Man_t* pGia, Abc_Cex_t* pCex) {
    ...
    for (i = 0; i <= pCex->iFrame; i++) {
        ...
        Gia_ManForEachPi(pGia, pObj, k) if (Abc_InfoHasBit(pCex->pData, iBit++))
        ...
        Gia_ManForEachRiRo(pGia, pObjRi, pObjRo, k) if (Abc_InfoHasBit((unsigned*)pGia->pData2, nObjs * i + Gia_ObjId(pGia, pObjRi)))
...
Abc_Cex_t* Gia_ManCexExtendToIncludeCurrentStates(Gia_Man_t* p, Abc_Cex_t* pCex) {
    ...
    Gia_ManForEachRi(p, pObjRi, k)
        pObjRi->fMark0
        = Abc_InfoHasBit(pCex->pData, iBit++);
    ...
    for (i = 0; i <= pCex->iFrame; i++) {
        ...
        Gia_ManForEachRiRo(p, pObjRi, pObjRo, k)
            pObjRo->fMark0
            = pObjRi->fMark0;
        Gia_ManForEachCi(p, pObj, k) if (pObj->fMark0)
...
Abc_Cex_t* Gia_ManCexExtendToIncludeAllObjects(Gia_Man_t* p, Abc_Cex_t* pCex) {
    ...
    for (i = 0; i <= pCex->iFrame; i++) {
        ...
        Gia_ManForEachRiRo(p, pObjRi, pObjRo, k)
            pObjRo->fMark0
            = pObjRi->fMark0;
        Gia_ManForEachObj(p, pObj, k) if (pObj->fMark0)
            Abc_InfoSetBit(pNew->pData, pNew->nPis * i + k);
        Gia_ManForEachAnd(p, pObj, k)
            pObj->fMark0
            = (Gia_ObjFanin0(pObj)->fMark0 ^ Gia_ObjFaninC0(pObj)) & (Gia_ObjFanin1(pObj)->fMark0 ^ Gia_ObjFaninC1(pObj));
...
Gia_Man_t* Gia_ManFramesForCexMin(Gia_Man_t* p, int nFrames) {
    ...
    for (f = 0; f < nFrames; f++) {
        Gia_ManForEachRo(p, pObj, i)
            pObj->Value
            = f ? Gia_ObjRoToRi(p, pObj)->Value : 0;
