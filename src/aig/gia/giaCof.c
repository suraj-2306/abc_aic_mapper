...
static inline void Cof_ManResetTravId(Cof_Man_t* p) {
    extern void Cof_ManCleanValue(Cof_Man_t * p);
    Cof_ManCleanValue(p);
    p->nTravIds = 1;
}
...
Cof_Man_t* Cof_ManCreateLogicSimple(Gia_Man_t* pGia) {
    ...
    Gia_ManForEachObj(pGia, pObj, i) {
        ...
        if (Gia_ObjIsAnd(pObj)) {
            ...
            pFanLog->Fanios[pFanLog->nFanins + pFanLog->Value++].iFan = pObjLog->Fanios[pObjLog->nFanins].iFan = Cof_ObjHandleDiff(pObjLog, pFanLog);
...
Vec_Ptr_t* Cof_ManCollectHighFanout(Cof_Man_t* p, int nNodes) {
    ...
    Cof_ManForEachObj(p, pObj, i) if (Cof_ObjIsCi(pObj) || Cof_ObjIsNode(pObj))
...
Vec_Int_t* Gia_ManCofVars(Gia_Man_t* p, int nFanLim) {
    ...
    Gia_ManForEachObj(p, pObj, i) if (Gia_ObjIsCand(pObj) && Gia_ObjRefNum(p, pObj) >= nFanLim)
        Vec_IntPush(vVars, i);
    ABC_FREE(p->pRefs);
    return vVars;
}
