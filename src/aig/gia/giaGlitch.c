...
static inline Gli_Obj_t* Gli_ManPi(Gli_Man_t* p, int v) {
    assert(v < Gli_ManPiNum(p));
    return Gli_ManCi(p, v);
}
static inline Gli_Obj_t* Gli_ManPo(Gli_Man_t* p, int v) {
    assert(v < Gli_ManPoNum(p));
    return Gli_ManCo(p, v);
}
static inline Gli_Obj_t* Gli_ManRo(Gli_Man_t* p, int v) {
    assert(v < Gli_ManRegNum(p));
    return Gli_ManCi(p, Gli_ManRegNum(p) + v);
}
static inline Gli_Obj_t* Gli_ManRi(Gli_Man_t* p, int v) {
    assert(v < Gli_ManRegNum(p));
    return Gli_ManCo(p, Gli_ManRegNum(p) + v);
}
...
void Gli_ObjAddFanin(Gli_Obj_t* pObj, Gli_Obj_t* pFanin) {
    ...
    pFanin->Fanios[pFanin->nFanins + pFanin->iFanout++] = pObj->Fanios[pObj->iFanin++] = pObj->Handle - pFanin->Handle;
}
...
void Gli_ManSetPiRandom(Gli_Man_t* p, float PiTransProb) {
    ...
    Gli_ManForEachCi(p, pObj, i) if (Multi * (Gia_ManRandom(0) & 0xffff) < PiTransProb) {
...
void Gli_ManSetPiFromSaved(Gli_Man_t* p, int iBit) {
    ...
    int i;
    Vec_IntClear(p->vCisChanged);
    Gli_ManForEachCi(p, pObj, i) if ((p->pSimInfoPrev[i] ^ pObj->uSimInfo) & (1 << iBit)) {
        Vec_IntPush(p->vCisChanged, pObj->Handle);
        pObj->fPhase ^= 1;
        pObj->fPhase2 ^= 1;
        pObj->nSwitches++;
...
void Gli_ManSimulateSeqPref(Gli_Man_t* p, int nPref) {
    ...
    Gli_ManForEachPi(p, pObj, i)
        pObj->uSimInfo
        = Gli_ManUpdateRandomInput(pObj->uSimInfo, 0.5);
    ...
    for (f = 0; f < nPref; f++) {
        ...
        Gli_ManForEachNode(p, pObj, i)
            pObj->uSimInfo
            = Gli_ManSimulateSeqNode(p, pObj);
        Gli_ManForEachRi(p, pObj, i)
            pObj->uSimInfo
            = Gli_ObjFanin(pObj, 0)->uSimInfo;
...
void Gli_ManSetDataSaved(Gli_Man_t* p, int iBit) {
    ...
    Gli_ManForEachCi(p, pObj, i)
        pObj->fPhase
        = pObj->fPhase2 = ((p->pSimInfoPrev[i] >> iBit) & 1);
    Gli_ManForEachNode(p, pObj, i)
        pObj->fPhase
        = pObj->fPhase2 = Gli_NodeComputeValue(pObj);
}
...
void Gli_ManSetPiRandomSeq(Gli_Man_t* p, float PiTransProb) {
    ...
    Gli_ManForEachCo(p, pObj, i)
        pObj->fPhase
        = pObj->fPhase2 = Gli_ObjFanin(pObj, 0)->fPhase;
    // set changed PIs
    Vec_IntClear(p->vCisChanged);
    Gli_ManForEachPi(p, pObj, i) if (Multi * (Gia_ManRandom(0) & 0xffff) < PiTransProb) {
        Vec_IntPush(p->vCisChanged, pObj->Handle);
        pObj->fPhase ^= 1;
        pObj->fPhase2 ^= 1;
        pObj->nSwitches++;
    ...
    // set changed ROs
    Gli_ManForEachRiRo(p, pObjRi, pObj, i) if (pObjRi->fPhase != pObj->fPhase) {
        Vec_IntPush(p->vCisChanged, pObj->Handle);
        pObj->fPhase ^= 1;
        pObj->fPhase2 ^= 1;
        pObj->nSwitches++;
