...
void Gia_ManRandomInfo(Vec_Ptr_t* vInfo, int iInputStart, int iWordStart, int iWordStop) {
    unsigned* pInfo;
    int i, w;
    Vec_PtrForEachEntryStart(unsigned*, vInfo, pInfo, i, iInputStart) for (w = iWordStart; w < iWordStop; w++)
        pInfo[w]
        = Gia_ManRandom(0);
}
...
void Gia_ManSetPhasePattern(Gia_Man_t* p, Vec_Int_t* vCiValues) {
    ...
    Gia_ManForEachObj(p, pObj, i) if (Gia_ObjIsCi(pObj))
        pObj->fPhase
        = Vec_IntEntry(vCiValues, Gia_ObjCioId(pObj));
    else Gia_ObjSetPhase(p, pObj);
}
...
Vec_Int_t* Gia_ManRequiredLevel(Gia_Man_t* p) {
    ...
    Gia_ManForEachCi(p, pObj, i)
        LevelMax
        = Abc_MaxInt(LevelMax, Vec_IntEntry(vRequired, Gia_ObjId(p, pObj)));
    Gia_ManForEachObj(p, pObj, i)
...
Vec_Int_t* Gia_ManDfsForCrossCut(Gia_Man_t* p, int fReverse) {
    ...
    if (fReverse) {
        Gia_ManForEachCoReverse(p, pObj, i) if (!Gia_ObjIsConst0(Gia_ObjFanin0(pObj)))
            Gia_ManDfsForCrossCut_rec(p, pObj, vNodes);
    } else {
        Gia_ManForEachCo(p, pObj, i) if (!Gia_ObjIsConst0(Gia_ObjFanin0(pObj)))
            Gia_ManDfsForCrossCut_rec(p, pObj, vNodes);
    }
    return vNodes;
}
...
Gia_Obj_t* Gia_ObjRecognizeMux(Gia_Obj_t* pNode, Gia_Obj_t** ppNodeT, Gia_Obj_t** ppNodeE) {
    ...
    if (Gia_ObjFanin1(pNode0) == Gia_ObjFanin1(pNode1) && (Gia_ObjFaninC1(pNode0) ^ Gia_ObjFaninC1(pNode1))) {
        ...
        if (Gia_ObjFaninC1(pNode0)) {                  // pNode2->p2 is positive phase of C
        ...
        if (Gia_ObjFaninC0(pNode0)) {                  // pNode2->p1 is positive phase of C
        ...
        //        if ( FrGia_IsComplement(pNode1->p1) )
        if (Gia_ObjFaninC0(pNode0)) {                  // pNode2->p2 is positive phase of C
            *ppNodeT = Gia_Not(Gia_ObjChild0(pNode1)); //pNode2->p1);
            *ppNodeE = Gia_Not(Gia_ObjChild1(pNode0)); //pNode1->p2);
            return Gia_ObjChild1(pNode1);              //pNode2->p2;
        } else {                                       // pNode1->p1 is positive phase of C
        ...
        //        if ( FrGia_IsComplement(pNode1->p2) )
        if (Gia_ObjFaninC1(pNode0)) {                  // pNode2->p1 is positive phase of C
            *ppNodeT = Gia_Not(Gia_ObjChild1(pNode1)); //pNode2->p2);
            *ppNodeE = Gia_Not(Gia_ObjChild0(pNode0)); //pNode1->p1);
            return Gia_ObjChild0(pNode1);              //pNode2->p1;
        } else {                                       // pNode1->p2 is positive phase of C
...
int Gia_NodeMffcMapping_rec(Gia_Man_t* p, int iObj, Vec_Int_t* vMapping, Vec_Int_t* vSupp) {
    ...
    Vec_IntForEachEntry(vSupp, iNode, i)
        Count
        += Gia_NodeMffcMapping_rec(p, iNode, vMapping, vSupp);
    return Count;
}
int Gia_NodeMffcMapping(Gia_Man_t* p) {
    ...
    Gia_ManForEachCoDriverId(p, Id, i)
        Count
        += Gia_NodeMffcMapping_rec(p, Id, vMapping, vSupp);
    p->pRefs = pRefsOld;
    Vec_IntFree(vSupp);
    p->vMapping = vMapping;
...
Vec_Int_t* Gia_ManGetDangling(Gia_Man_t* p) {
    ...
    Gia_ManForEachAnd(p, pObj, i) if (!pObj->fMark0)
...
void Gia_ManInvertConstraints(Gia_Man_t* pAig) {
    ...
    if (Gia_ManConstrNum(pAig) == 0)
        return;
    Gia_ManForEachPo(pAig, pObj, i) if (i >= Gia_ManPoNum(pAig) - Gia_ManConstrNum(pAig))
...
Vec_Int_t* Gia_ManPoXSim(Gia_Man_t* p, int nFrames, int fVerbose) {
    ...
    for (f = 0; f < nFrames; f++) {
        ...
        Gia_ManForEachPo(p, pObj, k) if (Vec_IntEntry(vRes, k) == nFrames && Gia_ObjTerSimGetX(pObj))
            Vec_IntWriteEntry(vRes, k, f),
            nLeft--;
        if (nLeft == 0)
            break;
    }
...
void Gia_AigerWriteLut(Gia_Man_t* p, char* pFileName) {
    ...
    // nodes
    Gia_ManForEachObj(p, pObj, i) if (i && Gia_ObjIsLut(p, i)) {
        word truth;
        pLuts[iLut].Type = 3;
        Gia_LutForEachFanin(p, i, iFan, k)
            pLuts[iLut]
                .pFans[k]
            = Gia_ManObj(p, iFan)->Value;
        pLuts[iLut].nFans = k;
...
void Gia_DumpLutSizeDistrib(Gia_Man_t* p, char* pFileName) {
    ...
    int i, Counts[10] = {0};
    Gia_ManForEachLut(p, i) if (Gia_ObjLutSize(p, i) > 0 && Gia_ObjLutSize(p, i) < 10)
...
int Gia_ManCountPosWithNonZeroDrivers(Gia_Man_t* p) {
    ...
    Gia_ManForEachCo(p, pObj, i)
        Count
        += Gia_ObjFaninLit0(pObj, Gia_ObjId(p, pObj)) != 0;
    return Count;
}
...
Gia_Man_t* Gia_ManDupWithMuxPos(Gia_Man_t* p) {
    ...
    for (i = 0; i < Vec_IntSize(vQuads) / 4; i++) {
        ...
        if ((Vec_BitEntry(vHeads, iObj) && Vec_BitEntry(vDatas, iObj)) || (Vec_BitEntry(vHeads, iData0) && Vec_BitEntry(vDatas, iData0)) || (Vec_BitEntry(vHeads, iData1) && Vec_BitEntry(vDatas, iData1))) {
...
int Gia_ManSumTotalOfSupportSizes(Gia_Man_t* p) {
    ...
    Gia_ManForEachCo(p, pObj, i)
        nResult
        += Vec_IntSize(Vec_WecEntry(vSupps, Gia_ObjFaninId0p(p, pObj)));
    Vec_WecFree(vSupps);
    return nResult;
}
...
int Gia_ManSumTotalOfSupportSizes2(Gia_Man_t* p) {
    ...
    for (r = 0; r < nRounds; r++) {
        ...
        Gia_ManForEachCo(p, pObj, i)
            nResult
            += Abc_TtCountOnes(Vec_WrdEntry(vSims, Gia_ObjFaninId0p(p, pObj)));
        for (i = 0; i < Limit; i++)
...
Gia_Man_t* Gia_ManComputeCofs2(Gia_Man_t* p) {
    ...
    Gia_ManForEachRi(p, pSink, o) {
        ...
        Vec_IntSort(vSupp, 0);
        for (m = 0; m < 5; m++) {
            Gia_ManForEachObjVec(vSupp, p, pObj, i) if (i >= Vec_IntSize(vSupp) - 5)
                pObj->Value
                = (i == Vec_IntSize(vSupp) - 5 + m) ? 1 : 0;
            ...
            Gia_ManForEachObjVec(vSupp, p, pObj, i) if (i >= Vec_IntSize(vSupp) - 5)
                pObj->Value
                = Abc_Var2Lit(1 + Gia_ObjCioId(pObj), 0);
        }
    }
    pNew = Gia_ManCleanup(pTemp = pNew);
    Gia_ManStop(pTemp);
...
Vec_Wrd_t* Gia_ManDetectSims(Gia_Man_t* p, int iCo, int nWords) {
    ...
    Gia_Obj_t *pObj = Gia_ManCo(p, iCo), *pTemp;
