...
Gia_Man_t* Slv_ManToGia(Slv_Man_t* p) {
    ...
    Slv_ManForEachObj(p, iObj) if (Slv_ObjIsCi(p, iObj))
...
void Gia_ManStructExperiment(Gia_Man_t* p) {
    ...
    Gia_ManForEachCo(p, pObj, i) {
        ...
        Vec_PtrForEachEntry(Gia_Man_t*, vGias, pUsed, k) if (Gia_ManCiNum(pTemp) == Gia_ManCiNum(pUsed) && Cec_ManVerifyTwo(pTemp, pUsed, 0) == 1) {
            ABC_SWAP(void*, Vec_PtrArray(vGias)[0], Vec_PtrArray(vGias)[k]);
            break;
        }
        else ABC_FREE(pTemp->pCexComb);
