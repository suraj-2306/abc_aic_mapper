...
static inline int Cbs2_VarDecLevel(Cbs2_Man_t* p, int iVar) {
    assert(!Cbs2_VarUnused(p, iVar));
    return Vec_IntEntry(&p->vLevReason, 3 * iVar);
}
static inline int Cbs2_VarReason0(Cbs2_Man_t* p, int iVar) {
    assert(!Cbs2_VarUnused(p, iVar));
    return Vec_IntEntry(&p->vLevReason, 3 * iVar + 1);
}
static inline int Cbs2_VarReason1(Cbs2_Man_t* p, int iVar) {
    assert(!Cbs2_VarUnused(p, iVar));
    return Vec_IntEntry(&p->vLevReason, 3 * iVar + 2);
}
static inline int* Cbs2_VarReasonP(Cbs2_Man_t* p, int iVar) {
    assert(!Cbs2_VarUnused(p, iVar));
...
static inline int Cbs2_QueHasNode(Cbs2_Que_t* p, int iObj) {
    int i, iTemp;
    Cbs2_QueForEachEntry(*p, iTemp, i) if (iTemp == iObj) return 1;
    return 0;
}
...
static inline int Cbs2_ManDecideHighest(Cbs2_Man_t* p) {
    ...
    Cbs2_QueForEachEntry(p->pJust, iObj, i) if (iObjMax == 0 || iObjMax < iObj)
        iObjMax
        = iObj;
    return iObjMax;
}
...
static inline Gia_Obj_t* Cbs2_ManDecideLowest(Cbs2_Man_t* p) {
    ...
    Cbs2_QueForEachEntry(p->pJust, iObj, i) if (iObjMin == 0 || iObjMin > iObj)
        iObjMin
        = iObj;
...
static inline int Cbs2_ManPropagateOne(Cbs2_Man_t* p, int iVar, int Level) {
    Gia_Obj_t* pVar = Gia_ManObj(p->pAig, iVar);
    int Value0, Value1;
    ...
    if (Cbs2_VarValue(p, iVar) == 1) {  // value is 1
...
int Cbs2_ManUpdateFrontier(Cbs2_Man_t* p, int iPropHeadOld, int* piDecLit) {
    ...
    *piDecLit = -1;
    ...
    Cbs2_QueForEachEntry(p->pJust, iVar, i) if (i == iJustTailOld) break;
...
void Cbs2_ManPrintFanouts(Cbs2_Man_t* p) {
    Gia_Obj_t* pObj;
    int iObj;
    Gia_ManForEachObj(p->pAig, pObj, iObj) if (Vec_IntEntry(&p->vFanout0, iObj))
