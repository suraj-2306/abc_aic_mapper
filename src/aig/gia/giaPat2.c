...
static inline int Min_ObjCioId(Min_Man_t* p, int i) {
    assert(i && !Min_ObjIsNode(p, i));
    return Min_ObjLit1(p, i);
}
...
static inline void Min_LitSetValL(Min_Man_t* p, int i, char v) {
    assert(v == 0 || v == 1);
    Vec_StrWriteEntry(&p->vValsL, i, v);
    Vec_StrWriteEntry(&p->vValsL, i ^ 1, (char)!v);
    Vec_IntPush(&p->vVis, Abc_Lit2Var(i));
}
...
static inline void Min_ManStartValsN(Min_Man_t* p) {
    Vec_StrGrow(&p->vValsN, Vec_IntCap(&p->vFans) / 2);
    Vec_StrFill(&p->vValsN, Min_ManObjNum(p), 2);
}
static inline void Min_ManStartValsL(Min_Man_t* p) {
    Vec_StrGrow(&p->vValsL, Vec_IntCap(&p->vFans));
    Vec_StrFill(&p->vValsL, Vec_IntSize(&p->vFans), 2);
}
static inline int Min_ManCheckCleanValsL(Min_Man_t* p) {
    int i;
    char c;
    Vec_StrForEachEntry(&p->vValsL, c, i) if (c != 2) return 0;
    return 1;
}
static inline void Min_ManCleanVisitedValL(Min_Man_t* p) {
    int i, iObj;
    Vec_IntForEachEntry(&p->vVis, iObj, i) Min_ObjCleanValL(p, iObj);
    Vec_IntClear(&p->vVis);
}
...
Min_Man_t* Min_ManFromGia(Gia_Man_t* p, Vec_Int_t* vOuts) {
    ...
    Gia_ManForEachCi(p, pObj, i)
        pObj->Value
        = Min_ManAppendCi(pNew);
    if (vOuts == NULL) {
        Gia_ManForEachAnd(p, pObj, i)
            pObj->Value
            = Min_ManAppendObj(pNew, Gia_ObjFaninLit0(pObj, i), Gia_ObjFaninLit1(pObj, i));
        Gia_ManForEachCo(p, pObj, i)
            pObj->Value
            = Min_ManAppendCo(pNew, Gia_ObjFaninLit0p(p, pObj));
    } else {
...
int Min_ManRemoveItem(Vec_Wec_t* vCexes, int iItem, int iFirst, int iLimit) {
    Vec_Int_t *vLevel, *vLevel0 = Vec_WecEntry(vCexes, iItem);
    int i;
    assert(iFirst <= iItem && iItem < iLimit);
    Vec_WecForEachLevelReverseStartStop(vCexes, vLevel, i, iLimit, iFirst) if (Vec_IntSize(vLevel) > 0) break;
...
int Min_ManBitPackTry(Vec_Wrd_t* vSimsPi, int nWords, int iPat, Vec_Int_t* vLits) {
    ...
    Vec_IntForEachEntry(vLits, Lit, i) {
        ...
        if (Abc_InfoHasBit((unsigned*)pCare, iPat) && Abc_InfoHasBit((unsigned*)pInfo, iPat) == Abc_LitIsCompl(Lit)) // Lit is in ms notation
            return 0;
    }
    Vec_IntForEachEntry(vLits, Lit, i) {
        word* pInfo = Vec_WrdEntryP(vSimsPi, nWords * Abc_Lit2Var(Lit - 2)); // Lit is based on ObjId
...
Vec_Int_t* Min_ManGetUnsolved(Gia_Man_t* p) {
    ...
    int i, Driver;
    Gia_ManForEachCoDriverId(p, Driver, i) if (Driver > 0)
        Vec_IntPush(vRes, i);
...
Vec_Wrd_t* Min_ManCollect(Gia_Man_t* p, int nConf, int nConf2, int nMaxTries, int nMinCexes, int fUseSim, int fUseSat, int fVerbose, int fVeryVerbose) {
    ...
    int nArgs = fVerbose ? printf("Generating patterns: Conf = %d (%d). Tries = %d. Pats = %d. Sim = %d. SAT = %d.\n",
                                  nConf, nConf2, nMaxTries, nMinCexes, fUseSim, fUseSat)
                         : 0;
