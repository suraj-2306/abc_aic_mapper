...
Sfm_Ntk_t* Gia_ManExtractMfs(Gia_Man_t* p) {
    word uTruth, *pTruth, uTruths6[6] = {ABC_CONST(0xAAAAAAAAAAAAAAAA), ABC_CONST(0xCCCCCCCCCCCCCCCC), ABC_CONST(0xF0F0F0F0F0F0F0F0), ABC_CONST(0xFF00FF00FF00FF00), ABC_CONST(0xFFFF0000FFFF0000), ABC_CONST(0xFFFFFFFF00000000)};
...
Gia_Man_t* Gia_ManInsertMfs(Gia_Man_t* p, Sfm_Ntk_t* pNtk, int fAllBoxes) {
    ...
    Gia_ManForEachObj(p, pObj, i) if (Gia_ObjCopyArray(p, i) > 0)
    ...
    // map primary inputs
    Gia_ManForEachCiId(p, Id, i) if (i < nRealPis)
    ...
    Vec_IntForEachEntry(vMapping, iLitNew, i) if (iLitNew > 0)
