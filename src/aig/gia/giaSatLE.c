...
static inline int Sle_SetLastCutIsContained(Vec_Int_t* vTemp, int* pBase) {
    ...
    Sle_ForEachCut(pList, pCut, i) if (Sle_CutIsUsed(pCut) && Sle_CutSize(pCut) <= Sle_CutSize(pBase) && (Sle_CutSign(pCut) & Sle_CutSign(pBase)) == Sle_CutSign(pCut) && Sle_SetCutIsContainedOrder(pBase, pCut)) return 1;
    return 0;
}
static inline void Sle_SetAddCut(Vec_Int_t* vTemp, int* pCut) {
    int i, *pBase, *pList = Vec_IntArray(vTemp);
    Sle_ForEachCut(pList, pBase, i) if (Sle_CutIsUsed(pBase) && Sle_CutSize(pCut) < Sle_CutSize(pBase) && (Sle_CutSign(pCut) & Sle_CutSign(pBase)) == Sle_CutSign(pCut) && Sle_SetCutIsContainedOrder(pBase, pCut))
...
Vec_Int_t* Sle_ManComputeCuts(Gia_Man_t* p, int nLutSize, int fVerbose) {
    ...
    Gia_ManForEachAndId(p, iObj)
        nCuts
        += Sle_ManCutMerge(p, iObj, vCuts, vTemp, nLutSize);
    if (fVerbose)
...
void Sle_ManDeriveInit(Sle_Man_t* p) {
    ...
    Gia_ManForEachAndId(p->pGia, iObj) {
        ...
        Sle_ForEachCut(pList, pCut, i) if (Sle_ManCheckContained(Sle_CutLeaves(pCut), Sle_CutSize(pCut), pFanins, nFanins)) {
            iFound = i;
            break;
        }
        if (iFound == -1) {
...
void Sle_ManDeriveCnf(Sle_Man_t* p, int nBTLimit, int fDynamic) {
    ...
    Gia_ManForEachCoDriverId(p->pGia, iObj, i) if (Vec_BitEntry(p->vMask, iObj)) // internal node
    {
    ...
    Gia_ManForEachAndId(p->pGia, iObj) {
        ...
        int *pCut, *pList = Sle_ManList(p, iObj);
        Sle_ForEachCut(pList, pCut, i) if (Sle_ManCutHasPisOnly(pCut, p->vMask)) {
...
int Sle_ManAddEdgeConstraints(Sle_Man_t* p, int nEdges) {
    ...
    Vec_WecForEachLevel(p->vFanoutEdges, vArray, iObj) {
        ...
        Vec_IntClear(vTemp);
        Vec_IntForEachEntry(vArray, EdgeJ, j) if (sat_solver_var_value(p->pSat, EdgeJ))
...
void Sle_ManDeriveResult(Sle_Man_t* p, Vec_Int_t* vEdge2, Vec_Int_t* vMapping) {
    ...
    Gia_ManForEachAndId(p->pGia, iObj) {
        ...
        if (!sat_solver_var_value(p->pSat, iObj))
            continue;
        Sle_ForEachCut(pList, pCut, iCut) if (sat_solver_var_value(p->pSat, iCutVar0 + iCut)) {
    ...
    Gia_ManForEachAndId(p->pGia, iObj) {
        ...
        //printf( "\n" );
        Vec_IntForEachEntry(vCutFans, iFanin, i) if (sat_solver_var_value(p->pSat, iFanin) && sat_solver_var_value(p->pSat, iEdgeVar0 + i)) {
            // verify edge
...
void Sle_ManExplore(Gia_Man_t* pGia, int nBTLimit, int DelayInit, int fDynamic, int fTwoEdges, int fVerbose) {
    ...
    for (Delay = p->nLevels; Delay >= 0; Delay--) {
        ...
        if (Delay < p->nLevels) {
            Gia_ManForEachCoDriverId(p->pGia, iLut, i) if (Vec_BitEntry(p->vMask, iLut)) // internal node
            {
