...
Vec_Int_t* Gia_ManDfsArrivals(Gia_Man_t* p, Vec_Int_t* vObjs) {
    ...
    if (pManTime) {
        ...
        Gia_ManForEachCi(p, pObj, j) if (j < Tim_ManPiNum(pManTime)) {
...
Vec_Int_t* Gia_ManDfsRequireds(Gia_Man_t* p, Vec_Int_t* vObjs, int ReqTime) {
    ...
    if (pManTime) {
        ...
        Gia_ManForEachCo(p, pObj, j) if (j >= nCoLimit) {
...
Vec_Int_t* Gia_ManDfsSlacks(Gia_Man_t* p) {
    ...
    if (Vec_IntSize(vObjs) > 0) {
        ...
        Vec_IntForEachEntry(vReqs, Req, i) if (Req != -1)
            Arrivals
            = Abc_MinInt(Arrivals, Req);
        //if ( Arrivals != 0 )
...
void Gia_ManDfsSlacksPrint(Gia_Man_t* p) {
    ...
    // compute slacks
    Vec_IntForEachEntry(vSlacks, Entry, i) if (Entry != -1)
...
Vec_Bit_t* Gia_ManGenUsed(Gia_Man_t* p, int fBuf) {
    ...
    Gia_ManForEachCo(p, pObj, i) if (Gia_ObjFaninC0(pObj) ^ fBuf)
...
void Gia_ManDumpVerilog(Gia_Man_t* p, char* pFileName, Vec_Int_t* vObjs, int fVerBufs) {
    ...
    Gia_ManForEachAnd(p, pObj, i) {
        int fSkip = 0;
        if (vObjs) {
            Vec_IntForEachEntry(vObjs, iObj, k) if (iObj == i) break;
