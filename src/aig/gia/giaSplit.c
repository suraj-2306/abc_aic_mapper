...
Vec_Int_t* Spl_ManFromWecMapping(Gia_Man_t* p, Vec_Wec_t* vMap) {
    ...
    Vec_WecForEachLevel(vMap, vVec, i) if (Vec_IntSize(vVec) > 0) {
...
*/
    ...
    Vec_IntForEachEntry(p->vAnds, iObj, i) if (Gia_ObjLutRefNumId(p->pGia, iObj))
...
int Spl_ManCountMarkedFanins(Gia_Man_t* p, int iObj, Vec_Bit_t* vMarks) {
    ...
    Vec_IntForEachEntry(vFanins, iFan, i) if (Vec_BitEntry(vMarks, iFan))
        Count++;
    return Count;
}
...
int Spl_ManFindOne(Spl_Man_t* p) {
    ...
    Gia_ManForEachLut2Vec(p->vNodes, p->pGia, vVec, iObj, i)
        Vec_IntForEachEntry(vVec, iFan, k) if (!Vec_BitEntry(p->vMarksNo, iFan) && !Vec_BitEntry(p->vMarksCIO, iFan) && !Gia_ObjLutRefNumId(p->pGia, iFan)) {
    ...
    Vec_IntClear(p->vInputs);
    Gia_ManForEachLut2Vec(p->vNodes, p->pGia, vVec, iObj, i)
        Vec_IntForEachEntry(vVec, iFan, k) if (!Vec_BitEntry(p->vMarksNo, iFan) && !Vec_BitEntry(p->vMarksCIO, iFan)) {
        Vec_IntPush(p->vCands, iFan);
        Vec_IntPush(p->vInputs, iFan);
    }
...
int Spl_ManLutMffcSize(Gia_Man_t* p, int iObj, Vec_Int_t* vTemp, Vec_Bit_t* vMarksAnd) {
    ...
    Vec_IntForEachEntry(vTemp, iTemp, i) if (!Vec_BitEntry(vMarksAnd, iTemp))
