...
static inline void CbsP_VarAssign(Gia_Obj_t* pVar) {
    assert(!pVar->fMark0);
    pVar->fMark0 = 1;
}
static inline void CbsP_VarUnassign(CbsP_Man_t* pMan, Gia_Obj_t* pVar) {
    assert(pVar->fMark0);
    pVar->fMark0 = 0;
    pVar->fMark1 = 0;
    pMan->vValue->pArray[Gia_ObjId(pMan->pAig, pVar)] = ~0;
}
static inline int CbsP_VarValue(Gia_Obj_t* pVar) {
    assert(pVar->fMark0);
    return pVar->fMark1;
}
static inline void CbsP_VarSetValue(Gia_Obj_t* pVar, int v) {
    assert(pVar->fMark0);
    pVar->fMark1 = v;
}
...
static inline int CbsP_VarDecLevel(CbsP_Man_t* p, Gia_Obj_t* pVar) {
    int Value = p->vValue->pArray[Gia_ObjId(p->pAig, pVar)];
    assert(Value != ~0);
    return Vec_IntEntry(p->vLevReas, 3 * Value);
}
static inline Gia_Obj_t* CbsP_VarReason0(CbsP_Man_t* p, Gia_Obj_t* pVar) {
    int Value = p->vValue->pArray[Gia_ObjId(p->pAig, pVar)];
    assert(Value != ~0);
    return pVar + Vec_IntEntry(p->vLevReas, 3 * Value + 1);
}
static inline Gia_Obj_t* CbsP_VarReason1(CbsP_Man_t* p, Gia_Obj_t* pVar) {
    int Value = p->vValue->pArray[Gia_ObjId(p->pAig, pVar)];
    assert(Value != ~0);
    return pVar + Vec_IntEntry(p->vLevReas, 3 * Value + 2);
}
...
static inline void CbsP_ManSaveModel(CbsP_Man_t* p, Vec_Int_t* vCex) {
    ...
    Vec_IntClear(vCex);
    p->pProp.iHead = 0;
    CbsP_QueForEachEntry(p->pProp, pVar, i) if (Gia_ObjIsCi(pVar))
...
static inline int CbsP_QueHasNode(CbsP_Que_t* p, Gia_Obj_t* pObj) {
    Gia_Obj_t* pTemp;
    int i;
    CbsP_QueForEachEntry(*p, pTemp, i) if (pTemp == pObj) return 1;
    return 0;
}
...
static inline Gia_Obj_t* CbsP_ManDecideHighest(CbsP_Man_t* p) {
    ...
    int i;
    CbsP_QueForEachEntry(p->pJust, pObj, i) if (pObjMax == NULL || pObjMax < pObj)
        pObjMax
        = pObj;
    return pObjMax;
}
...
static inline Gia_Obj_t* CbsP_ManDecideLowest(CbsP_Man_t* p) {
    ...
    int i;
    CbsP_QueForEachEntry(p->pJust, pObj, i) if (pObjMin == NULL || pObjMin > pObj)
        pObjMin
        = pObj;
    return pObjMin;
}
...
static inline int CbsP_ManPropagateOne(CbsP_Man_t* p, Gia_Obj_t* pVar, int Level) {
    ...
    if (CbsP_VarValue(pVar)) {          // value is 1
