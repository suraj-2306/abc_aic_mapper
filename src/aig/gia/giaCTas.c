...
static inline void Tas_VarAssign(Gia_Obj_t* pVar) {
    assert(!pVar->fMark0);
    pVar->fMark0 = 1;
}
static inline void Tas_VarUnassign(Gia_Obj_t* pVar) {
    assert(pVar->fMark0);
    pVar->fMark0 = 0;
    pVar->fMark1 = 0;
    pVar->Value = ~0;
}
static inline int Tas_VarValue(Gia_Obj_t* pVar) {
    assert(pVar->fMark0);
    return pVar->fMark1;
}
static inline void Tas_VarSetValue(Gia_Obj_t* pVar, int v) {
    assert(pVar->fMark0);
    pVar->fMark1 = v;
}
...
static inline int Tas_VarToLit(Tas_Man_t* p, Gia_Obj_t* pObj) {
    assert(Tas_VarIsAssigned(pObj));
    return Abc_Var2Lit(Gia_ObjId(p->pAig, pObj), !Tas_VarValue(pObj));
}
static inline int Tas_LitIsTrue(Gia_Obj_t* pObj, int Lit) {
    assert(Tas_VarIsAssigned(pObj));
...
static inline int Tas_VarDecLevel(Tas_Man_t* p, Gia_Obj_t* pVar) {
    assert(pVar->Value != ~0);
    return Vec_IntEntry(p->vLevReas, 3 * pVar->Value);
}
static inline Gia_Obj_t* Tas_VarReason0(Tas_Man_t* p, Gia_Obj_t* pVar) {
    assert(pVar->Value != ~0);
    return pVar + Vec_IntEntry(p->vLevReas, 3 * pVar->Value + 1);
}
static inline Gia_Obj_t* Tas_VarReason1(Tas_Man_t* p, Gia_Obj_t* pVar) {
    assert(pVar->Value != ~0);
    return pVar + Vec_IntEntry(p->vLevReas, 3 * pVar->Value + 2);
}
static inline int Tas_ClauseDecLevel(Tas_Man_t* p, int hClause) { return Tas_VarDecLevel(p, p->pClauses.pData[hClause]); }

static inline int Tas_VarHasReasonCls(Tas_Man_t* p, Gia_Obj_t* pVar) {
    assert(pVar->Value != ~0);
    return Vec_IntEntry(p->vLevReas, 3 * pVar->Value + 1) == 0 && Vec_IntEntry(p->vLevReas, 3 * pVar->Value + 2) != 0;
}
static inline Tas_Cls_t* Tas_VarReasonCls(Tas_Man_t* p, Gia_Obj_t* pVar) {
    assert(pVar->Value != ~0);
    return Tas_ClsFromHandle(p, Vec_IntEntry(p->vLevReas, 3 * pVar->Value + 2));
}
...
static inline int Tas_QueHasNode(Tas_Que_t* p, Gia_Obj_t* pObj) {
    Gia_Obj_t* pTemp;
    int i;
    Tas_QueForEachEntry(*p, pTemp, i) if (pTemp == pObj) return 1;
    return 0;
}
...
static inline Gia_Obj_t* Tas_ManFindActive(Tas_Man_t* p) {
    ...
    Tas_QueForEachEntry(p->pJust, pObj, i) {
        ...
        if (pObjMax == NULL || p->pActivity[Gia_ObjFaninId0(pObj, ObjId)] > BestCost || (p->pActivity[Gia_ObjFaninId0(pObj, ObjId)] == BestCost && pObjMax < Gia_ObjFanin0(pObj))) {
            pObjMax = Gia_ObjFanin0(pObj);
            BestCost = p->pActivity[Gia_ObjFaninId0(pObj, ObjId)];
        }
        if (p->pActivity[Gia_ObjFaninId1(pObj, ObjId)] > BestCost || (p->pActivity[Gia_ObjFaninId1(pObj, ObjId)] == BestCost && pObjMax < Gia_ObjFanin1(pObj))) {
...
static inline Gia_Obj_t* Tas_ManDecideHighestA(Tas_Man_t* p) {
    ...
    int i;
    Tas_QueForEachEntry(p->pJust, pObj, i) {
        if (pObjMax == NULL || p->pActivity[Gia_ObjId(p->pAig, pObjMax)] < p->pActivity[Gia_ObjId(p->pAig, pObj)])
            pObjMax = pObj;
    }
    return pObjMax;
}
...
static inline Gia_Obj_t* Tas_ManDecideLowest(Tas_Man_t* p) {
    ...
    int i;
    Tas_QueForEachEntry(p->pJust, pObj, i) if (pObjMin == NULL || pObjMin > pObj)
        pObjMin
        = pObj;
    return pObjMin;
}
...
static inline Tas_Cls_t* Tas_ManAllocCls(Tas_Man_t* p, int nSize) {
    ...
    pCls = Tas_ClsFromHandle(p, p->pStore.iCur);
    p->pStore.iCur += nSize;
...
*/
    ...
    if (Tas_VarValue(pVar)) {           // value is 1
...
int Tas_ManSolveArray(Tas_Man_t* p, Vec_Ptr_t* vObjs) {
    ...
    s_Counter4 = 0;
    Vec_IntClear(p->vModel);
    Vec_PtrForEachEntry(Gia_Obj_t*, vObjs, pObj, i) if (pObj == Gia_ManConst0(p->pAig)) return 1;
    assert(!p->pProp.iHead && !p->pProp.iTail);
    assert(!p->pJust.iHead && !p->pJust.iTail);
    assert(p->pClauses.iHead == 1 && p->pClauses.iTail == 1);
    p->Pars.nBTThis = p->Pars.nJustThis = p->Pars.nBTThisNc = 0;
    Vec_PtrForEachEntry(Gia_Obj_t*, vObjs, pObj, i) if (pObj != Gia_ManConst1(p->pAig) && !Tas_VarIsAssigned(Gia_Regular(pObj)))
        Tas_ManAssign(p, pObj, 0, NULL, NULL);
    if (!Tas_ManSolve_rec(p, 0) && !Tas_ManCheckLimits(p))
        Tas_ManSaveModel(p, p->vModel);
    else
...
int Tas_StorePatternTry(Vec_Ptr_t* vInfo, Vec_Ptr_t* vPres, int iBit, int* pLits, int nLits) {
    ...
    for (i = 0; i < nLits; i++) {
        ...
        if (Abc_InfoHasBit(pPres, iBit) && Abc_InfoHasBit(pInfo, iBit) == Abc_LitIsCompl(pLits[i]))
            return 0;
    }
    for (i = 0; i < nLits; i++) {
        pInfo = (unsigned*)Vec_PtrEntry(vInfo, Abc_Lit2Var(pLits[i]));
