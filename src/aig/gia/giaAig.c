...
Gia_Man_t* Gia_ManFromAigSwitch(Aig_Man_t* p) {
    ...
    Aig_ManForEachNode(p, pObj, i) if (Aig_ObjRefs(pObj) == 0) {
...
int Gia_ManTestChoices(Gia_Man_t* p) {
    ...
    Gia_ManForEachAnd(p, pObj, i) if (Gia_ObjSibl(p, i))
        Vec_IntWriteEntry(vPointed, Gia_ObjSibl(p, i), 1);
    Gia_ManCreateRefs(p);
    Gia_ManForEachAnd(p, pObj, i) if (Vec_IntEntry(vPointed, i) && Gia_ObjRefNumId(p, i) > 0) {
...
int Gia_ManSolveSat(Gia_Man_t* p) {
    ...
    if (RetValue == 0) {
        ...
        Gia_ManForEachPo(p, pObj, i) if (pObj->fMark0 != 1) break;
        if (i != Gia_ManPoNum(p))
