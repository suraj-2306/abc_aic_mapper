...
static inline int Nf_Cfg2Int(Nf_Cfg_t Mat) {
    union {
        int x;
        Nf_Cfg_t y;
    } v;
...
static inline Nf_Cfg_t Nf_Int2Cfg(int Int) {
    union {
        int x;
        Nf_Cfg_t y;
    } v;
...
static inline void Nf_ObjUpdateRequired(Nf_Man_t* p, int i, int c, int f) {
    if (Nf_ObjRequired(p, i, c) > f) Nf_ObjSetRequired(p, i, c, f);
}
...
static inline int Nf_CutHandle(int* pCutSet, int* pCut) {
    assert(pCut > pCutSet);
    return pCut - pCutSet;
}
static inline int* Nf_CutFromHandle(int* pCutSet, int h) {
    assert(h > 0);
    return pCutSet + h;
}
...
void Nf_StoCreateGateAdd(Vec_Mem_t* vTtMem, Vec_Wec_t* vTt2Match, Mio_Cell2_t* pCell, word uTruth, int* pFans, int nFans, Vec_Wec_t* vProfs, Vec_Int_t* vStore, int fPinFilter, int fPinPerm, int fPinQuick) {
    ...
    if (fPinFilter) {
        ...
        Vec_IntForEachEntryDouble(vArray, GateId, Entry, i) if (Nf_Int2Cfg(Entry).Phase == Mat.Phase && Nf_Int2Cfg(Entry).fCompl == Mat.fCompl) {
    ...
    if (!fPinPerm) // do not use  pin-permutation (improves delay when pin-delays differ)
    {
        ...
        {
            Vec_IntForEachEntryDouble(vArray, GateId, Entry, i) if (GateId == (int)pCell->Id && Abc_TtBitCount8[Nf_Int2Cfg(Entry).Phase] == Abc_TtBitCount8[Mat.Phase]) return;
        } else // reduce the number of matches less agressively
        {
            Vec_IntForEachEntryDouble(vArray, GateId, Entry, i) if (GateId == (int)pCell->Id && Nf_Int2Cfg(Entry).Phase == Mat.Phase) return;
        }
    }
...
int Nf_ManSetMapRefs(Nf_Man_t* p) {
    ...
    Gia_ManForEachCiId(p->pGia, Id, i) if (Nf_ObjMapRefNum(p, Id, 1)) {
...
int Nf_ManExtractWindow(void* pMan, Vec_Int_t* vRoots, Vec_Wec_t* vCuts, Vec_Wec_t* vObjCuts, Vec_Int_t* vSolCuts, Vec_Int_t* vCutGates, Vec_Wrd_t* vCutAreas, word* pInvArea, int StartVar, int nVars) {
    ...
    *pInvArea = p->InvAreaW;
        ...
        Nf_SetForEachCut(pCutSet, pCut, iCut) {
            ...
            if (Abc_Lit2Var(Nf_CutFunc(pCut)) < Vec_WecSize(p->vTt2Match)) {
                ...
                Vec_IntForEachEntryDouble(vArr, Info, Offset, i) {
                    ...
                    Nf_CfgForEachVarCompl(Cfg, nFans, iFanin, fComplF, k) if (pFans[iFanin] >= nInputs + 1) // internal node
                    {
    ...
    Gia_ManForEachCiId(p->pGia, iObj, c) if (Nf_ObjMapRefNum(p, iObj, 1))
