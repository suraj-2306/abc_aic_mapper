...
Shr_Man_t* Shr_ManAlloc(Gia_Man_t* pGia) {
    ...
    p->pGia = pGia;
    p->vFanMem = Vec_WrdAlloc(1000);
    Vec_WrdPush(p->vFanMem, -1);
...
static inline int Shr_ManDivPushOrderByLevel(Shr_Man_t* p, int iDiv) {
    ...
    for (iPlace = Vec_IntSize(p->vPrio) - 1; iPlace > 0; iPlace--)
        if (Gia_ObjLevel(p->pNew, Gia_ManObj(p->pNew, pArray[iPlace - 1])) > Gia_ObjLevel(p->pNew, Gia_ManObj(p->pNew, pArray[iPlace])))
...
static inline int Shr_ManCollectDivisors(Shr_Man_t* p, Vec_Int_t* vLeaves, int Limit, int nFanoutMax) {
    ...
    Vec_IntForEachEntry(p->vPrio, iDiv, i) {
        ...
        Shr_ObjForEachFanout(p, iDiv, iFan) {
            ...
            if (!Gia_ObjIsTravIdCurrentId(p->pNew, Gia_ObjFaninId0(pFan, iFan)) || !Gia_ObjIsTravIdCurrentId(p->pNew, Gia_ObjFaninId1(pFan, iFan)))
                continue;
    ...
    Gia_ManForEachObj1(p, pObj, i) {
        if (Gia_ObjIsCi(pObj)) {
            ...
            if (uTruth == 0 || ~uTruth == 0)
                ...
                Gia_ManForEachObjVec(pMan->vLeaves, p, pFanin, k) if (uTruth == Truth[k] || ~uTruth == Truth[k])
                    pObj->Value
                    = Abc_LitNotCond(pFanin->Value, ~uTruth == Truth[k]);
            if (pObj->Value != ~0)
                continue;
