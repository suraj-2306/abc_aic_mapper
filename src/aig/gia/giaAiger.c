...
Gia_Man_t* Gia_AigerReadFromMemory(char* pContents, int nFileSize, int fGiaSimple, int fSkipStrash, int fCheck) {
    ...
    pCur = (unsigned char*)pContents;
    while (*pCur != ' ')
        pCur++;
    pCur++;
    // read the number of objects
    nTotal = atoi((const char*)pCur);
    while (*pCur != ' ')
        pCur++;
    pCur++;
    // read the number of inputs
    nInputs = atoi((const char*)pCur);
    while (*pCur != ' ')
        pCur++;
    pCur++;
    // read the number of latches
    nLatches = atoi((const char*)pCur);
    while (*pCur != ' ')
        pCur++;
    pCur++;
    // read the number of outputs
    nOutputs = atoi((const char*)pCur);
    while (*pCur != ' ')
        pCur++;
    pCur++;
    // read the number of nodes
    nAnds = atoi((const char*)pCur);
    while (*pCur != ' ' && *pCur != '\n')
        pCur++;
    if (*pCur == ' ') {
        //        assert( nOutputs == 0 );
    ...
    if (pCur + 1 < (unsigned char*)pContents + nFileSize && *pCur == 'c') {
        ...
        while (pCur < (unsigned char*)pContents + nFileSize) {
            ...
            if (*pCur == 'a') {
                pCur++;
                vStr = Vec_StrStart(Gia_AigerReadInt(pCur));
                pCur += 4;
            ...
            // read number of constraints
                ...
                pCur++;
                assert(Gia_AigerReadInt(pCur) == 4);
                pCur += 4;
                pNew->nConstrs = Gia_AigerReadInt(pCur);
                pCur += 4;
            ...
            // read delay information
                ...
                assert(Gia_AigerReadInt(pCur) == 4);
                pCur += 4;
                pNew->nAnd2Delay = Gia_AigerReadInt(pCur);
                pCur += 4;
                if (fVerbose) printf("Finished reading extension \"d\".\n");
            } else if (*pCur == 'i') {
                pCur++;
                nInputs = Gia_AigerReadInt(pCur) / 4;
                pCur += 4;
                pNew->vInArrs = Vec_FltStart(nInputs);
                memcpy(Vec_FltArray(pNew->vInArrs), pCur, (size_t)4 * nInputs);
                pCur += 4 * nInputs;
                if (fVerbose) printf("Finished reading extension \"i\".\n");
            } else if (*pCur == 'o') {
                pCur++;
                nOutputs = Gia_AigerReadInt(pCur) / 4;
                pCur += 4;
                pNew->vOutReqs = Vec_FltStart(nOutputs);
                memcpy(Vec_FltArray(pNew->vOutReqs), pCur, (size_t)4 * nOutputs);
                pCur += 4 * nOutputs;
            ...
            // read equivalence classes
                ...
                pCur++;
                pCurTemp = pCur + Gia_AigerReadInt(pCur) + 4;
                pCur += 4;
            ...
            // read flop classes
                ...
                pCur++;
                assert(Gia_AigerReadInt(pCur) == 4 * Gia_ManRegNum(pNew));
                pCur += 4;
                pNew->vFlopClasses = Vec_IntStart(Gia_ManRegNum(pNew));
                memcpy(Vec_IntArray(pNew->vFlopClasses), pCur, (size_t)4 * Gia_ManRegNum(pNew));
                pCur += 4 * Gia_ManRegNum(pNew);
                if (fVerbose) printf("Finished reading extension \"f\".\n");
            }
            // read gate classes
            else if (*pCur == 'g') {
                pCur++;
                assert(Gia_AigerReadInt(pCur) == 4 * Gia_ManObjNum(pNew));
                pCur += 4;
                pNew->vGateClasses = Vec_IntStart(Gia_ManObjNum(pNew));
                memcpy(Vec_IntArray(pNew->vGateClasses), pCur, (size_t)4 * Gia_ManObjNum(pNew));
                pCur += 4 * Gia_ManObjNum(pNew);
            ...
            // read placement
                ...
                memcpy(pPlacement, pCur, (size_t)4 * Gia_ManObjNum(pNew));
                pCur += 4 * Gia_ManObjNum(pNew);
                assert(pCur == pCurTemp);
            ...
            // read configuration data
                ...
                pCurTemp = pCur + nSize + 4;
                pCur += 4;
                pNew->pCellStr = Abc_UtilStrsav((char*)pCur);
            ...
            // read choices
                ...
                pCurTemp = pCur + Gia_AigerReadInt(pCur) + 4;
                pCur += 4;
                nPairs = Gia_AigerReadInt(pCur);
                pCur += 4;
                for (i = 0; i < nPairs; i++) {
                    iRepr = Gia_AigerReadInt(pCur);
                    pCur += 4;
                    iNode = Gia_AigerReadInt(pCur);
                    pCur += 4;
            ...
            // read switching activity
                ...
                memcpy(pSwitching, pCur, (size_t)Gia_ManObjNum(pNew));
                pCur += Gia_ManObjNum(pNew);
                assert(pCur == pCurTemp);
                if (fVerbose) printf("Finished reading extension \"s\".\n");
            }
            // read timing manager
            ...
            // read object classes
                ...
                pCur++;
                pNew->vObjClasses = Vec_IntStart(Gia_AigerReadInt(pCur) / 4);
                pCur += 4;
    ...
    if (!fGiaSimple && !fSkipStrash && Gia_ManHasDangling(pNew)) {
        ...
        vRegClasses = pNew->vRegClasses;
        pNew->vRegClasses = NULL;
        vRegInits = pNew->vRegInits;
        pNew->vRegInits = NULL;
        vFlopMap = pNew->vFlopClasses;
        pNew->vFlopClasses = NULL;
        vGateMap = pNew->vGateClasses;
        pNew->vGateClasses = NULL;
        vObjMap = pNew->vObjClasses;
        pNew->vObjClasses = NULL;
        pManTime = (Tim_Man_t*)pNew->pManTime;
        pNew->pManTime = NULL;
        pAigExtra = pNew->pAigExtra;
        pNew->pAigExtra = NULL;
...
void Gia_AigerWrite(Gia_Man_t* pInit, char* pFileName, int fWriteSymbols, int fCompact, int fWriteNewLine) {
    ...
    if (p->vNamesNode && Vec_PtrSize(p->vNamesNode) != Gia_ManObjNum(p))
        ...
        Gia_ManForEachAnd(p, pObj, i) if (Vec_PtrEntry(p->vNamesNode, i))
