...
static inline void Cbs_VarAssign(Gia_Obj_t* pVar) {
    assert(!pVar->fMark0);
    pVar->fMark0 = 1;
}
static inline void Cbs_VarUnassign(Gia_Obj_t* pVar) {
    assert(pVar->fMark0);
    pVar->fMark0 = 0;
    pVar->fMark1 = 0;
    pVar->Value = ~0;
}
static inline int Cbs_VarValue(Gia_Obj_t* pVar) {
    assert(pVar->fMark0);
    return pVar->fMark1;
}
static inline void Cbs_VarSetValue(Gia_Obj_t* pVar, int v) {
    assert(pVar->fMark0);
    pVar->fMark1 = v;
}
...
static inline int Cbs_VarDecLevel(Cbs_Man_t* p, Gia_Obj_t* pVar) {
    assert(pVar->Value != ~0);
    return Vec_IntEntry(p->vLevReas, 3 * pVar->Value);
}
static inline Gia_Obj_t* Cbs_VarReason0(Cbs_Man_t* p, Gia_Obj_t* pVar) {
    assert(pVar->Value != ~0);
    return pVar + Vec_IntEntry(p->vLevReas, 3 * pVar->Value + 1);
}
static inline Gia_Obj_t* Cbs_VarReason1(Cbs_Man_t* p, Gia_Obj_t* pVar) {
    assert(pVar->Value != ~0);
...
static inline void Cbs_ManSaveModel(Cbs_Man_t* p, Vec_Int_t* vCex) {
    ...
    Vec_IntClear(vCex);
    p->pProp.iHead = 0;
    Cbs_QueForEachEntry(p->pProp, pVar, i) if (Gia_ObjIsCi(pVar))
...
static inline int Cbs_QueHasNode(Cbs_Que_t* p, Gia_Obj_t* pObj) {
    Gia_Obj_t* pTemp;
    int i;
    Cbs_QueForEachEntry(*p, pTemp, i) if (pTemp == pObj) return 1;
    return 0;
}
...
static inline Gia_Obj_t* Cbs_ManDecideHighest(Cbs_Man_t* p) {
    ...
    int i;
    Cbs_QueForEachEntry(p->pJust, pObj, i) if (pObjMax == NULL || pObjMax < pObj)
        pObjMax
        = pObj;
    return pObjMax;
}
...
static inline Gia_Obj_t* Cbs_ManDecideLowest(Cbs_Man_t* p) {
    ...
    int i;
    Cbs_QueForEachEntry(p->pJust, pObj, i) if (pObjMin == NULL || pObjMin > pObj)
        pObjMin
        = pObj;
    return pObjMin;
}
...
static inline int Cbs_ManPropagateOne(Cbs_Man_t* p, Gia_Obj_t* pVar, int Level) {
    ...
    if (Cbs_VarValue(pVar)) {           // value is 1
