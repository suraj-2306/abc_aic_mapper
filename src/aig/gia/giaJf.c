...
static inline void Jf_ObjCleanUnit(Gia_Obj_t* p) {
    assert(Jf_ObjIsUnit(p));
    p->fMark0 = 1;
}
...
static inline void Jf_CutSetSize(int* pCut, int s) {
    assert(s >= 0 && s < 16);
    pCut[0] ^= (Jf_CutSize(pCut) ^ s);
}
static inline void Jf_CutSetCost(int* pCut, int c) {
    assert(c >= 0 && c < 16);
    pCut[0] ^= ((Jf_CutCost(pCut) ^ c) << 4);
}
static inline void Jf_CutSetFunc(int* pCut, int f) {
    assert(f >= 0);
    pCut[0] ^= ((Jf_CutFunc(pCut) ^ f) << 8);
}
...
static inline int Jf_CutLit(int* pCut, int i) {
    assert(i);
    return pCut[i];
}
//static inline int    Jf_CutVar( int * pCut, int i )         { assert(i); return pCut[i];                               }
static inline int Jf_CutVar(int* pCut, int i) {
    assert(i);
...
Cnf_Dat_t* Jf_ManCreateCnfRemap(Gia_Man_t* p, Vec_Int_t* vLits, Vec_Int_t* vClas, int fAddOrCla) {
    ...
    Vec_IntForEachEntry(vLits, Entry, i)
        Gia_ManObj(p, Abc_Lit2Var(Entry))
            ->fMark0
        = 1;
    // create variable map
    pMap = ABC_FALLOC(int, Gia_ManObjNum(p));
    Gia_ManForEachObjReverse(p, pObj, i) if (pObj->fMark0)
        pObj->fMark0
        = 0,
        pMap[i] = nVars++;
    // relabel literals
...
void Jf_ManProfileClasses(Jf_Man_t* p) {
    ...
    Gia_ManForEachAnd(p->pGia, pObj, i) if (!Gia_ObjIsBuf(pObj) && Gia_ObjRefNumId(p->pGia, i)) {
...
int Jf_ObjCutFilterBoth(Jf_Man_t* p, Jf_Cut_t** pSto, int c) {
    ...
    for (k = 0; k < c; k++)
        if (pSto[c]->pCut[0] >= pSto[k]->pCut[0] && (pSto[c]->Sign & pSto[k]->Sign) == pSto[k]->Sign && Jf_CutIsContained1(pSto[c]->pCut, pSto[k]->pCut)) {
            pSto[c]->pCut[0] = -1;
            return c;
        }
    // filter other cuts using this cut
    for (k = last = 0; k < c; k++)
        if (!(pSto[c]->pCut[0] < pSto[k]->pCut[0] && (pSto[c]->Sign & pSto[k]->Sign) == pSto[c]->Sign && Jf_CutIsContained1(pSto[k]->pCut, pSto[c]->pCut))) {
            if (last++ == k)
                continue;
...
int Jf_ObjCutFilter(Jf_Man_t* p, Jf_Cut_t** pSto, int c) {
    ...
    if (p->pPars->fCutMin) {
        ...
        for (k = 0; k < c; k++)
            if (pSto[c]->pCut[0] >= pSto[k]->pCut[0] && (pSto[c]->Sign & pSto[k]->Sign) == pSto[k]->Sign && Jf_CutIsContainedOrder(pSto[c]->pCut, pSto[k]->pCut))
                return 0;
    }
    return 1;
}
...
int Jf_CutRef_rec(Jf_Man_t* p, int* pCut) {
    ...
    Jf_CutForEachVar(pCut, Var, i) if (!Gia_ObjRefIncId(p->pGia, Var) && !Jf_CutIsTriv(Jf_ObjCutBest(p, Var), Var))
        Count
        += Jf_CutRef_rec(p, Jf_ObjCutBest(p, Var));
    return Count;
}
int Jf_CutDeref_rec(Jf_Man_t* p, int* pCut) {
    int i, Var, Count = Jf_CutCost(pCut);
    Jf_CutForEachVar(pCut, Var, i) if (!Gia_ObjRefDecId(p->pGia, Var) && !Jf_CutIsTriv(Jf_ObjCutBest(p, Var), Var))
        Count
        += Jf_CutDeref_rec(p, Jf_ObjCutBest(p, Var));
    return Count;
}
...
static inline int Jf_ObjAddCutToStore(Jf_Man_t* p, Jf_Cut_t** pSto, int c, int cMax) {
    ...
    if (p->pPars->fCutMin) {
        for (k = last = iPivot + 1; k < c; k++)
            if (!(pSto[iPivot]->pCut[0] <= pSto[k]->pCut[0] && (pSto[iPivot]->Sign & pSto[k]->Sign) == pSto[iPivot]->Sign && Jf_CutIsContained1(pSto[k]->pCut, pSto[iPivot]->pCut))) {
                if (last++ == k)
                    continue;
                ABC_SWAP(Jf_Cut_t*, pSto[last - 1], pSto[k]);
            }
        ...
        for (k = last = iPivot + 1; k < c; k++)
            if (!(pSto[iPivot]->pCut[0] <= pSto[k]->pCut[0] && (pSto[iPivot]->Sign & pSto[k]->Sign) == pSto[iPivot]->Sign && Jf_CutIsContainedOrder(pSto[k]->pCut, pSto[iPivot]->pCut))) {
                if (last++ == k)
                    continue;
                ABC_SWAP(Jf_Cut_t*, pSto[last - 1], pSto[k]);
            }
...
int Jf_ManComputeDelay(Jf_Man_t* p, int fEval) {
    ...
    int i, Delay = 0;
    if (fEval) {
        Gia_ManForEachObj(p->pGia, pObj, i) if (Gia_ObjIsBuf(pObj))
            Jf_ObjPropagateBuf(p, pObj, 0);
...
void Jf_ManPropagateEla(Jf_Man_t* p, int fEdge) {
    ...
    Gia_ManForEachObjReverse(p->pGia, pObj, i) if (Gia_ObjIsBuf(pObj))
...
Gia_Man_t* Jf_ManDeriveMappingGia(Jf_Man_t* p) {
    ...
    Vec_IntForEachEntry(vMapping, iLit, i) if (iLit > 0)
