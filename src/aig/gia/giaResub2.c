...
int Gia_Rsb2ManInsert_rec(Vec_Int_t* vRes, int nPis, Vec_Int_t* vObjs, int iNode, Vec_Int_t* vResub, Vec_Int_t* vDivs, Vec_Int_t* vCopies, int iObj) {
    ...
    if (iObj == iNode) {
        ...
        if (Abc_Lit2Var(iTopLit) == 0)
            ...
            Vec_IntForEachEntryStop(vResub, iLit, k, Vec_IntSize(vResub) - 1) if (Abc_Lit2Var(iLit) < nVars)
    ...
    *ppArray = NULL;
    while ((iNode = Abc_ResubNodeToTry(&p->vTried, p->nPis + 1, p->iFirstPo)) > 0) {
        ...
        if (nResub == 0)
            ...
            Vec_IntFree(vRes);
            Vec_IntForEachEntry(&p->vTried, iTried, i) if (Vec_IntEntry(&p->vCopies, iTried) > Abc_Var2Lit(p->nPis, 0)) // internal node
...
int Gia_WinTryAddingNode(Gia_Man_t* p, int iPivot, int iPivot2, Vec_Wec_t* vLevels, Vec_Int_t* vNodes) {
    ...
    Vec_WecForEachLevel(vLevels, vLevel, k) {
        Gia_ManForEachObjVec(vLevel, p, pObj, i) if (vNodes == NULL) // it was a test run - unmark the node
...
int Gia_WinAddCiWithMaxFanouts(Gia_Man_t* p) {
    ...
    Gia_ManForEachCiId(p, Id, i) if (nMaxFan < Gia_ObjFanoutNumId(p, Id)) {
...
int Gia_WinNodeHasUnmarkedFanouts(Gia_Man_t* p, int iPivot) {
    int f, iFan;
    Gia_ObjForEachFanoutStaticId(p, iPivot, iFan, f) if (!Gia_ObjIsTravIdCurrentId(p, iFan)) return 1;
    return 0;
}
...
Vec_Int_t* Gia_RsbCiTranslate(Gia_Man_t* p, Vec_Int_t* vObjs, Vec_Int_t* vMap) {
    ...
    Vec_IntForEachEntry(vObjs, iObj, i) if (Gia_WinNodeHasUnmarkedFanouts(p, iObj))
...
int Gia_RsbSelectOneInput(Gia_Man_t* p, Vec_Wec_t* vLevels, Vec_Int_t* vIns) {
    ...
    Gia_Obj_t* pObj;
    Gia_ManForEachObjVec(vIns, p, pObj, i) if (Gia_ObjIsAnd(pObj)) {
...
Vec_Int_t* Gia_RsbFindOutputs(Gia_Man_t* p, Vec_Int_t* vWin, Vec_Int_t* vIns, Vec_Int_t* vRefs) {
    ...
    Gia_ManForEachObjVec(vIns, p, pObj, i)
        Gia_ObjSetTravIdCurrent(p, pObj);
    Gia_ManForEachObjVec(vWin, p, pObj, i) if (!Gia_ObjIsTravIdCurrent(p, pObj) && Gia_ObjIsAnd(pObj)) {
        Vec_IntAddToEntry(vRefs, Gia_ObjFaninId0p(p, pObj), 1);
        Vec_IntAddToEntry(vRefs, Gia_ObjFaninId1p(p, pObj), 1);
    }
    Gia_ManForEachObjVec(vWin, p, pObj, i) if (!Gia_ObjIsTravIdCurrent(p, pObj) && Gia_ObjFanoutNum(p, pObj) != Vec_IntEntry(vRefs, Gia_ObjId(p, pObj)))
...
Gia_Man_t* Gia_RsbDeriveGiaFromWindows(Gia_Man_t* p, Vec_Int_t* vWin, Vec_Int_t* vIns, Vec_Int_t* vOuts) {
    ...
    Gia_ManForEachObjVec(vIns, p, pObj, i)
        pObj->Value
        = Gia_ManAppendCi(pNew);
    Gia_ManForEachObjVec(vWin, p, pObj, i) if (!~pObj->Value)
        pObj->Value
