...
static inline int Of_CutHandle(int* pCutSet, int* pCut) {
    assert(pCut > pCutSet);
    return pCut - pCutSet;
}
static inline int* Of_CutFromHandle(int* pCutSet, int h) {
    assert(h > 0);
    return pCutSet + h;
}
...
static inline void Of_ObjUpdateRequired(Of_Man_t* p, int i, int x) {
    if (Of_ObjRequired(p, i) > x) Of_ObjSetRequired(p, i, x);
}
...
static inline int* Of_ObjCutBestP(Of_Man_t* p, int iObj) {
    assert(iObj > 0 && iObj < Gia_ManObjNum(p->pGia));
    return Of_ManCutSet(p, Of_ObjCutBest(p, iObj));
}
...
static inline int* Of_ObjCutBestP2(Of_Man_t* p, int iObj) {
    assert(iObj > 0 && iObj < Gia_ManObjNum(p->pGia));
    return Of_ManCutSet(p, Of_ObjCutBest2(p, iObj));
}
...
static inline int Of_ManComputeForwardCut(Of_Man_t* p, int iObj, int* pCut) {
    ...
    Of_CutForEachVar(pCut, iVar, k)
        Delay
        = Abc_MaxInt(Delay, Of_ObjDelay1(p, iVar) + DelayLut1);
    Of_CutSetDelay1(pCut, Delay);
    return Delay;
}
...
void Of_ManComputeForward1(Of_Man_t* p) {
    ...
    else Of_ManComputeForwardObj(p, i);
}

int Of_CutRef_rec(Of_Man_t* p, int* pCut) {
    int i, Var, Count = (p->Iter & 1) ? 1 : Of_CutArea(p, Of_CutSize(pCut));
    Of_CutForEachVar(pCut, Var, i) if (Of_ObjCutBest(p, Var) && !Of_ObjRefInc(p, Var))
        Count
        += Of_CutRef_rec(p, Of_ObjCutBestP(p, Var));
    return Count;
}
int Of_CutDeref_rec(Of_Man_t* p, int* pCut) {
    int i, Var, Count = (p->Iter & 1) ? 1 : Of_CutArea(p, Of_CutSize(pCut));
    Of_CutForEachVar(pCut, Var, i) if (Of_ObjCutBest(p, Var) && !Of_ObjRefDec(p, Var))
        Count
        += Of_CutDeref_rec(p, Of_ObjCutBestP(p, Var));
    return Count;
}
...
void Of_ManComputeForward2(Of_Man_t* p) {
    ...
    Gia_ManForEachAnd(p->pGia, pObj, i) if (Gia_ObjIsBuf(pObj))
        Of_ObjSetDelay1(p, i, Of_ObjDelay1(p, Gia_ObjFaninId0(pObj, i)));
    else Of_ManComputeForwardObj2(p, i);
}
...
static inline int Of_ManComputeOutputRequired(Of_Man_t* p, int fCleanRefs) {
    ...
    Gia_ManForEachCoDriverId(p->pGia, Id, i)
        Delay
        = Abc_MaxInt(Delay, Of_ObjDelay1(p, Id));
    Gia_ManForEachCoDriverId(p->pGia, Id, i) {
...
static inline int Of_ManComputeBackwardCut(Of_Man_t* p, int* pCut) {
    int k, iVar, Cost = 0;
    Of_CutForEachVar(pCut, iVar, k) if (!Of_ObjRefNum(p, iVar))
        Cost
        += Of_ObjFlow(p, iVar);
    return Cost;
}
...
void Of_ManComputeForwardDircon1(Of_Man_t* p) {
    ...
    else Of_ManComputeForwardDirconObj(p, i);
}
...
void Of_ManCreateSat(Of_Man_t* p, int nCutsAll, Vec_Int_t* vFirst, Vec_Int_t* vCutNum, Vec_Int_t* vBestNode, Vec_Int_t* vBestCut) {
    ...
    Gia_ManForEachCo(p->pGia, pObj, i)
        ...
        = 1;
    // set used nodes to 1
    Gia_ManForEachAnd(p->pGia, pObj, i) if (pObj->fMark0) {
...
Gia_Man_t* Of_ManDeriveMapping(Of_Man_t* p) {
    ...
    Gia_ManForEachAndId(p->pGia, i) {
        ...
        Of_CutForEachVarFlag(pCut, iVar, Flag, k) if (Flag) {
