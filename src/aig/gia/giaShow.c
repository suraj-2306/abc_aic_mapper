...
void Gia_WriteDotAigSimple(Gia_Man_t* p, char* pFileName, Vec_Int_t* vBold) {
    ...
    if (vBold)
        ...
        Gia_ManForEachObj(p, pNode, i) if (Gia_ObjIsXor(pNode) || Gia_ObjIsMux(p, pNode))
            pNode->fMark0
            = 1;
...
Vec_Int_t* Gia_ShowMapAdds(Gia_Man_t* p, Vec_Int_t* vAdds, int fFadds, Vec_Int_t* vBold) {
    ...
    Vec_Int_t* vMapAdds = Vec_IntStartFull(Gia_ManObjNum(p));
    int i, Entry;
    if (vBold)
...
Vec_Int_t* Gia_ShowMapXors(Gia_Man_t* p, Vec_Int_t* vXors) {
    Vec_Int_t* vMapXors = Vec_IntStartFull(Gia_ManObjNum(p));
    int i;
