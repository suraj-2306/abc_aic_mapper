...
Gia_Man_t* Gia_ManInsertWin(Gia_Man_t* p, Vec_Int_t* vOuts, Gia_Man_t* pWin) {
    ...
    Gia_ManForEachCi(pWin, pObj, i)
        pObj->Value
        = Gia_ManObj(p, Vec_IntEntry(vPis, i))->Value;
    // internal nodes
    ...
    Gia_ManForEachCo(pWin, pObj, i)
        Gia_ManObj(p, Vec_IntEntry(vPos, i))
            ->Value
        = Gia_ObjFanin0Copy(pObj);
    Gia_ManForEachAnd(p, pObj, i) if (!Gia_ObjIsTravIdCurrentId(p, i))
        pObj->Value
        = Gia_ManHashAnd(pNew, Gia_ObjFanin0Copy(pObj), Gia_ObjFanin1Copy(pObj));
...
Vec_Int_t* Gia_ManFindLatest(Gia_Man_t* p, int LevelMax, int nTimeWindow) {
    ...
    if (Gia_ManHasMapping(p)) {
        ...
        Gia_ManForEachCo(p, pObj, i) if (pLevels[Gia_ObjFaninId0p(p, pObj)] >= LevelMax)
            Vec_IntPush(vOuts, i);
        ABC_FREE(pLevels);
    } else {
        ...
        Gia_ManForEachCo(p, pObj, i) if (Gia_ObjLevel(p, pObj) >= LevelMax)
            Vec_IntPush(vOuts, i);
    }
    return vOuts;
}
