...
static inline int Pf_Mat2Int(Pf_Mat_t Mat) {
    union {
        int x;
        Pf_Mat_t y;
    } v;
...
static inline Pf_Mat_t Pf_Int2Mat(int Int) {
    union {
        int x;
        Pf_Mat_t y;
    } v;
...
static inline int Pf_CutHandle(int* pCutSet, int* pCut) {
    assert(pCut > pCutSet);
    return pCut - pCutSet;
}
static inline int* Pf_CutFromHandle(int* pCutSet, int h) {
    assert(h > 0);
    return pCutSet + h;
}
...
void Pf_StoCreateGateAdd(Pf_Man_t* pMan, word uTruth, int* pFans, int nFans, int CellId) {
    ...
    Vec_IntForEachEntryDouble(vArray, GateId, Entry, i) if (GateId == CellId && Pf_Int2Mat(Entry).Phase == Mat.Phase) break;
