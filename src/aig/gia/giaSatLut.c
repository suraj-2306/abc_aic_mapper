...
int Sbl_ManComputeDelay(Sbl_Man_t* p, int iLut, Vec_Int_t* vFanins) {
    ...
    Vec_IntForEachEntry(vFanins, iFan, k)
        Delay
        = Abc_MaxInt(Delay, Vec_IntEntry(p->vArrs, iFan) + 1);
    return Delay;
}
...
int Sbl_ManEvaluateMappingEdge(Sbl_Man_t* p, int DelayGlo) {
    ...
    Vec_WecForEachLevel(p->vWindow, vArray, i) if (Vec_IntSize(vArray) > 0)
...
int Sbl_ManCriticalFanin(Sbl_Man_t* p, int iLut, Vec_Int_t* vFanins) {
    ...
    Vec_IntForEachEntry(vFanins, iFan, k) if (Vec_IntEntry(p->vArrs, iFan) + 1 == Delay) return iFan;
    return -1;
}
...
static inline int Sbl_CutIsFeasible(word CutI1, word CutI2, word CutN1, word CutN2, int LutSize) {
    ...
    CutN1 &= CutN1 - 1;
    CutN2 &= CutN2 - 1;
    Count += (CutI1 != 0) + (CutI2 != 0) + (CutN1 != 0) + (CutN2 != 0);
    CutI1 &= CutI1 - 1;
    CutI2 &= CutI2 - 1;
    CutN1 &= CutN1 - 1;
    CutN2 &= CutN2 - 1;
...
static inline int Sbl_CutPushUncontained(Vec_Wrd_t* vCutsI1, Vec_Wrd_t* vCutsI2, Vec_Wrd_t* vCutsN1, Vec_Wrd_t* vCutsN2, word CutI1, word CutI2, word CutN1, word CutN2) {
    ...
    for (i = 0; i < vCutsI1->nSize; i++)
        if ((vCutsI1->pArray[i] & CutI1) == vCutsI1->pArray[i] && (vCutsI2->pArray[i] & CutI2) == vCutsI2->pArray[i] && (vCutsN1->pArray[i] & CutN1) == vCutsN1->pArray[i] && (vCutsN2->pArray[i] & CutN2) == vCutsN2->pArray[i])
            return 1;
    for (i = 0; i < vCutsI1->nSize; i++)
        if ((vCutsI1->pArray[i] & CutI1) != CutI1 || (vCutsI2->pArray[i] & CutI2) != CutI2 || (vCutsN1->pArray[i] & CutN1) != CutN1 || (vCutsN2->pArray[i] & CutN2) != CutN2) {
...
int Sbl_ManWindow2(Sbl_Man_t* p, int iPivot) {
    ...
    if (Count == 0)
        return 0;
    Vec_IntClear(p->vRoots);
    Vec_IntAppend(p->vRoots, vRoots);
    Vec_IntClear(p->vNodes);
    Vec_IntAppend(p->vNodes, vNodes);
    Vec_IntClear(p->vLeaves);
    Vec_IntAppend(p->vLeaves, vLeaves);
    Vec_IntClear(p->vAnds);
    Vec_IntAppend(p->vAnds, vAnds);
    //Vec_IntPrint( vRoots );
    //Vec_IntPrint( vAnds );
