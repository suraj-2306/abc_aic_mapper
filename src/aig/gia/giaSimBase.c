...
static inline void Gia_ManSimPatSimNot(Gia_Man_t* p, int i, Gia_Obj_t* pObj, int nWords, Vec_Wrd_t* vSims) {
    word* pSims = Vec_WrdArray(vSims) + nWords * i;
    int w;
    for (w = 0; w < nWords; w++)
        pSims[w] = ~pSims[w];
}
...
static inline void Gia_ManSimPatSimAnd3(Gia_Man_t* p, int i, Gia_Obj_t* pObj, int nWords, Vec_Wrd_t* vSims, Vec_Wrd_t* vSimsC) {
    ...
    word* pSimsC2 = Vec_WrdArray(vSimsC) + nWords * i;
    int w;
    if (Gia_ObjIsXor(pObj))
        for (w = 0; w < nWords; w++) {
            pSimsC0[w] |= pSimsC2[w];
...
void Gia_ManSimPatResim(Gia_Man_t* pGia, Vec_Int_t* vObjs, int nWords, Vec_Wrd_t* vSims) {
    Gia_Obj_t* pObj;
    int i;
    Gia_ManForEachObjVec(vObjs, pGia, pObj, i) if (i == 0)
...
int Gia_ManComputeTfos_rec(Gia_Man_t* p, int iObj, int iRoot, Vec_Int_t* vNode) {
    ...
    if (!Gia_ObjIsAnd(pObj))
        return 0;
    if (Gia_ManComputeTfos_rec(p, Gia_ObjFaninId0(pObj, iObj), iRoot, vNode) | Gia_ManComputeTfos_rec(p, Gia_ObjFaninId1(pObj, iObj), iRoot, vNode)) {
...
Vec_Wec_t* Gia_ManComputeTfos(Gia_Man_t* p) {
    ...
    Gia_ManForEachCiId(p, IdCi, i) {
        ...
        Vec_IntClear(vTemp);
        Gia_ManForEachCoId(p, IdCo, o) if (Gia_ManComputeTfos_rec(p, Gia_ObjFaninId0(Gia_ManObj(p, IdCo), IdCo), IdCi, vNode))
...
static inline void Gia_ManSimPatSimAnd2(Gia_Man_t* p, int i, Gia_Obj_t* pObj, int nWords, Vec_Wrd_t* vSims) {
    ...
    word* pSims3 = pSims + nWords * (2 * i + 1);
    int w;
...
Vec_Wrd_t* Gia_ManSimCombine(int nInputs, Vec_Wrd_t* vBase, Vec_Wrd_t* vAddOn, int nWordsUse) {
    ...
    int nWordsAddOn = Vec_WrdSize(vAddOn) / nInputs;
    int i, w;
...
Gia_SimRsbMan_t* Gia_SimRsbAlloc(Gia_Man_t* pGia) {
    ...
    p->pGia = pGia;
    p->nWords = Vec_WrdSize(pGia->vSimsPi) / Gia_ManCiNum(pGia);
    assert(Vec_WrdSize(pGia->vSimsPi) % Gia_ManCiNum(pGia) == 0);
...
void Gia_SimRsbTfo_rec(Gia_Man_t* p, int iObj, int iFanout, Vec_Int_t* vTfo) {
    ...
    if (Gia_ObjIsTravIdCurrentId(p, iObj))
        return;
    Gia_ObjSetTravIdCurrentId(p, iObj);
    Gia_ObjForEachFanoutStaticId(p, iObj, iFan, i) if (iFanout == -1 || iFan == iFanout)
...
word* Gia_SimRsbFunc(Gia_SimRsbMan_t* p, int iObj, Vec_Int_t* vFanins, int fOnSet) {
    ...
    word* pFanins[16] = {NULL};
    int s, b, iMint, i, iFanin;
    assert(Vec_IntSize(vFanins) <= 16);
    Vec_IntForEachEntry(vFanins, iFanin, i)
        pFanins[i]
        = Vec_WrdEntryP(p->vSimsObj, p->nWords * iFanin);
...
static inline void Gia_SimRsbSimAndCareSet(Gia_Man_t* p, int i, Gia_Obj_t* pObj, int nWords, Vec_Wrd_t* vSims, Vec_Wrd_t* vSims2) {
    ...
    word* pSims2 = Vec_WrdEntryP(vSims2, nWords * i);
    int w;
    if (Gia_ObjIsXor(pObj))
        for (w = 0; w < nWords; w++)
            pSims2[w] = (pSims0[w] ^ Diff0) ^ (pSims1[w] ^ Diff1);
...
word* Gia_SimRsbCareSet(Gia_SimRsbMan_t* p, int iObj, Vec_Int_t* vTfo) {
    ...
    word* pSims2 = Vec_WrdEntryP(p->vSimsObj2, p->nWords * iObj);
    int iNode, i;
...
void Gia_ObjSimCollect(Gia_SimRsbMan_t* p) {
    ...
    Vec_IntForEachEntry(p->vFanins, iTemp, i) {
        ...
        Gia_ObjForEachFanoutStaticId(p->pGia, iTemp, iFanout, k) if (Gia_ObjIsAnd(Gia_ManObj(p->pGia, iFanout)) && !Gia_ObjIsTravIdCurrentId(p->pGia, iFanout))
...
*/
    Vec_IntForEachEntry(vCands, iCand, i) {
        ...
        if (!Abc_TtIntersect(pDiv, p->pFunc[0], p->nWords, 0) && !Abc_TtIntersect(pDiv, p->pFunc[1], p->nWords, 1)) {
            (*pnBufs)++;
            if (fVerbose) printf("Level %3d : %d = buf(%d)\n", Gia_ObjLevelId(p->pGia, iObj), iObj, iCand);
            RetValue = 1;
        }
        if (!Abc_TtIntersect(pDiv, p->pFunc[0], p->nWords, 1) && !Abc_TtIntersect(pDiv, p->pFunc[1], p->nWords, 0)) {
            (*pnInvs)++;
            if (fVerbose) printf("Level %3d : %d = inv(%d)\n", Gia_ObjLevelId(p->pGia, iObj), iObj, iCand);
            RetValue = 1;
        }
    }
    return RetValue;
...
int Gia_ManSimRsb(Gia_Man_t* pGia, int nCands, int fVerbose) {
    ...
    Gia_ManForEachAnd(pGia, pObj, iObj)
        //if ( iObj == 6 )
        nCount
        += Gia_ObjSimRsb(p, iObj, nCands, fVerbose, &nBufs, &nInvs);
...
void Gia_ManSimRelAssignInputs(Gia_Man_t* p, int nWords, Vec_Wrd_t* vSims, int nWordsIn, Vec_Wrd_t* vSimsIn) {
    ...
    Gia_ManForEachCiId(p, Id, i) for (m = 0; m < nMints; m++)
...
Vec_Wrd_t* Gia_ManSimRel(Gia_Man_t* p, Vec_Int_t* vObjs, Vec_Wrd_t* vVals) {
    ...
    Vec_IntForEachEntry(vObjs, iObj, i) for (m = 0; m < nMints; m++) if ((m >> i) & 1)
        memset(Vec_WrdEntryP(vSims, iObj * nMints * nWords + nWords * m), 0xFF, sizeof(word) * nWords);
    else memset(Vec_WrdEntryP(vSims, iObj * nMints * nWords + nWords * m), 0x00, sizeof(word) * nWords);
    Gia_ManCleanPhase(p);
    Gia_ManForEachObjVec(vObjs, p, pObj, i)
        pObj->fPhase
        = 1;
    Gia_ManForEachAnd(p, pObj, i) if (!pObj->fPhase)
        Gia_ManSimPatSimAnd(p, i, pObj, nWords * nMints, vSims);
    Gia_ManForEachCo(p, pObj, i) if (!pObj->fPhase)
...
Vec_Int_t* Gia_Sim5CollectValues(word* pOffSet, word* pOnSet, int nWords) {
    Vec_Int_t* vBits = Vec_IntAlloc(64 * nWords);
    int i, Count[2] = {0};
    for (i = 0; i < 64 * nWords; i++)
...
int Gia_SimAbsRefine(Gia_SimAbsMan_t* p) {
    ...
    Vec_IntForEachEntry(p->vResub, iObj, b)
        pFanins[b]
        = Vec_WrdEntryP(p->vSims, p->nWords * iObj);
...
Vec_Int_t* Gia_SimAbsFind(Vec_Int_t* vValues, int Value) {
    Vec_Int_t* vSubset = Vec_IntAlloc(100);
    int i, Entry;
    Vec_IntForEachEntry(vValues, Entry, i) if (Entry == Value)
...
int Gia_RsbCost(Gia_RsbMan_t* p) {
    ...
    Vec_WecForEachLevelTwo(p->vSets[0], p->vSets[1], vLevel[0], vLevel[1], i)
        Cost
        += Vec_IntSize(vLevel[0]) * Vec_IntSize(vLevel[1]);
    return Cost;
}
void Gia_RsbPrint(Gia_RsbMan_t* p) {
    ...
    Vec_WecForEachLevelTwo(p->vSets[0], p->vSets[1], vLevel[0], vLevel[1], i) {
        for (n = 0; n < 2; n++) {
            printf("%5d : ", i);
            Extra_PrintBinary2(stdout, (unsigned*)&i, Vec_IntSize(p->vObjs));
            printf(" %d ", n);
...
int Gia_RsbCollectValid(Gia_RsbMan_t* p) {
    ...
    Vec_WecForEachLevelTwo(p->vSets[0], p->vSets[1], vLevel[0], vLevel[1], i) if (Vec_IntSize(vLevel[0]) && Vec_IntSize(vLevel[1]))
...
Vec_Int_t* Gia_SimCollectBest(Vec_Flt_t* vQuo) {
    ...
    Vec_FltForEachEntry(vQuo, Value, i) if (Value == ValueMax)
        Vec_IntPush(vRes, i);
    return vRes;
}
...
void Gia_ManUpdateCoPhase(Gia_Man_t* pNew, Gia_Man_t* pOld) {
    ...
    Gia_ManForEachCo(pNew, pObj, i) if (pObj->fPhase ^ Gia_ManCo(pOld, i)->fPhase) {
...
void Gia_ManCompareSims(Gia_Man_t* pHie, Gia_Man_t* pFlat, int nWords, int fVerbose) {
    ...
    printf("Hierarchical: ");
    Gia_ManPrintStats(pHie, NULL);
    printf("Flat:         ");
    Gia_ManPrintStats(pFlat, NULL);
...
Vec_Wrd_t* Gia_ManRelDerive(Gia_Man_t* p, Vec_Int_t* vObjs, Vec_Wrd_t* vSims) {
    ...
    Vec_Wrd_t* vCopy = Vec_WrdDup(vSims);
    Vec_Int_t* vLevel;
    ...
    Gia_ManForEachObjVec(vLevel, p, pObj, i) if (pObj->fPhase)
        Abc_TtClear(Vec_WrdEntryP(vCopy, Gia_ObjId(p, pObj) * nWords), nWords);
    else Gia_ManSimPatSimAnd(p, Gia_ObjId(p, pObj), pObj, nWords, vCopy);
    for (m = 0; m < nMints; m++) {
        Gia_ManForEachCo(p, pObj, i) {
