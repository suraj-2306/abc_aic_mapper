...
static inline void Emb_ManResetTravId(Emb_Man_t* p) {
    extern void Emb_ManCleanTravId(Emb_Man_t * p);
    Emb_ManCleanTravId(p);
    p->nTravIds = 1;
}
...
void Emb_ObjAddFanin(Emb_Obj_t* pObj, Emb_Obj_t* pFanin) {
    ...
    pFanin->Fanios[pFanin->nFanins + pFanin->iFanout++] = pObj->Fanios[pObj->iFanin++] = pObj->hHandle - pFanin->hHandle;
}
...
void Emb_ManCreateRefsSpecial(Gia_Man_t* p) {
    ...
    Gia_ManForEachAnd(p, pObj, i) {
        ...
        if (Gia_ObjIsCi(pFan0) || pFan0->fMark0 || Gia_ObjIsCi(pFan1) || pFan1->fMark0)
            continue;
...
void Emb_ManComputeDimensions(Emb_Man_t* p, int nDims) {
    ...
    if (Vec_IntSize(vComps) > 1) {
        ...
        Emb_ManForEachObjVec(vComps, p, pFanin, i) {
            ...
            pFanin->Fanios[pFanin->nFanins + pFanin->nFanouts - 1] = pObj->Fanios[i] = pObj->hHandle - pFanin->hHandle;
        }
    }
    Vec_IntFree(vComps);
