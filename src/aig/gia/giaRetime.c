...
void Gia_ManMarkAutonomous(Gia_Man_t* p) {
    ...
    Gia_ManIncrementTravId(p);
    Gia_ManForEachRo(p, pObj, i) if (pObj->fMark0)
        Gia_ObjSetTravIdCurrent(p, pObj);
    Gia_ManCleanMark0(p);
}
...
Gia_Man_t* Gia_ManRetimeDupForward(Gia_Man_t* p, Vec_Ptr_t* vCut) {
    ...
    Vec_PtrForEachEntry(Gia_Obj_t*, vCut, pObj, i)
        pObj->Value
        = Abc_LitNotCond(Gia_ManAppendCi(pNew), pObj->fPhase);
    ...
    Vec_PtrForEachEntry(Gia_Obj_t*, vCut, pObj, i) if (Gia_ObjIsAnd(pObj))
        pObj->Value
        = ~0;
...
Gia_Man_t* Gia_ManRetimeForwardOne(Gia_Man_t* p, int* pnRegFixed, int* pnRegMoves) {
    ...
    Gia_ManIncrementTravId(p);
    *pnRegFixed = 0;
    Gia_ManForEachRo(p, pObj, i) if (Gia_ObjIsTravIdPrevious(p, pObj))
        Gia_ObjSetTravIdCurrent(p, pObj);
    else(*pnRegFixed)++;
    // mark all the nodes that can be retimed forward
    *pnRegMoves = 0;
    Gia_ManForEachAnd(p, pObj, i) if (Gia_ObjIsTravIdCurrent(p, Gia_ObjFanin0(pObj)) && Gia_ObjIsTravIdCurrent(p, Gia_ObjFanin1(pObj))) {
