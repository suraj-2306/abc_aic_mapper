...
static inline void Cbs0_VarAssign(Gia_Obj_t* pVar) {
    assert(!pVar->fMark0);
    pVar->fMark0 = 1;
}
static inline void Cbs0_VarUnassign(Gia_Obj_t* pVar) {
    assert(pVar->fMark0);
    pVar->fMark0 = 0;
    pVar->fMark1 = 0;
}
static inline int Cbs0_VarValue(Gia_Obj_t* pVar) {
    assert(pVar->fMark0);
    return pVar->fMark1;
}
static inline void Cbs0_VarSetValue(Gia_Obj_t* pVar, int v) {
    assert(pVar->fMark0);
    pVar->fMark1 = v;
}
...
static inline void Cbs0_ManSaveModel(Cbs0_Man_t* p, Vec_Int_t* vCex) {
    ...
    Vec_IntClear(vCex);
    p->pProp.iHead = 0;
    Cbs0_QueForEachEntry(p->pProp, pVar, i) if (Gia_ObjIsCi(pVar))
...
static inline int Cbs0_QueHasNode(Cbs0_Que_t* p, Gia_Obj_t* pObj) {
    Gia_Obj_t* pTemp;
    int i;
    Cbs0_QueForEachEntry(*p, pTemp, i) if (pTemp == pObj) return 1;
    return 0;
}
...
static inline Gia_Obj_t* Cbs0_ManDecideHighest(Cbs0_Man_t* p) {
    ...
    int i;
    Cbs0_QueForEachEntry(p->pJust, pObj, i) if (pObjMax == NULL || pObjMax < pObj)
        pObjMax
        = pObj;
    return pObjMax;
}
...
static inline Gia_Obj_t* Cbs0_ManDecideLowest(Cbs0_Man_t* p) {
    ...
    int i;
    Cbs0_QueForEachEntry(p->pJust, pObj, i) if (pObjMin == NULL || pObjMin > pObj)
        pObjMin
        = pObj;
    return pObjMin;
}
...
static inline int Cbs0_ManPropagateOne(Cbs0_Man_t* p, Gia_Obj_t* pVar) {
    ...
    if (Cbs0_VarValue(pVar)) {          // value is 1
