...
Gia_Man_t* Gia_ManMuxRestructure(Gia_Man_t* p) {
    ...
    Gia_ManForEachObj1(p, pObj, i) {
        if (Gia_ObjIsCi(pObj))
            ...
            pObj->Value = Gia_ManAppendBuf(pNew, Gia_ObjFanin0Copy(pObj));
        else if (Gia_ObjIsMuxId(p, i) && Gia_ObjIsMuxId(p, Gia_ObjFaninId0(pObj, i)) && !Vec_BitEntry(vUsed, Gia_ObjFaninId0(pObj, i)) && Gia_ObjIsMuxId(p, Gia_ObjFaninId1(pObj, i)) && !Vec_BitEntry(vUsed, Gia_ObjFaninId1(pObj, i)) && Gia_ObjFaninId2(p, Gia_ObjFaninId0(pObj, i)) == Gia_ObjFaninId2(p, Gia_ObjFaninId1(pObj, i))) {
...
Gia_Man_t* Gia_ManDupMuxRestructure(Gia_Man_t* p) {
    ...
    pNew = Gia_ManMuxRestructure(pTemp = pNew);
    Gia_ManStop(pTemp);
...
int Gia_MuxRef_rec(Gia_Man_t* p, int iObj) {
    ...
    if (Gia_ObjRefInc(p, pObj))
        return 0;
    return Gia_MuxRef_rec(p, Gia_ObjFaninId0p(p, pObj)) + Gia_MuxRef_rec(p, Gia_ObjFaninId1p(p, pObj)) + Gia_MuxRef_rec(p, Gia_ObjFaninId2p(p, pObj)) + 1;
}
...
int Gia_MuxDeref_rec(Gia_Man_t* p, int iObj) {
    ...
    if (Gia_ObjRefDec(p, pObj))
        return 0;
    return Gia_MuxDeref_rec(p, Gia_ObjFaninId0p(p, pObj)) + Gia_MuxDeref_rec(p, Gia_ObjFaninId1p(p, pObj)) + Gia_MuxDeref_rec(p, Gia_ObjFaninId2p(p, pObj)) + 1;
}
...
Vec_Int_t* Gia_ManFindSharedInputs(Gia_Man_t* p) {
    ...
    Gia_ManForEachCo(p, pObj, i) {
        ...
        Gia_ManMarkTfi_rec(p, Gia_ObjFanin0(pObj));
        Gia_ManForEachCi(p, pObj2, k) if (Gia_ObjIsTravIdCurrent(p, pObj2))
            Vec_IntAddToEntry(vRes, k, 1);
    }
    k = 0;
    Vec_IntForEachEntry(vRes, Value, i) if (Value == Gia_ManCoNum(p))
...
Vec_Int_t* Gia_ManFindEquivClasses(Vec_Wec_t* vCofs) {
    ...
    Vec_WecForEachLevel(vCofs, vVec, i) {
        Vec_IntForEachEntry(vUnique, Lev, k) if (Vec_IntEqual(vVec, Vec_WecEntry(vCofs, Lev))) break;
        Vec_IntPush(vMap, k);
...
void Gia_ManFindDerive(Gia_Man_t* pNew, int nOuts, Vec_Int_t* vIns, Vec_Wec_t* vCofs, Vec_Int_t* vMap) {
    ...
    for (i = 0; i < nBits; i++) {
        ...
        Vec_IntForEachEntry(vMap, Value, k) if ((Value >> i) & 1)
    ...
    for (i = 0; i < nOuts; i++) {
        Vec_Int_t* vLevel;
        Vec_IntClear(vData);
        Vec_WecForEachLevel(vCofs, vLevel, k) if (Vec_IntEntry(vUsed, k))
