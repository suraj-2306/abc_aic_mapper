...
static inline int Gia_FanoutCreate(int FanId, int Num) {
    assert(Num < 2);
    return (FanId << 1) | Num;
}
...
static inline int Gia_ObjFanout0Int(Gia_Man_t* p, int ObjId) {
    assert(ObjId < p->nFansAlloc);
    return p->pFanData[5 * ObjId];
}
static inline int Gia_ObjFanoutNext(Gia_Man_t* p, int iFan) {
...
#define Gia_ObjForEachFanout(p, pObj, pFanout, iFan, i) \
    for (assert(p->pFanData), i = 0; (i < (int)(pObj)->nRefs) && (((iFan) = i ? Gia_ObjFanoutNext(p, iFan) : Gia_ObjFanout0Int(p, Gia_ObjId(p, pObj))), 1) && (((pFanout) = Gia_ManObj(p, iFan >> 1)), 1); i++)
