...
static inline Str_Obj_t* Str_NtkObj(Str_Ntk_t* p, int i) {
    assert(i < p->nObjs);
    return p->pObjs + i;
}
...
static inline int Str_ObjCreate(Str_Ntk_t* p, int Type, int nFanins, int* pFanins) {
    Str_Obj_t* pObj = p->pObjs + p->nObjs;
    int i;
...
static inline void Str_NtkPrintGroups(Str_Ntk_t* p) {
    Str_Obj_t* pObj;
    int nGroups, nMuxes;
    Str_NtkManForEachObj(p, pObj) if (pObj->Type == STR_MUX && pObj->iTop > 0) {
...
Vec_Int_t* Str_ManCreateRoots(Vec_Wec_t* vGroups, int nObjs) { // map tree MUXes into their classes
    Vec_Int_t* vRoots;
    Vec_Int_t* vGroup;
    int i, k, Entry;
...
void Str_ManCheckOverlap(Gia_Man_t* p, Vec_Wec_t* vGroups) { // check that members of each group are not in the TFI of each other
    ...
    Vec_WecForEachLevel(vGroups, vGroup, i)
        ...
        Vec_IntForEachEntry(vGroup, iObj2, n) if (iObj != iObj2 && Gia_ObjIsTravIdCurrentId(p, iObj2)) break;
...
static inline void Gia_ManSuperCollectXor_rec(Gia_Man_t* p, Gia_Obj_t* pObj) {
    ...
    if (!Gia_ObjIsXor(pObj) || Gia_ObjRefNum(p, pObj) > 1 ||
...
static inline void Gia_ManSuperCollectAnd_rec(Gia_Man_t* p, Gia_Obj_t* pObj) {
    if (Gia_IsComplement(pObj) || !Gia_ObjIsAndReal(p, pObj) || Gia_ObjRefNum(p, pObj) > 1 ||
        //        Gia_ObjRefNum(p, pObj) > 3 ||
        //        (Gia_ObjRefNum(p, pObj) == 2 && (Gia_ObjRefNum(p, Gia_ObjFanin0(pObj)) == 1 || Gia_ObjRefNum(p, Gia_ObjFanin1(pObj)) == 1)) ||
        Vec_IntSize(p->vSuper) > STR_SUPER) {
        Vec_IntPush(p->vSuper, Gia_ObjToLit(p, pObj));
...
void Str_ManNormalize_rec(Str_Ntk_t* pNtk, Gia_Man_t* p, Gia_Obj_t* pObj, Vec_Wec_t* vGroups, Vec_Int_t* vRoots) {
    ...
    if (Gia_ObjIsMux(p, pObj)) {
        ...
        Gia_ManForEachObjVec(vGroup, p, pRoot, i) {
            ...
            // set mapping
            Gia_ManForEachObjVec(p->vSuper, p, pMux, k)
                Str_NtkObj(pNtk, Abc_Lit2Var(pMux->Value))
                    ->iTop
                = Abc_Lit2Var(pRoot->Value);
            pNtk->nTrees++;
        }
        assert(~pObj->Value);
        // set mapping
        ...
        Gia_ManForEachObjVec(vGroup, p, pRoot, i)
            Str_NtkObj(pNtk, Abc_Lit2Var(pRoot->Value))
                ->iTop
            = Abc_Lit2Var(pObj->Value);
        pNtk->nGroups++;
...
void Str_NtkBalanceMulti(Gia_Man_t* pNew, Str_Ntk_t* p, Str_Obj_t* pObj, Vec_Int_t* vDelay, int nLutSize) {
    ...
    while (Vec_IntSize(vSuper) > 2) {
        ...
        iBest = iBest2 = -1;
        CostBest = CostBest2 = -1, OccurBest = OccurBest2 = -1;
        for (i = 0; i < iStop; i++)
            for (k = i + 1; k < iStop; k++) {
