...
void Gia_ManTerAnalyze2(Vec_Ptr_t* vStates, int nRegs) {
    ...
    Vec_PtrForEachEntry(unsigned*, vStates, pTemp, i) for (w = 0; w < nStateWords; w++)
        pStates[w]
        |= pTemp[w];
...
int Gia_ManFindEqualFlop(Vec_Ptr_t* vFlops, int iFlop, int nFlopWords) {
    ...
    Vec_PtrForEachEntryStop(unsigned*, vFlops, pTemp, i, iFlop) if (!memcmp(pTemp, pFlop, sizeof(unsigned) * nFlopWords)) return i;
    return -1;
}
