...
int Gia_ManEvalEdgeDelay(Gia_Man_t* p) {
    ...
    Gia_ManForEachCoDriverId(p, iLut, k)
        DelayMax
        = Abc_MaxInt(DelayMax, Vec_IntEntry(p->vEdgeDelay, iLut));
    return DelayMax;
}
...
int Edg_ManAssignEdgeNew(Gia_Man_t* p, int nEdges, int fVerbose) {
    ...
    for (i = 0; i < 10000; i++) {
        ...
        if (DelayPrev > DelayD) {
            ...
            Vec_IntFreeP(&vEdge1);
            vEdge1 = Vec_IntDup(p->vEdge1);
            Vec_IntFreeP(&vEdge2);
            vEdge2 = Vec_IntDup(p->vEdge2);
            DelayPrev = DelayD;
            iLast = i;
        }
        if (fVerbose)
        ...
        Vec_WecForEachLevel(vEdges, vLevel, k) {
            Vec_IntForEachEntry(vLevel, iObj, j) if (Edg_ObjImprove(p, iObj, nEdges, DelayD, fVerbose)) // improved
                break;
