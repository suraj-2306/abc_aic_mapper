...
int Gia_ManQuantVerify_rec(Gia_Man_t* p, int iObj, int CiId) {
    ...
    return Gia_ManQuantVerify_rec(p, Gia_ObjFaninId0(pObj, iObj), CiId) || Gia_ManQuantVerify_rec(p, Gia_ObjFaninId1(pObj, iObj), CiId);
}
void Gia_ManQuantVerify(Gia_Man_t* p, int iObj) {
    word* pInfo = Gia_ManQuantInfoId(p, iObj);
    int i, CiId;
...
void Gia_ManQuantSetSuppCi(Gia_Man_t* p, Gia_Obj_t* pObj) {
    ...
    if (p->iSuppPi == 64 * p->nSuppWords) {
        word Data;
        int w, Count = 0, Size = Vec_WrdSize(p->vSuppWords);
...
void Gia_ManQuantMarkUsedCis(Gia_Man_t* p, int (*pFuncCiToKeep)(void*, int), void* pData) {
    ...
    Vec_IntForEachEntry(&p->vSuppVars, CiId, i) if (!pFuncCiToKeep(pData, CiId)) // quant var
