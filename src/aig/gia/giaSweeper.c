...
static inline int Swp_ManLit2Lit(Swp_Man_t* p, int Lit) {
    assert(Vec_IntEntry(p->vId2Lit, Abc_Lit2Var(Lit)));
    return Abc_Lit2LitL(Vec_IntArray(p->vId2Lit), Lit);
}
static inline void Swp_ManSetObj2Lit(Swp_Man_t* p, int Id, int Lit) {
    assert(Lit > 0);
    Vec_IntSetEntry(p->vId2Lit, Id, Lit);
}
...
Vec_Int_t* Gia_SweeperGraft(Gia_Man_t* pDst, Vec_Int_t* vProbes, Gia_Man_t* pSrc) {
    ...
    Gia_ManForEachPi(pSrc, pObj, i)
        pObj->Value
        = vProbes ? Gia_SweeperProbeLit(pDst, Vec_IntEntry(vProbes, i)) : Gia_Obj2Lit(pDst, Gia_ManPi(pDst, i));
    Gia_ManForEachAnd(pSrc, pObj, i)
        pObj->Value
        = Gia_ManHashAnd(pDst, Gia_ObjFanin0Copy(pObj), Gia_ObjFanin1Copy(pObj));
...
Gia_Man_t* Gia_SweeperFraigTest(Gia_Man_t* pInit, int nWords, int nConfs, int fVerbose) {
    ...
    Gia_ManForEachPo(p, pObj, i) if (i < Gia_ManPoNum(p) - p->nConstrs) // this is the user's output
