...
void Gia_ManOrigIdsRemap(Gia_Man_t* p, Gia_Man_t* pNew) {
    ...
    Gia_ManForEachObj1(p, pObj, i) if (~pObj->Value && Abc_Lit2Var(pObj->Value) && Vec_IntEntry(p->vIdsOrig, i) != -1 && Vec_IntEntry(pNew->vIdsOrig, Abc_Lit2Var(pObj->Value)) == -1)
...
Vec_Int_t* Gia_ManOrigIdsRemapPairs(Vec_Int_t* vEquivPairs, int nObjs) {
    ...
    Vec_IntForEachEntry(vMap2Smaller, One, i) if (One >= 0)
...
int Gia_ManCheckTopoOrder(Gia_Man_t* p) {
    ...
    Gia_ManForEachCo(p, pObj, i)
        RetValue
        &= Gia_ManCheckTopoOrder_rec(p, Gia_ObjFanin0(pObj));
    return RetValue;
}
...
Gia_Man_t* Gia_ManEquivReduce(Gia_Man_t* p, int fUseAll, int fDualOut, int fSkipPhase, int fVerbose) {
    ...
    Gia_ManForEachObj(p, pObj, i) if (Gia_ObjReprObj(p, i) != NULL) break;
...
int Gia_ManEquivSetColors(Gia_Man_t* p, int fVerbose) {
    ...
    Gia_ManForEachPo(p, pObj, i)
        nNodes[i & 1]
        += Gia_ManEquivSetColor_rec(p, Gia_ObjFanin0(pObj), i & 1);
    //    Gia_ManForEachObj( p, pObj, i )
...
Gia_Man_t* Gia_ManSpecReduce(Gia_Man_t* p, int fDualOut, int fSynthesis, int fSpeculate, int fSkipSome, int fVerbose) {
    ...
    if (fSkipSome) {
        ...
        int iLit, nAddPos = 0;
        Vec_IntForEachEntry(vGuide, iLit, i) if (iLit)
            nAddPos++;
        if (nAddPos)
...
Gia_Man_t* Gia_ManSpecReduceInitFrames(Gia_Man_t* p, Abc_Cex_t* pInit, int nFramesMax, int* pnFrames, int fDualOut, int nMinOutputs) {
    ...
    for (f = 1;; f++) {
        ...
        if ((nMinOutputs == 0 && Gia_ManPoNum(pFrames) >= nLits / 2 + 1) || (nMinOutputs != 0 && Gia_ManPoNum(pFrames) >= nMinOutputs))
            break;
        if (f == nFramesMax)
            break;
...
void Gia_ManEquivTransform(Gia_Man_t* p, int fVerbose) {
    ...
    Gia_ManForEachObj(p, pObj, i) if (Gia_ObjIsCi(pObj) || Gia_ObjIsAnd(pObj))
        assert(Gia_ObjColors(p, i));
...
void Gia_ManEquivMark(Gia_Man_t* p, char* pFileName, int fSkipSome, int fVerbose) {
    ...
    if (fSkipSome) {
        ...
        // count the number of non-zero entries
        nAddPos = 0;
        Vec_IntForEachEntry(vTrace, iLit, i) if (iLit)
            nAddPos++;
        // check the number
...
*/
        ...
        Vec_IntForEachEntry(vClass, iNode, k) if (iNode != iRepr)
...
Gia_Man_t* Gia_ManEquivToChoices(Gia_Man_t* p, int nSnapshots) {
    ...
    Gia_ManForEachCi(p, pObj, i)
        pObj->Value
        = Gia_ManAppendCi(pNew);
    Gia_ManForEachRo(p, pObj, i) if ((pRepr = Gia_ObjReprObj(p, Gia_ObjId(p, pObj)))) {
    ...
    Vec_IntFree(vNodes);
    Gia_ManForEachCo(p, pObj, i) if (i % nSnapshots == 0)
        Gia_ManAppendCo(pNew, Gia_ObjFanin0Copy(pObj));
    Gia_ManHashStop(pNew);
    Gia_ManSetRegNum(pNew, Gia_ManRegNum(p));
...
int Cec4_ManMarkIndependentClasses_rec(Gia_Man_t* p, int iObj) {
    ...
    if (Gia_ObjIsCi(pObj))
        return 1;
    assert(Gia_ObjIsAnd(pObj));
    if (Cec4_ManMarkIndependentClasses_rec(p, Gia_ObjFaninId0(pObj, iObj)) && Cec4_ManMarkIndependentClasses_rec(p, Gia_ObjFaninId1(pObj, iObj)))
        return 1;
...
int Cec4_ManSatSolverChoices_rec(Gia_Man_t* pCho, Gia_Man_t* p, Gia_Man_t* pNew, int iObj) {
    if (!Gia_ObjIsClass(p, iObj))
        ...
        Gia_ClassForEachObj(p, iRepr, iObj) if (p->pReprs[iObj].fColorA)
...
Gia_Man_t* Cec4_ManSatSolverChoices(Gia_Man_t* p, Gia_Man_t* pNew) {
    ...
    Gia_ManForEachCo(pNew, pObj, i)
        pObj->Value
        = Gia_ManAppendCo(pCho, Gia_ObjFanin0Copy(pObj));
