...
int Gia_ManConeMark(Gia_Man_t* p, int iOut, int Limit) {
    ...
    Gia_ManForEachObjVec(vRoots, p, pObj, i) if (Gia_ManConeMark_rec(p, pObj, vRoots, Limit)) break;
...
*/
    // count references
    Gia_ManCreateRefs(p);
    vCount = Vec_IntAllocArray(p->pRefs, Gia_ManObjNum(p));
    p->pRefs = NULL;
