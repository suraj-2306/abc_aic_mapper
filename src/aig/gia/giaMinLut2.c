...
int Gia_ManFindOrAddNode(Tree_Sto_t* pSto, int iVar, int Truth, word* pCof) {
    ...
    if (iVar > 5) {
        ...
        Vec_IntForEachEntry(pSto->vCofs + iVar, Obj, k) if (Abc_TtEqual(pSto->pMem + Obj, pCof, nWords)) return 1;
        Vec_IntPush(pSto->vCofs + iVar, pCof - pSto->pMem);
    } else {
        Vec_IntForEachEntry(pSto->vCofs + iVar, Obj, k) if (Obj == Truth) return 1;
...
static inline word Abc_Tt6Min_rec(word uF, word uR, int nVars, Vec_Wrd_t* vNodes) {
    word uF0, uF1, uR0, uR1, uRes0, uRes1, uRes2;
    int i, Var;
    assert(nVars <= 6);
    assert((uF & uR) == 0);
    if (!uF && !uR)
    ...
    assert(Var >= 0);
    if (1 && vNodes)
        Vec_WrdForEachEntry(vNodes, uRes2, i) if (!(uF & ~uRes2) && !(uRes2 & uR)) return uRes2;
...
static inline word Abc_TtSimple6Min_rec(Gia_Man_t* p, word uF, word uC, int nVars, Vec_Wrd_t* vNodes, int* piLit, int* pPerm) {
    word uF0, uF1, uC0, uC1, uRes0, uRes1, uRes2;
    int i, Var, iLit0, iLit1;
    word uFC = uF & uC;
    word uRC = ~uF & uC;
    assert(nVars <= 6);
    *piLit = 0;
    ...
    if (1 && vNodes) {
        int iLit;
        Vec_WrdForEachEntryDouble(vNodes, uRes2, iLit, i) if (!((uF ^ uRes2) & uC)) {
            *piLit = (unsigned)iLit;
            return uRes2;
        }
        else if (!((uF ^ ~uRes2) & uC)) {
...
static inline word Abc_TtGia6Min_rec(Gia_Man_t* p, word uF, word uR, int nVars, Vec_Wrd_t* vNodes, int* piLit, int* pPerm) {
    ...
    *piLit = 0;
    ...
    if (1 && vNodes) {
        int iLit;
        Vec_WrdForEachEntryDouble(vNodes, uRes2, iLit, i) if (!(uF & ~uRes2) && !(uRes2 & uR)) {
            *piLit = (unsigned)iLit;
            return uRes2;
        }
        else if (!(uF & uRes2) && !(~uRes2 & uR)) {
