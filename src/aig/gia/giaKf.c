...
static inline int* Kf_ObjCutBest(Kf_Man_t* p, int i) {
...
static inline void Kf_SetSelectBest(Kf_Set_t* p, int fArea, int fSort) {
    ...
    for (i = 0; i <= p->nLutSize; i++)
        Kf_ListForEachCut(p, i, pCut)
            nCuts
            = Kf_SetStoreAddOne(p, nCuts, p->nCutNum - 1, pCut, fArea);
...
static inline int Kf_SetRemoveDuplicates(Kf_Set_t* p, int nLeaves, word Sign) {
    Kf_Cut_t* pCut;
    Kf_ListForEachCut(p, nLeaves, pCut) if (pCut->Sign == Sign && Kf_SetCutDominatedByThis(p, pCut)) return 1;
    return 0;
}
    ...
    for (i = 0; i <= p->nLutSize; i++)
        Kf_ListForEachCutP(p, i, pCut0, pPlace) {
            ...
            for (k = 0; k < pCut0->nLeaves; k++)
                Kf_ListForEachCut(p, k, pCut1) if ((pCut0->Sign & pCut1->Sign) == pCut1->Sign && Kf_SetCutDominatedByThis(p, pCut1)) {
                    k = pCut0->nLeaves;
                    p->nCuts--;
                    break;
                }
...
static inline int Kf_SetRemoveDuplicatesSimple(Kf_Set_t* p, Kf_Cut_t* pCutNew) {
    Kf_Cut_t* pCut;
    Kf_ListForEachCut(p, pCutNew->nLeaves, pCut) if (pCut->Sign == pCutNew->Sign && Kf_SetCutIsContainedSimple(pCut, pCutNew)) return 1;
    return 0;
}
static inline void Kf_SetFilterSimple(Kf_Set_t* p) {
    ...
    for (i = 0; i <= p->nLutSize; i++)
        Kf_ListForEachCutP(p, i, pCut0, pPlace) {
            for (k = 0; k < pCut0->nLeaves; k++)
                Kf_ListForEachCut(p, k, pCut1) if ((pCut0->Sign & pCut1->Sign) == pCut1->Sign && Kf_SetCutIsContainedSimple(pCut0, pCut1)) {
                    k = pCut0->nLeaves;
                    p->nCuts--;
                    break;
                }
...
static inline int Kf_SetRemoveDuplicatesOrder(Kf_Set_t* p, Kf_Cut_t* pCutNew) {
    Kf_Cut_t* pCut;
    Kf_ListForEachCut(p, pCutNew->nLeaves, pCut) if (pCut->Sign == pCutNew->Sign && Kf_SetCutIsContainedOrder(pCut, pCutNew)) return 1;
    return 0;
}
static inline void Kf_SetFilterOrder(Kf_Set_t* p) {
    ...
    for (i = 0; i <= p->nLutSize; i++)
        Kf_ListForEachCutP(p, i, pCut0, pPlace) {
            for (k = 0; k < pCut0->nLeaves; k++)
                Kf_ListForEachCut(p, k, pCut1) if ((pCut0->Sign & pCut1->Sign) == pCut1->Sign && Kf_SetCutIsContainedOrder(pCut0, pCut1)) {
                    k = pCut0->nLeaves;
                    p->nCuts--;
                    break;
                }
...
static inline int Kf_CutLeaf(int* pCut, int i) {
    assert(i);
...
int Kf_ManComputeDelay(Kf_Man_t* p, int fEval) {
    ...
    int i, Delay = 0;
    if (fEval) {
        Gia_ManForEachAnd(p->pGia, pObj, i) if (Gia_ObjRefNum(p->pGia, pObj) > 0)
...
void Kf_ManComputeCuts(Kf_Man_t* p) {
    ...
    Gia_ManForEachObjReverse(p->pGia, pObj, k) if (Gia_ObjIsAnd(pObj) && Vec_IntEntry(vFanins, k) == 0)
    ...
    for (i = 0; i < nProcs; i++) {
        ...
        ThData[i].Status = 0;
        ThData[i].clkUsed = 0;
        status = pthread_create(WorkerThread + i, NULL, Kf_WorkerThread, (void*)(ThData + i));
        assert(status == 0);
    }
    ...
    Vec_IntForEachEntry(vFanins, iFan, k) if (iFan != 0) {
        printf("%d -> %d    ", k, iFan);
