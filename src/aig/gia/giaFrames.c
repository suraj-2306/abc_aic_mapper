...
void Gia_ManFraSupports(Gia_ManFra_t* p) {
    ...
    for (f = p->pPars->nFrames - 1; f >= 0; f--) {
        ...
        if (vIns)
            Gia_ManForEachObjVec(vIns, p->pAig, pObj, i) if (Gia_ObjIsRo(p->pAig, pObj))
...
Gia_Man_t* Gia_ManFramesInit(Gia_Man_t* pAig, Gia_ParFra_t* pPars) {
    ...
    pFrames = Gia_ManStart(Vec_VecSizeSize((Vec_Vec_t*)p->vIns) + Vec_VecSizeSize((Vec_Vec_t*)p->vAnds) + Vec_VecSizeSize((Vec_Vec_t*)p->vOuts));
...
Gia_Man_t* Gia_ManFrames(Gia_Man_t* pAig, Gia_ParFra_t* pPars) {
    ...
    for (f = 0; f < pPars->nFrames; f++) {
        if (f == 0) {
            ...
            Gia_ManForEachRo(pAig, pObj, i)
                pObj->Value
                = Gia_ObjRoToRi(pAig, pObj)->Value;
        }
        Gia_ManForEachPi(pAig, pObj, i)
            pObj->Value
            = Gia_Obj2Lit(pFrames, Gia_ManPi(pFrames, f * Gia_ManPiNum(pAig) + i));
        if (!pPars->fDisableSt)
            Gia_ManForEachAnd(pAig, pObj, i)
                pObj->Value
                = Gia_ManHashAnd(pFrames, Gia_ObjFanin0Copy(pObj), Gia_ObjFanin1Copy(pObj));
        else
            Gia_ManForEachAnd(pAig, pObj, i)
                pObj->Value
                = Gia_ManAppendAnd2(pFrames, Gia_ObjFanin0Copy(pObj), Gia_ObjFanin1Copy(pObj));
        if (vPoLits) {
            if (!pPars->fDisableSt)
        ...
        if (f == pPars->nFrames - 1) {
            if (vPoLits)
                Gia_ManForEachPo(pAig, pObj, i)
                    pObj->Value
                    = Gia_ManAppendCo(pFrames, Vec_IntEntry(vPoLits, i));
...
Gia_Man_t* Gia_ManFramesInitSpecial(Gia_Man_t* pAig, int nFrames, int fVerbose) {
    ...
    for (f = 0; nFrames == 0 || f < nFrames; f++) {
        ...
        Gia_ManForEachRo(pAig, pObj, i)
            pObj->Value
            = f ? Gia_ObjRoToRi(pAig, pObj)->Value : 0;
        Gia_ManForEachPi(pAig, pObj, i)
            pObj->Value
            = Gia_ManAppendCi(pFrames);
        Gia_ManForEachAnd(pAig, pObj, i)
            pObj->Value
            = Gia_ManHashAnd(pFrames, Gia_ObjFanin0Copy(pObj), Gia_ObjFanin1Copy(pObj));
        Gia_ManForEachPo(pAig, pObj, i) if (Gia_ObjFanin0Copy(pObj) != 0) break;
