...
Cnf_Dat_t* Mf_ManDeriveCnf(Mf_Man_t* p, int fCnfObjIds, int fAddOrCla) {
    ...
    if (fCnfObjIds) {
        ...
        Gia_ManForEachAndReverseId(p->pGia, Id) if (Mf_ObjMapRefNum(p, Id))
            Vec_IntWriteEntry(vCnfIds, Id, Id),
            iVar++;
    ...
    if (fAddOrCla) {
        ...
        Gia_ManForEachCoId(p->pGia, Id, i)
            pCnf->pClauses[0][iLit++]
            = Abc_Var2Lit(pCnfIds[Id], 0);
    }
    ...
    if (fCnfObjIds) {
        ...
        if (p->pGia != p->pGia0) // diff managers - create map for CIs/COs
        {
            ...
            Gia_ManForEachCiId(p->pGia0, Id, i)
                pCnf->pVarNums[Id]
                = pCnfIds[Gia_ManCiIdToId(p->pGia, i)];
            Gia_ManForEachCoId(p->pGia0, Id, i)
                pCnf->pVarNums[Id]
                = pCnfIds[Gia_ManCoIdToId(p->pGia, i)];
            /*
...
void Mf_ManSetFlowRefs(Gia_Man_t* p, Vec_Int_t* vRefs) {
    ...
    Gia_ManForEachCoDriverId(p, Id, i) if (Gia_ObjIsAnd(Gia_ManObj(p, Id)))
...
int Mf_ManSetMapRefs(Mf_Man_t* p) {
    ...
    Gia_ManForEachCoDriverId(p->pGia, Id, i)
        Delay
        = Abc_MaxInt(Delay, Mf_ManObj(p, Id)->Delay);
    // check delay target
...
static inline int Mf_CutAreaMffc(Mf_Man_t* p, int iObj) {
    return Mf_ObjMapRefNum(p, iObj) ? Mf_CutAreaRefed(p, Mf_ObjCutBest(p, iObj)) : Mf_CutAreaDerefed(p, Mf_ObjCutBest(p, iObj));
}
...
int Mf_ManMappingFromMapping(Mf_Man_t* p) {
    ...
    Gia_ManForEachAnd(pGia, pObj, iObj) if (Mf_ObjMapRefNum(p, iObj)) {
...
void Mf_ManPrintFanoutProfile(Mf_Man_t* p, Vec_Int_t* vFanCounts) {
    ...
    Vec_IntForEachEntry(vFanCounts, Count, i) if (Count && Gia_ObjIsAnd(Gia_ManObj(pGia, i)))
...
void Mf_ManOptimizationOne(Mf_Man_t* p, int iObj) {
    ...
    Gia_ObjForEachFanoutStaticId(pGia, iObj, iFanout, i) if (Gia_ObjIsCo(Gia_ManObj(pGia, iFanout))) return;
    ...
    Gia_ObjForEachFanoutStaticId(pGia, iObj, iFanout, i)
        printf("Node  "),
        nAreaSum += Mf_ManPrintMfccStats(p, iFanout);
    // calculate the shared MFFC
    Gia_ObjForEachFanoutStaticId(pGia, iObj, iFanout, i)
        Mf_ObjMapRefInc(p, iFanout);
    Gia_ObjForEachFanoutStaticId(pGia, iObj, iFanout, i)
        ppCuts[nCuts++]
        = Mf_ObjCutBest(p, iFanout);
...
void Mf_ManOptimization(Mf_Man_t* p) {
    ...
    Vec_IntForEachEntry(pGia->vFanoutNums, Count, i) if (Count >= 2 && Count <= nOutMax && Gia_ObjIsAnd(Gia_ManObj(pGia, i)))
