...
Vec_Int_t* Gia_ManSimSimulateOne(Gia_Man_t* p, Vec_Int_t* vPat) {
    ...
    for (k = f = 0; f < Vec_IntSize(vPat) / Gia_ManPiNum(p); f++) {
        ...
        Gia_ManForEachAnd(p, pObj, i)
            pObj->fMark1
            = (Gia_ObjFanin0(pObj)->fMark1 ^ Gia_ObjFaninC0(pObj)) & (Gia_ObjFanin1(pObj)->fMark1 ^ Gia_ObjFaninC1(pObj));
        Gia_ManForEachCo(p, pObj, i)
            pObj->fMark1
            = (Gia_ObjFanin0(pObj)->fMark1 ^ Gia_ObjFaninC0(pObj));
...
int Gia_ManBuiltInSimCheckOver(Gia_Man_t* p, int iLit0, int iLit1) {
    ...
    word* pInfo1 = Gia_ManBuiltInData(p, Abc_Lit2Var(iLit1));
    int w;
    assert(p->fBuiltInSim || p->fIncrSim);
...
int Gia_ManBuiltInSimPack(Gia_Man_t* p, Vec_Int_t* vPat) {
    ...
    for (i = 0; i < p->iPatsPi; i++) {
        Vec_IntForEachEntry(vPat, iLit, k) if (Abc_TtGetBit(Gia_ManBuiltInDataPi(p, Abc_Lit2Var(iLit)), i) && Abc_TtGetBit(Gia_ManBuiltInData(p, 1 + Abc_Lit2Var(iLit)), i) == Abc_LitIsCompl(iLit)) break;
...
int Gia_ManBuiltInSimAddPat(Gia_Man_t* p, Vec_Int_t* vPat) {
    ...
    if (fOverflow) {
        int iVar;
        Vec_IntForEachEntry(&p->vSuppVars, iVar, k) if (Abc_TtGetBit(Gia_ManBuiltInDataPi(p, iVar), iPat))
