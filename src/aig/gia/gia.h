...
static inline int Gia_ManChoiceNum(Gia_Man_t* p) {
    int c = 0;
    if (p->pSibls) {
        int i;
        for (i = 0; i < p->nObjs; i++)
...
static inline int Gia_ObjId(Gia_Man_t* p, Gia_Obj_t* pObj) {
    assert(p->pObjs <= pObj && pObj < p->pObjs + p->nObjs);
    return pObj - p->pObjs;
}
...
static inline int Gia_ObjFaninNum(Gia_Man_t* p, Gia_Obj_t* pObj) {
    if (Gia_ObjIsMux(p, pObj)) return 3;
    if (Gia_ObjIsAnd(pObj)) return 2;
    if (Gia_ObjIsCo(pObj)) return 1;
    return 0;
}
static inline int Gia_ObjWhatFanin(Gia_Man_t* p, Gia_Obj_t* pObj, Gia_Obj_t* pFanin) {
    if (Gia_ObjFanin0(pObj) == pFanin) return 0;
    if (Gia_ObjFanin1(pObj) == pFanin) return 1;
    if (Gia_ObjFanin2(p, pObj) == pFanin) return 2;
    assert(0);
    return -1;
}
...
static inline void Gia_ObjSetGateLevel(Gia_Man_t* p, Gia_Obj_t* pObj) {
...
static inline void Gia_ObjSetTravIdCurrent(Gia_Man_t* p, Gia_Obj_t* pObj) {
    assert(Gia_ObjId(p, pObj) < p->nTravIdsAlloc);
...
static inline int Gia_ObjUpdateTravIdCurrent(Gia_Man_t* p, Gia_Obj_t* pObj) {
    if (Gia_ObjIsTravIdCurrent(p, pObj)) return 1;
    Gia_ObjSetTravIdCurrent(p, pObj);
    return 0;
}
static inline int Gia_ObjUpdateTravIdPrevious(Gia_Man_t* p, Gia_Obj_t* pObj) {
    if (Gia_ObjIsTravIdPrevious(p, pObj)) return 1;
...
static inline int Gia_ObjUpdateTravIdCurrentId(Gia_Man_t* p, int Id) {
    if (Gia_ObjIsTravIdCurrentId(p, Id)) return 1;
    Gia_ObjSetTravIdCurrentId(p, Id);
    return 0;
}
static inline int Gia_ObjUpdateTravIdPreviousId(Gia_Man_t* p, int Id) {
    if (Gia_ObjIsTravIdPreviousId(p, Id)) return 1;
    Gia_ObjSetTravIdPreviousId(p, Id);
    return 0;
}

static inline void Gia_ManTimeClean(Gia_Man_t* p) {
    int i;
    assert(p->vTiming != NULL);
    Vec_FltFill(p->vTiming, 3 * Gia_ManObjNum(p), 0);
    for (i = 0; i < Gia_ManObjNum(p); i++)
        Vec_FltWriteEntry(p->vTiming, 3 * i + 1, (float)(ABC_INFINITY));
}
static inline void Gia_ManTimeStart(Gia_Man_t* p) {
    assert(p->vTiming == NULL);
    p->vTiming = Vec_FltAlloc(0);
    Gia_ManTimeClean(p);
}
...
static inline void Gia_ObjTerSimSetC(Gia_Obj_t* pObj) {
    pObj->fMark0 = 0;
    pObj->fMark1 = 0;
}
static inline void Gia_ObjTerSimSet0(Gia_Obj_t* pObj) {
    pObj->fMark0 = 1;
    pObj->fMark1 = 0;
}
static inline void Gia_ObjTerSimSet1(Gia_Obj_t* pObj) {
    pObj->fMark0 = 0;
    pObj->fMark1 = 1;
}
static inline void Gia_ObjTerSimSetX(Gia_Obj_t* pObj) {
    pObj->fMark0 = 1;
    pObj->fMark1 = 1;
}
...
static inline void Gia_ObjSetColor(Gia_Man_t* p, int Id, int c) {
    if (c)
...
static inline int Gia_ObjVisitColor(Gia_Man_t* p, int Id, int c) {
    int x;
    if (c) {
        x = p->pReprs[Id].fColorB;
        p->pReprs[Id].fColorB = 1;
    } else {
        x = p->pReprs[Id].fColorA;
        p->pReprs[Id].fColorA = 1;
    }
    return x;
}
...
static inline int Gia_ObjHasSameRepr(Gia_Man_t* p, int i, int k) {
    assert(k);
    return i ? (Gia_ObjRepr(p, i) == Gia_ObjRepr(p, k) && Gia_ObjRepr(p, i) != GIA_VOID) : Gia_ObjRepr(p, k) == 0;
}
...
#    define Gia_ManForEachCell(p, i)               \
        for (i = 2; i < 2 * Gia_ManObjNum(p); i++) \
            if (!Gia_ObjIsCell(p, i)) {            \
            } else
...
#    define Gia_ManForEachBuf(p, pObj, i)                                                             \
        for (i = Gia_ManBufNum(p) ? 0 : p->nObjs; (i < p->nObjs) && ((pObj) = Gia_ManObj(p, i)); i++) \
            if (!Gia_ObjIsBuf(pObj)) {                                                                \
            } else
#    define Gia_ManForEachBufId(p, i)              \
        for (i = 0; (i < p->nObjs); i++)           \
            if (!Gia_ObjIsBuf(Gia_ManObj(p, i))) { \
            } else
