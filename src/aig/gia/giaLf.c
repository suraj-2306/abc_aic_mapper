...
static inline int Lf_BestCutIndex(Lf_Bst_t* p) {
    if (p->Cut[0].fUsed) return 0;
    if (p->Cut[1].fUsed) return 1;
    return 2;
}
...
#define Lf_CutForEachVar(pCut, Var, i)                                   \
    for (i = 0; i < (int)pCut->nLeaves && (Var = pCut->pLeaves[i]); i++) \
        if (Lf_ObjOff(p, Var) < 0) {                                     \
        } else
...
void Lf_ManAnalyzeCoDrivers(Gia_Man_t* p, int* pnDrivers, int* pnInverts) {
    ...
    Gia_ManForEachCo(p, pObj, i)
        * Vec_IntEntryP(vMarks, Gia_ObjFaninId0p(p, pObj))
        |= Gia_ObjFaninC0(pObj) ? 2 : 1;
    Vec_IntForEachEntry(vMarks, Entry, i)
        nDrivers
        += (int)(Entry != 0),
        nInverts += (int)(Entry == 3);
    Vec_IntFree(vMarks);
    *pnDrivers = nDrivers;
    *pnInverts = nInverts;
}
...
static inline int Lf_ManPrepareSet(Lf_Man_t* p, int iObj, int Index, Lf_Cut_t** ppCutSet) {
        ...
        return Lf_CutCreateUnit((*ppCutSet = (Lf_Cut_t*)CutTemp[Index]), iObj);
    {
        ...
        *ppCutSet = Lf_ManFetchSet(p, iObj);
        Lf_CutSetForEachCut(p->nCutWords, *ppCutSet, pCut, i, nCutNum) if (pCut->nLeaves == LF_NO_LEAF) return i;
        return i;
    }
}
...
static inline void Lf_MemRecycle(Lf_Mem_t* p) {
    void* pPlace;
    int i;
    Vec_PtrForEachEntry(void*, &p->vPages, pPlace, i) if (pPlace)
...
static inline int Lf_CutComputeTruth6(Lf_Man_t* p, Lf_Cut_t* pCut0, Lf_Cut_t* pCut1, int fCompl0, int fCompl1, Lf_Cut_t* pCutR, int fIsXor) {
    ...
    int nOldSupp = pCutR->nLeaves, truthId, fCompl;
    word t;
...
float Lf_CutRef_rec(Lf_Man_t* p, Lf_Cut_t* pCut) {
    ...
    int i, Var;
    Lf_CutForEachVar(pCut, Var, i) if (!Lf_ObjMapRefInc(p, Var))
        Count
        += Lf_CutRef_rec(p, Lf_ObjCutBestNew(p, Var, (Lf_Cut_t*)CutTemp));
    return Count;
}
float Lf_CutDeref_rec(Lf_Man_t* p, Lf_Cut_t* pCut) {
    ...
    float Count = Lf_CutArea(p, pCut);
    int i, Var;
    Lf_CutForEachVar(pCut, Var, i) if (!Lf_ObjMapRefDec(p, Var))
        Count
        += Lf_CutDeref_rec(p, Lf_ObjCutBestNew(p, Var, (Lf_Cut_t*)CutTemp));
    return Count;
}
...
void Lf_ObjMergeOrder(Lf_Man_t* p, int iObj) {
    ...
    if (Gia_ObjIsMuxId(p->pGia, iObj)) {
        ...
        Lf_CutSetForEachCut(nCutWords, pCutSet0, pCut0, i, nCuts0) if ((int)pCut0->nLeaves <= nLutSize)
            ...
            if (p->pPars->nLutSizeMux && p->pPars->nLutSizeMux == (int)pCutsR[nCutsR]->nLeaves && Lf_ManFindCofVar(Lf_CutTruth(p, pCutsR[nCutsR]), Abc_Truth6WordNum(nLutSize), pCutsR[nCutsR]->nLeaves) == -1)
                continue;
            Lf_CutParams(p, pCutsR[nCutsR], Required, FlowRefs, pObj);
            nCutsR = Lf_SetAddCut(pCutsR, nCutsR, nCutNum);
        }
...
void Lf_ManSetFlowRefs(Gia_Man_t* p, Vec_Flt_t* vRefs, Vec_Int_t* vOffsets) {
    ...
    Gia_ManForEachCoDriverId(p, Id, i) if (Gia_ObjIsAndNotBuf(Gia_ManObj(p, Id)))
...
void Lf_ManCountMapRefs(Lf_Man_t* p) {
    ...
    if (p->pGia->pManTime != NULL) {
        ...
        Gia_ManForEachAndReverse(p->pGia, pObj, i) if (Gia_ObjIsBuf(pObj))
            Lf_ObjSetRequired(p, Gia_ObjFaninId0(pObj, i), Lf_ObjRequired(p, i));
        else if (Lf_ObjMapRefNum(p, i))
            Lf_ManCountMapRefsOne(p, i);
    }
...
Lf_Man_t* Lf_ManAlloc(Gia_Man_t* pGia, Jf_Par_t* pPars) {
    ...
    Gia_ManForEachAndId(pGia, i) if (!Gia_ObjIsBuf(Gia_ManObj(pGia, i)))
...
void Lf_ManComputeMapping(Lf_Man_t* p) {
    ...
    if (p->pGia->pManTime != NULL) {
        ...
        Gia_ManForEachAnd(p->pGia, pObj, i) if (!Gia_ObjIsBuf(pObj))
            Lf_ObjMergeOrder(p, i);
    }
