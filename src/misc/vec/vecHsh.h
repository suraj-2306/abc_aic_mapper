...
static inline word Hsh_IntWord(int iData, int iNext) {
    Hsh_IntObjWord_t Obj = {{iData, iNext}};
    return Obj.wWord;
}
...
static inline int Hsh_Int1ManAdd(Hsh_Int1Man_t* p, int Data) {
    ...
    if (Vec_IntSize(p->vData) > Vec_IntSize(p->vTable)) {
        ...
        Vec_IntForEachEntry(p->vData, Entry, i) {
            ...
            Vec_IntWriteEntry(p->vNext, i, *pPlace);
            *pPlace = i;
        }
    }
