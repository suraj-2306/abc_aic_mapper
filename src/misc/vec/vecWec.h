...
static inline void Vec_WecKeepLevels(Vec_Wec_t* p, int Limit) {
    Vec_Int_t* vLevel;
    int i, k = 0;
    Vec_WecForEachLevel(p, vLevel, i) if (Vec_IntSize(vLevel) > Limit) {
...
static inline int Vec_WecMaxLevelSize(Vec_Wec_t* p) {
    ...
    Vec_WecForEachLevel(p, vTemp, i)
        Res
        = Abc_MaxInt(Res, Vec_IntSize(vTemp));
    return Res;
}
...
static inline Vec_Int_t* Vec_WecCollectFirsts(Vec_Wec_t* p) {
    ...
    Vec_WecForEachLevel(p, vLevel, i) if (Vec_IntSize(vLevel) > 0)
...
static inline void Vec_WecRemoveEmpty(Vec_Wec_t* vCubes) {
    Vec_Int_t* vCube;
    int i, k = 0;
    Vec_WecForEachLevel(vCubes, vCube, i) if (Vec_IntSize(vCube) > 0)
        vCubes->pArray[k++]
        = *vCube;
    else ABC_FREE(vCube->pArray);
