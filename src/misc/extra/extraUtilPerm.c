...
static inline void Abc_ZddSetVarIJ(Abc_ZddMan* p, int i, int j, int v) {
    assert(i < j);
    p->pT2V[i * p->nPermSize + j] = v;
}
static inline int Abc_ZddVarIJ(Abc_ZddMan* p, int i, int j) {
    assert(i < j);
    return p->pT2V[i * p->nPermSize + j];
}
...
Abc_ZddMan* Abc_ZddManAlloc(int nVars, int nObjs) {
    ...
    p->nMemory = sizeof(Abc_ZddMan) / 4 + p->nUniqueMask + 1 + p->nObjsAlloc + (p->nCacheMask + 1) * sizeof(Abc_ZddEnt) / 4 + p->nObjsAlloc * sizeof(Abc_ZddObj) / 4;
    return p;
}
...
void Abc_ZddGiaTest(Gia_Man_t* pGia) {
    ...
    Gia_ManForEachCi(pGia, pObj, i)
        pObj->Value
        = Abc_ZddIthVar(Gia_ObjId(pGia, pObj));
    ...
    Gia_ManForEachAnd(pGia, pObj, i)
        Paths
        += Abc_ZddCountPaths(p, pObj->Value);
