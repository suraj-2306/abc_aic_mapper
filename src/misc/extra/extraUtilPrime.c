...
void Abc_GenCountDump(Vec_Int_t* vPrimes, int nVars, char* pFileName) {
    ...
    fprintf(pFile, ".o %d\n", 1);
    fprintf(pFile, ".p %d\n", Vec_IntSize(vPrimes));
    Vec_IntForEachEntry(vPrimes, Prime, i) for (k = nVars - 1; k >= 0; k--)
...
static inline int* Tab_ManCube(Tab_Man_t* p, int i) {
    assert(i >= 0 && i < p->nCubes);
    return p->pCubes + i * (p->nVars + 1);
}
static inline Tab_Ent_t* Tab_ManEnt(Tab_Man_t* p, int i) {
    assert(i >= -1 && i < p->nTable);
    return i >= 0 ? p->pTable + i : NULL;
}
