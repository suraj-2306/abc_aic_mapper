...
Gia_Man_t* Abc_GraphDeriveGia(Vec_Wec_t* vNodes, Vec_Int_t* vEdges) {
    ...
    Vec_WecForEachLevel(vNodes, vLevel, i)
        pCache[i]
        = ABC_FALLOC(int, Vec_IntSize(vLevel));
...
int Abc_GraphBuildState(Vec_Int_t* vState, int e, int x, Vec_Int_t* vEdges, Vec_Int_t* vLife, Vec_Wec_t* vFronts, int* pFront, Vec_Int_t* vStateNew, int fVerbose) {
    ...
    if (x) {
        // union comp
        int First = -1;
        Vec_IntForEachEntry(vFront2, Node, i) if ((pFront[Node] & 0xFFFF) == pEquivs[0] || (pFront[Node] & 0xFFFF) == pEquivs[1]) {
            if (First == -1)
                First = Node;
            pFront[Node] = (pFront[Node] & 0xFFFF0000) | First;
        }
...
void Abc_GraphBuildFrontier(int nSize, Vec_Int_t* vEdges, Vec_Int_t* vLife, Vec_Wec_t* vFronts, int fVerbose) {
    ...
    for (e = 0; e < nEdges; e++) {
        ...
        if (fVerbose) {
            ...
            printf("Frontier: ");
            Vec_IntPrint(Vec_WecEntry(vFronts, e));
            printf("\n");
        }
        for (s = 0; s < nStates; s++) {
...
void Abc_GraphSolve(Gia_Man_t* pGia) {
    ...
    for (i = 0; i < nIters; i++) {
        ...
        // collect new minterm
        Mint2 = 0;
        Vec_IntForEachEntry(vLits, iLit, k) if (!Abc_LitIsCompl(iLit))
            Mint2
            |= ((word)1) << (nVars - 1 - k);
        if (Mint1 == 0)
            Mint1 = Mint2;
