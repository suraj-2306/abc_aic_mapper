...
avl_tree*
    avl_init_table(compar) int (*compar)();
{
    avl_tree* tree;
...
static void
    do_rebalance(stack_nodep, stack_n) register avl_node*** stack_nodep;
...
int* error;
{
    ...
    if (node->left != NIL(avl_node) && (*compar)(node->left->key, node->key) > 0) {
        (void)printf("Bad ordering between 0x%08x and 0x%08x",
                     node, node->left);
        ++*error;
    }

    if (node->right != NIL(avl_node) && (*compar)(node->key, node->right->key) > 0) {
        (void)printf("Bad ordering between 0x%08x and 0x%08x",
                     node, node->right);
        ++*error;
    }
