...
static inline unsigned xSat_Float2Uint(xFloat_t f) {
    union {
        xFloat_t f;
        unsigned u;
    } temp;
...
static inline xFloat_t xSat_Uint2Float(unsigned u) {
    union {
        xFloat_t f;
        unsigned u;
    } temp;
...
static inline xFloat_t xSat_FloatCreate(unsigned Exp, unsigned Mnt) {
    xFloat_t res;
    res.Exp = Exp;
    res.Mnt = Mnt;
    return res;
...
static inline float xSat_Float2Float(xFloat_t a) {
    assert(a.Exp < 127);
    return Abc_Int2Float(((a.Exp + 127) << 23) | ((a.Mnt & 0x7FFF) << 8));
}
static inline xFloat_t xSat_FloatFromFloat(float a) {
    int A = Abc_Float2Int(a);
    assert(a >= 1.0);
    return xSat_FloatCreate((A >> 23) - 127, 0x8000 | ((A >> 8) & 0x7FFF));
}
