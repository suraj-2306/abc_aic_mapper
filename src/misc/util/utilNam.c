...
Abc_Nam_t* Abc_NamStart(int nObjs, int nAveSize) {
    ...
    Vec_IntGrow(&p->vInt2Handle, nObjs);
    Vec_IntPush(&p->vInt2Handle, -1);
    Vec_IntGrow(&p->vInt2Next, nObjs);
...
void Abc_NamSave(Abc_Nam_t* p, char* pFileName) {
    ...
    int h, i;
    if (pFile == NULL) {
        printf("Count node open input file %s\n", pFileName);
        return;
    }
    Vec_IntForEachEntryStart(&p->vInt2Handle, h, i, 1)
...
Abc_Nam_t* Abc_NamLoad(char* pFileName) {
    ...
    if (pFile == NULL) {
        printf("Count node open output file %s\n", pFileName);
        return NULL;
    }
...
int Abc_NamMemUsed(Abc_Nam_t* p) {
    if (p == NULL)
        return 0;
    return sizeof(Abc_Nam_t) + p->iHandle + sizeof(int) * p->nBins + sizeof(int) * (p->vInt2Handle.nSize + p->vInt2Next.nSize);
}
...
int Abc_NamMemAlloc(Abc_Nam_t* p) {
    if (p == NULL)
        return 0;
    return sizeof(Abc_Nam_t) + p->nStore + sizeof(int) * p->nBins + sizeof(int) * (p->vInt2Handle.nCap + p->vInt2Next.nCap);
}
