...
#    define ABC_REALLOC(type, obj, num) \
        ((obj) ? ((type*)realloc((char*)(obj), sizeof(type) * (size_t)(num))) : ((type*)malloc(sizeof(type) * (size_t)(num))))
...
static inline int Abc_Float2Int(float Val) {
    union {
        int x;
        float y;
    } v;
...
static inline float Abc_Int2Float(int Num) {
    union {
        int x;
        float y;
    } v;
...
static inline word Abc_Dbl2Word(double Dbl) {
    union {
        word x;
        double y;
    } v;
...
static inline double Abc_Word2Dbl(word Num) {
    union {
        word x;
        double y;
    } v;
...
static inline int Abc_Base10Log(unsigned n) {
    int r;
    if (n < 2) return (int)n;
    for (r = 0, n--; n; n /= 10, r++) {};
    return r;
}
static inline int Abc_Base16Log(unsigned n) {
    int r;
    if (n < 2) return (int)n;
    for (r = 0, n--; n; n /= 16, r++) {};
    return r;
...
static inline char* Abc_UtilStrsavTwo(char* s, char* a) {
    char* r;
    if (!a) return Abc_UtilStrsav(s);
    r = ABC_ALLOC(char, strlen(s) + strlen(a) + 1);
    sprintf(r, "%s%s", s, a);
    return r;
}
static inline char* Abc_UtilStrsavNum(char* s, int n) {
    char* r;
    if (!s) return NULL;
    r = ABC_ALLOC(char, strlen(s) + 12 + 1);
    sprintf(r, "%s%d", s, n);
    return r;
}
