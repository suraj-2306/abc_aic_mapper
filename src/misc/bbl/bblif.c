...
int Bbl_ManSaveSop(Bbl_Man_t* p, char* pSop, int nVars) {
    ...
    if (iFunc == Vec_StrSize(p->pFncs)) { // store this SOP
...
int Bbl_ObjIdOriginal(Bbl_Man_t* pMan, Bbl_Obj_t* p) {
    assert(0);
    return Vec_IntEntry(pMan->vObj2Id, p->Id);
}
...
Bbl_Obj_t* Bbl_ObjFaninNext(Bbl_Obj_t* p, Bbl_Obj_t* pPrev) {
    Bbl_Obj_t* pFanin;
    int i;
    Bbl_ObjForEachFanin_int(p, pFanin, i) if (pFanin == pPrev) break;
