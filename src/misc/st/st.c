...
#define EQUAL(func, x, y) \
    ((((func) == st__numcmp) || ((func) == st__ptrcmp)) ? (st__NUMCMP((x), (y)) == 0) : ((*func)((x), (y)) == 0))

#define do_hash(key, table)                                                                                                                      \
    ((table->hash == st__ptrhash) ? st__PTRHASH((key), (table)->num_bins) : (table->hash == st__numhash) ? st__NUMHASH((key), (table)->num_bins) \
...
int st__numhash(const char* x, int size) {
    return st__NUMHASH(x, size);
}

int st__ptrhash(const char* x, int size) {
    return st__PTRHASH(x, size);
}

int st__numcmp(const char* x, const char* y) {
    return st__NUMCMP(x, y);
}

int st__ptrcmp(const char* x, const char* y) {
    return st__NUMCMP(x, y);
}
