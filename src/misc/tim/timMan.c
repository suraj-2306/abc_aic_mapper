...
Tim_Man_t* Tim_ManTrim(Tim_Man_t* p, Vec_Int_t* vBoxPres) {
    ...
    if (Tim_ManBoxNum(p))
        Tim_ManForEachBox(p, pBox, i) if (Vec_IntEntry(vBoxPres, i)) {
...
Vec_Int_t* Tim_ManAlignTwo(Tim_Man_t* pSpec, Tim_Man_t* pImpl) {
    ...
    Tim_ManForEachBox(pImpl, pBox, i) if (pBox->iCopy < 0 || pBox->iCopy >= Tim_ManBoxNum(pSpec)) return NULL;
...
float* Tim_ManGetArrTimes(Tim_Man_t* p) {
    ...
    Tim_Obj_t* pObj;
    int i;
    Tim_ManForEachPi(p, pObj, i) if (pObj->timeArr != 0.0) break;
    if (i == Tim_ManPiNum(p))
        return NULL;
...
float* Tim_ManGetReqTimes(Tim_Man_t* p) {
    ...
    Tim_Obj_t* pObj;
    int i, k = 0;
    Tim_ManForEachPo(p, pObj, i) if (pObj->timeReq != TIM_ETERNITY) break;
    if (i == Tim_ManPoNum(p))
        return NULL;
...
void Tim_ManPrint(Tim_Man_t* p) {
    ...
    // print CI info
    pPrev = p->pCis;
    Tim_ManForEachPi(p, pObj, i) if (pPrev->timeArr != pObj->timeArr || pPrev->timeReq != pObj->timeReq) break;
    if (i == Tim_ManCiNum(p))
    ...
    // print CO info
    pPrev = p->pCos;
    Tim_ManForEachPo(p, pObj, i) if (pPrev->timeArr != pObj->timeArr || pPrev->timeReq != pObj->timeReq) break;
    if (i == Tim_ManCoNum(p))
    ...
    if (Tim_ManBoxNum(p) > 0)
        Tim_ManForEachBox(p, pBox, i) {
            ...
            Tim_ManBoxForEachInput(p, pBox, pObj, k) if (pPrev->timeArr != pObj->timeArr || pPrev->timeReq != pObj->timeReq) break;
            ...
            Tim_ManBoxForEachOutput(p, pBox, pObj, k) if (pPrev->timeArr != pObj->timeArr || pPrev->timeReq != pObj->timeReq) break;
