...
static inline Tim_Obj_t* Tim_ManCi(Tim_Man_t* p, int i) {
    assert(i < p->nCis);
    return p->pCis + i;
}
static inline Tim_Obj_t* Tim_ManCo(Tim_Man_t* p, int i) {
    assert(i < p->nCos);
    return p->pCos + i;
}
...
static inline Tim_Obj_t* Tim_ManBoxInput(Tim_Man_t* p, Tim_Box_t* pBox, int i) {
    assert(i < pBox->nInputs);
    return p->pCos + pBox->Inouts[i];
}
static inline Tim_Obj_t* Tim_ManBoxOutput(Tim_Man_t* p, Tim_Box_t* pBox, int i) {
    assert(i < pBox->nOutputs);
    return p->pCis + pBox->Inouts[pBox->nInputs + i];
}
...
#    define Tim_ManForEachPi(p, pObj, i)                         \
        Tim_ManForEachCi(p, pObj, i) if (pObj->iObj2Box >= 0) {} \
        else
#    define Tim_ManForEachPo(p, pObj, i)                         \
        Tim_ManForEachCo(p, pObj, i) if (pObj->iObj2Box >= 0) {} \
        else
