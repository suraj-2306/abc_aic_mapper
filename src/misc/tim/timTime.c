...
float Tim_ManGetCiArrival(Tim_Man_t* p, int iCi) {
    ...
    if (p->fUseTravId)
        Tim_ManBoxForEachInput(p, pBox, pObj, i) if (pObj->TravId != p->nTravIds)
    ...
    Tim_ManBoxForEachOutput(p, pBox, pObjRes, i) {
        ...
        Tim_ManBoxForEachInput(p, pBox, pObj, k) if (pDelays[k] != -ABC_INFINITY)
            DelayBest
            = Abc_MaxInt(DelayBest, pObj->timeArr + pDelays[k]);
...
float Tim_ManGetCoRequired(Tim_Man_t* p, int iCo) {
    ...
    if (p->fUseTravId)
        Tim_ManBoxForEachOutput(p, pBox, pObj, i) if (pObj->TravId != p->nTravIds)
