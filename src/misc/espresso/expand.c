...
*/
void expand1(BB, CC, RAISE, FREESET, OVEREXPANDED_CUBE, SUPER_CUBE, INIT_LOWER, num_covered, c)
...
#else
        {
            ...
            if ((last = cube.inword) != -1) {
                ...
                if ((x = ~(x | x >> 1) & cube.inmask))
                    if ((dist = count_ones(x)) > 1) goto exit_if;
                for (w = 1; w < last; w++) {
                    x = p[w] & r[w];
                    if ((x = ~(x | x >> 1) & DISJOINT))
                        if (dist == 1 || (dist += count_ones(x)) > 1) goto exit_if;
                }
            }
        }
        {
