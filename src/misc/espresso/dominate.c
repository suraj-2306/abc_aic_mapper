...
sm_matrix* A;
{
    ...
    for (prow = A->first_row; prow != 0; prow = prow->next_row) {
        ...
        for (p = least_col->first_row; p != 0; p = pnext) {
            ...
            if ((prow1->length > prow->length) || (prow1->length == prow->length && prow1->row_num > prow->row_num)) {
    ...
    for (pcol = A->first_col; pcol != 0; pcol = next_col) {
        ...
        for (p = least_row->first_col; p != 0; p = p->next_col) {
            ...
            if (weight != 0 && weight[pcol1->col_num] > weight[pcol->col_num])
                continue;
            if ((pcol1->length > pcol->length) || (pcol1->length == pcol->length && pcol1->col_num > pcol->col_num)) {
