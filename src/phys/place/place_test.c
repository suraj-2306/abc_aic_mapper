...
void readBookshelfNets(char* filename) {
    ...
    while (fgets(buf, 1024, netsFile)) {
        ...
        if (concreteNets[id].m_numTerms < 0 || concreteNets[id].m_numTerms > 100000) {
            printf("ERROR: Bad net degree\n");
            exit(1);
        }
        concreteNets[id].m_terms = malloc(sizeof(ConcreteCell*) * concreteNets[id].m_numTerms);

        // read terms
        t = 0;
        while (t < concreteNets[id].m_numTerms && fgets(buf, 1024, netsFile)) {
            if (buf[0] == '\n' || buf[0] == '#') continue;
...
void readBookshelfPlacement(char* filename) {
    ...
    while (fgets(buf, 1024, plFile)) {
        ...
        // hfixed
        cell->m_fixed = strtok(NULL, DELIMITERS) && (tok = strtok(NULL, DELIMITERS)) && !strcmp(tok, "\\FIXED");
    }
