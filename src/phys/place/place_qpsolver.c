...
qps_cgmin(qps_problem_t* p) {
    ...
    for (i = 0; i < 2 * n; i++) {
        ...
        qps_settp(p);
        p->priv_f = qps_func(p);
        if (fabs((p->priv_f) - fp) <= (fabs(p->priv_f) + fabs(fp) + QPS_EPS) * QPS_TOL / 2.0) {
            break;
        }
        fp = p->priv_f;
        qps_dfunc(p, xi);
...
qps_solve_inner(qps_problem_t* p) {
    ...
    if (p->max_enable || p->loop_num) {
        if (p->max_enable == 1 || (p->loop_num && p->loop_k == 0)) {
            ...
            if (p->priv_f < p->priv_fprev && (p->priv_fprev - p->priv_f) > QPS_DEC_CHANGE * fabs(p->priv_fprev)) {
    ...
    if (p->loop_num) {
        ...
        for (i = p->loop_num; i--;) {
            ...
            if (p->priv_lt[i] > z || (p->loop_k < QPS_RELAX_ITER && p->loop_penalty[i] * p->priv_lt[i] < -z)) {
                p->loop_done = 0;
#if (QPS_DEBUG > 1)
    ...
    if (p->max_enable) {
        ...
        for (i = p->num_cells; i--;) {
            z = -(p->x[i]);
            t += z * z;
            if (z > QPS_TOL || (p->max_enable < QPS_RELAX_ITER && p->priv_mxl[i] * z < -QPS_MAX_TOL)) {
                p->max_done = 0;
#if (QPS_DEBUG > 4)
            ...
            z = (p->x[i] - p->max_x);
            t += z * z;
            if (z > QPS_TOL || (p->max_enable < QPS_RELAX_ITER && p->priv_mxh[i] * z < -QPS_MAX_TOL)) {
                p->max_done = 0;
#if (QPS_DEBUG > 4)
            ...
            z = -(p->y[i]);
            t += z * z;
            if (z > QPS_TOL || (p->max_enable < QPS_RELAX_ITER && p->priv_myl[i] * z < -QPS_MAX_TOL)) {
                p->max_done = 0;
#if (QPS_DEBUG > 4)
            ...
            z = (p->y[i] - p->max_y);
            t += z * z;
            if (z > QPS_TOL || (p->max_enable < QPS_RELAX_ITER && p->priv_myh[i] * z < -QPS_MAX_TOL)) {
                p->max_done = 0;
#if (QPS_DEBUG > 4)
