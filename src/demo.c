...
int main(int argc, char* argv[]) {
    ...
    if (fUseResyn2) {
        sprintf(Command,
                "balance; rewrite -l; refactor -l; balance; rewrite -l; rewrite "
                "-lz; balance; refactor -lz; rewrite -lz; balance");
        if (Cmd_CommandExecute(pAbc, Command)) {
            fprintf(stdout, "Cannot execute command \"%s\".\n", Command);
            return 1;
        }
    ...
    printf("Rewriting = %6.2f sec   ",
           (float)(clkResyn) / (float)(CLOCKS_PER_SEC));
