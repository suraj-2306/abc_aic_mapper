...
struct Kit_Node_t_ {
    ...
    // other info
    union {
        int iFunc; // the function of the node (BDD or AIG)
        void* pFunc;
...
static inline Kit_DsdObj_t* Kit_DsdNtkObj(Kit_DsdNtk_t* pNtk, int Id) {
    assert(Id >= 0 && Id < pNtk->nVars + pNtk->nNodes);
    return Id < pNtk->nVars ? NULL : pNtk->pNodes[Id - pNtk->nVars];
}
static inline Kit_DsdObj_t* Kit_DsdNtkRoot(Kit_DsdNtk_t* pNtk) { return Kit_DsdNtkObj(pNtk, Abc_Lit2Var(pNtk->Root)); }
static inline int Kit_DsdLitIsLeaf(Kit_DsdNtk_t* pNtk, int Lit) {
    int Id = Abc_Lit2Var(Lit);
    assert(Id >= 0 && Id < pNtk->nVars + pNtk->nNodes);
    return Id < pNtk->nVars;
}
static inline unsigned Kit_DsdLitSupport(Kit_DsdNtk_t* pNtk, int Lit) {
    int Id = Abc_Lit2Var(Lit);
    assert(Id >= 0 && Id < pNtk->nVars + pNtk->nNodes);
    return pNtk->pSupps ? (Id < pNtk->nVars ? (1 << Id) : pNtk->pSupps[Id - pNtk->nVars]) : 0;
}
...
static inline Kit_Edge_t Kit_EdgeCreate(int Node, int fCompl) {
    Kit_Edge_t eEdge = {(unsigned)fCompl, (unsigned)Node};
    return eEdge;
}
...
static inline unsigned Kit_EdgeToInt_(Kit_Edge_t m) {
    union {
        Kit_Edge_t x;
        unsigned y;
    } v;
...
static inline Kit_Edge_t Kit_IntToEdge_(unsigned m) {
    union {
        Kit_Edge_t x;
        unsigned y;
    } v;
...
static inline Kit_Node_t* Kit_GraphVar(Kit_Graph_t* pGraph) {
    assert(Kit_GraphIsVar(pGraph));
    return Kit_GraphNode(pGraph, pGraph->eRoot.Node);
}
static inline int Kit_GraphVarInt(Kit_Graph_t* pGraph) {
    assert(Kit_GraphIsVar(pGraph));
...
static inline unsigned Kit_BitMask(int nBits) {
    assert(nBits <= 32);
