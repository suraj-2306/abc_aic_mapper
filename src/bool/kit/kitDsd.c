...
unsigned* Kit_DsdTruthComputeNode_rec(Kit_DsdMan_t* p, Kit_DsdNtk_t* pNtk, int Id) {
    ...
    Kit_DsdObjForEachFanin(pNtk, pObj, iLit, i)
        pTruthFans[i]
        = Kit_DsdTruthComputeNode_rec(p, pNtk, Abc_Lit2Var(iLit));
...
*/
    Kit_DsdObjForEachFanin(pNtk, pObj, iLit, i) if (Abc_LitIsCompl(iLit))
...
unsigned* Kit_DsdTruthComputeNodeOne_rec(Kit_DsdMan_t* p, Kit_DsdNtk_t* pNtk, int Id, unsigned uSupp) {
    ...
    // collect the truth tables of the fanins
    if (uSupp) {
        Kit_DsdObjForEachFanin(pNtk, pObj, iLit, i) if (uSupp != (uSupp & ~Kit_DsdLitSupport(pNtk, iLit)))
            pTruthFans[i]
            = Kit_DsdTruthComputeNodeOne_rec(p, pNtk, Abc_Lit2Var(iLit), uSupp);
        else {
            pTruthFans[i] = NULL;
            nPartial = 1;
        }
    ...
    if (pObj->Type == KIT_DSD_AND) {
        Kit_TruthFill(pTruthRes, pNtk->nVars);
        Kit_DsdObjForEachFanin(pNtk, pObj, iLit, i) if (pTruthFans[i])
            Kit_TruthAndPhase(pTruthRes, pTruthRes, pTruthFans[i], pNtk->nVars, 0, Abc_LitIsCompl(iLit));
        return pTruthRes;
    }
    if (pObj->Type == KIT_DSD_XOR) {
    ...
    if (uSupp && nPartial) {
        ...
        Kit_DsdObjForEachFanin(pNtk, pObj, iLit, i) if (pTruthFans[i]) break;
        assert(i < pObj->nFans);
        return pTruthFans[i];
    }
    /*
...
unsigned* Kit_DsdTruthComputeNodeTwo_rec(Kit_DsdMan_t* p, Kit_DsdNtk_t* pNtk, int Id, unsigned uSupp, int iVar, unsigned* pTruthDec) {
    ...
    if (uSuppFan != (uSuppFan & uSupp)) {
        ...
        if (pObj->Type == KIT_DSD_AND) {
            ...
            Kit_DsdObjForEachFanin(pNtk, pObj, iLit, i) if (pfBoundSet[i])
                Kit_TruthAndPhase(pTruthDec, pTruthDec, pTruthFans[i], pNtk->nVars, 0, Abc_LitIsCompl(iLit));
            else Kit_TruthAndPhase(pTruthRes, pTruthRes, pTruthFans[i], pNtk->nVars, 0, Abc_LitIsCompl(iLit));
            return pTruthRes;
        }
        if (pObj->Type == KIT_DSD_XOR) {
            Kit_TruthIthVar(pTruthRes, pNtk->nVars, iVar);
        ...
        // find the only non-empty component
        Kit_DsdObjForEachFanin(pNtk, pObj, iLit, i) if (pfBoundSet[i]) break;
        assert(i < pObj->nFans);
...
int Kit_DsdExpandNode_rec(Kit_DsdNtk_t* pNew, Kit_DsdNtk_t* p, int iLit) {
    ...
    if (pObj->nFans == 3 && (pTruthNew[0] == 0xCACACACA || pTruthNew[0] == 0xC5C5C5C5 || pTruthNew[0] == 0x3A3A3A3A || pTruthNew[0] == 0x35353535)) {
...
unsigned Kit_DsdGetSupports_rec(Kit_DsdNtk_t* p, int iLit) {
    ...
    Kit_DsdObjForEachFanin(p, pObj, iFaninLit, k)
        uSupport
        |= Kit_DsdGetSupports_rec(p, iFaninLit);
...
int Kit_DsdFindLargeBox_rec(Kit_DsdNtk_t* pNtk, int Id, int Size) {
    ...
    Kit_DsdObjForEachFanin(pNtk, pObj, iLit, i)
        RetValue
        |= Kit_DsdFindLargeBox_rec(pNtk, Abc_Lit2Var(iLit), Size);
    return RetValue;
}
...
int Kit_DsdCountAigNodes_rec(Kit_DsdNtk_t* pNtk, int Id) {
    ...
    Kit_DsdObjForEachFanin(pNtk, pObj, iLit, i)
        RetValue
        += Kit_DsdCountAigNodes_rec(pNtk, Abc_Lit2Var(iLit));
    return RetValue;
}
    ...
    for (i = pObj->nFans - 1; i >= 0; i--) {
        ...
        if (nFans0 == 1 && nFans1 == 1) {
            ...
            if ((fEquals[0][0] && fEquals[0][1]) || (fEquals[1][0] && fEquals[1][1])) {
                ...
                pRes->nRefs++;
                pRes->nFans = 3;
                pRes->pFans[0] = pObj->pFans[iLit0];
                pObj->pFans[iLit0] = 127;
                uSupp &= ~(1 << iLit0);
                pRes->pFans[1] = pObj->pFans[iLit1];
                pObj->pFans[iLit1] = 127;
                uSupp &= ~(1 << iLit1);
                pRes->pFans[2] = pObj->pFans[i];
                pObj->pFans[i] = 2 * pRes->Id; // remains in support
                // update the node
        ...
        for (k = i + 1; k < pObj->nFans; k++) {
            ...
            pRes->nRefs++;
            pRes->nFans = 2;
            pRes->pFans[0] = pObj->pFans[k];
            pObj->pFans[k] = 2 * pRes->Id; // remains in support
            pRes->pFans[1] = pObj->pFans[i];
            pObj->pFans[i] = 127;
            uSupp &= ~(1 << i);
