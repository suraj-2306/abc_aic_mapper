...
DdNode* Kit_GraphToBdd(DdManager* dd, Kit_Graph_t* pGraph) {
    ...
    Kit_GraphForEachNode(pGraph, pNode, i) {
        ...
        pNode->pFunc = Cudd_bddAnd(dd, bFunc0, bFunc1);
        Cudd_Ref((DdNode*)pNode->pFunc);
    }

    // deref the intermediate results
    bFunc = (DdNode*)pNode->pFunc;
    Cudd_Ref(bFunc);
    Kit_GraphForEachNode(pGraph, pNode, i)
...
DdNode* Kit_TruthToBdd_rec(DdManager* dd, unsigned* pTruth, int iBit, int nVars, int nVarsTotal, int fMSBonTop) {
    ...
    bF0 = Kit_TruthToBdd_rec(dd, pTruth, iBit, nVars - 1, nVarsTotal, fMSBonTop);
    Cudd_Ref(bF0);
    bF1 = Kit_TruthToBdd_rec(dd, pTruth, iBit + (1 << (nVars - 1)), nVars - 1, nVarsTotal, fMSBonTop);
    Cudd_Ref(bF1);
    bF = Cudd_bddIte(dd, dd->vars[Var], bF1, bF0);
    Cudd_Ref(bF);
...
int Kit_SopFactorVerify(Vec_Int_t* vCover, Kit_Graph_t* pFForm, int nVars) {
    ...
    // get the functions
    bFunc1 = Kit_SopToBdd(dd, cSop, nVars);
    Cudd_Ref(bFunc1);
    bFunc2 = Kit_GraphToBdd(dd, pFForm);
    Cudd_Ref(bFunc2);
    ...
    if (bFunc1 != bFunc2) {
        int s;
        Extra_bddPrint(dd, bFunc1);
        printf("\n");
        Extra_bddPrint(dd, bFunc2);
        printf("\n");
        s = 0;
    }
