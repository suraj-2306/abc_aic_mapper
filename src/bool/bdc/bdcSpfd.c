...
void Bdc_SpfdPrint(Bdc_Nod_t* pNode, int Level, Vec_Ptr_t* vLevels, word Truth) {
    ...
    Extra_PrintHex(stdout, (unsigned*)&pNode->Truth, 6);
    printf("   ");
    Extra_PrintHex(stdout, (unsigned*)&Diff, 6);
    printf("   ");
...
void Bdc_SpfdDecompose(word Truth, int nVars, int nCands, int nGatesMax) {
    ...
    for (c = i = 0; i < nVars; i++)
        for (j = i + 1; j < nVars; j++) {
            pNode[c].Truth = pNode0[i].Truth & pNode0[j].Truth;
            pNode[c].iFan0n = i;
            pNode[c].iFan1n = j;
            pNode[c++].Type = 0;
            pNode[c].Truth = ~pNode0[i].Truth & pNode0[j].Truth;
    ...
    for (n = 2; n <= nGatesMax; n++) {
        ...
        for (k = 0; k < n - 1; k++) {
            ...
            for (i = 0; i < Count0; i++)
                for (j = 0; j < Count1; j++) {
                    pNode[c].Truth = pNode0[i].Truth & pNode1[j].Truth;
                    pNode[c].iFan0g = k;
                    pNode[c].iFan1g = n - 1;
                    pNode[c].iFan0n = i;
                    pNode[c].iFan1n = j;
        ...
        for (i = 0; i < Count1; i++)
            for (j = i + 1; j < Count1; j++) {
                pNode[c].Truth = pNode1[i].Truth & pNode1[j].Truth;
                pNode[c].iFan0g = n - 1;
                pNode[c].iFan1g = n - 1;
                pNode[c].iFan0n = i;
                pNode[c].iFan1n = j;
...
int Bdc_SpfdMark0(Bdc_Ent_t* p, Bdc_Ent_t* pEnt) {
    ...
    if (pEnt->fMark0)
        return 0;
    pEnt->fMark0 = 1;
    return pEnt->fMark1 + Bdc_SpfdMark0(p, p + pEnt->iFan0) + Bdc_SpfdMark0(p, p + pEnt->iFan1);
}
int Bdc_SpfdMark1(Bdc_Ent_t* p, Bdc_Ent_t* pEnt) {
    ...
    if (pEnt->fMark1)
        return 0;
    pEnt->fMark1 = 1;
    return pEnt->fMark0 + Bdc_SpfdMark1(p, p + pEnt->iFan0) + Bdc_SpfdMark1(p, p + pEnt->iFan1);
}
...
word Bdc_SpfdFindBest(Vec_Wrd_t* vDivs, Vec_Int_t* vWeights, word F0, word F1, int* pCost) {
    ...
    Extra_PrintHex(stdout, (unsigned*)&FuncBest, 6);
    printf("\n");
    return FuncBest;
}
...
int Bdc_SpfdDecomposeTestOne(word t, Vec_Wrd_t* vDivs, Vec_Int_t* vWeights) {
    ...
    int i, Cost = 0;
    printf("Trying: ");
    Extra_PrintHex(stdout, (unsigned*)&t, 6);
    printf("\n");
