...
void Bdc_SuppMinimize2(Bdc_Man_t* p, Bdc_Isf_t* pIsf) {
    ...
    if (p->pPars->fVerbose)
        clk = Abc_Clock();
    // compute support
    pIsf->uSupp = Kit_TruthSupport(pIsf->puOn, p->nVars) | Kit_TruthSupport(pIsf->puOff, p->nVars);
...
void Bdc_SuppMinimize(Bdc_Man_t* p, Bdc_Isf_t* pIsf) {
    ...
    pIsf->uSupp = 0;
    for (v = 0; v < p->nVars; v++) {
        if (!Kit_TruthVarInSupport(pIsf->puOn, p->nVars, v) && !Kit_TruthVarInSupport(pIsf->puOff, p->nVars, v))
            continue;
        if (Kit_TruthVarIsVacuous(pIsf->puOn, pIsf->puOff, p->nVars, v)) {
            Kit_TruthExist(pIsf->puOn, p->nVars, v);
            Kit_TruthExist(pIsf->puOff, p->nVars, v);
