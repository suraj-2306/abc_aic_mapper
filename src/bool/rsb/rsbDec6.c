...
void Rsb_DecPrintTable(word* pCexes, int nGs, int nGsAll, Vec_Int_t* vTries) {
    ...
    for (c = 0; iStart < Vec_IntSize(vTries); c++) {
        ...
        for (i = 0; i < 4; i++)
            pCands[i] = -1;
        nCands = 0;
        Vec_IntForEachEntryStart(vTries, Cand, i, iStart) if (Cand == -1) {
            iStart = i + 1;
            break;
        }
        else pCands[nCands++] = Cand;
        assert(nCands <= 4);
        // print them
        for (i = 0; i < 4; i++)
            if (pCands[i] >= 0)
...
void Rsb_DecPrintFunc(Rsb_Man_t* p, unsigned Truth4, word* f, word** ppGs, int nGs, int nVarsAll) {
    ...
    Kit_DsdPrintFromTruth((unsigned*)&wIsop, nVars);
    printf("\n");

    // print functions
    printf("Func   : ");
    Abc_TtPrintBinary(f, nVarsAll); //printf( "\n" );
    Kit_DsdPrintFromTruth((unsigned*)f, nVarsAll);
    printf("\n");
    for (i = 0; i < nGs; i++) {
        printf("Div%3d : ", i);
        Kit_DsdPrintFromTruth((unsigned*)ppGs[i], nVarsAll);
        printf("\n");
    }
    printf("Solution : ");
