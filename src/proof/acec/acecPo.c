...
void Gia_PolynPrintStats(Vec_Wec_t* vPolyn) {
    ...
    Vec_IntForEachEntry(vCountsN, Entry, i) if (Entry)
        printf("  -2^%d appears %d times\n", Abc_AbsInt(i) - 1, Entry);
    Vec_IntForEachEntry(vCountsP, Entry, i) if (Entry)
        printf("  +2^%d appears %d times\n", Abc_AbsInt(i) - 1, Entry);
...
static inline int Gia_PolynBuildAdd(Hsh_VecMan_t* pHashC, Hsh_VecMan_t* pHashM, Vec_Int_t* vCoefs, Vec_Wec_t* vLit2Mono, Vec_Int_t* vTempC, Vec_Int_t* vTempM) {
...
***********************************************************************/
static inline int Gia_PolynHandleOne(Hsh_VecMan_t* pHashC, Hsh_VecMan_t* pHashM, Vec_Int_t* vCoefs, Vec_Wec_t* vLit2Mono, Vec_Int_t* vTempC, Vec_Int_t* vTempM, int iMono, int iLitOld, int iLitNew0, int iLitNew1) {
...
static inline void Gia_PolynPrepare4(Vec_Int_t* vTempC[4], Vec_Int_t* vTempM[4], Vec_Int_t* vConst, Vec_Int_t* vMono, int iObj, int iFan0, int iFan1) {
    ...
    Vec_IntForEachEntry(vMono, Entry, k) if (Entry != iObj) for (i = 0; i < 4; i++)
...
Vec_Wec_t* Gia_PolynBuildNew(Gia_Man_t* pGia, Vec_Wec_t* vSign, Vec_Int_t* vRootLits, int nExtra, Vec_Int_t* vLeaves, Vec_Int_t* vNodes, int fSigned, int fVerbose, int fVeryVerbose) {
    ...
    Vec_IntForEachEntryReverse(vNodes, iObj, i) {
        ...
        Vec_IntForEachEntry(vArray, iMono, k) if ((iConst = Vec_IntEntry(vCoefs, iMono)) > 0) {
            Vec_Int_t* vArrayC = Hsh_VecReadEntry(pHashC, iConst);
            Vec_Int_t* vArrayM = Hsh_VecReadEntry(pHashM, iMono);
