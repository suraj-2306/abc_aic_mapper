...
Vec_Wec_t* Gia_PolynComputeMap(Vec_Int_t* vAdds, int nObjs) {
    ...
    Vec_Wec_t* vMap = Vec_WecStart(nObjs);
    int i;
...
Vec_Int_t* Gia_PolynCoreOrder_int(Gia_Man_t* pGia, Vec_Int_t* vAdds, Vec_Wec_t* vMap, Vec_Int_t* vRoots, Vec_Int_t** pvIns) {
    ...
    while (1) {
        ...
        Vec_IntForEachEntry(vRoots, Entry1, i) {
            ...
            Vec_IntForEachEntryDouble(vLevel, Index, Entry2, k) if (Vec_BitEntry(vIsRoot, Entry2)) break;
    ...
    if (pvIns) {
        *pvIns = Vec_IntAlloc(Vec_BitSize(vIsRoot));
        Vec_BitForEachEntryStart(vIsRoot, Driver, i, 1) if (Driver)
            Vec_IntPush(*pvIns, i);
    }
    Vec_BitFree(vIsRoot);
    return vOrder;
...
void Gia_PolyCollectRoots_rec(Vec_Int_t* vAdds, Vec_Wec_t* vMap, Vec_Bit_t* vMarks, int iBox, Vec_Int_t* vRoots) {
    ...
    for (k = 0; k < 3; k++) {
        ...
        if (Carry == 0)
            continue;
        Vec_IntForEachEntryDouble(vLevel, Index, Sum, i) if (Vec_IntEntry(vAdds, 6 * Index + 4) == Carry && !Vec_BitEntry(vMarks, Sum)) {
...
Gia_Man_t* Gia_PolynCoreDupTree(Gia_Man_t* p, Vec_Int_t* vAddCos, Vec_Int_t* vLeaves, Vec_Int_t* vNodes, int fAddCones) {
    ...
    if (fAddCones) {
        ...
        Gia_ManForEachObjVec(vLeaves, p, pObj, i)
            pObj->Value
            = Gia_PolynCoreDupTreePlus_rec(pNew, p, pObj);
    } else {
        Gia_ManForEachObjVec(vLeaves, p, pObj, i)
            pObj->Value
            = Gia_ManAppendCi(pNew);
