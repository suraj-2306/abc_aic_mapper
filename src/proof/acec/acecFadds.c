...
int Dtc_ObjComputeTruth(Gia_Man_t* p, int iObj, int* pCut, int* pTruth) {
    unsigned Truth, Truths[3] = {0xAA, 0xCC, 0xF0};
    int i;
    for (i = 1; i <= pCut[0]; i++)
    ...
    if (Truth == 0x96 || Truth == 0x69)
        return 1;
    if (Truth == 0xE8 || Truth == 0xD4 || Truth == 0xB2 || Truth == 0x71 || Truth == 0x17 || Truth == 0x2B || Truth == 0x4D || Truth == 0x8E)
        return 2;
    return 0;
}
...
void Gia_ManCollectOneChain(Gia_Man_t* p, Vec_Int_t* vFadds, int iFaddTop, Vec_Int_t* vMap, Vec_Int_t* vChain) {
    int iFadd;
    Vec_IntClear(vChain);
    for (iFadd = iFaddTop; iFadd >= 0 && !Gia_ObjIsTravIdCurrentId(p, Vec_IntEntry(vFadds, 5 * iFadd + 3)) && !Gia_ObjIsTravIdCurrentId(p, Vec_IntEntry(vFadds, 5 * iFadd + 4));
...
Vec_Wec_t* Gia_ManCollectTopmost(Gia_Man_t* p, Vec_Int_t* vFadds, Vec_Int_t* vMap, int nFaddMin) {
    ...
    Dtc_ForEachFadd(vFadds, i) if ((iFadd = Vec_IntEntry(vMap, Vec_IntEntry(vFadds, 5 * i + 0))) >= 0)
...
void Gia_ManDupFadd(Gia_Man_t* pNew, Gia_Man_t* p, Vec_Int_t* vChain, Vec_Int_t* vFadds, Vec_Int_t* vMap, Vec_Wec_t* vChains, Vec_Int_t* vMap2Chain, Vec_Int_t* vTruths) {
    ...
    Vec_IntForEachEntry(vChain, iFadd, i) for (k = 0; k < 3; k++) {
        if (i && !k) continue;
