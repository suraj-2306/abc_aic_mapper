...
void Acec_VerifyBoxLeaves(Acec_Box_t* pBox, Vec_Bit_t* vIgnore) {
    ...
    Vec_WecForEachLevel(pBox->vLeafLits, vLevel, i)
        Vec_IntForEachEntry(vLevel, iLit, k) if (Gia_ObjIsAnd(Gia_ManObj(pBox->pGia, Abc_Lit2Var(iLit))) && !Vec_BitEntry(vIgnore, Abc_Lit2Var(iLit)))
...
void Acec_TreeFilterOne(Gia_Man_t* p, Vec_Int_t* vAdds, Vec_Int_t* vTree) {
    ...
    Gia_ManForEachAnd(p, pObj, i) {
        if (Vec_BitEntry(vIsRoot, Gia_ObjFaninId0(pObj, i)) || Vec_BitEntry(vIsRoot, Gia_ObjFaninId1(pObj, i)) || Vec_BitEntry(vMarked, Gia_ObjFaninId0(pObj, i)) || Vec_BitEntry(vMarked, Gia_ObjFaninId1(pObj, i)))
...
void Acec_TreeFilterOne2(Gia_Man_t* p, Vec_Int_t* vAdds, Vec_Int_t* vTree) {
    ...
    // mark TFI of leaves
    Gia_ManForEachAnd(p, pObj, i) if (Vec_BitEntry(vIsLeaf, i))
...
void Acec_TreeVerifyPhases2(Gia_Man_t* p, Vec_Int_t* vAdds, Vec_Wec_t* vBoxes) {
    ...
    Vec_WecForEachLevel(vBoxes, vLevel, i)
        Vec_IntForEachEntry(vLevel, Box, k) for (n = 0; n < 3; n++) {
...
void Acec_TreeVerifyConnections(Gia_Man_t* p, Vec_Int_t* vAdds, Vec_Wec_t* vBoxes) {
    ...
    // count fanouts
    Vec_WecForEachLevel(vBoxes, vLevel, i)
        Vec_IntForEachEntry(vLevel, Box, k) for (n = 0; n < 3; n++) if (Vec_IntEntry(vCounts, Vec_IntEntry(vAdds, 6 * Box + n)) != -1)
    ...
    if (Vec_IntCountLarger(vCounts, 1) == 0)
        ...
        Vec_IntForEachEntry(vCounts, Box, i) if (Box > 1)
...
Acec_Box_t* Acec_CreateBox(Gia_Man_t* p, Vec_Int_t* vAdds, Vec_Int_t* vTree) {
    ...
    Vec_WecForEachLevelReverse(pBox->vAdds, vLevel, i)
        Vec_IntForEachEntry(vLevel, Box, k) if (!Vec_BitEntry(vIsLeaf, Vec_IntEntry(vAdds, 6 * Box + 4))) {
