...
Vec_Int_t* Acec_RewriteTop(Gia_Man_t* p, Acec_Box_t* pBox) {
    ...
    Vec_WecForEachLevel(pBox->vRootLits, vLevel, iStart) if (Abc_Lit2Var(Vec_IntEntry(vLevel, 0)) == Driver) break;
...
Gia_Man_t* Acec_RewriteReplace(Gia_Man_t* p, Vec_Int_t* vRes) {
    ...
    Gia_ManForEachCo(pNew, pObj, i) if (Gia_ObjPhase(pObj) != Gia_ObjPhase(Gia_ManCo(p, i)))
