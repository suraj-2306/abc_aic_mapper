...
int Acec_ManCheckCarryMap(Gia_Man_t* p, int Carry, Vec_Int_t* vAdds, Vec_Int_t* vCarryMap) {
    ...
    assert(iBox >= 0);
    return Vec_IntEntry(vCarryMap, Vec_IntEntry(vAdds, 6 * iBox + 0)) >= 0 || Vec_IntEntry(vCarryMap, Vec_IntEntry(vAdds, 6 * iBox + 1)) >= 0 || Vec_IntEntry(vCarryMap, Vec_IntEntry(vAdds, 6 * iBox + 2)) >= 0;
}
...
Vec_Int_t* Acec_ManPoolTopMost(Gia_Man_t* p, Vec_Int_t* vAdds) {
    ...
    for (i = 0; 6 * i < Vec_IntSize(vAdds); i++)
        if (!Vec_BitEntry(vMarks, Vec_IntEntry(vAdds, 6 * i + 3)) && !Vec_BitEntry(vMarks, Vec_IntEntry(vAdds, 6 * i + 4)))
            Vec_IntPush(vTops, i);
