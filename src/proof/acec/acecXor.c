...
void Acec_CheckXors(Gia_Man_t* p, Vec_Int_t* vXors) {
    ...
    Vec_IntForEachEntry(vCount2, Entry, i) if (Entry > 1)
...
Vec_Int_t* Acec_OrderTreeRoots(Gia_Man_t* p, Vec_Int_t* vAdds, Vec_Int_t* vXorRoots, Vec_Int_t* vRanks) {
    ...
    //Vec_IntPrint( vMove );
    // find reodering
    Vec_IntForEachEntry(vMove, Entry, i) if (Entry == -1 && Vec_IntFind(vMove, i) >= 0) break;
    assert(i < Vec_IntSize(vMove));
    while (1) {
        Vec_IntPush(vOrder, Vec_IntEntry(vXorRoots, i));
        Entry = i;
        Vec_IntForEachEntry(vMove, This, i) if (This == Entry) break;
