...
int Ssw_ManSweepLatch(Ssw_Man_t* p) {
    ...
    Saig_ManForEachLo(p->pAig, pObj, i) {
        ...
        if (Ssw_ObjIsConst1Cand(p->pAig, pObj))
            ...
            Vec_PtrForEachEntry(Aig_Obj_t*, vClass, pTemp, k) if (Aig_ObjRepr(p->pAig, pTemp) == pObj) {
        ...
        if (p->pPars->nSatVarMax && p->pMSat->nSatVars > p->pPars->nSatVarMax && p->nRecycleCalls > p->pPars->nRecycleCalls) {
