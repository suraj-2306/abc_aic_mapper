...
void Ssw_SmlSavePatternAigPhase(Ssw_Man_t* p, int f) {
    ...
    Aig_ManForEachCi(p->pAig, pObj, i) if (Aig_ObjPhaseReal(Ssw_ObjFrame(p, pObj, f)))
...
void Ssw_SmlSavePatternAig(Ssw_Man_t* p, int f) {
    ...
    int i;
    memset(p->pPatWords, 0, sizeof(unsigned) * p->nPatWords);
    Aig_ManForEachCi(p->pAig, pObj, i) if (Ssw_ManGetSatVarValue(p, pObj, f))
        Abc_InfoSetBit(p->pPatWords, i);
}
...
int Ssw_ManSweepBmc(Ssw_Man_t* p) {
    ...
    for (f = 0; f < p->pPars->nFramesK; f++) {
        ...
        Saig_ManForEachLo(p->pAig, pObj, i)
            p->fRefined
            |= Ssw_ManSweepNode(p, pObj, f, 1, NULL);
        // sweep internal nodes
