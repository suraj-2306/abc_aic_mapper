...
int Ssw_ManSweepResimulateDynLocal(Ssw_Man_t* p, int f) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, p->vResimClasses, pRepr, i)
        RetValue2
        += Ssw_ClassesRefineOneClass(p->ppClasses, pRepr, 1);
...
int Ssw_ManSweepDyn(Ssw_Man_t* p) {
    ...
    Aig_ManForEachObj(p->pAig, pObj, i) {
        ...
        if (p->pMSat->pSat == NULL || (p->pPars->nSatVarMax2 && p->pMSat->nSatVars > p->pPars->nSatVarMax2 && p->nRecycleCalls > p->pPars->nRecycleCalls2)) {
            // resimulate
            if (p->nPatterns > 0) {
                p->iNodeLast = i;
