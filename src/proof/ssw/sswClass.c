...
***********************************************************************/
void Ssw_ClassesSetData(Ssw_Cla_t* p, void* pManData, unsigned (*pFuncNodeHash)(void*, Aig_Obj_t*), // returns hash key of the node
                        int (*pFuncNodeIsConst)(void*, Aig_Obj_t*),                                 // returns 1 if the node is a constant
                        int (*pFuncNodesAreEqual)(void*, Aig_Obj_t*, Aig_Obj_t*))                   // returns 1 if nodes are equal up to a complement
{
    p->pManData = pManData;
...
void Ssw_ClassesCheck(Ssw_Cla_t* p) {
    ...
    Aig_ManForEachObj(p->pAig, pObj, i)
        nCands1
        += Ssw_ObjIsConst1Cand(p->pAig, pObj);
...
void Ssw_ClassesPrint(Ssw_Cla_t* p, int fVeryVerbose) {
    ...
    Aig_ManForEachObj(p->pAig, pObj, i) if (Ssw_ObjIsConst1Cand(p->pAig, pObj))
        Abc_Print(1, "%d(%d,%d,%d) ", pObj->Id, pObj->Level,
                  Aig_SupportSize(p->pAig, pObj), Aig_NodeMffcSupp(p->pAig, pObj, 0, NULL));
    Abc_Print(1, "}\n");
...
void Ssw_ClassesRemoveNode(Ssw_Cla_t* p, Aig_Obj_t* pObj) {
    ...
    if (p->pClassSizes[pRepr->Id] == 2) {
        ...
        Ssw_ClassForEachNode(p, pRepr, pTemp, i) if (pTemp != pObj)
            p->pId2Class[pRepr->Id][k++]
            = pTemp;
...
int Ssw_ClassesPrepareRehash(Ssw_Cla_t* p, Vec_Ptr_t* vCands, int fConstCorr) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vCands, pObj, i) {
        ...
        if (ppTable[iEntry] == NULL) {
            ...
            if (Ssw_ObjNext(ppNexts, pRepr) == NULL) { // this will be the second entry
                p->pClassSizes[pRepr->Id]++;
                nEntries++;
            }
...
Ssw_Cla_t* Ssw_ClassesPrepare(Aig_Man_t* pAig, int nFramesK, int fLatchCorr, int fConstCorr, int fOutputCorr, int nMaxLevs, int fVerbose) {
    ...
    if (fOutputCorr) {
        ...
        Aig_ManForEachObj(p->pAig, pObj, i)
            pObj->fMarkB
            = 0;
        Saig_ManForEachPo(p->pAig, pObj, i) if (Aig_ObjIsCand(Aig_ObjFanin0(pObj)))
            Aig_ObjFanin0(pObj)
                ->fMarkB
            = 1;
        Aig_ManForEachObj(p->pAig, pObj, i) if (pObj->fMarkB)
            Vec_PtrPush(vCands, pObj);
        Aig_ManForEachObj(p->pAig, pObj, i)
            pObj->fMarkB
            = 0;
    ...
    for (i = 1; i < nIters; i++) {
        ...
        Vec_PtrClear(vCands);
        Aig_ManForEachObj(p->pAig, pObj, k) if (Ssw_ObjIsConst1Cand(p->pAig, pObj))
            Vec_PtrPush(vCands, pObj);
...
int Ssw_ClassesRefineOneClass(Ssw_Cla_t* p, Aig_Obj_t* pReprOld, int fRecursive) {
    ...
    Ssw_ClassForEachNode(p, pReprOld, pObj, i) if (p->pFuncNodesAreEqual(p->pManData, pReprOld, pObj))
        Vec_PtrPush(p->vClassOld, pObj);
    else Vec_PtrPush(p->vClassNew, pObj);
...
int Ssw_ClassesRefine(Ssw_Cla_t* p, int fRecursive) {
    ...
    Ssw_ManForEachClass(p, ppClass, i)
        nRefis
        += Ssw_ClassesRefineOneClass(p, ppClass[0], fRecursive);
    return nRefis;
}
...
int Ssw_ClassesRefineGroup(Ssw_Cla_t* p, Vec_Ptr_t* vReprs, int fRecursive) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vReprs, pObj, i)
        nRefis
        += Ssw_ClassesRefineOneClass(p, pObj, fRecursive);
    return nRefis;
}
...
int Ssw_ClassesRefineConst1Group(Ssw_Cla_t* p, Vec_Ptr_t* vRoots, int fRecursive) {
    ...
    Vec_PtrClear(p->vClassNew);
    Vec_PtrForEachEntry(Aig_Obj_t*, vRoots, pObj, i) if (!p->pFuncNodeIsConst(p->pManData, pObj))
