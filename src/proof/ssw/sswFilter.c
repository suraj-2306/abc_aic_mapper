...
void Ssw_ManRefineByFilterSim(Ssw_Man_t* p, int nFrames) {
    ...
    Saig_ManForEachLi(p->pAig, pObj, i)
        pObj->fMarkB
        = Abc_InfoHasBit(p->pPatWords, Saig_ManPiNum(p->pAig) + i);
...
void Ssw_ManRollForward(Ssw_Man_t* p, int nFrames) {
    ...
    Saig_ManForEachLi(p->pAig, pObj, i) if (pObj->fMarkB ^ Abc_InfoHasBit(p->pPatWords, Saig_ManPiNum(p->pAig) + i))
...
void Ssw_ManFindStartingState(Ssw_Man_t* p, Abc_Cex_t* pCex) {
    ...
    // record the new pattern
    Saig_ManForEachLo(p->pAig, pObj, i) if (pObj->fMarkB ^ Abc_InfoHasBit(p->pPatWords, Saig_ManPiNum(p->pAig) + i))
        Abc_InfoXorBit(p->pPatWords, Saig_ManPiNum(p->pAig) + i);
}
    ...
    for (f = 0; f < p->pPars->nFramesK; f++) {
        ...
        if (f == p->nFrames - 1) {
            ...
            for (f1 = 0; f1 < p->nFrames; f1++) {
                Aig_ManForEachObj(p->pAig, pObj, i)
                    pNodeToFrames[2 * p->nFrames * pObj->Id + f1]
