...
void Gia_ManDomTest(Gia_Man_t* p) {
    ...
    Gia_ManForEachAnd(p, pObj, i) if (Vec_IntEqual(Vec_WecEntry(vSupps, i), Vec_WecEntry(vDomeds, i)))
        Vec_IntPush(vDoms, i);
    Vec_WecFree(vSupps);
    Vec_WecFree(vDomeds);
...
void Gia_ManCountFanoutlessFlops(Gia_Man_t* p) {
    ...
    int Counter = 0;
    Gia_ManCreateRefs(p);
    Gia_ManForEachRo(p, pObj, i) if (Gia_ObjRefNum(p, pObj) == 0)
        Counter++;
...
int Abs_GiaSortNodes(Gia_Man_t* p, Vec_Int_t* vSupp) {
    ...
    int i, RetValue;
    Gia_ManForEachObjVec(vSupp, p, pObj, i) if (i < nSize && Gia_ObjRefNum(p, pObj) == 0 && !Gia_ObjIsRo(p, pObj)) // add removable leaves
    {
        assert(pObj->fMark1);
        Vec_IntPush(vSupp, Gia_ObjId(p, pObj));
    }
    RetValue = Vec_IntSize(vSupp) - nSize;
    Gia_ManForEachObjVec(vSupp, p, pObj, i) if (i < nSize && !(Gia_ObjRefNum(p, pObj) == 0 && !Gia_ObjIsRo(p, pObj))) // add non-removable leaves
        Vec_IntPush(vSupp, Gia_ObjId(p, pObj));
...
int Abs_GiaCheckTruth(word* pTruth, int nSize, int nSize0) {
    ...
    if (nSize0 == 2) {
        for (i = 0; i < nStr; i++)
            if (((unsigned)pStr[i] & 0xF) == 0x0 || (((unsigned)pStr[i] >> 4) & 0xF) == 0x0 || ((unsigned)pStr[i] & 0xF) == 0xF || (((unsigned)pStr[i] >> 4) & 0xF) == 0xF)
                return 0;
        return 1;
    }
    assert(nSize0 >= 3);
