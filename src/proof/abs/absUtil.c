...
Vec_Int_t* Gia_VtaConvertFromGla(Gia_Man_t* p, Vec_Int_t* vGla, int nFrames) {
    ...
    for (i = 0; i < nFrames; i++)
        for (k = 0; k <= i; k++)
            Vec_IntForEachEntry(vGla, Entry, j) if (Entry)
...
Vec_Int_t* Gia_FlaConvertToGla(Gia_Man_t* p, Vec_Int_t* vFla) {
    ...
    Gia_ManForEachPi(p, pObj, i)
        Gia_ObjSetTravIdCurrent(p, pObj);
    Gia_ManForEachRo(p, pObj, i) if (!Vec_IntEntry(vFla, i))
        Gia_ObjSetTravIdCurrent(p, pObj);
    ...
    Gia_ManForEachPo(p, pObj, i)
        Gia_FlaConvertToGla_rec(p, Gia_ObjFanin0(pObj), vGla);
    Gia_ManForEachRi(p, pObj, i) if (Vec_IntEntry(vFla, i))
        Gia_FlaConvertToGla_rec(p, Gia_ObjFanin0(pObj), vGla);
    return vGla;
}
...
Vec_Int_t* Gia_GlaConvertToFla(Gia_Man_t* p, Vec_Int_t* vGla) {
    ...
    Gia_ManForEachRo(p, pObj, i) if (Vec_IntEntry(vGla, Gia_ObjId(p, pObj)))
...
int Gia_GlaCountNodes(Gia_Man_t* p, Vec_Int_t* vGla) {
    Gia_Obj_t* pObj;
    int i, Count = 0;
    Gia_ManForEachAnd(p, pObj, i) if (Vec_IntEntry(vGla, Gia_ObjId(p, pObj)))
        Count++;
    return Count;
}
