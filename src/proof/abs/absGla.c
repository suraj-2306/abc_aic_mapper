...
static inline Vec_Int_t* Ga2_ObjCnf0(Ga2_Man_t* p, Gia_Obj_t* pObj) {
    assert(Ga2_ObjId(p, pObj) >= 0);
    return (Vec_Int_t*)Vec_PtrEntry(p->vCnfs, 2 * Ga2_ObjId(p, pObj));
}
static inline Vec_Int_t* Ga2_ObjCnf1(Ga2_Man_t* p, Gia_Obj_t* pObj) {
    assert(Ga2_ObjId(p, pObj) >= 0);
    return (Vec_Int_t*)Vec_PtrEntry(p->vCnfs, 2 * Ga2_ObjId(p, pObj) + 1);
}

static inline int Ga2_ObjIsAbs0(Ga2_Man_t* p, Gia_Obj_t* pObj) {
    assert(Ga2_ObjId(p, pObj) >= 0);
    return Ga2_ObjId(p, pObj) >= 0 && Ga2_ObjId(p, pObj) < p->LimAbs;
}
static inline int Ga2_ObjIsLeaf0(Ga2_Man_t* p, Gia_Obj_t* pObj) {
    assert(Ga2_ObjId(p, pObj) >= 0);
    return Ga2_ObjId(p, pObj) >= p->LimAbs && Ga2_ObjId(p, pObj) < p->LimPpi;
}
...
***********************************************************************/
int Ga2_ManBreakTree_rec(Gia_Man_t* p, Gia_Obj_t* pObj, int fFirst, int N) { // breaks a tree rooted at the node into N-feasible subtrees
    int Val0, Val1;
    if (pObj->fPhase && !fFirst)
        return 1;
...
int Ga2_ManCheckNodesAnd(Gia_Man_t* p, Vec_Int_t* vNodes) {
    Gia_Obj_t* pObj;
    int i;
    Gia_ManForEachObjVec(vNodes, p, pObj, i) if ((!Gia_ObjFanin0(pObj)->fPhase && Gia_ObjFaninC0(pObj)) || (!Gia_ObjFanin1(pObj)->fPhase && Gia_ObjFaninC1(pObj))) return 0;
    return 1;
}
...
void Ga2_ManAddAbsClauses(Ga2_Man_t* p, int f) {
    ...
    Gia_ManForEachObjVec(p->vAbs, p->pGia, pObj, i) if (i >= p->LimAbs)
...
void Ga2_ManAddToAbs(Ga2_Man_t* p, Vec_Int_t* vToAdd) {
    ...
    Gia_ManForEachObjVec(vToAdd, p->pGia, pObj, i) {
        vLeaves = Ga2_ObjLeaves(p->pGia, pObj);
        Gia_ManForEachObjVec(vLeaves, p->pGia, pFanin, k) if (Ga2_ObjId(p, pFanin) == -1)
...
void Ga2_ManShrinkAbs(Ga2_Man_t* p, int nAbs, int nValues, int nSatVars) {
    ...
    Vec_PtrForEachEntry(Vec_Int_t*, p->vId2Lit, vMap, i) {
        ...
        Vec_IntForEachEntryStart(vMap, Entry, k, p->LimAbs) if (Entry >= 2 * nSatVars)
...
Vec_Int_t* Ga2_ManAbsDerive(Gia_Man_t* p) {
    ...
    Gia_ManForEachRo(p, pObj, i) if (pObj->fPhase && Vec_IntEntry(p->vGateClasses, Gia_ObjId(p, pObj)))
        Vec_IntPush(vToAdd, Gia_ObjId(p, pObj));
    Gia_ManForEachAnd(p, pObj, i) if (pObj->fPhase && Vec_IntEntry(p->vGateClasses, i))
...
*/
    ...
    pCex->iFrame = p->pPars->iFrame;
    for (f = 0; f <= p->pPars->iFrame; f++)
        Gia_ManForEachObjVec(vMap, p->pGia, pObj, k) if (Ga2_ObjSatValue(p, pObj, f))
...
Vec_Int_t* Ga2_ManRefine(Ga2_Man_t* p) {
    ...
    k = 0;
    Gia_ManForEachObjVec(vVec, p->pGia, pObj, i) if (!Ga2_ObjIsAbs(p, pObj))
    ...
    if (p->pPars->fVerbose) {
        ...
        if (pPars->fDumpMabs) {
            ...
            }
            {
                ...
                Vec_Int_t* vMap = pAig->vGateClasses;
                pAig->vGateClasses = NULL;
                pAig->vGateClasses = Vec_IntStart(Gia_ManObjNum(pAig));
                Vec_IntWriteEntry(pAig->vGateClasses, 0, 1);
