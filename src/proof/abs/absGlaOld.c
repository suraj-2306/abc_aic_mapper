...
// object procedures
static inline int Gla_ObjId(Gla_Man_t* p, Gla_Obj_t* pObj) {
    assert(pObj > p->pObjs && pObj < p->pObjs + p->nObjs);
    return pObj - p->pObjs;
}
static inline Gla_Obj_t* Gla_ManObj(Gla_Man_t* p, int i) {
    assert(i >= 0 && i < p->nObjs);
    return i ? p->pObjs + i : NULL;
}
...
void Gia_GlaPrepareCexAndMap(Gla_Man_t* p, Abc_Cex_t** ppCex, Vec_Int_t** pvMap) {
    ...
    Gla_ManForEachObjAbs(p, pObj, i) {
        ...
        Gla_ObjForEachFanin(p, pObj, pFanin, k) if (!pFanin->fAbs)
    ...
    for (f = 0; f <= p->pPars->iFrame; f++)
        Gia_ManForEachObjVec(vMap, p->pGia, pGiaObj, k) if (Gla_ObjSatValue(p, Gia_ObjId(p->pGia, pGiaObj), f))
...
void Gia_ManRefSetAndPropFanout_rec(Gla_Man_t* p, Gia_Obj_t* pObj, int f, Vec_Int_t* vSelect, int Sign) {
    ...
    Gia_ObjForEachFanoutStatic(p->pGia, pObj, pFanout, k) {
        ...
        if (((pRef0->Value ^ Gia_ObjFaninC0(pFanout)) == 0 && pRef0->fVisit) || ((pRef1->Value ^ Gia_ObjFaninC1(pFanout)) == 0 && pRef1->fVisit) || (((pRef0->Value ^ Gia_ObjFaninC0(pFanout)) == 1 && pRef0->fVisit) && ((pRef1->Value ^ Gia_ObjFaninC1(pFanout)) == 1 && pRef1->fVisit)))
            Gia_ManRefSetAndPropFanout_rec(p, pFanout, f, vSelect, Sign);
    }
}
...
void Gla_ManVerifyUsingTerSim(Gla_Man_t* p, Vec_Int_t* vPis, Vec_Int_t* vPPis, Vec_Int_t* vRoAnds, Vec_Int_t* vCos, Vec_Int_t* vRes) {
    ...
    for (f = 0; f <= p->pPars->iFrame; f++) {
        ...
        Gia_ManForEachObjVec(vRes, p->pGia, pObj, i) if (Gla_ObjSatValue(p, Gia_ObjId(p->pGia, pObj), f))
            Gia_ObjTerSimSet1(pObj);
        else Gia_ObjTerSimSet0(pObj);
...
*/
    ...
    for (f = 0; f <= p->pPars->iFrame; f++) {
        // constant
        pRef = Gla_ObjRef(p, Gia_ManConst0(p->pGia), f);
        Gla_ObjClearRef(pRef);
        pRef->Value = 0;
        pRef->Prio = 0;
        pRef->Sign = Sign;
        // primary input
        ...
        Gia_ManForEachObjVec(vRoAnds, p->pGia, pObj, i) {
            ...
            if (p->pObj2Obj[Gia_ObjId(p->pGia, pObj)] != ~0 && Gla_ManCheckVar(p, p->pObj2Obj[Gia_ObjId(p->pGia, pObj)], f) && (int)pRef->Value != Gla_ObjSatValue(p, Gia_ObjId(p->pGia, pObj), f)) {
...
Gla_Man_t* Gla_ManStart(Gia_Man_t* pGia0, Abs_Par_t* pPars) {
    ...
    Vec_IntFree(p->pCnf->vMapping);
    p->pCnf->vMapping = vMappingNew;
    ABC_FREE(p->pCnf->pObj2Count);
    p->pCnf->pObj2Count = pObj2Count;
    ABC_FREE(p->pCnf->pObj2Clause);
    p->pCnf->pObj2Clause = pObj2Clause;

    // count the number of variables
    p->nObjs = 1;
    Gia_ManForEachObj(p->pGia, pObj, i) if (p->pCnf->pObj2Count[i] >= 0)
        pObj->Value
        = p->nObjs++;
    else pObj->Value = ~0;
...
Vec_Int_t* Gla_ManCollectPPis(Gla_Man_t* p, Vec_Int_t* vPis) {
    ...
    Gla_ManForEachObjAbs(p, pObj, i) {
        assert(pObj->fConst || pObj->fRo || pObj->fAnd);
        Gla_ObjForEachFanin(p, pObj, pFanin, k) if (!pFanin->fPi && !pFanin->fAbs)
