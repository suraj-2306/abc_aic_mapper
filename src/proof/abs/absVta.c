...
static inline Vta_Obj_t* Vta_ManObj(Vta_Man_t* p, int i) {
    assert(i >= 0 && i < p->nObjs);
    return i ? p->pObjs + i : NULL;
}
static inline int Vta_ObjId(Vta_Man_t* p, Vta_Obj_t* pObj) {
...
Abc_Cex_t* Vga_ManDeriveCex(Vta_Man_t* p) {
    ...
    Vta_ManForEachObjObj(p, pThis, pObj, i) if (Gia_ObjIsPi(p->pGia, pObj) && sat_solver2_var_value(p->pSat, Vta_ObjId(p, pThis)))
...
void Vta_ManSatVerify(Vta_Man_t* p) {
    ...
    Vta_ManForEachObj(p, pThis, i)
        pThis->Value
        = (sat_solver2_var_value(p->pSat, i) ? VTA_VAR1 : VTA_VAR0);
...
*/
    ...
    if (pTop->Prio == 0)
        ...
        Vta_ManForEachObjObjVec(vTermsToAdd, p, pThis, pObj, i) if (!Gia_ObjIsPi(p->pGia, pObj))
...
void Vga_ManRollBack(Vta_Man_t* p, int nObjOld) {
    ...
    p->nObjs = nObjOld;
    Vec_IntForEachEntry(p->vAddedNew, Entry, i) if (Entry < p->nObjs) {
        pThis = Vta_ManObj(p, Entry);
