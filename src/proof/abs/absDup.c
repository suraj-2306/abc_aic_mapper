...
Gia_Man_t* Gia_ManDupAbsFlops(Gia_Man_t* p, Vec_Int_t* vFlopClasses) {
    ...
    Gia_ManForEachRo(p, pObj, i) if (!Vec_IntEntry(vFlopClasses, i))
        pObj->Value
        = Gia_ManAppendCi(pNew);
    // create ROs
    Gia_ManForEachRo(p, pObj, i) if (Vec_IntEntry(vFlopClasses, i))
    ...
    // create RIs
    Gia_ManForEachRi(p, pObj, i) if (Vec_IntEntry(vFlopClasses, i)) {
        Gia_ManDupAbsFlops_rec(pNew, Gia_ObjFanin0(pObj));
        Gia_ManAppendCo(pNew, Gia_ObjFanin0Copy(pObj));
        nFlops++;
    }
...
Gia_Man_t* Gia_ManDupAbsGates(Gia_Man_t* p, Vec_Int_t* vGateClasses) {
    ...
    // create RIs
    Gia_ManForEachObjVec(vFlops, p, pObj, i)
        Gia_ObjRoToRi(p, pObj)
            ->Value
        = Gia_ManAppendCo(pNew, Gia_ObjFanin0Copy(Gia_ObjRoToRi(p, pObj)));
