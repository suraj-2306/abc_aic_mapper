...
static inline void Rf2_ObjPrint(Rf2_Man_t* p, Gia_Obj_t* pRoot) {
    ...
    Gia_ManForEachObjVec(p->vMap, p->pGia, pObj, i) if (!Gia_ObjIsPi(p->pGia, pObj))
...
void Rf2_ManCollect(Rf2_Man_t* p) {
    ...
    Gia_ManForEachObjVec(p->vObjs, p->pGia, pObj, i) if (Gia_ObjIsRo(p->pGia, pObj))
...
static inline int Rf2_ManCountPpis(Rf2_Man_t* p) {
    Gia_Obj_t* pObj;
    int i, Counter = 0;
    Gia_ManForEachObjVec(p->vMap, p->pGia, pObj, i) if (!Gia_ObjIsPi(p->pGia, pObj)) // this is PPI
        Counter++;
    return Counter;
}
...
static inline void Rf2_ManProcessVector(Vec_Int_t* p, int Limit) {
    ...
    k = 0;
    Vec_IntForEachEntry(p, Entry, i) if (Gia_WordCountOnes((unsigned)Entry) <= Limit)
        Vec_IntWriteEntry(p, k++, Entry);
    Vec_IntShrink(p, k);
    Vec_IntSort(p, 0);
    k = 0;
    Vec_IntForEachEntry(p, Entry, i) {
        Vec_IntForEachEntryStop(p, Entry2, j, i) if ((Entry2 & Entry) == Entry2) // Entry2 is a subset of Entry
            break;
