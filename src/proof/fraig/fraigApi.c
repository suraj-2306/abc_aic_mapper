...
Fraig_Node_t* Fraig_ManReadIthNode(Fraig_Man_t* p, int i) {
    assert(i < p->vNodes->nSize);
    return p->vNodes->pArray[i];
}
...
int Fraig_NodeComparePhase(Fraig_Node_t* p1, Fraig_Node_t* p2) {
    assert(!Fraig_IsComplement(p1));
    assert(!Fraig_IsComplement(p2));
    return p1->fInv ^ p2->fInv;
}
...
Fraig_Node_t* Fraig_NodeMux(Fraig_Man_t* p, Fraig_Node_t* pC, Fraig_Node_t* pT, Fraig_Node_t* pE) {
    ...
    pAnd1 = Fraig_NodeAndCanon(p, pC, pT);
    Fraig_Ref(pAnd1);
    pAnd2 = Fraig_NodeAndCanon(p, Fraig_Not(pC), pE);
    Fraig_Ref(pAnd2);
