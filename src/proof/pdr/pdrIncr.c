...
int IPdr_ManRestoreClauses(Pdr_Man_t* p, Vec_Vec_t* vClauses, Vec_Int_t* vMap) {
    ...
    if (vMap) {
        Pdr_Set_t* pSet;
        int j, k;
        Vec_VecForEachEntry(Pdr_Set_t*, vClauses, pSet, i, j) for (k = 0; k < pSet->nLits; k++)
            pSet->Lits[k]
            = Abc_Lit2LitV(Vec_IntArray(vMap), pSet->Lits[k]);
    }
    ...
    while (1) {
        ...
        */
        if (p->pPars->fUseAbs && p->vAbsFlops && !fRefined) {
            ...
            Vec_VecForEachEntry(Pdr_Set_t*, p->vClauses, pSet, i, j) for (k = 0; k < pSet->nLits; k++)
                Vec_IntWriteEntry(p->vAbsFlops, Abc_Lit2Var(pSet->Lits[k]), 1);
        }
