...
void Ssc_GiaSimProcessRefined(Gia_Man_t* p, Vec_Int_t* vRefined) {
    ...
    Vec_IntForEachEntry(vRefined, i, k) if (Gia_ObjIsHead(p, i))
...
int Ssc_GiaClassesRefine(Gia_Man_t* p) {
    ...
    Gia_ManForEachCand(p, pObj, i) if (Gia_ObjIsTail(p, i))
        Counter
...
void Ssc_GiaClassesCheckPairs(Gia_Man_t* p, Vec_Int_t* vDisPairs) {
    ...
    Vec_IntForEachEntryDouble(vDisPairs, iRepr, iObj, i) if (iRepr == Gia_ObjRepr(p, iObj))
