...
Aig_Man_t* Fra_FraigInduction(Aig_Man_t* pManAig, Fra_Ssw_t* pParams) {
    ...
    for (nIter = 0; p->pCla->fRefinement; nIter++) {
        ...
        if (p->pCla->fRefinement && nLitsOld == Fra_ClassesCountLits(p->pCla) && nImpsOld == (p->pCla->vImps ? Vec_IntSize(p->pCla->vImps) : 0) && nHotsOld == (p->vOneHots ? Fra_OneHotCount(p, p->vOneHots) : 0)) {
...
int Fra_FraigInductionTest(char* pFileName, Fra_Ssw_t* pParams) {
    ...
    Aig_ManForEachObj(pMan, pObj, i) if ((pRepr = pMan->pReprs[pObj->Id])) {
