...
void Fra_ClassesPostprocess(Fra_Cla_t* p) {
    ...
    k = 0;
    Vec_PtrForEachEntry(Aig_Obj_t**, p->vClasses, ppClass, i) if (ppClass[1] != NULL)
...
void Fra_ClassesSelectRepr(Fra_Cla_t* p) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t**, p->vClasses, pClass, i) {
        ...
        for (c = 0; pClass[c]; c++) {
            ...
            //            nSuppSizeCur = 1;
            if (nSuppSizeMin > nSuppSizeCur || (nSuppSizeMin == nSuppSizeCur && pNodeMin->Level > pClass[c]->Level)) {
...
Aig_Man_t* Fra_ClassesDeriveAig(Fra_Cla_t* p, int nFramesK) {
    ...
    for (f = 0; f < nFramesAll; f++) {
        ...
        Aig_ManForEachLiSeq(p->pAig, pObj, i)
            pLatches[k++]
            = Fra_ObjChild0Equ(ppEquivs, pObj);
