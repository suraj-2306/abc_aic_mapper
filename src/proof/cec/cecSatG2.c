...
static inline int Cec4_ObjSetSatId(Gia_Man_t* p, Gia_Obj_t* pObj, int Num) {
...
static inline void Cec4_ObjCleanSatId(Gia_Man_t* p, Gia_Obj_t* pObj) {
    assert(Cec4_ObjSatId(p, pObj) != -1);
...
void Cec4_RefineOneClassIter(Gia_Man_t* p, int iRepr) {
    ...
    Gia_ClassForEachObj1(p, iRepr, iRepr2) if (Cec4_ObjSimEqual(p, iRepr, iRepr2))
        iPrev
        = iRepr2;
    else break;
...
Abc_Cex_t* Cec4_ManDeriveCex(Gia_Man_t* p, int iOut, int iPat) {
    ...
    if (iPat == -1)
        return pCex;
    Gia_ManForEachCiId(p, Id, i) if (Abc_InfoHasBit((unsigned*)Cec4_ObjSim(p, Id), iPat))
...
void Cec4_ManConvertToLits(Gia_Man_t* p, Vec_Int_t* vRes) {
    ...
    int i, iRepr;
    Gia_ManSetPhase(p);
    Gia_ManForEachObj(p, pObj, i) if ((iRepr = Vec_IntEntry(vRes, i)) >= 0)
