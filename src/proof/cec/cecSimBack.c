...
int Cec_ObjSatVerify(Gia_Man_t* p, Gia_Obj_t* pRoot, int Value) {
    ...
    Gia_ManForEachCi(p, pObj, i) if (!Gia_ObjIsTravIdCurrent(p, pObj))
        Gia_ObjTerSimSetX(pObj),
        printf("x");
    else if (pObj->fMark0)
        Gia_ObjTerSimSet1(pObj),
        printf("1");
    else Gia_ObjTerSimSet0(pObj), printf("0");
    printf(" ");
...
word Cec_ManCheckSat2_rec(Gia_Man_t* p, Gia_Obj_t* pObj, word Value, Vec_Wrd_t* vValues) {
    ...
    if (Value) {
        return Cec_ManCheckSat2_rec(p, pFan0, !Gia_ObjFaninC0(pObj), vValues) && Cec_ManCheckSat2_rec(p, pFan1, !Gia_ObjFaninC1(pObj), vValues);
    }
...
int Cec_ManCheckSat_rec(Gia_Man_t* p, Gia_Obj_t* pObj, int Value) {
    ...
    pFan1 = Gia_ObjFanin1(pObj);
    if (Value) {
        return Cec_ManCheckSat_rec(p, pFan0, !Gia_ObjFaninC0(pObj)) && Cec_ManCheckSat_rec(p, pFan1, !Gia_ObjFaninC1(pObj));
    }
    if (Gia_ObjIsTravIdCurrent(p, pFan0)) // already assigned
    {
        if (Gia_ObjFaninC0(pObj) == (int)pFan0->fMark0) // justified
