...
Gia_Man_t* Cec_ManFraSpecReduction(Cec_ManFra_t* p) {
    ...
    Gia_ManForEachObj1(p->pAig, pObj, i) {
        ...
        if (Gia_ObjIsCo(pObj))
            continue;
        if (piCopies[Gia_ObjFaninId0(pObj, i)] == -1 || piCopies[Gia_ObjFaninId1(pObj, i)] == -1)
            continue;
        ...
        if (Abc_LitRegular(iNode) == Abc_LitRegular(iRepr))
            continue;
        if (p->pPars->nLevelMax && (Gia_ObjLevelId(p->pAig, i) > p->pPars->nLevelMax || Gia_ObjLevelId(p->pAig, Abc_Lit2Var(iRepr)) > p->pPars->nLevelMax))
            continue;
        if (p->pPars->fDualOut) {
...
int Cec_ManFraClassesUpdate_rec(Gia_Obj_t* pObj) {
    ...
    if (Gia_ObjIsCi(pObj) || Gia_ObjIsConst0(pObj))
        return 0;
    Result = (Cec_ManFraClassesUpdate_rec(Gia_ObjFanin0(pObj)) | Cec_ManFraClassesUpdate_rec(Gia_ObjFanin1(pObj)));
...
int Cec_ManFraClassesUpdate(Cec_ManFra_t* p, Cec_ManSim_t* pSim, Cec_ManPat_t* pPat, Gia_Man_t* pNew) {
    ...
    if (p->pPars->nDepthMax != 1) {
        ...
        Gia_ManForEachAnd(p->pAig, pObjOld, k)
            pObjOld->fMark0
            |= (Gia_ObjFanin0(pObjOld)->fMark0 | Gia_ObjFanin1(pObjOld)->fMark0);
        ...
        // clean marks
        Gia_ManForEachAnd(p->pAig, pObjOld, k) if (pObjOld->fMark1) {
            pObjOld->fMark0 = 0;
            pObjOld->fMark1 = 0;
        }
    }
