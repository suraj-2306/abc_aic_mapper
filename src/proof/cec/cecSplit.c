...
void Cec_GiaSplitExplore(Gia_Man_t* p) {
    ...
    Gia_ManForEachAnd(p, pObj, i) {
        if (!Gia_ObjRecognizeExor(pObj, &pFan0, &pFan1))
            continue;
        if (Gia_ObjRefNum(p, Gia_ObjFanin0(pObj)) > 1 && Gia_ObjRefNum(p, Gia_ObjFanin1(pObj)) > 1)
            continue;
        printf("%5d : ", Counter++);
...
int Gia_SplitCofVar2(Gia_Man_t* p, int* pnFanouts, int* pnCost) {
    ...
    if (p->pRefs == NULL)
        Gia_ManCreateRefs(p);
    Gia_ManForEachPi(p, pObj, i) if (CostBest < Gia_ObjRefNum(p, pObj))
        iBest
        = i,
        CostBest = Gia_ObjRefNum(p, pObj);
    assert(iBest >= 0);
...
Abc_Cex_t* Cec_SplitDeriveModel(Gia_Man_t* p, Cnf_Dat_t* pCnf, sat_solver* pSat) {
    ...
    Gia_ManForEachPi(p, pObj, i)
        pModel[i]
        = sat_solver_var_value(pSat, pCnf->pVarNums[Gia_ObjId(p, pObj)]);
    if (p->vCofVars)
        Vec_IntForEachEntry(p->vCofVars, iLit, i)
            pModel[Abc_Lit2Var(iLit)]
            = !Abc_LitIsCompl(iLit);
...
int Cec_GiaSplitTestInt(Gia_Man_t* p, int nProcs, int nTimeOut, int nIterMax, int LookAhead, int fVerbose, int fVeryVerbose, int fSilent) {
    ...
    for (i = 0; i < nProcs; i++) {
        ...
        ThData[i].nVars = -1;
        ThData[i].nConfs = -1;
        status = pthread_create(WorkerThread + i, NULL, Cec_GiaSplitWorkerThread, (void*)(ThData + i));
        assert(status == 0);
    }
    // look at the threads
    while (fWorkToDo) {
