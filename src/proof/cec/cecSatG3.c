...
static inline int Cec5_ObjSetSatId(Gia_Man_t* p, Gia_Obj_t* pObj, int Num) {
    assert(Cec5_ObjSatId(p, pObj) == -1);
    Gia_ObjSetCopy2Array(p, Gia_ObjId(p, pObj), Num);
    Vec_IntPush(&p->vSuppVars, Gia_ObjId(p, pObj));
    if (Gia_ObjIsCi(pObj)) Vec_IntPushTwo(&p->vCopiesTwo, Gia_ObjId(p, pObj), Num);
    assert(Vec_IntSize(&p->vVarMap) == Num);
    Vec_IntPush(&p->vVarMap, Gia_ObjId(p, pObj));
    return Num;
}
static inline void Cec5_ObjCleanSatId(Gia_Man_t* p, Gia_Obj_t* pObj) {
    assert(Cec5_ObjSatId(p, pObj) != -1);
    Gia_ObjSetCopy2Array(p, Gia_ObjId(p, pObj), -1);
}
...
void Cec5_CollectSuper_rec(Gia_Obj_t* pObj, Vec_Ptr_t* vSuper, int fFirst, int fUseMuxes) {
    ...
    if (Gia_IsComplement(pObj) || Gia_ObjIsCi(pObj) || (!fFirst && Gia_ObjValue(pObj) > 1) || (fUseMuxes && pObj->fMark0)) {
...
void Cec5_RefineOneClassIter(Gia_Man_t* p, int iRepr) {
    ...
    Gia_ClassForEachObj1(p, iRepr, iRepr2) if (Cec5_ObjSimEqual(p, iRepr, iRepr2))
        iPrev
        = iRepr2;
    else break;
...
Abc_Cex_t* Cec5_ManDeriveCex(Gia_Man_t* p, int iOut, int iPat) {
    ...
    if (iPat == -1)
        return pCex;
    Gia_ManForEachCiId(p, Id, i) if (Abc_InfoHasBit((unsigned*)Cec5_ObjSim(p, Id), iPat))
...
int Cec5_ManPackAddPatternTry(Gia_Man_t* p, int iBit, Vec_Int_t* vLits) {
    ...
    Vec_IntForEachEntry(vLits, Lit, i) {
        word* pInfo = Vec_WrdEntryP(p->vSims, p->nSimWords * Abc_Lit2Var(Lit));
        word* pPres = Vec_WrdEntryP(p->vSimsPi, p->nSimWords * Abc_Lit2Var(Lit));
        if (Abc_InfoHasBit((unsigned*)pPres, iBit) && Abc_InfoHasBit((unsigned*)pInfo, iBit) != Abc_LitIsCompl(Lit))
            return 0;
    }
    Vec_IntForEachEntry(vLits, Lit, i) {
        word* pInfo = Vec_WrdEntryP(p->vSims, p->nSimWords * Abc_Lit2Var(Lit));
...
int Cec5_ManSolveTwo(Cec5_Man_t* p, int iObj0, int iObj1, int fPhase, int* pfEasy, int fVerbose, int fEffort) {
    abctime clk;
    int nBTLimit = fEffort ? p->pPars->nBTLimitPo : (Vec_BitEntry(p->vFails, iObj0) || Vec_BitEntry(p->vFails, iObj1)) ? Abc_MaxInt(1, p->pPars->nBTLimit / 10)
                                                                                                                       : p->pPars->nBTLimit;
    ...
    *pfEasy = 0;
    ...
    p->nCallsSince++;
    if (p->nCallsSince > p->pPars->nCallsRecycle && Vec_IntSize(&p->pNew->vSuppVars) > p->pPars->nSatVarMax && p->pPars->nSatVarMax)
...
int Cec5_ManPerformSweeping(Gia_Man_t* p, Cec_ParFra_t* pPars, Gia_Man_t** ppNew, int fSimOnly, int fCbs, int approxLim, int subBatchSz, int adaRecycle) {
    ...
    if (fCbs) {
        ...
        if (pPars->fVerbose) {
            printf("cbs: clim = %4d jlim = %4d\n", pCbs->Pars.nBTLimit, pCbs->Pars.nJustLimit);
        }
    }
...
resume:
    for (; i < Gia_ManObjNum(p) && (pObj = Gia_ManObj(p, i)); i++) {
        ...
        Vec_BitWriteEntry(pMan->vCexSite, i, Vec_BitEntry(pMan->vCexSite, Gia_ObjFaninId0(pObj, i)) | Vec_BitEntry(pMan->vCexSite, Gia_ObjFaninId1(pObj, i)));
        ...
        if (Gia_ObjIsAnd(pObjNew))
            if (Vec_BitEntry(pMan->vFails, Gia_ObjFaninId0(pObjNew, Abc_Lit2Var(pObj->Value))) || Vec_BitEntry(pMan->vFails, Gia_ObjFaninId1(pObjNew, Abc_Lit2Var(pObj->Value))))
    ...
    if (ppNew) {
        Gia_ManForEachCo(p, pObj, i)
            pObj->Value
            = Gia_ManAppendCo(pMan->pNew, Gia_ObjFanin0Copy(pObj));
