...
int Cec_ManSeqResimulateCounter(Gia_Man_t* pAig, Cec_ParSim_t* pPars, Abc_Cex_t* pCex) {
    ...
    Gia_ManRandom(1);
    vSimInfo = Vec_PtrAllocSimInfo(Gia_ManRegNum(pAig) + Gia_ManPiNum(pAig) * (pPars->nFrames + pCex->iFrame + 1), 1);
...
int Cec_ManCountNonConstOutputs(Gia_Man_t* pAig) {
    ...
    if (pAig->pReprs == NULL)
        return -1;
    Gia_ManForEachPo(pAig, pObj, i) if (!Gia_ObjIsConst(pAig, Gia_ObjFaninId0p(pAig, pObj)))
        Counter++;
    return Counter;
}
...
int Cec_ManCheckNonTrivialCands(Gia_Man_t* pAig) {
    ...
    Gia_ManForEachObj(pAig, pObj, i) if (Gia_ObjIsCand(pObj) && !pObj->fMark0 && Gia_ObjRepr(pAig, i) != GIA_VOID) {
        RetValue = 1;
        break;
    }
...
int Cec_ManSeqSemiformal(Gia_Man_t* pAig, Cec_ParSmf_t* pPars) {
    ...
    for (r = 0; r < pPars->nRounds; r++) {
        ...
        vSimInfo = Vec_PtrAllocSimInfo(Gia_ManRegNum(pAig) + Gia_ManPiNum(pAig) * (nFramesReal + nAddFrames), pPars->nWords);
