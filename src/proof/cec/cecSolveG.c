...
void CecG_CollectSuper_rec(Gia_Obj_t* pObj, Vec_Ptr_t* vSuper, int fFirst, int fUseMuxes, int fUseSuper) {
    ...
    if (Gia_IsComplement(pObj) || Gia_ObjIsCi(pObj) || (!fFirst && Gia_ObjValue(pObj) > 1) || (fUseMuxes && Gia_ObjIsMuxType(pObj))) {
...
int CecG_ManSatCheckNode(Cec_ManSat_t* p, Gia_Obj_t* pObj) {
    ...
    if (p->pSat == NULL || (p->pPars->nSatVarMax && sat_solver_varnum(p->pSat) > p->pPars->nSatVarMax && p->nCallsSince > p->pPars->nCallsRecycle))
