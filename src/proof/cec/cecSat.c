...
static inline int Cec2_ObjSetSatId(Gia_Man_t* p, Gia_Obj_t* pObj, int Num) {
    assert(Cec2_ObjSatId(p, pObj) == -1);
    Gia_ObjSetCopy2Array(p, Gia_ObjId(p, pObj), Num);
    return Num;
}
static inline void Cec2_ObjCleanSatId(Gia_Man_t* p, Gia_Obj_t* pObj) {
    assert(Cec2_ObjSatId(p, pObj) != -1);
...
void Cec2_CollectSuper_rec(Gia_Man_t* p, Gia_Obj_t* pObj, Vec_Ptr_t* vSuper, int fFirst, int fUseMuxes) {
    ...
    if (Gia_IsComplement(pObj) || Gia_ObjIsCi(pObj) || (!fFirst && (p->pRefs ? Gia_ObjRefNum(p, pObj) : Gia_ObjValue(pObj)) > 1) || (fUseMuxes && pObj->fMark0)) {
...
Abc_Cex_t* Cec2_ManDeriveCex(Gia_Man_t* p, int iOut, int iPat) {
    ...
    if (iPat == -1)
        return pCex;
    Gia_ManForEachCiId(p, Id, i) if (Abc_InfoHasBit((unsigned*)Cec2_ObjSim(p, Id), iPat))
...
void Cec2_ManSimClassRefineOne(Gia_Man_t* p, int iRepr) {
    ...
    Gia_ClassForEachObj1(p, iRepr, iRepr2) if (Cec2_ObjSimEqual(p, iRepr, iRepr2))
        iPrev
        = iRepr2;
    else break;
...
int Cec2_ManPerformSweeping(Gia_Man_t* p, Cec2_Par_t* pPars, Gia_Man_t** ppNew) {
    ...
    if (ppNew) {
        Gia_ManForEachAnd(p, pObj, i) if (!~pObj->Value)
            pObj->Value
