...
int Cec_ManSimulationOne(Gia_Man_t* pAig, Cec_ParSim_t* pPars) {
    ...
    if ((pAig->pReprs == NULL && (RetValue = Cec_ManSimClassesPrepare(pSim, -1))) || (RetValue == 0 && (RetValue = Cec_ManSimClassesRefine(pSim))))
...
Gia_Man_t* Cec_ManSatSweeping(Gia_Man_t* pAig, Cec_ParFra_t* pPars, int fSilent) {
    ...
    pIni->pReprs = pAig->pReprs;
    pAig->pReprs = NULL;
    pIni->pNexts = pAig->pNexts;
    pAig->pNexts = NULL;
