...
static inline int Cec3_ObjSetSatId(Gia_Man_t* p, Gia_Obj_t* pObj, int Num) {
    assert(Cec3_ObjSatId(p, pObj) == -1);
    Gia_ObjSetCopy2Array(p, Gia_ObjId(p, pObj), Num);
    return Num;
}
static inline void Cec3_ObjCleanSatId(Gia_Man_t* p, Gia_Obj_t* pObj) {
    assert(Cec3_ObjSatId(p, pObj) != -1);
...
Abc_Cex_t* Cec3_ManDeriveCex(Gia_Man_t* p, int iOut, int iPat) {
    ...
    if (iPat == -1)
        return pCex;
    Gia_ManForEachCiId(p, Id, i) if (Abc_InfoHasBit((unsigned*)Cec3_ObjSim(p, Id), iPat))
...
void Cec3_ManSimClassRefineOne(Gia_Man_t* p, int iRepr) {
    ...
    Gia_ClassForEachObj1(p, iRepr, iRepr2) if (Cec3_ObjSimEqual(p, iRepr, iRepr2))
        iPrev
        = iRepr2;
    else break;
