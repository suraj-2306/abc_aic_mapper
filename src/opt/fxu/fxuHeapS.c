...
void Fxu_HeapSingleUpdate(Fxu_HeapSingle* p, Fxu_Single* pSingle) {
    ...
    if (FXU_HEAP_SINGLE_PARENT_EXISTS(p, pSingle) && FXU_HEAP_SINGLE_WEIGHT(pSingle) > FXU_HEAP_SINGLE_WEIGHT(FXU_HEAP_SINGLE_PARENT(p, pSingle)))
        Fxu_HeapSingleMoveUp(p, pSingle);
    else if (FXU_HEAP_SINGLE_CHILD1_EXISTS(p, pSingle) && FXU_HEAP_SINGLE_WEIGHT(pSingle) < FXU_HEAP_SINGLE_WEIGHT(FXU_HEAP_SINGLE_CHILD1(p, pSingle)))
        Fxu_HeapSingleMoveDn(p, pSingle);
    else if (FXU_HEAP_SINGLE_CHILD2_EXISTS(p, pSingle) && FXU_HEAP_SINGLE_WEIGHT(pSingle) < FXU_HEAP_SINGLE_WEIGHT(FXU_HEAP_SINGLE_CHILD2(p, pSingle)))
        Fxu_HeapSingleMoveDn(p, pSingle);
}
...
void Fxu_HeapSingleMoveDn(Fxu_HeapSingle* p, Fxu_Single* pSingle) {
    ...
    ppSingle = &FXU_HEAP_SINGLE_CURRENT(p, pSingle);
    while (FXU_HEAP_SINGLE_CHILD1_EXISTS(p, *ppSingle)) { // if Child1 does not exist, Child2 also does not exists
        ...
        if (FXU_HEAP_SINGLE_CHILD2_EXISTS(p, *ppSingle)) {
            ...
            // consider two cases
            if (FXU_HEAP_SINGLE_WEIGHT(*ppSingle) >= FXU_HEAP_SINGLE_WEIGHT(*ppChild1) && FXU_HEAP_SINGLE_WEIGHT(*ppSingle) >= FXU_HEAP_SINGLE_WEIGHT(*ppChild2)) { // Var is larger than both, skip
                break;
            } else { // Var is smaller than one of them, then swap it with larger
        ...
        {
            // consider two cases
            if (FXU_HEAP_SINGLE_WEIGHT(*ppSingle) >= FXU_HEAP_SINGLE_WEIGHT(*ppChild1)) { // Var is larger than Child1, skip
                break;
            } else { // Var is smaller than Child1, then swap them
                Fxu_HeapSingleSwap(ppSingle, ppChild1);
                // update the pointer
                ppSingle = ppChild1;
            }
