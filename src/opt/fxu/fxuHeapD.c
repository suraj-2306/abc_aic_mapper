...
void Fxu_HeapDoubleUpdate(Fxu_HeapDouble* p, Fxu_Double* pDiv) {
    ...
    if (FXU_HEAP_DOUBLE_PARENT_EXISTS(p, pDiv) && FXU_HEAP_DOUBLE_WEIGHT(pDiv) > FXU_HEAP_DOUBLE_WEIGHT(FXU_HEAP_DOUBLE_PARENT(p, pDiv)))
        Fxu_HeapDoubleMoveUp(p, pDiv);
    else if (FXU_HEAP_DOUBLE_CHILD1_EXISTS(p, pDiv) && FXU_HEAP_DOUBLE_WEIGHT(pDiv) < FXU_HEAP_DOUBLE_WEIGHT(FXU_HEAP_DOUBLE_CHILD1(p, pDiv)))
        Fxu_HeapDoubleMoveDn(p, pDiv);
    else if (FXU_HEAP_DOUBLE_CHILD2_EXISTS(p, pDiv) && FXU_HEAP_DOUBLE_WEIGHT(pDiv) < FXU_HEAP_DOUBLE_WEIGHT(FXU_HEAP_DOUBLE_CHILD2(p, pDiv)))
        Fxu_HeapDoubleMoveDn(p, pDiv);
}
...
void Fxu_HeapDoubleMoveDn(Fxu_HeapDouble* p, Fxu_Double* pDiv) {
    ...
    ppDiv = &FXU_HEAP_DOUBLE_CURRENT(p, pDiv);
    while (FXU_HEAP_DOUBLE_CHILD1_EXISTS(p, *ppDiv)) { // if Child1 does not exist, Child2 also does not exists
        ...
        if (FXU_HEAP_DOUBLE_CHILD2_EXISTS(p, *ppDiv)) {
            ...
            // consider two cases
            if (FXU_HEAP_DOUBLE_WEIGHT(*ppDiv) >= FXU_HEAP_DOUBLE_WEIGHT(*ppChild1) && FXU_HEAP_DOUBLE_WEIGHT(*ppDiv) >= FXU_HEAP_DOUBLE_WEIGHT(*ppChild2)) { // Div is larger than both, skip
                break;
            } else { // Div is smaller than one of them, then swap it with larger
        ...
        {
            // consider two cases
            if (FXU_HEAP_DOUBLE_WEIGHT(*ppDiv) >= FXU_HEAP_DOUBLE_WEIGHT(*ppChild1)) { // Div is larger than Child1, skip
                break;
            } else { // Div is smaller than Child1, then swap them
                Fxu_HeapDoubleSwap(ppDiv, ppChild1);
                // update the pointer
                ppDiv = ppChild1;
            }
