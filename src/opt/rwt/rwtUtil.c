...
Rwt_Node_t* Rwt_ManAddNode(Rwt_Man_t* p, Rwt_Node_t* p0, Rwt_Node_t* p1, int fExor, int Level, int Volume) {
    ...
    if (fExor)
        ...
        uTruth = (Rwt_IsComplement(p0) ? ~Rwt_Regular(p0)->uTruth : Rwt_Regular(p0)->uTruth) & (Rwt_IsComplement(p1) ? ~Rwt_Regular(p1)->uTruth : Rwt_Regular(p1)->uTruth) & 0xFFFF;
    // create the new node
    pNew = (Rwt_Node_t*)Mem_FixedEntryFetch(p->pMmNode);
    pNew->Id = p->vForest->nSize;
    pNew->TravId = 0;
...
void Rwt_ManLoadFromArray(Rwt_Man_t* p, int fVerbose) {
    ...
    if (fVerbose) {
        ...
        printf("The number of nodes loaded = %d.  ", nEntries);
        ABC_PRT("Loading", Abc_Clock() - clk);
    }
}
