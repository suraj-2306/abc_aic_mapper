...
int Lpk_Decompose_rec(Lpk_Man_t* pMan, Lpk_Fun_t* p) {
    ...
    if (pResDsd && (pResDsd->nBSVars == (int)p->nLutK || pResDsd->nBSVars == (int)p->nLutK - 1) && pResDsd->AreaEst <= (int)p->nAreaLim && pResDsd->DelayEst <= (int)p->nDelayLim) {
        clk = Abc_Clock();
    ...
    if (pResMux && pResMux->nSuppSizeS <= (int)p->nLutK && pResMux->nSuppSizeL <= (int)p->nLutK)
        ...
        if (pResMux->AreaEst < pResDsd->AreaEst || (pResMux->AreaEst == pResDsd->AreaEst && pResMux->nSuppSizeL < pResDsd->nSuppSizeL) || (pResMux->AreaEst == pResDsd->AreaEst && pResMux->nSuppSizeL == pResDsd->nSuppSizeL && pResMux->DelayEst < pResDsd->DelayEst))
            pResDsd = NULL;
        else
            pResMux = NULL;
    }
