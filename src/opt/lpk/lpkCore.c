...
Abc_Obj_t* Abc_NtkLutMinDecompose(Abc_Ntk_t* pNtk, Vec_Ptr_t* vLeaves, word* pTruth, int nLutSize, int Required) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pObj, i) {
        pObj->Level = 0;
        Abc_ObjForEachFanin(pObj, pFanin, k) if (pObj->Level < pFanin->Level)
            pObj->Level
            = pFanin->Level;
        pObj->Level++;
    }
...
int Lpk_ResynthesizeNode(Lpk_Man_t* p) {
    ...
    for (i = 0; i < p->nEvals; i++) {
        ...
        if (Kit_DsdNonDsdSizeMax(pDsdNtk) > p->pPars->nLutSize && nSuppSize >= ((int)pCut->nNodes - (int)pCut->nNodesDup - 1) * (p->pPars->nLutSize - 1) + 1) {
            Kit_DsdNtkFree(pDsdNtk);
            continue;
        }
