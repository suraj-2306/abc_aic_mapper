...
int Sfm_NodeResubSolve(Sfm_Ntk_t* p, int iNode, int f, int fRemoveOnly) {
    ...
    Sfm_ObjForEachFanin(p, iNode, iFanin, i) if (i != f)
        Vec_IntPush(p->vDivIds, Sfm_ObjSatVar(p, iFanin));
    else iFaninRem = iFanin;
    assert(iFaninRem != -1);
    // find fanin to skip
    ...
    while (1) {
        ...
        Vec_WrdForEachEntry(p->vDivCexes, uSign, iVar) if (uSign == uMask && Vec_IntEntry(p->vDivs, iVar) != iFaninSkip) break;
...
finish:
    if (p->pPars->fVeryVerbose) {
        ...
        Kit_DsdPrintFromTruth((unsigned*)&uTruth, Vec_IntSize(p->vDivIds));
        printf("\n");
    }
    if (iVar == -1)
        p->nRemoves++;
...
int Sfm_NodeResub(Sfm_Ntk_t* p, int iNode) {
    ...
    Sfm_ObjForEachFanin(p, iNode, iFanin, i) if (Sfm_ObjIsNode(p, iFanin) && Sfm_ObjFanoutNum(p, iFanin) == 1) {
