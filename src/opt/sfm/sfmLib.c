...
int Sfm_LibFindComplInputGate(Vec_Wrd_t* vFuncs, int iGate, int nFanins, int iFanin, int* piFaninNew) {
    ...
    word uTruth, uTruthSwap;
    int i;
    assert(iFanin >= 0 && iFanin < nFanins);
    if (piFaninNew) *piFaninNew = iFanin;
    Vec_WrdForEachEntry(vFuncs, uTruth, i) if (uTruth == uTruthFlip) return i;
    if (iFanin - 1 >= 0) {
        if (piFaninNew) *piFaninNew = iFanin - 1;
        uTruthSwap = Abc_Tt6SwapAdjacent(uTruthFlip, iFanin - 1);
        Vec_WrdForEachEntry(vFuncs, uTruth, i) if (uTruth == uTruthSwap) return i;
    }
    if (iFanin + 1 < nFanins) {
        if (piFaninNew) *piFaninNew = iFanin + 1;
...
void Sfm_LibTruth8Two(Mio_Cell2_t* pCellBot, Mio_Cell2_t* pCellTop, int InTop, word* pRes) {
    word uTruthBot[4], *puFanins[SFM_SUPP_MAX];
    int i, k;
...
int Sfm_LibFindAreaMatch(Sfm_Lib_t* p, word* pTruth, int nFanins, int* piObj) {
    ...
    if (iFunc == -1)
        return -1;
    Sfm_LibForEachSuper(p, pObj, iFunc) break;
    if (piObj)
        *piObj = pObj - p->pObjs;
    return pObj->Area;
}
int Sfm_LibFindDelayMatches(Sfm_Lib_t* p, word* pTruth, int* pFanins, int nFanins, Vec_Ptr_t* vGates, Vec_Ptr_t* vFans) {
    ...
    Vec_PtrClear(vFans);
    // look for gate
    assert(!Abc_TtIsConst0(pTruth, p->nWords) && !Abc_TtIsConst1(pTruth, p->nWords) && !Abc_TtEqual(pTruth, s_Truth8[0], p->nWords) && !Abc_TtOpposite(pTruth, s_Truth8[0], p->nWords));
