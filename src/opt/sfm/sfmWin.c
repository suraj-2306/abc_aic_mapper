...
int Sfm_NtkCheckOverlap_rec(Sfm_Ntk_t* p, int iThis, int iNode) {
    ...
    Sfm_ObjForEachFanin(p, iThis, iFanin, i) if (Sfm_NtkCheckOverlap_rec(p, iFanin, iNode)) return 1;
    return 0;
}
...
static inline int Sfm_NtkCheckRoot(Sfm_Ntk_t* p, int iNode, int nLevelMax) {
    ...
    Sfm_ObjForEachFanout(p, iNode, iFanout, i) if (Sfm_ObjIsPo(p, iFanout) || Sfm_ObjLevel(p, iFanout) > nLevelMax) //|| !Sfm_NtkCheckOverlap(p, iFanout, iNode) )
        return 1;
    return 0;
}
...
static inline int Sfm_ObjIsUseful(Sfm_Ntk_t* p, int iNode) {
    ...
    if (!Sfm_ObjIsFixed(p, iNode))
        return 1;
    Sfm_ObjForEachFanout(p, iNode, iFanout, i) if (!Sfm_ObjIsFixed(p, iFanout)) return 1;
    return 0;
}
...
int Sfm_NtkCollectTfi_rec(Sfm_Ntk_t* p, int iNode, Vec_Int_t* vNodes) {
    ...
    if (Sfm_ObjIsTravIdCurrent(p, iNode))
        return 0;
    Sfm_ObjSetTravIdCurrent(p, iNode);
    Sfm_ObjForEachFanin(p, iNode, iFanin, i) if (Sfm_NtkCollectTfi_rec(p, iFanin, vNodes)) return 1;
    Vec_IntPush(vNodes, iNode);
...
int Sfm_NtkCreateWindow(Sfm_Ntk_t* p, int iNode, int fVerbose) {
    ...
    if (!p->pPars->nWinSizeMax || Vec_IntSize(p->vDivs) < p->pPars->nWinSizeMax + 0) {
        Sfm_NtkIncrementTravId2(p);
        Vec_IntForEachEntry(p->vDivs, iTemp, i) if (!p->pPars->nWinSizeMax || Vec_IntSize(p->vDivs) < p->pPars->nWinSizeMax + 0)
...
*/
    ...
    // compact divisors
    k = 0;
    Vec_IntForEachEntry(p->vDivs, iTemp, i) if (!Sfm_ObjIsTravIdCurrent2(p, iTemp) && Sfm_ObjIsUseful(p, iTemp))
        Vec_IntWriteEntry(p->vDivs, k++, iTemp);
    ...
    if (p->pPars->nTfoLevMax > 0 && !Sfm_NtkCheckRoot(p, iNode, Sfm_ObjLevel(p, iNode) + p->pPars->nTfoLevMax)) {
        ...
        Sfm_NtkIncrementTravId(p);
        Vec_IntForEachEntry(p->vRoots, iTemp, i) if (Sfm_NtkCollectTfi_rec(p, iTemp, p->vOrder)) {
        ...
        if (Vec_IntSize(p->vRoots) > 0)
            Vec_IntForEachEntry(p->vTfo, iTemp, i) if (Sfm_NtkCollectTfi_rec(p, iTemp, p->vOrder)) {
                Vec_IntClear(p->vRoots);
                Vec_IntClear(p->vTfo);
                Vec_IntClear(p->vOrder);
                break;
        ...
        if (Vec_IntSize(p->vRoots) > 0)
            Vec_IntForEachEntry(p->vDivs, iTemp, i) if (Sfm_NtkCollectTfi_rec(p, iTemp, p->vOrder)) {
                Vec_IntClear(p->vRoots);
                Vec_IntClear(p->vTfo);
                Vec_IntClear(p->vOrder);
                break;
