...
word Sfm_ComputeInterpolant(Sfm_Ntk_t* p) {
    ...
    while (1) {
        ...
        for (i = 0; i < nFinal; i++) {
            ...
            iVar = Vec_IntFind(p->vDivIds, Abc_Lit2Var(pFinal[i]));
            assert(iVar >= 0);
    ...
    Vec_IntForEachEntry(p->vDivVars, iVar, i) if (Vec_IntEntry(p->vValues, i) ^ sat_solver_var_value(p->pSat, iVar)) // insert 1
    {
