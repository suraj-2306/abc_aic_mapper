...
void Sfm_DecStop(Sfm_Dec_t* p) {
    ...
    Abc_Obj_t* pObj;
    int i;
    Abc_NtkForEachNode(pNtk, pObj, i) if ((int)pObj->Level != Abc_ObjLevelNew(pObj))
...
static inline word Sfm_ObjSimulate2(Abc_Obj_t* pObj) {
    ...
    int i;
    word uFanins[6];
    Abc_ObjForEachFanin(pObj, pFanin, i) if ((pFanin->iTemp & SFM_MASK_PIVOT))
        uFanins[i]
        = Sfm_DecObjSim2(p, pFanin);
    else uFanins[i] = Sfm_DecObjSim(p, pFanin);
    return Exp_Truth6(Abc_ObjFaninNum(pObj), vExpr, uFanins);
}
...
static inline word Sfm_ObjFindCareSet(Abc_Ntk_t* pNtk, Vec_Int_t* vRoots) {
    ...
    Abc_NtkForEachObjVec(vRoots, pNtk, pObj, i)
        Res
        |= Sfm_DecObjSim(p, pObj) ^ Sfm_DecObjSim2(p, pObj);
    return Res;
}
...
int Sfm_DecMffcArea(Abc_Ntk_t* pNtk, Vec_Int_t* vMffc) {
    ...
    Abc_NtkForEachObjVec(vMffc, pNtk, pObj, i)
        nAreaMffc
        += Scl_Flt2Int(Mio_GateReadArea((Mio_Gate_t*)pObj->pData));
    return nAreaMffc;
}
...
*/
    ...
    if (Var == -1 && fCofactor) {
        ...
        Vec_IntForEachEntryReverse(&p->vObjInMffc, Var, i) if (Vec_IntFind(&p->vObjDec, Var) == -1) break;
...
int Abc_NtkDfsCheck_rec(Abc_Obj_t* pObj, Abc_Obj_t* pPivot) {
    ...
    Abc_ObjForEachFanin(pObj, pFanin, i) if (!Abc_NtkDfsCheck_rec(pFanin, pPivot)) return 0;
    return 1;
}
void Abc_NtkDfsReverseOne_rec(Abc_Obj_t* pObj, Vec_Int_t* vTfo, int nLevelMax, int nFanoutMax) {
    ...
    if (Abc_ObjFanoutNum(pObj) <= nFanoutMax) {
        Abc_ObjForEachFanout(pObj, pFanout, i) if (Abc_ObjIsCo(pFanout) || Abc_ObjLevel(pFanout) > nLevelMax) break;
...
int Abc_NtkDfsOne_rec(Abc_Obj_t* pObj, Vec_Int_t* vTfi, int nLevelMin, int CiLabel) {
    ...
    Abc_ObjForEachFanin(pObj, pFanin, i)
        pObj->iTemp
        |= Abc_NtkDfsOne_rec(pFanin, vTfi, nLevelMin, CiLabel);
    Vec_IntPush(vTfi, Abc_ObjId(pObj));
...
void Sfm_DecMarkMffc(Abc_Obj_t* pPivot, int nLevelMin, int nMffcMax, int fVeryVerbose, Vec_Int_t* vMffc, Vec_Int_t* vInMffc, Sfm_Tim_t* pTim, Sfm_Mit_t* pMit) {
    Abc_Obj_t *pFanin, *pFanin2, *pFanin3, *pObj;
    int i, k, n;
    assert(nMffcMax > 0);
    ...
    if (pMit != NULL) {
        ...
        Abc_ObjForEachFanin(pPivot, pFanin, i) if (Sfm_DecNodeIsMffcInput2(pFanin, nLevelMin, pMit, pPivot))
            Vec_IntPushUnique(vInMffc, Abc_ObjId(pFanin));
        Abc_ObjForEachFanin(pPivot, pFanin, i)
            Abc_ObjForEachFanin(pFanin, pFanin2, k) if (Sfm_DecNodeIsMffcInput2(pFanin2, nLevelMin, pMit, pPivot))
                Vec_IntPushUnique(vInMffc, Abc_ObjId(pFanin2));
        Abc_ObjForEachFanin(pPivot, pFanin, i)
            Abc_ObjForEachFanin(pFanin, pFanin2, k)
                Abc_ObjForEachFanin(pFanin2, pFanin3, n) if (Sfm_DecNodeIsMffcInput2(pFanin3, nLevelMin, pMit, pPivot))
        ...
        // collect MFFC inputs (these are low-delay nodes close to the pivot)
        Vec_IntClear(vInMffc);
        Abc_ObjForEachFanin(pPivot, pFanin, i) if (Sfm_DecNodeIsMffcInput(pFanin, nLevelMin, pTim, pPivot))
            Vec_IntPushUnique(vInMffc, Abc_ObjId(pFanin));
        Abc_ObjForEachFanin(pPivot, pFanin, i)
            Abc_ObjForEachFanin(pFanin, pFanin2, k) if (Sfm_DecNodeIsMffcInput(pFanin2, nLevelMin, pTim, pPivot))
                Vec_IntPushUnique(vInMffc, Abc_ObjId(pFanin2));
        Abc_ObjForEachFanin(pPivot, pFanin, i)
            Abc_ObjForEachFanin(pFanin, pFanin2, k)
                Abc_ObjForEachFanin(pFanin2, pFanin3, n) if (Sfm_DecNodeIsMffcInput(pFanin3, nLevelMin, pTim, pPivot))
                    Vec_IntPushUnique(vInMffc, Abc_ObjId(pFanin3));
...
*/
        ...
        // collect MFFC
        Abc_ObjForEachFanin(pPivot, pFanin, i) if (Sfm_DecNodeIsMffc(pFanin, nLevelMin) && Vec_IntSize(vMffc) < nMffcMax)
            Vec_IntPushUnique(vMffc, Abc_ObjId(pFanin));
        Abc_ObjForEachFanin(pPivot, pFanin, i) if (Sfm_DecNodeIsMffc(pFanin, nLevelMin) && Vec_IntSize(vMffc) < nMffcMax)
            Abc_ObjForEachFanin(pFanin, pFanin2, k) if (Sfm_DecNodeIsMffc(pFanin2, nLevelMin) && Vec_IntSize(vMffc) < nMffcMax)
                Vec_IntPushUnique(vMffc, Abc_ObjId(pFanin2));
        Abc_ObjForEachFanin(pPivot, pFanin, i) if (Sfm_DecNodeIsMffc(pFanin, nLevelMin) && Vec_IntSize(vMffc) < nMffcMax)
            Abc_ObjForEachFanin(pFanin, pFanin2, k) if (Sfm_DecNodeIsMffc(pFanin2, nLevelMin) && Vec_IntSize(vMffc) < nMffcMax)
                Abc_ObjForEachFanin(pFanin2, pFanin3, n) if (Sfm_DecNodeIsMffc(pFanin3, nLevelMin) && Vec_IntSize(vMffc) < nMffcMax)
        ...
        Vec_IntClear(vInMffc);
        Abc_NtkForEachObjVec(vMffc, pPivot->pNtk, pObj, i)
            Abc_ObjForEachFanin(pObj, pFanin, k) if (Abc_NodeIsTravIdCurrent(pFanin) && pFanin->iTemp == SFM_MASK_PI)
                Vec_IntPushUnique(vInMffc, Abc_ObjId(pFanin));
...
int Sfm_DecExtract(Abc_Ntk_t* pNtk, Sfm_Par_t* pPars, Abc_Obj_t* pPivot, Vec_Int_t* vRoots, Vec_Int_t* vGates, Vec_Wec_t* vFanins, Vec_Int_t* vMap, Vec_Int_t* vTfi, Vec_Int_t* vTfo, Vec_Int_t* vMffc, Vec_Int_t* vInMffc, Sfm_Tim_t* pTim, Sfm_Mit_t* pMit) {
    ...
    // compute roots
    Vec_IntClear(vRoots);
    Abc_NtkForEachObjVec(vTfo, pNtk, pObj, i) if (pObj->iTemp != Abc_ObjFanoutNum(pObj))
    ...
    Abc_NtkForEachObjVecStart(vTfi, pNtk, pObj, i, nTfiSize) if (pObj->iTemp != SFM_MASK_INPUT)
        Abc_ObjForEachFanin(pObj, pFanin, k) if (pFanin->iTemp == SFM_MASK_INPUT)
            pFanin->iTemp
        = SFM_MASK_FANIN;
    ...
    Vec_IntClear(vMap);
    Vec_IntClear(vGates);
    Abc_NtkForEachObjVec(vTfi, pNtk, pObj, i) if (pObj->iTemp == SFM_MASK_PI)
    ...
    if (fVeryVerbose)
        printf("\nSides:\n");
    Abc_NtkForEachObjVec(vTfi, pNtk, pObj, i) if (pObj->iTemp == (SFM_MASK_PI | SFM_MASK_INPUT) || pObj->iTemp == SFM_MASK_FANIN)
    ...
    if (fVeryVerbose)
        printf("\nTFO:\n");
    Abc_NtkForEachObjVec(vTfi, pNtk, pObj, i) if (pObj->iTemp >= SFM_MASK_MFFC)
...
Abc_Obj_t* Sfm_DecInsert(Abc_Ntk_t* pNtk, Abc_Obj_t* pPivot, int Limit, Vec_Int_t* vGates, Vec_Wec_t* vFanins, Vec_Int_t* vMap, Vec_Ptr_t* vGateHandles, int GateBuf, int GateInv, Vec_Wrd_t* vFuncs, Vec_Int_t* vNewNodes, Sfm_Mit_t* pMit) {
    ...
    if (Limit + 1 == Vec_IntSize(vGates)) {
        ...
        if (Gate == GateBuf) {
            ...
            Abc_Obj_t* pFanout;
            Abc_ObjForEachFanout(pPivot, pFanout, i) if (!Abc_ObjIsNode(pFanout) || Sfm_LibFindComplInputGate(vFuncs, Mio_GateReadValue((Mio_Gate_t*)pFanout->pData), Abc_ObjFaninNum(pFanout), Abc_NodeFindFanin(pFanout, pPivot), NULL) == -1) break;
            // update fanouts
...
void Abc_NtkAreaOpt2(Sfm_Dec_t* p) {
    ...
    Abc_NtkForEachCo(p->pNtk, pObj, i) if (Abc_ObjIsNode(Abc_ObjFanin0(pObj)) && !Abc_ObjFanin0(pObj)->fMarkB) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vFront, pObj, i) {
        ...
        if (pObjNew != NULL) {
            ...
            pObjNew = pObj;
        Abc_ObjForEachFanin(pObjNew, pFanin, k) if (Abc_ObjIsNode(pFanin) && Abc_ObjFaninNum(pObjNew) > 0 && !pFanin->fMarkB) {
            pFanin->fMarkB = 1;
