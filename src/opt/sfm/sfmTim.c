...
static inline int Sfm_TimArrMaxId(Sfm_Tim_t* p, int Id) {
    int* a = Sfm_TimArrId(p, Id);
    return Abc_MaxInt(a[0], a[1]);
}

static inline int Sfm_TimArrMax(Sfm_Tim_t* p, Abc_Obj_t* pNode) {
    int* a = Sfm_TimArr(p, pNode);
    return Abc_MaxInt(a[0], a[1]);
}
static inline void Sfm_TimSetReq(Sfm_Tim_t* p, Abc_Obj_t* pNode, int t) {
    int* r = Sfm_TimReq(p, pNode);
    r[0] = r[1] = t;
}
static inline int Sfm_TimSlack(Sfm_Tim_t* p, Abc_Obj_t* pNode) {
    int *r = Sfm_TimReq(p, pNode), *a = Sfm_TimArr(p, pNode);
...
int Sfm_TimTrace(Sfm_Tim_t* p) {
    ...
    Abc_NtkForEachCo(p->pNtk, pObj, i)
        Delay
        = Abc_MaxInt(Delay, Sfm_TimArrMax(p, Abc_ObjFanin0(pObj)));
...
int Sfm_TimPriorityNodes(Sfm_Tim_t* p, Vec_Int_t* vCands, int Window) {
    ...
    Vec_IntClear(vCands);
    Vec_WecForEachLevel(&p->vLevels, vLevel, i)
        Abc_NtkForEachObjVec(vLevel, p->pNtk, pObj, k) if (!pObj->fMarkA)
