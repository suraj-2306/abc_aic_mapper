...
void Fxch_DivSepareteCubes(Vec_Int_t* vDiv,
    ...
    else Vec_IntPush(vCube0, Abc_Lit2Var(Lit));
...
int Fxch_DivRemoveLits(Vec_Int_t* vCube0,
    ...
    Vec_IntForEachEntry(vDiv, Lit, i) if (Abc_LitIsCompl(Abc_Lit2Var(Lit)))
        CountN
        += Vec_IntRemove1(vCube0, Abc_Lit2Var(Lit));
    else CountP += Vec_IntRemove1(vCube0, Abc_Lit2Var(Lit));

    Vec_IntForEachEntry(vDiv, Lit, i)
        Count
        += Vec_IntRemove1(vCube1, Abc_Lit2Var(Lit));
    ...
    if (Vec_IntSize(vDiv) == 4) {
        ...
        if (Lit0 == Abc_LitNot(Lit1) && ret == 2) {
            *fCompl = 1;
            ...
            Vec_IntForEachEntry(vDiv, Lit, i)
                ret
                += Vec_IntRemove1(vCube0, (Abc_Lit2Var(Lit) ^ (fCompl && i > 1)));

            Vec_IntForEachEntry(vDiv, Lit, i)
                ret
                += Vec_IntRemove1(vCube1, (Abc_Lit2Var(Lit) ^ (fCompl && i > 1)));
        }
    }
