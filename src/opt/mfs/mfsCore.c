...
void Abc_NtkMfsPowerResub(Mfs_Man_t* p, Mfs_Par_t* pPars) {
    ...
    Abc_NtkForEachNode(pNtk, pNode, k) {
        ...
        // Abc_NtkMfsEdgePower( p, pNode );
        // try replacing area critical fanins
        Abc_ObjForEachFanin(pNode, pFanin, i) if (Abc_MfsObjProb(p, pFanin) >= 0.35 && Abc_NtkMfsSolveSatResub(p, pNode, i, 0, 0)) continue;
    }
    ...
    Abc_NtkForEachNode(pNtk, pNode, k) {
        ...
        Abc_ObjForEachFanin(pNode, pFanin, i) if (Abc_MfsObjProb(p, pFanin) >= 0.2 && Abc_NtkMfsSolveSatResub(p, pNode, i, 1, 0)) continue;
    }
    /*
    Abc_NtkForEachNode( pNtk, pNode, k )
    {
