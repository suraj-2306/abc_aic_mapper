...
int Sbc_ManAddInternalToPath_rec(Gia_Man_t* p, int iObj, Vec_Bit_t* vPath) {
    ...
    Gia_LutForEachFanin(p, iObj, iFan, k)
        Value
        |= Sbc_ManAddInternalToPath_rec(p, iFan, vPath);
    if (Value)
...
void Sbc_ManCriticalPath_rec(Gia_Man_t* p, int* pLevels, int iObj, int LevelFan, Vec_Bit_t* vPath, int Slack) {
    ...
    assert(Gia_ObjIsAnd(pObj));
    Gia_LutForEachFanin(p, iObj, iFan, k) if (pLevels[iFan] + Slack >= LevelFan - 1)
...
Vec_Bit_t* Sbc_ManCriticalPath(Gia_Man_t* p) {
    ...
    Gia_ManIncrementTravId(p);
    Gia_ManForEachCoDriverId(p, iDriver, k) if ((pLevels[iDriver] == nLevels) && iDriver)
...
void Sbc_ManDelayTrace(Gia_Man_t* p) {
    ...
    Gia_ManForEachCoDriverId(p, iFan, i) if (pLevels[iFan] == nLevels)
    ...
    Gia_ManForEachLut(p, i)
        Gia_LutForEachFanin(p, i, iFan, k)
            nEdgesAll
        += (Vec_BitEntry(vPath, i) && Vec_BitEntry(vPath, iFan));
