...
Sbd_Man_t* Sbd_ManStart(Gia_Man_t* pGia, Sbd_Par_t* pPars) {
    ...
    Gia_ManRandom(1);
    Gia_ManForEachCiId(pGia, Id, i) for (w = 0; w < p->pPars->nWords; w++)
        Sbd_ObjSim0(p, Id)[w]
        = Gia_ManRandomW(0);
    // cut enumeration
    if (pPars->fMoreCuts)
...
int Sbd_ManWindow(Sbd_Man_t* p, int Pivot) {
    ...
    Vec_IntForEachEntry(p->vTfo, Node, i) if (Abc_LitIsCompl(Node)) // root
...
void Sbd_ManPrintObj(Sbd_Man_t* p, int Pivot) {
    ...
    Vec_IntForEachEntryStop(p->vWinObjs, Id, i, nDivs)
        printf("%3d : ", Id),
        Extra_PrintBinary(stdout, (unsigned*)Sbd_ObjSim0(p, Id), 64), printf("\n");
...
int Sbd_ManDelay(Sbd_Man_t* p) {
    ...
    Gia_ManForEachCoDriverId(p->pGia, Id, i)
        Delay
        = Abc_MaxInt(Delay, Vec_IntEntry(p->vLutLevs, Id));
    return Delay;
}
...
void Sbd_ManFindCut(Sbd_Man_t* p, int Node, Vec_Int_t* vCutLits) {
    ...
    // collect
    pCut[0] = 0;
    Gia_ManForEachObjVec(vCutLits, p->pGia, pTemp, i) if (pTemp->fMark1) {
