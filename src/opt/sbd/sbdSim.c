...
void Sbd_GiaSimRoundBack2(Gia_Man_t* p) {
    ...
    Gia_ManForEachCoId(p, Id, i) for (w = 0; w < nWords; w++)
        Sbd_ObjSims(p, Id)[w]
        = Gia_ManRandomW(0);
    // transfer to nodes
...
int Sbd_GiaSatOne_rec(Gia_Man_t* p, Gia_Obj_t* pObj, int Value, int fFirst, int iPat) {
    ...
    if (Value)
        return Sbd_GiaSatOne_rec(p, Gia_ObjFanin0(pObj), !Gia_ObjFaninC0(pObj), fFirst, iPat) && Sbd_GiaSatOne_rec(p, Gia_ObjFanin1(pObj), !Gia_ObjFaninC1(pObj), fFirst, iPat);
    if (fFirst)
...
void Sbd_GiaSimRound(Gia_Man_t* p, int fTry, Vec_Int_t** pvMap) {
    ...
    if (fTry) {
        ...
        Gia_ManForEachCiId(p, Id, i) for (w = 0; w < nWords; w++)
            Sbd_ObjSims(p, Id)[w]
            = Gia_ManRandomW(0) << !w;
    }
