...
void Sbd_StoSaveBestDelayCut(Sbd_Sto_t* p, int iObj, int* pCut) {
    Sbd_Cut_t* pCutBest = p->ppCuts[p->iCutBest];
    int i;
    assert(iObj == p->Pivot);
...
int Sbd_StoObjBestCut(Sbd_Sto_t* p, int iObj, int nSize, int* pLeaves) {
    ...
    for (i = 0; i < p->nCutsR; i++) {
        ...
        if (nSize && (int)p->ppCuts[i]->nLeaves != nSize)
            continue;
        if ((int)p->ppCuts[i]->nLeaves > p->nLutSize && (int)p->ppCuts[i]->nSlowLeaves <= 1 && (int)p->ppCuts[i]->nTopLeaves <= p->nLutSize - 1 && (pCutBest == NULL || Sbd_CutCompare2(pCutBest, p->ppCuts[i]) == 1))
            pCutBest = p->ppCuts[i];
    }
    if (fVerbose && iObj % 1000 == 0) {
