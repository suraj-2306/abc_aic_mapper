...
static inline int Dau_Dsd6DecomposeTripleVarsInner(Dau_Dsd_t* p, word* pTruth, int* pVars, int nVars, int v, unsigned uSupports) {
    ...
    if (fEqual0 || fEqual1) {
        ...
        Abc_TtSwapVars(pTruth, nVars, iVar1, nVars - 1);
        nVars--;
        // remove iVar0
        iVar0 = Dau_DsdFindVarDef(pVars, nVars, VarId);
        ABC_SWAP(int, pVars[iVar0], pVars[nVars - 1]);
        Abc_TtSwapVars(pTruth, nVars, iVar0, nVars - 1);
        nVars--;
        // find the new var
...
int Dau_Dsd6DecomposeTripleVars(Dau_Dsd_t* p, word* pTruth, int* pVars, int nVars) {
    ...
    while (1) {
        ...
        for (v = nVars - 1; v >= 0; v--) {
            ...
            if (Abc_TtSuppOnlyOne(uSupports & (~uSupports >> 1) & 0x55555555) && Abc_TtSuppOnlyOne(~uSupports & (uSupports >> 1) & 0x55555555)) // one unique variable in each cofactor
            {
