...
void Dtt_PrintMulti1(Dtt_Man_t* p) {
    ...
    for (n = 0; n < 16; n++) {
        ...
        Vec_IntForEachEntryStart(vTimes, Entry, i, 1) if (Prev == Entry)
            Count++;
        else {
            assert(Prev < Entry);
    ...
    for (i = 0; i < p->nPerms; i++) {
        for (k = 0; k < p->nComps; k++) {
            if (Dtt_Check(tFun, tGoal, (unsigned)tCur, pType)) {
                ...
                {
                    if (!NPout)
                        Dtt_ProcessType(pType, 3);
                    else
                        Dtt_ProcessType(pType, 2);
                }
                *NP = 0;
                if (!NPout) {
                    for (j = 0; j < 5; j++)
...
void Dtt_EnumerateLf(int nVars, int nNodeMax, int fDelay, int fMulti, int fVerbose, char* pFileName) {
    ...
    Dtt_Man_t* p = Dtt_ManAlloc(nVars, fMulti);
    int n, i, j;
    ...
    for (n = 1; n <= nNodeMax; n++) {
        for (i = 0, j = n - 1; i < n; i++, j = j - 1 + fDelay)
            if (i <= j) {
