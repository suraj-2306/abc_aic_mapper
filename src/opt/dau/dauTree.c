...
static inline int Dss_FunWordNum(Dss_Fun_t* p) {
    assert(p->nFans >= 2);
    return (p->nFans + 4) / 8 + (((p->nFans + 4) & 7) > 0);
}
...
static inline int Dss_ObjFaninC(Dss_Obj_t* pObj, int i) {
...
static inline Dss_Obj_t* Dss_ObjFanin(Vec_Ptr_t* p, Dss_Obj_t* pObj, int i) {
    assert(i < (int)pObj->nFans);
    return Dss_VecObj(p, Abc_Lit2Var(pObj->pFans[i]));
}
static inline Dss_Obj_t* Dss_ObjChild(Vec_Ptr_t* p, Dss_Obj_t* pObj, int i) {
    assert(i < (int)pObj->nFans);
    return Dss_Lit2Obj(p, pObj->pFans[i]);
}
...
#define Dss_VecForEachNode(vVec, pObj, i)                                                                            \
    Vec_PtrForEachEntry(Dss_Obj_t*, vVec, pObj, i) if (pObj->Type == DAU_DSD_CONST0 || pObj->Type == DAU_DSD_VAR) {} \
    else
...
int Dss_NtkCollectPerm_rec(Dss_Ntk_t* p, Dss_Obj_t* pObj, int* pPermDsd, int* pnPerms) {
    ...
    Dss_ObjForEachChild(p->vObjs, pObj, pChild, k) if (Dss_NtkCollectPerm_rec(p, pChild, pPermDsd, pnPerms))
        pObj->pFans[k]
        = (unsigned char)Abc_LitRegular((int)pObj->pFans[k]);
    return 0;
}
...
unsigned* Dss_ObjHashLookup(Dss_Man_t* p, int Type, Vec_Int_t* vFaninLits, word* pTruth) {
    ...
    for (; *pSpot; pSpot = (unsigned*)Vec_IntEntryP(p->vNexts, pObj->Id)) {
        pObj = Dss_VecObj(p->vObjs, *pSpot);
        if ((int)pObj->Type == Type && (int)pObj->nFans == Vec_IntSize(vFaninLits) && !memcmp(pObj->pFans, Vec_IntArray(vFaninLits), sizeof(int) * pObj->nFans) && (pTruth == NULL || !memcmp(Dss_ObjTruth(pObj), pTruth, (size_t)Abc_TtByteNum(pObj->nFans)))) // equal
            return pSpot;
    }
    return pSpot;
}
...
Dss_Ent_t** Dss_ManCacheLookup(Dss_Man_t* p, Dss_Ent_t* pEnt) {
    ...
    for (; *pSpot; pSpot = &(*pSpot)->pNext) {
        if ((*pSpot)->iDsd0 == pEnt->iDsd0 && (*pSpot)->iDsd1 == pEnt->iDsd1 && (*pSpot)->nShared == pEnt->nShared && !memcmp((*pSpot)->pShared, pEnt->pShared, sizeof(char) * 2 * pEnt->nShared)) // equal
        {
...
int Dss_ManCheckNonDec_rec(Dss_Man_t* p, Dss_Obj_t* pObj) {
    ...
    if (pObj->Type == DAU_DSD_PRIME)
        return 1;
    Dss_ObjForEachFanin(p->vObjs, pObj, pFanin, i) if (Dss_ManCheckNonDec_rec(p, pFanin)) return 1;
    return 0;
}
...
int Dss_ObjCheckTransparent(Dss_Man_t* p, Dss_Obj_t* pObj) {
    ...
    if (pObj->Type == DAU_DSD_AND)
        return 0;
    if (pObj->Type == DAU_DSD_XOR) {
        Dss_ObjForEachFanin(p->vObjs, pObj, pFanin, i) if (Dss_ObjCheckTransparent(p, pFanin)) return 1;
        return 0;
    }
    if (pObj->Type == DAU_DSD_MUX) {
