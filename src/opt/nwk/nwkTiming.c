...
float Nwk_NodeComputeArrival(Nwk_Obj_t* pObj, int fUseSorting) {
    ...
    if (pLutLib == NULL) {
        Nwk_ObjForEachFanin(pObj, pFanin, k) if (tArrival < Nwk_ObjArrival(pFanin) + 1.0)
            tArrival
            = Nwk_ObjArrival(pFanin) + 1.0;
    } else if (!pLutLib->fVarPinDelays) {
        pDelays = pLutLib->pLutDelays[Nwk_ObjFaninNum(pObj)];
        Nwk_ObjForEachFanin(pObj, pFanin, k) if (tArrival < Nwk_ObjArrival(pFanin) + pDelays[0])
            tArrival
            = Nwk_ObjArrival(pFanin) + pDelays[0];
    } else {
        pDelays = pLutLib->pLutDelays[Nwk_ObjFaninNum(pObj)];
        if (fUseSorting) {
            Nwk_ManDelayTraceSortPins(pObj, pPinPerm, pPinDelays);
            Nwk_ObjForEachFanin(pObj, pFanin, k) if (tArrival < Nwk_ObjArrival(Nwk_ObjFanin(pObj, pPinPerm[k])) + pDelays[k])
                tArrival
                = Nwk_ObjArrival(Nwk_ObjFanin(pObj, pPinPerm[k])) + pDelays[k];
        } else {
            Nwk_ObjForEachFanin(pObj, pFanin, k) if (tArrival < Nwk_ObjArrival(pFanin) + pDelays[k])
                tArrival
                = Nwk_ObjArrival(pFanin) + pDelays[k];
        }
    }
...
float Nwk_NodePropagateRequired(Nwk_Obj_t* pObj, int fUseSorting) {
    ...
    if (pLutLib == NULL) {
        ...
        Nwk_ObjForEachFanin(pObj, pFanin, k) if (Nwk_ObjRequired(pFanin) > tRequired)
...
float Nwk_ManDelayTraceLut(Nwk_Man_t* pNtk) {
    ...
    tArrival = -TIM_ETERNITY;
    Nwk_ManForEachPo(pNtk, pObj, i) if (tArrival < Nwk_ObjArrival(pObj))
        tArrival
        = Nwk_ObjArrival(pObj);
...
int Nwk_ObjLevelNew(Nwk_Obj_t* pObj) {
    ...
    Nwk_ObjForEachFanin(pObj, pFanin, i)
        Level
        = Abc_MaxInt(Level, Nwk_ObjLevel(pFanin));
