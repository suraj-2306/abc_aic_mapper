...
void Aig_ManSpeedupNode(Nwk_Man_t* pNtk, Aig_Man_t* pAig, Nwk_Obj_t* pNode, Vec_Ptr_t* vLeaves, Vec_Ptr_t* vTimes) {
    ...
    Vec_PtrForEachEntry(Nwk_Obj_t*, vLeaves, pObj, i)
        Vec_PtrForEachEntry(Nwk_Obj_t*, vLeaves, pObj2, k) if (i != k && Aig_Regular((Aig_Obj_t*)pObj->pCopy) == Aig_Regular((Aig_Obj_t*)pObj2->pCopy)) {
    ...
    Vec_PtrForEachEntry(Nwk_Obj_t*, vTimes, pObj, k) for (nSkip = (1 << k), i = 0; i < nCofs; i += 2 * nSkip) {
        pAnd = (Aig_Obj_t*)pObj->pCopy;
...
unsigned Nwk_ManDelayTraceTCEdges(Nwk_Man_t* pNtk, Nwk_Obj_t* pNode, float tDelta, int fUseLutLib) {
    ...
    if (pLutLib == NULL) {
        Nwk_ObjForEachFanin(pNode, pFanin, k) if (tRequired < Nwk_ObjArrival(pFanin) + 1.0 + tDelta)
            uResult
            |= (1 << k);
    } else if (!pLutLib->fVarPinDelays) {
        pDelays = pLutLib->pLutDelays[Nwk_ObjFaninNum(pNode)];
        Nwk_ObjForEachFanin(pNode, pFanin, k) if (tRequired < Nwk_ObjArrival(pFanin) + pDelays[0] + tDelta)
            uResult
            |= (1 << k);
    } else {
        pDelays = pLutLib->pLutDelays[Nwk_ObjFaninNum(pNode)];
        Nwk_ManDelayTraceSortPins(pNode, pPinPerm, pPinDelays);
        Nwk_ObjForEachFanin(pNode, pFanin, k) if (tRequired < Nwk_ObjArrival(Nwk_ObjFanin(pNode, pPinPerm[k])) + pDelays[k] + tDelta)
            uResult
            |= (1 << pPinPerm[k]);
    }
    return uResult;
...
Aig_Man_t* Nwk_ManSpeedup(Nwk_Man_t* pNtk, int fUseLutLib, int Percentage, int Degree, int fVerbose, int fVeryVerbose) {
    ...
    if (fVerbose) {
        ...
        Nwk_ManForEachNode(pNtk, pNode, i) {
            Nwk_ObjForEachFanin(pNode, pFanin, k) if (!Nwk_ObjIsCi(pFanin) && Nwk_ObjSlack(pFanin) < tDelta)
                Counter++;
    ...
    Nwk_ManForEachNode(pNtk, pNode, i) {
        ...
        nTimeCris = 0;
        Nwk_ObjForEachFanin(pNode, pFanin, k) if (!Nwk_ObjIsCi(pFanin) && (puTCEdges[pNode->Id] & (1 << k)))
            nTimeCris++;
        ...
        if (nTimeCris) {
            Nwk_ObjForEachFanin(pNode, pFanin, k) if (!Nwk_ObjIsCi(pFanin) && (puTCEdges[pNode->Id] & (1 << k)))
                Nwk_ObjForEachFanin(pFanin, pFanin2, k2) if (puTCEdges[pFanin->Id] & (1 << k2))
    ...
    Aig_ManForEachNode(pAig, pAnd, i) if (Aig_ObjEquiv(pAig, pAnd)) {
