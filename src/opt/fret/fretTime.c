...
void trace2(Abc_Obj_t* pObj) {
    ...
    print_node(pObj);
    Abc_ObjForEachFanin(pObj, pNext, i) if (pNext->Level >= pObj->Level - 1) {
        trace2(pNext);
        break;
    }
}
...
void Abc_FlowRetime_ConstrainConserv_forw(Abc_Ntk_t* pNtk) {
    ...
    Vec_PtrForEachEntryReverse(Abc_Obj_t*, vNodes, pObj, i) {
        ...
        Abc_ObjForEachFanin(pObj, pNext, j) {
            if (Abc_NodeIsTravIdCurrent(pNext) && pObj->Level < pNext->Level)
    ...
    Vec_PtrForEachEntryReverse(Abc_Obj_t*, vNodes, pObj, i) {
        ...
        Abc_ObjForEachFanin(pObj, pNext, j) if (Abc_NodeIsTravIdCurrent(pNext) && !Abc_ObjIsLatch(pNext))
            assert(pNext->fMarkB);
    }
    Vec_PtrForEachEntryReverse(Abc_Obj_t*, vNodes, pObj, i)
        pObj->fMarkB
...
void Abc_FlowRetime_ConstrainConserv_back(Abc_Ntk_t* pNtk) {
    ...
    Vec_PtrForEachEntryReverse(Abc_Obj_t*, vNodes, pObj, i) {
        ...
        Abc_ObjForEachFanout(pObj, pNext, j) {
            ...
            if (Abc_NodeIsTravIdCurrent(pNext) && (int)pObj->Level < l)
                pObj->Level = l;
        }
    ...
    Vec_PtrForEachEntryReverse(Abc_Obj_t*, vNodes, pObj, i) {
        ...
        Abc_ObjForEachFanout(pObj, pNext, j) if (Abc_NodeIsTravIdCurrent(pNext) && !Abc_ObjIsLatch(pNext))
            assert(pNext->fMarkB);
    }
    Vec_PtrForEachEntryReverse(Abc_Obj_t*, vNodes, pObj, i)
        pObj->fMarkB
...
void Abc_FlowRetime_ConstrainExact_forw(Abc_Obj_t* pObj) {
    ...
    Vec_PtrForEachEntryReverse(Abc_Obj_t*, vNodes, pCur, i) {
        ...
        if (pReg == pCur) {
            ...
            Abc_ObjForEachFanin(pReg, pNext, j)
                pNext->Level
                = MAX(pNext->Level, pReg->Level + (Abc_ObjIsNode(pReg) ? 1 : 0));
    ...
    Vec_PtrForEachEntryReverse(Abc_Obj_t*, vNodes, pCur, i) {
        pReg = Abc_ObjRegular(pCur);
        if (pReg != pCur) {
            Abc_ObjForEachFanin(pReg, pNext, j) if (!Abc_ObjIsLatch(pNext))
                pNext->Level
                = MAX(pNext->Level, pReg->Level + (Abc_ObjIsNode(pReg) ? 1 : 0));
...
void Abc_FlowRetime_ConstrainExact_back(Abc_Obj_t* pObj) {
    ...
    Vec_PtrForEachEntryReverse(Abc_Obj_t*, vNodes, pCur, i) {
        pReg = Abc_ObjRegular(pCur);
        if (pReg != pCur) {
            Abc_ObjForEachFanout(pReg, pNext, j) if (!Abc_ObjIsLatch(pNext))
                pNext->Level
                = MAX(pNext->Level, pReg->Level + (Abc_ObjIsNode(pReg) ? 1 : 0));
...
void Abc_FlowRetime_ConstrainExactAll(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachObj(pNtk, pObj, i) if (Vec_PtrSize(FTIMEEDGES(pObj))) {
        pArray = Vec_PtrReleaseArray(FTIMEEDGES(pObj));
        ABC_FREE(pArray);
    }
    pManMR->nExactConstraints = 0;
    ...
    Abc_NtkForEachObj(pNtk, pObj, i) if (!Abc_ObjIsLatch(pObj) && FTEST(pObj, CONSERVATIVE) && !FTEST(pObj, BLOCK)) if (!Vec_PtrSize(FTIMEEDGES(pObj)))
...
void Abc_FlowRetime_Dfs_forw(Abc_Obj_t* pObj, Vec_Ptr_t* vNodes) {
    ...
    Abc_ObjForEachFanout(pObj, pNext, i) if (!Abc_NodeIsTravIdCurrent(pNext))
...
void Abc_FlowRetime_Dfs_back(Abc_Obj_t* pObj, Vec_Ptr_t* vNodes) {
    ...
    Abc_ObjForEachFanin(pObj, pNext, i) if (!Abc_NodeIsTravIdCurrent(pNext))
...
int Abc_FlowRetime_RefineConstraints() {
    ...
    // remember nodes
    if (pManMR->fIsForward) {
        Abc_NtkForEachObj(pNtk, pObj, i) if (!FTEST(pObj, VISITED_R))
            pObj->fMarkC
            = 1;
    } else {
        Abc_NtkForEachObj(pNtk, pObj, i) if (!FTEST(pObj, VISITED_E))
            pObj->fMarkC
            = 1;
    }
    ...
    if (pManMR->fIsForward) {
        Abc_NtkForEachObj(pNtk, pObj, i) {
            if (pObj->fMarkC && FTEST(pObj, VISITED_R) && FTEST(pObj, CONSERVATIVE) && count < maxTighten) {
                count++;
                Abc_FlowRetime_ConstrainExact(pObj);
            }
            pObj->fMarkC = 0;
        ...
        Abc_NtkForEachObj(pNtk, pObj, i) {
            if (pObj->fMarkC && FTEST(pObj, VISITED_E) && FTEST(pObj, CONSERVATIVE) && count < maxTighten) {
                count++;
                Abc_FlowRetime_ConstrainExact(pObj);
            }
            pObj->fMarkC = 0;
