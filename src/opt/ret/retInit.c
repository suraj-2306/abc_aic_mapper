...
Vec_Int_t* Abc_NtkRetimeInitialValues(Abc_Ntk_t* pNtkCone, Vec_Int_t* vValues, int fVerbose) {
    ...
    if (fVerbose) { ABC_PRT("SAT solving time", Abc_Clock() - clk); }
    // analyze the result
    if (RetValue == 1)
...
int Abc_NtkRetimeVerifyModel(Abc_Ntk_t* pNtkCone, Vec_Int_t* vValues, int* pModel) {
    ...
    Abc_NtkForEachPi(pNtkCone, pObj, i)
        pObj->pCopy
        = (Abc_Obj_t*)(ABC_PTRUINT_T)pModel[i];
    // simulate the internal nodes
    vNodes = Abc_NtkDfs(pNtkCone, 0);
    Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pObj, i)
        pObj->pCopy
        = (Abc_Obj_t*)(ABC_PTRUINT_T)Abc_ObjSopSimulate(pObj);
    Vec_PtrFree(vNodes);
    // compare the outputs
    ...
    Abc_NtkForEachPo(pNtkCone, pObj, i)
        Counter
        += (Vec_IntEntry(vValues, i) != (int)(ABC_PTRUINT_T)pObj->pCopy);
    if (Counter > 0)
...
void Abc_NtkRetimeTranferToCopy(Abc_Ntk_t* pNtk) {
    Abc_Obj_t* pObj;
    int i;
    Abc_NtkForEachObj(pNtk, pObj, i) if (Abc_ObjIsLatch(pObj))
        pObj->pCopy
        = (Abc_Obj_t*)(ABC_PTRUINT_T)Abc_LatchIsInit1(pObj);
}
...
void Abc_NtkRetimeTranferFromCopy(Abc_Ntk_t* pNtk) {
    ...
    Abc_NtkForEachObj(pNtk, pObj, i) if (Abc_ObjIsLatch(pObj))
        pObj->pData
        = (void*)(ABC_PTRUINT_T)(pObj->pCopy ? ABC_INIT_ONE : ABC_INIT_ZERO);
}
...
void Abc_NtkRetimeInsertLatchValues(Abc_Ntk_t* pNtk, Vec_Int_t* vValues) {
    ...
    Abc_NtkForEachObj(pNtk, pObj, i) if (Abc_ObjIsLatch(pObj))
        pObj->pCopy
        = (Abc_Obj_t*)(ABC_PTRUINT_T)Counter++;
    Abc_NtkForEachObj(pNtk, pObj, i) if (Abc_ObjIsLatch(pObj))
        pObj->pData
        = (Abc_Obj_t*)(ABC_PTRUINT_T)(vValues ? (Vec_IntEntry(vValues, (int)(ABC_PTRUINT_T)pObj->pCopy) ? ABC_INIT_ONE : ABC_INIT_ZERO) : ABC_INIT_DC);
}
...
void Abc_NtkCycleInitStateSop(Abc_Ntk_t* pNtk, int nFrames, int fVerbose) {
    ...
    Abc_NtkForEachPi(pNtk, pObj, i)
        pObj->pCopy
        = (Abc_Obj_t*)(ABC_PTRUINT_T)(rand() & 1);
    ...
    Abc_NtkForEachLatch(pNtk, pObj, i)
        pObj->pData
        = (void*)(ABC_PTRUINT_T)(Abc_ObjFanout0(pObj)->pCopy ? ABC_INIT_ONE : ABC_INIT_ZERO);
}
