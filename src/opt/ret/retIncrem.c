...
int Abc_NtkRetimeFinalizeLatches(Abc_Ntk_t* pNtk, st__table* tLatches, int nIdMaxStart, int fUseOldNames) {
    ...
    vCisOld = pNtk->vCis;
    pNtk->vCis = NULL;
    vCisNew = Vec_PtrAlloc(100);
    vCosOld = pNtk->vCos;
    pNtk->vCos = NULL;
    vCosNew = Vec_PtrAlloc(100);
    vBoxesOld = pNtk->vBoxes;
    pNtk->vBoxes = NULL;
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vCisOld, pObj, i) if (!Abc_ObjIsPi(pObj) && Abc_ObjFaninNum(pObj) == 0 && Abc_ObjFanoutNum(pObj) == 0)
        Abc_NtkDeleteObj(pObj);
    Vec_PtrForEachEntry(Abc_Obj_t*, vCosOld, pObj, i) if (!Abc_ObjIsPo(pObj) && Abc_ObjFaninNum(pObj) == 0 && Abc_ObjFanoutNum(pObj) == 0)
        Abc_NtkDeleteObj(pObj);
    // set the new arrays
    pNtk->vCis = vCisNew;
    Vec_PtrFree(vCisOld);
...
int Abc_NtkRetimeNodeIsEnabled(Abc_Obj_t* pObj, int fForward) {
    ...
    if (fForward) {
        Abc_ObjForEachFanin(pObj, pNext, i) if (!Abc_ObjIsLatch(pNext)) return 0;
    } else {
        Abc_ObjForEachFanout(pObj, pNext, i) if (!Abc_ObjIsLatch(pNext)) return 0;
    }
    return 1;
}
...
void Abc_NtkRetimeShareLatches(Abc_Ntk_t* pNtk, int fInitial) {
    ...
    Abc_NtkForEachObj(pNtk, pFanin, i) {
        ...
        // get the first latch
        pLatchTop = NULL;
        Abc_ObjForEachFanout(pFanin, pLatchTop, k) if (Abc_ObjIsLatch(pLatchTop)) break;
