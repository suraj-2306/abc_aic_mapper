...
Vec_Int_t* Abc_NtkRetimeGetLags(Abc_Ntk_t* pNtk, int nIterLimit, int fVerbose) {
    ...
    Abc_NtkForEachNode(pNtk, pNode, i) if (Abc_NodeGetLValue(pNode) < -ABC_INFINITY / 2)
...
int Abc_NtkRetimeForPeriod(Abc_Ntk_t* pNtk, Vec_Ptr_t* vNodes, Vec_Ptr_t* vLatches, int Fi, int nMaxIters, int fVerbose) {
    ...
    Abc_NtkForEachObj(pNtk, pObj, i) if (Abc_ObjFaninNum(pObj) == 0)
        Abc_NodeSetLValue(pObj, 0);
    else Abc_NodeSetLValue(pObj, -ABC_INFINITY);
...
int Abc_NtkRetimePosOverLimit(Abc_Ntk_t* pNtk, int Fi) {
    Abc_Obj_t* pObj;
    int i;
    Abc_NtkForEachPo(pNtk, pObj, i) if (Abc_NodeGetLValue(Abc_ObjFanin0(pObj)) > Fi) return 1;
    return 0;
}
