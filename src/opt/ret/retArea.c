...
void Abc_NtkRetimeMinAreaPrepare(Abc_Ntk_t* pNtk, int fForward) {
    ...
    if (fForward) {
        ...
        Abc_NtkForEachObj(pNtk, pObj, i) if (pObj->fMarkA)
            Abc_ObjForEachFanin(pObj, pFanin, k) if (!pFanin->fMarkA)
...
void Abc_NtkRetimeMinAreaUpdateLatches(Abc_Ntk_t* pNtk, Vec_Ptr_t* vMinCut, int fForward, int fUseOldNames) {
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vBoxes, pObj, i) if (!Abc_ObjIsLatch(pObj))
    ...
    Vec_PtrForEachEntry(Abc_Obj_t*, vMinCut, pObj, i) {
        if (Abc_ObjIsCi(pObj) && fForward) {
            ...
            Abc_ObjForEachFanout(pObj, pNext, k) if (pNext->fMarkA) break;
            if (k < Abc_ObjFanoutNum(pObj)) {
                ...
                Vec_PtrForEachEntry(Abc_Obj_t*, vNodes, pNext, k) if (pNext->fMarkA)
