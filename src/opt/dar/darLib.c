...
struct Dar_LibDat_t_ // library object data
{
    union {
        ...
        int iGunc;
    };                      // the corresponding AIG node if it exists
...
int Dar_LibEval_rec(Dar_LibObj_t* pObj, int Out, int nNodesSaved, int Required, float* pPower) {
    ...
    if (pPower) {
        ...
        pData->dProb = (pObj->fCompl0 ? 1.0 - pData0->dProb : pData0->dProb) * (pObj->fCompl1 ? 1.0 - pData1->dProb : pData1->dProb);
        *pPower = Power0 + 2.0 * pData0->dProb * (1.0 - pData0->dProb) + Power1 + 2.0 * pData1->dProb * (1.0 - pData1->dProb);
    }
    return Area + 1;
}
...
void Dar_LibEval(Dar_Man_t* p, Aig_Obj_t* pRoot, Dar_Cut_t* pCut, int Required, int* pnMffcSize) {
    ...
    for (Out = 0; Out < s_DarLib->nSubgr0[Class]; Out++) {
        ...
        if (nNodesGained < 0 || (nNodesGained == 0 && !p->pPars->fUseZeros))
            continue;
        if (nNodesGained < p->GainBest || (nNodesGained == p->GainBest && s_DarLib->pDatas[pObj->Num].Level >= p->LevelBest))
            continue;
...
int Dar2_LibEval(Gia_Man_t* p, Vec_Int_t* vCutLits, unsigned uTruth, int fKeepLevel, Vec_Int_t* vLeavesBest2) {
    ...
    for (Out = 0; Out < s_DarLib->nSubgr0[Class]; Out++) {
        ...
        nNodesGained = nNodesSaved - nNodesAdded;
        if (fKeepLevel) {
            if (s_DarLib->pDatas[pObj->Num].Level > p_LevelBest || (s_DarLib->pDatas[pObj->Num].Level == p_LevelBest && nNodesGained <= p_GainBest))
                continue;
        } else {
            if (nNodesGained < p_GainBest || (nNodesGained == p_GainBest && s_DarLib->pDatas[pObj->Num].Level >= p_LevelBest))
                continue;
        }
        // remember this possibility
