...
Cut_Cut_t* Cut_CutMergeTwo3(Cut_Man_t* p, Cut_Cut_t* pCut0, Cut_Cut_t* pCut1) {
    ...
    nLeaves0 = pCut0->nLeaves;
    nLeaves1 = pCut1->nLeaves;
    if (nLeaves0 == Limit) {     // the case when one of the cuts is the largest
        if (nLeaves1 == Limit) { // the case when both cuts are the largest
            for (i = 0; i < nLeaves0; i++) {
...
Cut_Cut_t* Cut_CutMergeTwo4(Cut_Man_t* p, Cut_Cut_t* pCut0, Cut_Cut_t* pCut1) {
    ...
    // consider two cuts
    Limit = p->pParams->nVarsMax;
    if (pCut0->nLeaves == (unsigned)Limit) {     // the case when one of the cuts is the largest
        if (pCut1->nLeaves == (unsigned)Limit) { // the case when both cuts are the largest
