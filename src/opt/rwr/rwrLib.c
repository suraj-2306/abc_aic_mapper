...
save:
    ...
    k = 5;
    nNodes = 0;
    Vec_PtrForEachEntryStart(Rwr_Node_t*, p->vForest, p0, i, 5) if (p0->uTruth == p->puCanons[p0->uTruth]) {
    ...
    k = 5;
    Vec_PtrForEachEntryStart(Rwr_Node_t*, p->vForest, p0, i, 5) if (p0->fUsed) {
...
Rwr_Node_t* Rwr_ManTryNode(Rwr_Man_t* p, Rwr_Node_t* p0, Rwr_Node_t* p1, int fExor, int Level, int Volume) {
    ...
    if (fExor) {
        ...
        uTruth = (Rwr_IsComplement(p0) ? ~Rwr_Regular(p0)->uTruth : Rwr_Regular(p0)->uTruth) & (Rwr_IsComplement(p1) ? ~Rwr_Regular(p1)->uTruth : Rwr_Regular(p1)->uTruth) & 0xFFFF;
