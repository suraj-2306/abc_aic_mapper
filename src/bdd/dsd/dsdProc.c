...
Dsd_Node_t* dsdKernelDecompose_rec(Dsd_Manager_t* pDsdMan, DdNode* bFunc0) {
    ...
    // check cache
    if (st__lookup(pDsdMan->Table, (char*)bF, (char**)&pTableEntry)) { // the entry is present
        HashSuccess++;
    ...
    if (bLowR->index == CUDD_CONST_INDEX || bHigh->index == CUDD_CONST_INDEX) { // one of the cofactors in the constant
        if (bHigh == b1)                                                        // bHigh cannot be equal to b0, because then it will be complemented
            ...
            /////////////////////////////////////////////////////////////////
            {
                ...
                pLR = Dsd_Regular(pL);
                bSuppNew = Cudd_bddAnd(dd, bVarCur, pLR->S);
                Cudd_Ref(bSuppNew);
        ...
        else // if ( bHigh != const ) // meaning that bLow should be a constant
        {
            ...
            pHR = Dsd_Regular(pH);
            bSuppNew = Cudd_bddAnd(dd, bVarCur, pHR->S);
            Cudd_Ref(bSuppNew);
            if (bLow == b0)
                /////////////////////////////////////////////////////////////////
...
*/
    ...
    if (fContained) {
        ...
        if (pSmallR == pLR) { // Low is Small => High is Large
            pSmall = pL;
            pLarge = pH;
            fLowIsLarge = 0;
        } else { // vice versa
        ...
        if (pLargeR->Type == DSD_NODE_PRIME) //&& pLargeR->nDecs != pSmallR->nDecs )
        {
            ...
            if (g != pLargeR->nDecs) { // decomposition is found
                if (fFoundComp)
                    if (fLowIsLarge)
        ...
        {      // for example F = OR(ab,c,d), F(a=0) = OR(c,d), F(a=1) = OR(b,c,d)
            ...
            if (pLargeR->Type == pSmallR->Type && (pLargeR->Type == DSD_NODE_EXOR || (pSmallR->Type == DSD_NODE_OR && ((pLarge == pLargeR) == (pSmall == pSmallR))))) {
        ...
        if (pComp) // the decomposition is possible!
        {
            ...
            if (pLargeR->Type == DSD_NODE_OR) // Figure 4 of Matsunaga's paper
            {
                ...
                if ((fComp1 ^ fComp2) == fComp3) { // decomposition exists = consider 4 cases
                    ...
                    bFuncNew = Cudd_bddAndAbstract(dd, bFTemp, Cudd_Not(bFuncComp), pCompR->S);
                    Cudd_Ref(bFuncNew);
                    ...
                    } else {                     // pComp is not complemented
            ...
            {                                          // decomposition always exists = consider 4 cases
                ...
                bFuncNew = Cudd_bddXor(dd, bFTemp, pComp->G);
                Cudd_Ref(bFuncNew);
    ...
    if (pLR->Type == pHR->Type && pLR->Type != DSD_NODE_BUF && (pLR->Type != DSD_NODE_OR || ((pL == pLR && pH == pHR) || (pL != pLR && pH != pHR))) && (pLR->Type != DSD_NODE_PRIME || pLR->nDecs == pHR->nDecs)) {
        ...
        if (nCommon) {
            ...
            {                             // at this point we have the function F and the group of common components C
                ...
                bFuncNew = Cudd_bddAndAbstract(dd, bFTemp, Cudd_Not(bCommF), bCommS);
                Cudd_Ref(bFuncNew);
                ...
                if (pL != pLR)
                    pThis = Dsd_Not(pThis);
                goto EXIT;
            } else if (pLR->Type == DSD_NODE_EXOR) // Figure 3 of Matsunaga's paper
            {
                // compute the sum total of the common components and the union of their supports
                ...
                bFuncNew = Cudd_bddXor(dd, bF, bCommF);
                Cudd_Ref(bFuncNew);
                Cudd_RecursiveDeref(dd, bCommF);
                ...
                if (fCompExor)
                    pThis = Dsd_Not(pThis);
                goto EXIT;
            } else if (pLR->Type == DSD_NODE_PRIME && (nCommon == pLR->nDecs - 1 || nCommon == pLR->nDecs)) {
                ...
                if (nCommon == pLR->nDecs) { // all the components are the same
                    ...
                    for (m = 0; m < pLR->nDecs; m++) {
                        ...
                        if (Dsd_CheckRootFunctionIdentity(dd, bLow, bHigh, pTempL->G, Cudd_Not(pTempH->G)) && Dsd_CheckRootFunctionIdentity(dd, bLow, bHigh, Cudd_Not(pTempL->G), pTempH->G)) {
                            pLastDiffL = pTempL;
                            pLastDiffH = pTempH;
...
*/
                    ...
                    if (fCompComp) { // put the equal components into pCommon, so that they could be copied into the new dec entry
                        nCommon = 0;
                        for (m = 0; m < pLR->nDecs; m++)
                ...
                } else { // the differing components are known - check that they have compatible PRIME function
                    ...
                    if (Dsd_CheckRootFunctionIdentity(dd, bLow, bHigh, Cudd_Not(pLastDiffL->G), Cudd_Not(pLastDiffH->G)) && Dsd_CheckRootFunctionIdentity(dd, bLow, bHigh, pLastDiffL->G, pLastDiffH->G))
                        fCompComp = 1;
                    else if (Dsd_CheckRootFunctionIdentity(dd, bLow, bHigh, pLastDiffL->G, Cudd_Not(pLastDiffH->G)) && Dsd_CheckRootFunctionIdentity(dd, bLow, bHigh, Cudd_Not(pLastDiffL->G), pLastDiffH->G))
                        fCompComp = 2;
    ...
    //////////////////////////////////////////////////////////////////////
    {
        ...
        if ((((pLR->Type == DSD_NODE_PRIME && nSuppL == pLR->nDecs) || (pHR->Type == DSD_NODE_PRIME && nSuppH == pHR->nDecs)) && pLR->S == pHR->S) || ((pLR->Type == DSD_NODE_PRIME && nSuppL == pLR->nDecs) && (pHR->Type == DSD_NODE_PRIME && nSuppH == pHR->nDecs))) {
            s_Case4CallsSpecial++;
            ...
            while (SuppL != b1 || SuppH != b1) {
                ...
                if (!fEqualLevel) {
                    ...
                    while (Extra_bddSuppOverlapping(dd, pCur->S, bSuppLower)) { // get the next component
                        pPrev = pCur;
                    ...
                    if (pPrev == NULL || pPrev->Type == DSD_NODE_PRIME) { // if there is no previous component, or if the previous component is PRIME
                    ...
                    } else // all other types
                    {
                        ...
                        if (nNonOverlap == 1) { // one one component was found, which is the original one
                    ...
                    if (TopLevL < TopLevH) {
                        SuppL = Cudd_bddExistAbstract(dd, bTemp = SuppL, bSuppSubract);
                        Cudd_Ref(SuppL);
                        Cudd_RecursiveDeref(dd, bTemp);
                    } else {
                        SuppH = Cudd_bddExistAbstract(dd, bTemp = SuppH, bSuppSubract);
                        Cudd_Ref(SuppH);
                        Cudd_RecursiveDeref(dd, bTemp);
                    }
                ...
                else // if ( fEqualLevel ) -- they have the same top level var
                {
                    ...
                    // look for the possibility to subtract more than one component
                    if (!pPrevL || !pPrevH || pPrevL->Type != pPrevH->Type || pPrevL->Type == DSD_NODE_PRIME || fPolarityCurL != fPolarityCurH) { // there is no way to extract more than one
                        ...
                        if (nCommon == 0 || nCommon == 1) { // one one component was found, which is the original one
...
int dsdKernelFindCommonComponents(Dsd_Manager_t* pDsdMan, Dsd_Node_t* pL, Dsd_Node_t* pH, Dsd_Node_t*** pCommon, Dsd_Node_t** pLastDiffL, Dsd_Node_t** pLastDiffH) {
    ...
    int iCurL = 0;
    int iCurH = 0;
    while (iCurL < pL->nDecs && iCurH < pH->nDecs) { // both did not run out
        ...
        if (TopVar == bSLcur->index && TopVar == bSHcur->index) {
            ...
            iCurL++;
            iCurH++;
        } else if (TopVar == bSLcur->index) { // the components cannot be equal
...
void dsdKernelComputeSumOfComponents(Dsd_Manager_t* pDsdMan, Dsd_Node_t** pCommon, int nCommon, DdNode** pCompF, DdNode** pCompS, int fExor) {
    ...
    for (i = 0; i < nCommon; i++) {
        ...
        if (pCompS) {
            // add to the support
            bS = Cudd_bddAnd(dd, bTemp = bS, pDER->S);
            Cudd_Ref(bS);
            Cudd_RecursiveDeref(dd, bTemp);
        }
    }
...
int dsdKernelVerifyDecomposition(Dsd_Manager_t* pDsdMan, Dsd_Node_t* pDE) {
    ...
    if (pR->Type == DSD_NODE_CONST1)
        ...
        DdNode* bNewFunc = Dsd_TreeGetPrimeFunctionOld(dd, pR, 1);
        Cudd_Ref(bNewFunc);
        ...
        bRes = Cudd_bddVectorCompose(dd, bNewFunc, bGVars);
        Cudd_Ref(bRes);
