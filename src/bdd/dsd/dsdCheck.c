...
void Dsd_CheckCacheAllocate(int nEntries) {
    ...
    if (pCache->nTableSize != nRequested) { // the current size is different
...
int Dsd_CheckRootFunctionIdentity_rec(DdManager* dd, DdNode* bF1, DdNode* bF2, DdNode* bC1, DdNode* bC2) {
    ...
    if (pCache->pTable[HKey].bX[0] == bF1 && pCache->pTable[HKey].bX[1] == bF2 && pCache->pTable[HKey].bX[2] == bC1 && pCache->pTable[HKey].bX[3] == bC2) {
        pCache->nSuccess++;
