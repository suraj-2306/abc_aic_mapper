...
void Dsd_TreePrint_rec(FILE* pFile, Dsd_Node_t* pNode, int fComp, char* pInputNames[], char* pOutputName, int nOffset, int* pSigCounter, int fShortNames) {
    ...
    assert(pNode->Type == DSD_NODE_BUF || pNode->Type == DSD_NODE_CONST1 || pNode->Type == DSD_NODE_PRIME || pNode->Type == DSD_NODE_OR || pNode->Type == DSD_NODE_EXOR);
...
DdNode* Dsd_TreeGetPrimeFunctionOld(DdManager* dd, Dsd_Node_t* pNode, int fRemap) {
    ...
    for (i = 0; i < pNode->nDecs; i++)
        ...
        {
            bCube0 = Extra_bddFindOneCube(dd, Cudd_Not(pNode->pDecs[i]->G));
            Cudd_Ref(bCube0);
            bCof0 = Cudd_Cofactor(dd, bNewFunc, bCube0);
            Cudd_Ref(bCof0);
            Cudd_RecursiveDeref(dd, bCube0);

            bCube1 = Extra_bddFindOneCube(dd, pNode->pDecs[i]->G);
            Cudd_Ref(bCube1);
            bCof1 = Cudd_Cofactor(dd, bNewFunc, bCube1);
            Cudd_Ref(bCof1);
            ...
            bNewFunc = Cudd_bddIte(dd, dd->vars[pNode->pDecs[i]->S->index], bCof1, bCof0);
            Cudd_Ref(bNewFunc);
    ...
    if (fRemap) {
        ...
        bNewFunc = Cudd_bddPermute(dd, bTemp = bNewFunc, Permute);
        Cudd_Ref(bNewFunc);
