...
static inline int Llb_ObjSetPath(Aig_Obj_t* pObj, Aig_Obj_t* pNext) {
    pObj->pData = (void*)pNext;
    return 1;
}
...
static inline Aig_Obj_t* Llb_ObjGetFanoutPath(Aig_Man_t* p, Aig_Obj_t* pObj) {
    ...
    Aig_ObjForEachFanout(p, pObj, pFanout, iFanout, i) if (Llb_ObjGetPath(pFanout) == pObj) return pFanout;
    return NULL;
}
...
Vec_Ptr_t* Llb_ManCutMap(Aig_Man_t* p, Vec_Ptr_t* vResult, Vec_Ptr_t* vSupps) {
    ...
    Aig_ManForEachObj(p, pObj, i) {
        if (!Aig_ObjIsCi(pObj) && !Aig_ObjIsNode(pObj))
            continue;
        Vec_PtrForEachEntry(Vec_Int_t*, vMaps, vMap, k) if (Vec_IntEntry(vMap, i)) break;
...
int Llb_ManCutPiNum(Aig_Man_t* p, Vec_Ptr_t* vMinCut) {
    Aig_Obj_t* pObj;
    int i, Counter = 0;
    Vec_PtrForEachEntry(Aig_Obj_t*, vMinCut, pObj, i) if (Saig_ObjIsPi(p, pObj))
        Counter++;
    return Counter;
}
...
int Llb_ManCutLoNum(Aig_Man_t* p, Vec_Ptr_t* vMinCut) {
    Aig_Obj_t* pObj;
    int i, Counter = 0;
    Vec_PtrForEachEntry(Aig_Obj_t*, vMinCut, pObj, i) if (Saig_ObjIsLo(p, pObj))
        Counter++;
    return Counter;
}
...
int Llb_ManCutVolume_rec(Aig_Man_t* p, Aig_Obj_t* pObj) {
    ...
    return 1 + Llb_ManCutVolume_rec(p, Aig_ObjFanin0(pObj)) + Llb_ManCutVolume_rec(p, Aig_ObjFanin1(pObj));
}
...
Vec_Ptr_t* Llb_ManCutSupp(Aig_Man_t* p, Vec_Ptr_t* vLower, Vec_Ptr_t* vUpper) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vLower, pObj, i) if (Aig_ObjIsTravIdCurrent(p, pObj))
...
Vec_Ptr_t* Llb_ManCutRange(Aig_Man_t* p, Vec_Ptr_t* vLower, Vec_Ptr_t* vUpper) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vUpper, pObj, i) if (!Aig_ObjIsTravIdCurrent(p, pObj))
...
Vec_Ptr_t* Llb_ManFlowFindBestCut(Aig_Man_t* p, Vec_Ptr_t* vLower, Vec_Ptr_t* vUpper, int Num) {
    ...
    for (s = 1; s < Aig_ManRegNum(p); s += 5) {
        Vec_PtrForEachEntry(Aig_Obj_t*, vLower, pObj, i) {
            ...
            if (Vol >= VolCmp && (iMinCut == -1 || iMinCut > Vec_PtrSize(vMinCut) || (iMinCut == Vec_PtrSize(vMinCut) && iVolBest < Vol))) {
                iBest = i;
