...
int Llb_NonlinFindBestVar(DdManager* dd, DdNode* bFunc, Aig_Man_t* pAig) {
    ...
    Saig_ManForEachLo(pAig, pObj, i) {
        ...
        bCof = Cudd_bddAnd(dd, bFunc, Cudd_Not(bVar));
        Cudd_Ref(bCof);
        ...
        bCof = Cudd_bddAnd(dd, bFunc, bVar);
        Cudd_Ref(bCof);
...
void Llb_NonlinTrySubsetting(DdManager* dd, DdNode* bFunc) {
    ...
    bNew = Cudd_SubsetHeavyBranch(dd, bFunc, Cudd_SupportSize(dd, bFunc), 1000);
    Cudd_Ref(bNew);
...
*/
    ...
    if (Vec_PtrSize(p->vRings) > 1) {
        bState = Llb_CoreComputeCube(p->dd, vVarsNs, 1, pValues);
        Cudd_Ref(bState);
    }
...
int Llb_NonlinReachability(Llb_Mnn_t* p) {
    ...
    p->dd->bFunc = Llb_NonlinComputeInitState(p->pAig, p->dd);
    Cudd_Ref(p->dd->bFunc); // current
    p->ddG->bFunc = Llb_NonlinComputeInitState(p->pAig, p->ddG);
    Cudd_Ref(p->ddG->bFunc); // reached
    p->ddG->bFunc2 = Llb_NonlinComputeInitState(p->pAig, p->ddG);
    Cudd_Ref(p->ddG->bFunc2); // frontier
