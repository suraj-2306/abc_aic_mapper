...
DdNode* Llb_Nonlin4ComputeBad(DdManager* dd, Aig_Man_t* pAig, Vec_Int_t* vOrder) {
    ...
    Aig_ManForEachCi(pAig, pObj, i)
        pObj->pData
        = Cudd_bddIthVar(dd, Llb_ObjBddVar(vOrder, pObj));
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vNodes, pObj, i) {
        ...
        if (bBdd == NULL) {
            Vec_PtrForEachEntry(Aig_Obj_t*, vNodes, pObj, i) if (Aig_ObjIsNode(pObj) && pObj->pData != NULL)
    ...
    if (bResult) {
        ...
        if (bResult != NULL) {
            bResult = Cudd_bddExistAbstract(dd, bTemp = bResult, bCube);
            Cudd_Ref(bResult);
            Cudd_RecursiveDeref(dd, bTemp);
...
Vec_Ptr_t* Llb_Nonlin4DerivePartitions(DdManager* dd, Aig_Man_t* pAig, Vec_Int_t* vOrder) {
    ...
    Aig_ManForEachCi(pAig, pObj, i)
        pObj->pData
        = Cudd_bddIthVar(dd, Llb_ObjBddVar(vOrder, pObj));
    Aig_ManForEachNode(pAig, pObj, i) if (Llb_ObjBddVar(vOrder, pObj) >= 0) {
    ...
    // early termination
finish:
    Aig_ManForEachNode(pAig, pObj, i) if (pObj->pData)
        Cudd_RecursiveDeref(dd, (DdNode*)pObj->pData);
...
Vec_Int_t* Llb_Nonlin4CollectHighRefNodes(Aig_Man_t* pAig, int nFans) {
    ...
    Aig_ManForEachNode(pAig, pObj, i) if (Aig_ObjRefs(pObj) >= nFans)
        pObj->fMarkA
        = 1;
    // unmark flop drivers
    Saig_ManForEachLi(pAig, pObj, i)
    ...
    Aig_ManForEachNode(pAig, pObj, i) if (pObj->fMarkA)
...
*/
    ...
    Aig_ManForEachCi(pAig, pObj, i) if (Llb_ObjBddVar(vOrder, pObj) < 0) {
        //            if ( Saig_ObjIsLo(pAig, pObj) )
...
DdNode* Llb_Nonlin4ComputeCube(DdManager* dd, Aig_Man_t* pAig, Vec_Int_t* vOrder, char* pValues, int Flag) {
    ...
    Saig_ManForEachLiLo(pAig, pObjLi, pObjLo, i) {
        ...
        if (pValues[Llb_ObjBddVar(vOrder, pObjLo)] != 1)
            bVar = Cudd_Not(bVar);
        // create cube
        bRes = Cudd_bddAnd(dd, bTemp = bRes, bVar);
        Cudd_Ref(bRes);
        Cudd_RecursiveDeref(dd, bTemp);
    }
    Cudd_Deref(bRes);
...
void Llb_Nonlin4RecordState(Aig_Man_t* pAig, Vec_Int_t* vOrder, unsigned* pState, char* pValues, int fBackward) {
    Aig_Obj_t *pObjLo, *pObjLi;
    int i;
    Saig_ManForEachLiLo(pAig, pObjLi, pObjLo, i) if (pValues[Llb_ObjBddVar(vOrder, fBackward ? pObjLi : pObjLo)] == 1)
...
Vec_Ptr_t* Llb_Nonlin4Multiply(DdManager* dd, DdNode* bCube, Vec_Ptr_t* vParts) {
    ...
    Vec_PtrForEachEntry(DdNode*, vParts, bFunc, i) {
        bTemp = Cudd_bddAnd(dd, bFunc, bCube);
        Cudd_Ref(bTemp);
...
Vec_Ptr_t* Llb_Nonlin4DeriveCex(Llb_Mnx_t* p, int fBackward, int fVerbose) {
    ...
    bOneCube = Cudd_bddIntersect(p->dd, (DdNode*)Vec_PtrEntryLast(p->vRings), p->bBad);
    Cudd_Ref(bOneCube);
    ...
    if (Vec_PtrSize(p->vRings) > 1) {
        bState = Llb_Nonlin4ComputeCube(p->dd, p->pAig, p->vOrder, pValues, fBackward);
        Cudd_Ref(bState);
    }
    ...
    Vec_PtrForEachEntryReverse(DdNode*, p->vRings, bRing, v) {
        ...
        bImage = Llb_Nonlin4Image(p->dd, vRootsNew, NULL, vVars2Q);
        Cudd_Ref(bImage);
        Llb_Nonlin4Deref(p->dd, vRootsNew);

        // intersect with the previous set
        bOneCube = Cudd_bddIntersect(p->dd, bImage, bRing);
        Cudd_Ref(bOneCube);
...
int Llb_Nonlin4Reachability(Llb_Mnx_t* p) {
    ...
    if (p->pPars->fBackward) {
        ...
        if (!p->pPars->fSkipOutCheck) {
            p->bBad = Llb_Nonlin4ComputeInitState(p->dd, p->pAig, p->vOrder, p->pPars->fBackward);
            Cudd_Ref(p->bBad);
        }
        // create init state
        if (p->pPars->fCluster)
        ...
        p->bCurrent = Llb_Nonlin4ComputeInitState(p->dd, p->pAig, p->vOrder, p->pPars->fBackward);
        Cudd_Ref(p->bCurrent);
    }
    // perform iterations
...
void Llb_MnxCheckNextStateVars(Llb_Mnx_t* p) {
    ...
    Saig_ManForEachLi(p->pAig, pObj, i) if (Saig_ObjIsLo(p->pAig, Aig_ObjFanin0(pObj))) {
...
Aig_Man_t* Llb_ReachableStates(Aig_Man_t* pAig) {
    ...
    bReached = Extra_TransferPermute(pMnn->dd, dd, pMnn->bReached, Vec_IntArray(vPermute));
    Cudd_Ref(bReached);
