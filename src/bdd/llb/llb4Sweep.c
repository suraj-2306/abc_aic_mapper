...
*/
        ...
        if (Cudd_DagSize(bFunc) > nBddLimit) {
            ...
            if (Cudd_DagSize(bFunc0) >= Cudd_DagSize(bFunc1)) {
                ...
                Aig_ObjFanin0(pObj)->pData = bFunc;
                Cudd_Ref(bFunc);
                ...
                Aig_ObjFanin1(pObj)->pData = bFunc;
                Cudd_Ref(bFunc);
...
DdManager* Llb4_Nonlin4SweepBadStates(Aig_Man_t* pAig, Vec_Int_t* vOrder, int nVars) {
    ...
    bMonitor = Llb4_Nonlin4SweepBadMonitor(pAig, vOrder, dd);
    Cudd_Ref(bMonitor);
    Cudd_AutodynEnable(dd, CUDD_REORDER_SYMM_SIFT);
    bImage = Llb_Nonlin4Image(dd, vParts, bMonitor, vVars2Q);
    Cudd_Ref(bImage);
...
void Llb4_Nonlin4Sweep(Aig_Man_t* pAig, int nSweepMax, int nClusterMax, DdManager** pdd, Vec_Int_t** pvOrder, Vec_Ptr_t** pvGroups, int fVerbose) {
    ...
    ddWork->bFunc = Cudd_bddTransfer(ddBad, ddWork, ddBad->bFunc);
    Cudd_Ref(ddWork->bFunc);
    Cudd_RecursiveDeref(ddBad, ddBad->bFunc);
    ddBad->bFunc = NULL;
