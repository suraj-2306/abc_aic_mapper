...
#define Llb_MgrForEachVar(p, pVar, i)                                    \
    for (i = 0; (i < p->nVars) && (((pVar) = Llb_MgrVar(p, i)), 1); i++) \
        if (pVar == NULL) {                                              \
        } else
// iterator over parts
#define Llb_MgrForEachPart(p, pPart, i)                                        \
    for (i = 0; (i < p->iPartFree) && (((pPart) = Llb_MgrPart(p, i)), 1); i++) \
        if (pPart == NULL) {                                                   \
        } else
...
DdNode* Llb_NonlinCreateCube1(Llb_Mgr_t* p, Llb_Prt_t* pPart) {
    ...
    Llb_PartForEachVar(p, pPart, pVar, i) {
        ...
        bCube = Cudd_bddAnd(p->dd, bTemp = bCube, Cudd_bddIthVar(p->dd, pVar->iVar));
        Cudd_Ref(bCube);
...
DdNode* Llb_NonlinCreateCube2(Llb_Mgr_t* p, Llb_Prt_t* pPart1, Llb_Prt_t* pPart2) {
    ...
    Llb_PartForEachVar(p, pPart1, pVar, i) {
        ...
        if (Vec_IntSize(pVar->vParts) != 2)
            continue;
        if ((Vec_IntEntry(pVar->vParts, 0) == pPart1->iPart && Vec_IntEntry(pVar->vParts, 1) == pPart2->iPart) || (Vec_IntEntry(pVar->vParts, 0) == pPart2->iPart && Vec_IntEntry(pVar->vParts, 1) == pPart1->iPart)) {
            bCube = Cudd_bddAnd(p->dd, bTemp = bCube, Cudd_bddIthVar(p->dd, pVar->iVar));
            Cudd_Ref(bCube);
            Cudd_RecursiveDeref(p->dd, bTemp);
        }
...
int Llb_NonlinHasSingletonVars(Llb_Mgr_t* p, Llb_Prt_t* pPart) {
    Llb_Var_t* pVar;
    int i;
    Llb_PartForEachVar(p, pPart, pVar, i) if (Vec_IntSize(pVar->vParts) == 1) return 1;
    return 0;
}
...
int Llb_NonlinQuantify1(Llb_Mgr_t* p, Llb_Prt_t* pPart, int fSubset) {
    ...
    if (fSubset) {
        ...
        pPart->bFunc = Cudd_LargestCube(p->dd, bTemp = pPart->bFunc, &Length);
        ...
        bCube = Llb_NonlinCreateCube1(p, pPart);
        Cudd_Ref(bCube);
        //        assert( !Cudd_IsConstant(bCube) );
        // derive new function
        pPart->bFunc = Cudd_bddExistAbstract(p->dd, bTemp = pPart->bFunc, bCube);
        Cudd_Ref(pPart->bFunc);
        Cudd_RecursiveDeref(p->dd, bTemp);
    ...
    Llb_PartForEachVar(p, pPart, pVar, i) if (p->pSupp[pVar->iVar]) {
...
int Llb_NonlinQuantify2(Llb_Mgr_t* p, Llb_Prt_t* pPart1, Llb_Prt_t* pPart2) {
    ...
    // create cube to be quantified
    bCube = Llb_NonlinCreateCube2(p, pPart1, pPart2);
    Cudd_Ref(bCube);
    if (fVerbose) {
        printf("\n");
        printf("\n");
        ...
        Extra_bddPrintSupport(p->dd, bCube);
        printf("\n");
    }
...
Vec_Ptr_t* Llb_NonlinBuildBdds(Aig_Man_t* p, Vec_Ptr_t* vLower, Vec_Ptr_t* vUpper, DdManager* dd) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vLower, pObj, i)
        pObj->pData
        = Cudd_bddIthVar(dd, Aig_ObjId(pObj));
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, vUpper, pObj, i) {
        if (Aig_ObjIsNode(pObj)) {
            bProd = Cudd_bddXnor(dd, Cudd_bddIthVar(dd, Aig_ObjId(pObj)), (DdNode*)pObj->pData);
            Cudd_Ref(bProd);
        } else {
            assert(Saig_ObjIsLi(p, pObj));
            bBdd0 = Cudd_NotCond((DdNode*)Aig_ObjFanin0(pObj)->pData, Aig_ObjFaninC0(pObj));
            bProd = Cudd_bddXnor(dd, Cudd_bddIthVar(dd, Aig_ObjId(pObj)), bBdd0);
            Cudd_Ref(bProd);
        }
...
int Llb_NonlinNextPartitions(Llb_Mgr_t* p, Llb_Prt_t** ppPart1, Llb_Prt_t** ppPart2) {
    ...
    Llb_MgrForEachVar(p, pVar, i) if (pVarBest == NULL || pVarBest->nScore > pVar->nScore)
        pVarBest
        = pVar;
    if (pVarBest == NULL)
        return 0;
...
DdNode* Llb_NonlinImage(Aig_Man_t* pAig, Vec_Ptr_t* vLeaves, Vec_Ptr_t* vRoots, int* pVars2Q, DdManager* dd, DdNode* bCurrent, int fReorder, int fVerbose, int* pOrder) {
    ...
    // remove singles
    Llb_MgrForEachPart(p, pPart, i) if (Llb_NonlinHasSingletonVars(p, pPart))
