...
*/
    ...
    Vec_PtrForEachEntryStart(unsigned*, vStates, pNext, i, 1) {
        ...
        Saig_ManForEachPi(pAig, pObj, k) if (sat_solver_var_value(pSat, pCnf->pVarNums[Aig_ObjId(pObj)]))
    ...
    Vec_IntClear(vAssumps);
    if (iCexPo >= 0) {
        Saig_ManForEachPo(pAig, pObj, k) if (k == iCexPo)
...
Vec_Ptr_t* Llb4_Nonlin4VerifyCex(Aig_Man_t* pAig, Abc_Cex_t* p) {
    ...
    Saig_ManForEachLo(pAig, pObj, i)
        pObj->fMarkB
        = 0; //Abc_InfoHasBit(p->pData, iBit++);
    // do not require equal flop count in the AIG and in the CEX
    iBit = p->nRegs;
    for (i = 0; i <= p->iFrame; i++) {
        // save current state
        Saig_ManForEachLo(pAig, pObj, k) if (pObj->fMarkB)
