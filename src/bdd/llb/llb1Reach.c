...
DdNode* Llb_ManConstructGroupBdd(Llb_Man_t* p, Llb_Grp_t* pGroup) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, pGroup->vIns, pObj, i)
        pObj->pData
        = Cudd_bddIthVar(p->dd, Vec_IntEntry(p->vObj2Var, Aig_ObjId(pObj)));
    Vec_PtrForEachEntry(Aig_Obj_t*, pGroup->vNodes, pObj, i) {
        ...
        if (pObj->pData == NULL) {
            Vec_PtrForEachEntryStop(Aig_Obj_t*, pGroup->vNodes, pObj, k, i) if (pObj->pData)
                Cudd_RecursiveDeref(p->dd, (DdNode*)pObj->pData);
            return NULL;
        }
        Cudd_Ref((DdNode*)pObj->pData);
    }
    bRes = Cudd_ReadOne(p->dd);
    Cudd_Ref(bRes);
    Vec_PtrForEachEntry(Aig_Obj_t*, pGroup->vOuts, pObj, i) {
        ...
        bXor = Cudd_bddXor(p->dd, bBdd0, bBdd1);
        Cudd_Ref(bXor);
...
DdNode* Llb_ManConstructQuantCubeIntern(Llb_Man_t* p, Llb_Grp_t* pGroup, int iGrpPlace, int fBackward) {
    ...
    Vec_PtrForEachEntry(Aig_Obj_t*, pGroup->vIns, pObj, i) {
        ...
        bRes = Cudd_bddAnd(p->dd, bTemp = bRes, bVar);
        Cudd_Ref(bRes);
        Cudd_RecursiveDeref(p->dd, bTemp);
    }
    Vec_PtrForEachEntry(Aig_Obj_t*, pGroup->vOuts, pObj, i) {
...
DdNode* Llb_ManComputeInitState(Llb_Man_t* p, DdManager* dd) {
    ...
    Saig_ManForEachLo(p->pAig, pObj, i) {
        ...
        bRes = Cudd_bddAnd(dd, bTemp = bRes, Cudd_Not(bVar));
        Cudd_Ref(bRes);
...
DdNode* Llb_ManComputeImage(Llb_Man_t* p, DdNode* bInit, int fBackward) {
    ...
    for (k = 1; k < p->pMatrix->nCols - 1; k++) {
        ...
        bCube = Llb_ManConstructQuantCubeIntern(p, pGroup, Index, fBackward);
        Cudd_Ref(bCube);
    ...
    if (fCheckSupport) {
        bCube = Cudd_Support(p->dd, bImage);
        Cudd_Ref(bCube);
...
DdNode* Llb_ManCreateConstraints(Llb_Man_t* p, Vec_Int_t* vHints, int fUseNsVars) {
    ...
    Aig_ManForEachCi(p->pAigGlo, pObj, i)
        pObj->pData
        = Aig_ManCi(p->pAig, i)->pData;
    // derive consraints
    bConstr = Cudd_ReadOne(p->dd);
    Cudd_Ref(bConstr);
    Vec_IntForEachEntry(vHints, Entry, i) {
        if (Entry != 0 && Entry != 1)
            continue;
        bFunc = Llb_ManConstructOutBdd(p->pAigGlo, Aig_ManObj(p->pAigGlo, i), p->dd);
        Cudd_Ref(bFunc);
        bFunc = Cudd_NotCond(bFunc, Entry); // restrict to not constraint
        // make the product
        bConstr = Cudd_bddAnd(p->dd, bTemp = bConstr, bFunc);
        Cudd_Ref(bConstr);
        Cudd_RecursiveDeref(p->dd, bTemp);
...
*/
    ...
    // get the last cube
    bOneCube = Cudd_bddIntersect(p->ddR, (DdNode*)Vec_PtrEntryLast(p->vRings), p->ddR->bFunc);
    Cudd_Ref(bOneCube);
    ...
    Saig_ManForEachPi(p->pAig, pObj, i) if (pValues[Saig_ManRegNum(p->pAig) + i] == 1)
        Abc_InfoSetBit(pCex->pData, nPiOffset + i);

    // write state in terms of NS variables
    if (Vec_PtrSize(p->vRings) > 1) {
        bState = Llb_CoreComputeCube(p->dd, p->vGlo2Ns, 1, pValues);
        Cudd_Ref(bState);
    }
    ...
    Vec_PtrForEachEntryReverse(DdNode*, p->vRings, bRing, v) {
        ...
        bImage = Extra_TransferPermute(p->dd, p->ddR, bTemp = bImage, Vec_IntArray(p->vCs2Glo));
        Cudd_Ref(bImage);
        Cudd_RecursiveDeref(p->dd, bTemp);
        //Extra_bddPrintSupport( p->ddR, bImage );  printf( "\n" );

        // intersect with the previous set
        bOneCube = Cudd_bddIntersect(p->ddR, bImage, bRing);
        Cudd_Ref(bOneCube);
...
int Llb_ManReachability(Llb_Man_t* p, Vec_Int_t* vHints, DdManager** pddGlo) {
    ...
    // derive constraints
    bConstrCs = Llb_ManCreateConstraints(p, vHints, 0);
    Cudd_Ref(bConstrCs);
    bConstrNs = Llb_ManCreateConstraints(p, vHints, 1);
    Cudd_Ref(bConstrNs);
    ...
    if (p->ddG->bFunc) {
        bReached = p->ddG->bFunc;
        p->ddG->bFunc = NULL;
        bCurrent = Extra_TransferPermute(p->ddG, p->dd, bReached, pGlo2Cs);
        Cudd_Ref(bCurrent);
    } else {
        bReached = Llb_ManComputeInitState(p, p->ddG);
        Cudd_Ref(bReached);
        bCurrent = Llb_ManComputeInitState(p, p->dd);
        Cudd_Ref(bCurrent);
    }
    ...
    for (nIters = 0; nIters < p->pPars->nIterMax; nIters++) {
        ...
        if (p->pPars->TimeLimit && Abc_Clock() > p->pPars->TimeTarget) {
            ...
            bCurrent = NULL;
            Cudd_RecursiveDeref(p->dd, bConstrCs);
            bConstrCs = NULL;
            Cudd_RecursiveDeref(p->dd, bConstrNs);
            bConstrNs = NULL;
        ...
        if (vHints) {
            bCurrent = Cudd_bddAnd(p->dd, bTemp = bCurrent, bConstrCs);
            Cudd_Ref(bCurrent);
            Cudd_RecursiveDeref(p->dd, bTemp);
        }

        // quantify variables appearing only in the init state
        bCube = Llb_ManConstructQuantCubeIntern(p, (Llb_Grp_t*)Vec_PtrEntry(p->vGroups, 0), 0, 0);
        Cudd_Ref(bCube);
        bCurrent = Cudd_bddExistAbstract(p->dd, bTemp = bCurrent, bCube);
        Cudd_Ref(bCurrent);
        Cudd_RecursiveDeref(p->dd, bTemp);
        Cudd_RecursiveDeref(p->dd, bCube);
        ...
        // restrict reachable states using constraints
        if (vHints) {
            bNext = Cudd_bddAnd(p->dd, bTemp = bNext, bConstrNs);
            Cudd_Ref(bNext);
            Cudd_RecursiveDeref(p->dd, bTemp);
        }
