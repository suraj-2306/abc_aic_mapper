...
int Llb_ManMaxFanoutCi(Aig_Man_t* pAig) {
    ...
    Aig_ManForEachCi(pAig, pObj, i) if (WeightMax < Aig_ObjRefs(pObj)) {
...
Vec_Int_t* Llb_ManCollectHighFanoutObjects(Aig_Man_t* pAig, int nCandMax, int fCisOnly) {
    ...
    do {
        ...
        for (i = 0; i < Vec_IntSize(vResult) - 1; i++)
            if (Aig_ObjRefs(Aig_ManObj(pAig, Vec_IntEntry(vResult, i))) < Aig_ObjRefs(Aig_ManObj(pAig, Vec_IntEntry(vResult, i + 1)))) {
