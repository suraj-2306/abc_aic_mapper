...
Vec_Int_t* Llb_Nonlin4FindOrder(Aig_Man_t* pAig, int* pCounter) {
    ...
    Aig_ManForEachNode(pAig, pObj, i) if (Aig_ObjLevel(pObj) > 3)
        pObj->fMarkA
        = 1;
...
DdNode* Llb_Nonlin4FindPartitions_rec(DdManager* dd, Aig_Obj_t* pObj, Vec_Int_t* vOrder, Vec_Ptr_t* vRoots) {
    ...
    if (Aig_ObjIsCo(pObj)) {
        ...
        bPart = Cudd_bddXnor(dd, Cudd_bddIthVar(dd, Llb_ObjBddVar(vOrder, pObj)), bBdd0);
        Cudd_Ref(bPart);
    ...
    if (Llb_ObjBddVar(vOrder, pObj) >= 0) {
        ...
        bPart = Cudd_bddXnor(dd, vVar, bBdd);
        Cudd_Ref(bPart);
        Vec_PtrPush(vRoots, bPart);
...
int Llb_Nonlin4CountTerms(DdManager* dd, Aig_Man_t* pAig, Vec_Int_t* vOrder, DdNode* bFunc, int fCo, int fFlop) {
    ...
    Aig_Obj_t* pObj;
    int i, Counter = 0;
    bSupp = Cudd_Support(dd, bFunc);
    Cudd_Ref(bSupp);
    if (!fCo && !fFlop) {
        Saig_ManForEachPi(pAig, pObj, i) if (Llb_ObjBddVar(vOrder, pObj) >= 0)
            Counter
            += Cudd_bddLeq(dd, bSupp, Cudd_bddIthVar(dd, Llb_ObjBddVar(vOrder, pObj)));
    } else if (fCo && !fFlop) {
        Saig_ManForEachPo(pAig, pObj, i) if (Llb_ObjBddVar(vOrder, pObj) >= 0)
            Counter
            += Cudd_bddLeq(dd, bSupp, Cudd_bddIthVar(dd, Llb_ObjBddVar(vOrder, pObj)));
    } else if (!fCo && fFlop) {
        Saig_ManForEachLo(pAig, pObj, i) if (Llb_ObjBddVar(vOrder, pObj) >= 0)
            Counter
            += Cudd_bddLeq(dd, bSupp, Cudd_bddIthVar(dd, Llb_ObjBddVar(vOrder, pObj)));
    } else if (fCo && fFlop) {
        Saig_ManForEachLi(pAig, pObj, i) if (Llb_ObjBddVar(vOrder, pObj) >= 0)
            Counter
            += Cudd_bddLeq(dd, bSupp, Cudd_bddIthVar(dd, Llb_ObjBddVar(vOrder, pObj)));
    }
