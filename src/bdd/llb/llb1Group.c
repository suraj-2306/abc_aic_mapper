...
Llb_Grp_t* Llb_ManGroupCreateFromCuts(Llb_Man_t* pMan, Vec_Int_t* vCut1, Vec_Int_t* vCut2) {
    ...
    Aig_ManForEachObjVec(vCut2, pMan->pAig, pObj, i) if (!Aig_ObjIsTravIdCurrent(pMan->pAig, pObj))
        Vec_PtrPush(p->vOuts, pObj);
    ...
    // collect marked Cut1
    Aig_ManForEachObjVec(vCut1, pMan->pAig, pObj, i) if (Aig_ObjIsTravIdCurrent(pMan->pAig, pObj))
        Vec_PtrPush(p->vIns, pObj);
...
void Llb_ManPrintSpan(Llb_Man_t* p) {
    ...
    Vec_PtrForEachEntry(Llb_Grp_t*, p->vGroups, pGroup, i) {
        Vec_PtrForEachEntry(Aig_Obj_t*, pGroup->vIns, pVar, k) if (Vec_IntEntry(p->vVarBegs, pVar->Id) == i)
            Span++;
        Vec_PtrForEachEntry(Aig_Obj_t*, pGroup->vOuts, pVar, k) if (Vec_IntEntry(p->vVarBegs, pVar->Id) == i)
            Span++;

        SpanMax = Abc_MaxInt(SpanMax, Span);
        printf("%d ", Span);

        Vec_PtrForEachEntry(Aig_Obj_t*, pGroup->vIns, pVar, k) if (Vec_IntEntry(p->vVarEnds, pVar->Id) == i)
            Span--;
        Vec_PtrForEachEntry(Aig_Obj_t*, pGroup->vOuts, pVar, k) if (Vec_IntEntry(p->vVarEnds, pVar->Id) == i)
            Span--;
    }
    printf("\n");
    printf("Max = %d\n", SpanMax);
...
int Llb_ManGroupHasVar(Llb_Man_t* p, int iGroup, int iVar) {
    ...
    Aig_Obj_t* pObj;
    int i;
    Vec_PtrForEachEntry(Aig_Obj_t*, pGroup->vIns, pObj, i) if (pObj->Id == iVar) return 1;
    Vec_PtrForEachEntry(Aig_Obj_t*, pGroup->vOuts, pObj, i) if (pObj->Id == iVar) return 1;
    return 0;
}
