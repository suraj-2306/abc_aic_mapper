...
// dynamic variable reordering should be disabled when this function is running
{
    ...
    for (i = 0; i < nLuts; i++) {
        ...
        bVarsCCube = Extra_bddBitsToCube(dd, (1 << nVars) - 1, nVars, bVars, 1);
        Cudd_Ref(bVarsCCube);
        ...
        bVarsInCube = Extra_bddBitsToCube(dd, (1 << nVars) - 1, nVars, bVars, 1);
        Cudd_Ref(bVarsInCube);
        ...
        if (i == nLuts - 1) {
            ...
            p->bRelation = Cudd_bddIte(dd, bVar, p->pbCols[0], p->pbCols[1]);
            Cudd_Ref(p->bRelation);
        } else {
            abctime clk2 = Abc_Clock();
            //          p->bRelation = PerformTheEncoding( dd, p->pbCols, p->nCols, bVarsCube, bCVars, p->nMulti, &p->nSimple );  Cudd_Ref( p->bRelation );
            p->bRelation = Extra_bddEncodingNonStrict(dd, p->pbCols, p->nCols, bVarsCube, bCVars, p->nMulti, &p->nSimple);
            Cudd_Ref(p->bRelation);
        ...
        // get the codes from the relation (these are not necessarily cubes)
        {
            int c;
            for (c = 0; c < p->nCols; c++) {
                p->pbCodes[c] = Cudd_bddAndAbstract(dd, p->bRelation, p->pbCols[c], bVarsCube);
                Cudd_Ref(p->pbCodes[c]);
            }
        }
...
void WriteLUTSintoBLIFfile(FILE* pFile, DdManager* dd, LUT** pLuts, int nLuts, DdNode** bCVars, char** pNames, int nNames, char* FileName) {
    ...
    for (i = 0; i < nLuts; i++) {
        ...
        bCube = Extra_bddBitsToCube(dd, (1 << p->nMulti) - 1, p->nMulti, bCVars, 1);
        Cudd_Ref(bCube);

        // write each output of the LUT
        for (o = 0; o < p->nMulti; o++) {
            // get the cofactor of this output
            bCof = Cudd_Cofactor(dd, p->bRelation, bCVars[o]);
            Cudd_Ref(bCof);
            // quantify the remaining variables to get the function
            bFunc = Cudd_bddExistAbstract(dd, bCof, bCube);
            Cudd_Ref(bFunc);
