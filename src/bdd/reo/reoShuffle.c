...
DdNode* reoShuffle(reo_man* p, DdManager* dd, DdNode* bFunc, int* pPerm, int* pPermInv) {
    ...
    bFuncRes = reoTransferUnitsToNodes_rec(p, p->pTops[0]);
    Cudd_Ref(bFuncRes);
    ...
    if (p->fVerify) {
        DdNode* bFuncPerm;
        bFuncPerm = Cudd_bddPermute(dd, bFunc, pPermInv);
        Cudd_Ref(bFuncPerm);
...
*/
    ...
    bRemap = Extra_bddRemapUp(dd, Func);
    Cudd_Ref(bRemap);

    clk = Abc_Clock();
    Temp = reoShuffle(pReo, dd, bRemap, pOrder, pOrderInv);
    Cudd_Ref(Temp);
...

    {
        DdNode* bFuncPerm;
        clk = Abc_Clock();
        bFuncPerm = Cudd_bddPermute(dd, bRemap, pOrderInv);
        Cudd_Ref(bFuncPerm);
