...
void Extra_ReorderTest(DdManager* dd, DdNode* Func) {
    ...
    Temp = Extra_Reorder(pReo, dd, Func, pOrder);
    Cudd_Ref(Temp);
    //Extra_DumpDot( dd, &Temp, 1, "afterReo.dot", 0 );

    Temp1 = Extra_ReorderCudd(dd, Func, NULL);
    Cudd_Ref(Temp1);
...
DdNode* Extra_ReorderCudd(DdManager* dd, DdNode* aFunc, int pPermuteReo[]) {
    ...
    aFuncReorder = Extra_TransferPermute(dd, ddReorder, aFunc, Permute);
    Cudd_Ref(aFuncReorder);
    ...
    // transfer this function into the new manager in such a way that ordering of vars does not change
    aFuncNew = Extra_TransferPermute(ddReorder, dd, aFuncReorder, Permute);
    Cudd_Ref(aFuncNew);
...
int Extra_bddReorderTest(DdManager* dd, DdNode* bF) {
    ...
    //    clk1 = Abc_Clock();
    bFmin = Cudd_bddTransfer(dd, s_ddmin, bF);
    Cudd_Ref(bFmin);
...
int Extra_addReorderTest(DdManager* dd, DdNode* aF) {
    ...
    clk1 = Abc_Clock();
    bF = Cudd_addBddPattern(dd, aF);
    Cudd_Ref(bF);
    bFmin = Cudd_bddTransfer(dd, s_ddmin, bF);
    Cudd_Ref(bFmin);
    Cudd_RecursiveDeref(dd, bF);
    aFmin = Cudd_BddToAdd(s_ddmin, bFmin);
    Cudd_Ref(aFmin);
    Cudd_RecursiveDeref(s_ddmin, bFmin);
