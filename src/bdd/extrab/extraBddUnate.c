    ...
    DdNode* bFunc) /* the function whose symmetries are computed */
{
    ...
    Cudd_Ref(bSupp);
    zRes = Extra_zddUnateInfoCompute(dd, bFunc, bSupp);
    Cudd_Ref(zRes);
...
int Extra_bddCheckUnateNaive(
    ...
    bCof0 = Cudd_Cofactor(dd, bF, Cudd_Not(Cudd_bddIthVar(dd, iVar)));
    Cudd_Ref(bCof0);
    bCof1 = Cudd_Cofactor(dd, bF, Cudd_bddIthVar(dd, iVar));
    Cudd_Ref(bCof1);
