    ...
    if (fPrintTruth) {
        ...
        if (nVars == 0)
            ...
            for (d = (1 << (nVars - 2)) - 1; d >= 0; d--) {
                int Value = 0;
                for (s = 0; s < 4; s++) {
                    bCube = Extra_bddBitsToCube(dd, 4 * d + s, nVars, dd->vars, 0);
                    Cudd_Ref(bCube);
                    bPart = Cudd_Cofactor(dd, OnSet, bCube);
                    Cudd_Ref(bPart);
...
*/
    ...
    if (XVars == NULL) {
        if (fSuppType == 0) { // assume that the support includes the first nVars of the manager
            assert(nVars);
            for (v = 0; v < nVars; v++)
                s_XVars[v] = Cudd_bddIthVar(dd, v);
        } else if (fSuppType == 1) { // assume that the support includes the topmost nVars of the manager
            assert(nVars);
            for (v = 0; v < nVars; v++)
        ...
        } else // determine the support
        {
            ...
            // determine support
            SuppOn = Cudd_Support(dd, OnSet);
            Cudd_Ref(SuppOn);
            SuppOff = Cudd_Support(dd, OffSet);
            Cudd_Ref(SuppOff);
            Supp = Cudd_bddAnd(dd, SuppOn, SuppOff);
            Cudd_Ref(Supp);
    ...
    for (v = 0; v < nCellsVer; v++) {
        ...
        // find vertical cube
        CubeVerBDD = Extra_bddBitsToCube(dd, GrayCode(v), nVarsVer, s_XVars + nVarsHor, 1);
        Cudd_Ref(CubeVerBDD);
        ...
        for (h = 0; h < nCellsHor; h++) {
            ...
            CubeHorBDD = Extra_bddBitsToCube(dd, GrayCode(h), nVarsHor, s_XVars, 1);
            Cudd_Ref(CubeHorBDD);
            Prod = Cudd_bddAnd(dd, CubeHorBDD, CubeVerBDD);
            Cudd_Ref(Prod);
            Cudd_RecursiveDeref(dd, CubeHorBDD);

            ValueOnSet = Cudd_Cofactor(dd, OnSet, Prod);
            Cudd_Ref(ValueOnSet);
            ValueOffSet = Cudd_Cofactor(dd, OffSet, Prod);
            Cudd_Ref(ValueOffSet);
    ...
    DdNode** YVars) /* the flag which determines how support is computed */
{
    ...
    for (v = 0; v < nCellsVer; v++) {
        ...
        CubeVerBDD = Extra_bddBitsToCube(dd, GrayCode(v), nXVars, XVars, 1);
        Cudd_Ref(CubeVerBDD);
        ...
        for (h = 0; h < nCellsHor; h++) {
            ...
            CubeHorBDD = Extra_bddBitsToCube(dd, GrayCode(h), nYVars, YVars, 1);
            Cudd_Ref(CubeHorBDD);
            Prod = Cudd_bddAnd(dd, CubeHorBDD, CubeVerBDD);
            Cudd_Ref(Prod);
            Cudd_RecursiveDeref(dd, CubeHorBDD);
