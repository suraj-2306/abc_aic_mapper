...
DdNode* Extra_bddSpaceFromFunctionFast(DdManager* dd, DdNode* bFunc) {
    ...
    for (lev = 0; lev < dd->size; lev++)
        if (pSupport[dd->invperm[lev]]) { // var "dd->invperm[lev]" on level "lev" should go to level 2*Counter;
            ...
            pCompose[dd->invperm[2 * Counter]] = Cudd_bddXor(dd, dd->vars[dd->invperm[2 * Counter]], dd->vars[dd->invperm[2 * Counter + 1]]);
            Cudd_Ref(pCompose[dd->invperm[2 * Counter]]);
            // add this variable to the cube
            bCube = Cudd_bddAnd(dd, bTemp = bCube, dd->vars[dd->invperm[2 * Counter]]);
            Cudd_Ref(bCube);
    ...
    bFunc1 = Cudd_bddPermute(dd, bFunc, pPermute);
    Cudd_Ref(bFunc1);
    // compose to gate the function depending on both vars
    bFunc2 = Cudd_bddVectorCompose(dd, bFunc1, pCompose);
    Cudd_Ref(bFunc2);
    // gate the vector space
    // L(a) = ForAll x [ F(x) = F(x+a) ] = Not( Exist x [ F(x) (+) F(x+a) ] )
    bSpaceShift = Cudd_bddXorExistAbstract(dd, bFunc1, bFunc2, bCube);
    Cudd_Ref(bSpaceShift);
    bSpaceShift = Cudd_Not(bSpaceShift);
    // permute the space back into the original mapping
    bSpace = Cudd_bddPermute(dd, bSpaceShift, pPermuteBack);
    Cudd_Ref(bSpace);
...
DdNode* Extra_bddSpaceReduce(DdManager* dd, DdNode* bFunc, DdNode* bCanonVars) {
    DdNode* bNegCube;
    DdNode* bResult;
    bNegCube = Extra_bddSupportNegativeCube(dd, bCanonVars);
    Cudd_Ref(bNegCube);
    bResult = Cudd_Cofactor(dd, bFunc, bNegCube);
    Cudd_Ref(bResult);
...
DdNode* Extra_bddSpaceEquations(DdManager* dd, DdNode* bSpace) {
    ...
    DdNode* zEquPos;
    DdNode* zEquNeg;
    zEquPos = Extra_bddSpaceEquationsPos(dd, bSpace);
    Cudd_Ref(zEquPos);
    zEquNeg = Extra_bddSpaceEquationsNeg(dd, bSpace);
    Cudd_Ref(zEquNeg);
    zRes = Cudd_zddUnion(dd, zEquPos, zEquNeg);
    Cudd_Ref(zRes);
...
DdNode** Extra_bddSpaceExorGates(DdManager* dd, DdNode* bFuncRed, DdNode* zEquations) {
    ...
    while (zEquRem != z0) {
        // extract one product
        zExor = Extra_zddSelectOneSubset(dd, zEquRem);
        Cudd_Ref(zExor);
        // remove it from the set
        zEquRem = Cudd_zddDiff(dd, zTemp = zEquRem, zExor);
        Cudd_Ref(zEquRem);
