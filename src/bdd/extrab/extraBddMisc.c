...
DdNode* Extra_bddRemapUp(
    ...
    bRes = Cudd_bddPermute(dd, bF, pPermute);
    Cudd_Ref(bRes);
...
int Extra_bddSuppCheckContainment(DdManager* dd, DdNode* bL, DdNode* bH, DdNode** bLarge, DdNode** bSmall) {
    ...
    while (bSL != b1 || bSH != b1) {
        if (bSL == b1) { // Low component has no vars; High components has some vars
            fLcontainsH = 0;
            if (fHcontainsL == 0)
                return 0;
            else
        ...
        if (TopVar == bSL->index && TopVar == bSH->index) { // they are on the same level
...
DdNode* Extra_bddFindOneCube(DdManager* dd, DdNode* bF) {
    ...
    for (v = 0; v < dd->size; v++)
        if (s_Temp[v] == 0) {
            //          Cube &= !s_XVars[v];
            bCube = Cudd_bddAnd(dd, bTemp = bCube, Cudd_Not(dd->vars[v]));
            Cudd_Ref(bCube);
            Cudd_RecursiveDeref(dd, bTemp);
        } else if (s_Temp[v] == 1) {
            //          Cube &=  s_XVars[v];
            bCube = Cudd_bddAnd(dd, bTemp = bCube, dd->vars[v]);
            Cudd_Ref(bCube);
            Cudd_RecursiveDeref(dd, bTemp);
        }
    Cudd_Deref(bCube);
...
DdNode* Extra_bddGetOneCube(DdManager* dd, DdNode* bFunc) {
    ...
    bRes0 = Extra_bddGetOneCube(dd, bFunc0);
    Cudd_Ref(bRes0);

    if (bRes0 != b0) {
        bRes = Cudd_bddAnd(dd, bRes0, Cudd_Not(dd->vars[bFuncR->index]));
        Cudd_Ref(bRes);
        Cudd_RecursiveDeref(dd, bRes0);
    } else {
        Cudd_RecursiveDeref(dd, bRes0);
        // try to find the cube with the positive literal
        bRes1 = Extra_bddGetOneCube(dd, bFunc1);
        Cudd_Ref(bRes1);
        assert(bRes1 != b0);
        bRes = Cudd_bddAnd(dd, bRes1, dd->vars[bFuncR->index]);
        Cudd_Ref(bRes);
...
DdNode* Extra_bddComputeRangeCube(DdManager* dd, int iStart, int iStop) {
    ...
    for (i = iStart; i < iStop; i++) {
        bProd = Cudd_bddAnd(dd, bTemp = bProd, dd->vars[i]);
        Cudd_Ref(bProd);
        Cudd_RecursiveDeref(dd, bTemp);
    }
    Cudd_Deref(bProd);
...
DdNode* Extra_bddBitsToCube(DdManager* dd, int Code, int CodeWidth, DdNode** pbVars, int fMsbFirst) {
    ...
    for (z = 0; z < CodeWidth; z++) {
        ...
        bResult = Cudd_bddAnd(dd, bTemp = bResult, bVar);
        Cudd_Ref(bResult);
        Cudd_RecursiveDeref(dd, bTemp);
    }
    Cudd_Deref(bResult);
...
DdNode* Extra_bddCreateAnd(DdManager* dd, int nVars) {
    DdNode *bFunc, *bTemp;
    int i;
    bFunc = Cudd_ReadOne(dd);
    Cudd_Ref(bFunc);
    for (i = 0; i < nVars; i++) {
        bFunc = Cudd_bddAnd(dd, bTemp = bFunc, Cudd_bddIthVar(dd, i));
        Cudd_Ref(bFunc);
        Cudd_RecursiveDeref(dd, bTemp);
    }
    Cudd_Deref(bFunc);
...
DdNode* Extra_bddCreateOr(DdManager* dd, int nVars) {
    DdNode *bFunc, *bTemp;
    int i;
    bFunc = Cudd_ReadLogicZero(dd);
    Cudd_Ref(bFunc);
    for (i = 0; i < nVars; i++) {
        bFunc = Cudd_bddOr(dd, bTemp = bFunc, Cudd_bddIthVar(dd, i));
        Cudd_Ref(bFunc);
        Cudd_RecursiveDeref(dd, bTemp);
    }
    Cudd_Deref(bFunc);
...
DdNode* Extra_bddCreateExor(DdManager* dd, int nVars) {
    ...
    Cudd_Ref(bFunc);
    for (i = 0; i < nVars; i++) {
        bFunc = Cudd_bddXor(dd, bTemp = bFunc, Cudd_bddIthVar(dd, i));
        Cudd_Ref(bFunc);
        Cudd_RecursiveDeref(dd, bTemp);
    }
    Cudd_Deref(bFunc);
...
{
    if ((zC->index & 1) == 0) { /* the top variable is present in positive polarity and maybe in negative */

        DdNode* Temp = cuddE(zC);
        *zC1 = cuddT(zC);
        if (cuddIZ(dd, Temp->index) == cuddIZ(dd, zC->index) + 1) { /* Temp is not a terminal node 
             * top var is present in negative polarity */
            *zC2 = cuddE(Temp);
            *zC0 = cuddT(Temp);
        } else { /* top var is not present in negative polarity */
            *zC2 = Temp;
            *zC0 = dd->zero;
        }
    } else { /* the top variable is present only in negative */
        *zC1 = dd->zero;
        *zC2 = cuddE(zC);
        *zC0 = cuddT(zC);
    }
...
int Extra_bddCountCubes(DdManager* dd, DdNode** pFuncs, int nFuncs, int fMode, int nLimit, int* pGuide) {
    ...
    for (i = 0; i < nFuncs; i++) {
        ...
        if (!pF1 && !pF0)
            break;
        if (!pF0)
            pGuide[i] = 1, Count = Count1; // use pos
        else if (!pF1)
            pGuide[i] = 0, Count = Count0; // use neg
...
DdNode* extraBddChangePolarity(
    ...
    if ((bRes = cuddCacheLookup2(dd, extraBddChangePolarity, bFunc, bVars)))
        ...
        else // if ( LevelF <= LevelV )
        {
            ...
            if (LevelF == LevelV) { // swap the cofactors
                DdNode* bTemp;
                bTemp = bRes0;
                bRes0 = bRes1;
                bRes1 = bTemp;
...
void Extra_TestAndPerm(DdManager* ddF, DdNode* bF, DdNode* bG) {
    ...
    bG2 = Extra_TransferLevelByLevel(ddF, ddG, bG);
    Cudd_Ref(bG2);
    ...
    Counter = 0;
    clk = Abc_Clock();
    bRes2 = Extra_bddAndPermute(ddF, bF, ddG, bG2, NULL);
    Cudd_Ref(bRes2);
...
void Extra_GraphExperiment() {
    ...
    for (e = 0; Edges[e][0]; e++) {
        ...
        Cudd_Ref(zEdge);
        for (n = 0; Edges[e][n]; n++) {
            zVar = cuddZddGetNode(dd, Edges[e][n], DD_ONE(dd), DD_ZERO(dd));
            Cudd_Ref(zVar);
            zEdge = Cudd_zddUnateProduct(dd, zTemp = zEdge, zVar);
            Cudd_Ref(zEdge);
            Cudd_RecursiveDerefZdd(dd, zTemp);
            Cudd_RecursiveDerefZdd(dd, zVar);
        }
        zGraph = Cudd_zddUnion(dd, zTemp = zGraph, zEdge);
        Cudd_Ref(zGraph);
        Cudd_RecursiveDerefZdd(dd, zTemp);
    ...
    double d)      /* average density of elements in combinations */
{
    ...
    for (c = 0; c < k; c++) {
        ...
        if (c) { /* at least one combination is already included */
            ...
            if (Aux != Result) {
                ...
                c--;
                continue;
            } else { /* Aux is the same node as Result */
                Cudd_Deref(Aux);
            }
        }
...
void Extra_ZddTest() {
    ...
    for (i = 1; i <= 10; i++) {
        int K = K0 * i;
        DdNode* zRandSet = Extra_zddRandomSet(dd, N, K, 0.5);
        Cudd_Ref(zRandSet);
