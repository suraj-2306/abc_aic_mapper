    ...
    int nVars)        // nVars is the column multiplicity, [log2(nFuncs)]
{
    ...
    Cudd_Ref(bResult);
    for (i = 0; i < nFuncs; i++) {
        bCube = Extra_bddBitsToCube(dd, i, nVars, pbVars, 1);
        Cudd_Ref(bCube);
        bProd = Cudd_bddAnd(dd, bCube, pbFuncs[i]);
        Cudd_Ref(bProd);
    ...
    int* pSimple)       // pSimple gets the number of code variables taken from the input varibles without change
{
    ...
    bEncoded = Extra_bddEncodingBinary(dd, pbColumns, nColumns, pCVars, nMulti);
    Cudd_Ref(bEncoded);
    ...
    //  clk = Abc_Clock();
    bResult = CreateTheCodes_rec(dd, bEncoded, 0, pCVars);
    Cudd_Ref(bResult);
...
st__table* Extra_bddNodePathsUnderCut(DdManager* dd, DdNode* bFunc, int CutLevel) {
    ...
    // go through the table CutNodes and create the BDD and the path to be returned
    {
        ...
        st__foreach_item(CutNodes, gen, (const char**)&aNode, (char**)&bSum) {
            ...
            bNode = Cudd_addBddPattern(dd, aNode);
            Cudd_Ref(bNode);
...
void extraProfileUpdateTopLevel(st__table* tNodeTopRef, int TopLevelNew, DdNode* node) {
    ...
    if (st__find_or_add(tNodeTopRef, (char*)node, (char***)&pTopLevel)) { // the node is already referenced
...
{
    DdNode* bRes;
    if (Level == s_nVarsBest) { // the terminal case, when we need to remap the encoded function
...
*/
        ...
        // put the entries from the table into the temporary array
        {
            ...
            st__foreach_item(CutNodes, gen, (const char**)&bCode, (char**)&bColumn) {
                if (bCode == b0) { // the unused part of the columns
        ...
        if (s_MultiStart - Level == 0) // we reached the bottom level of recursion
        {
            ...
            bRes = Extra_bddEncodingBinary(dd, s_pbTemp, nCols, pCVars + Level, s_MultiStart - Level);
            Cudd_Ref(bRes);
        }
        ...
        bCof0 = Cudd_Cofactor(dd, bEncoded, Cudd_Not(bVarNext));
        Cudd_Ref(bCof0);
        bCof1 = Cudd_Cofactor(dd, bEncoded, bVarNext);
        Cudd_Ref(bCof1);

        // call recursively
        bRes0 = CreateTheCodes_rec(dd, bCof0, Level + 1, pCVars);
        Cudd_Ref(bRes0);
        bRes1 = CreateTheCodes_rec(dd, bCof1, Level + 1, pCVars);
        Cudd_Ref(bRes1);
        ...
        bProd0 = Cudd_bddAnd(dd, Cudd_Not(bVarNext), Cudd_Not(pCVars[Level]));
        Cudd_Ref(bProd0);
        bProd1 = Cudd_bddAnd(dd, bVarNext, pCVars[Level]);
        Cudd_Ref(bProd1);

        bProd0 = Cudd_bddAnd(dd, bTemp = bProd0, bRes0);
        Cudd_Ref(bProd0);
        Cudd_RecursiveDeref(dd, bTemp);
        Cudd_RecursiveDeref(dd, bRes0);

        bProd1 = Cudd_bddAnd(dd, bTemp = bProd1, bRes1);
        Cudd_Ref(bProd1);
        Cudd_RecursiveDeref(dd, bTemp);
        Cudd_RecursiveDeref(dd, bRes1);

        bRes = Cudd_bddOr(dd, bProd0, bProd1);
        Cudd_Ref(bRes);
...
// if we successfully finish this procedure, Level also stands for how many encoding variabled we saved
{
    ...
    for (bTempV = bVarsCol; bTempV != b1; bTempV = cuddT(bTempV)) {
        ...
        if (!fBreak) {
            ...
            if (nMulti - 1 > 0) {
                bVarsRem = Cudd_bddExistAbstract(dd, bVarsCol, bVarTop);
                Cudd_Ref(bVarsRem);
...
// returns the cost and the new set of variables (bVars & bVarTop)
{
    ...
    bVarsRes = Cudd_bddAnd(dd, bVars, bVarTop);
    Cudd_Ref(bVarsRes);
...
DdNode* ComputeVarSetAndCountMinterms2(DdManager* dd, DdNode* bVars, DdNode* bVarTop, unsigned* Cost) {
    ...
    bCof = Cudd_Cofactor(dd, s_Encoded, bVarsRes);
    Cudd_Ref(bCof);
    bFun = Cudd_bddExistAbstract(dd, bCof, s_VarAll);
    Cudd_Ref(bFun);
...
unsigned Extra_CountMintermsSimple(DdNode* bFunc, unsigned max) {
    ...
    if (HHTable2[HKey].Arg1 == bFunc && HHTable2[HKey].Arg2 == max)
        ...
        // min = min0/2 + min1/2;
        unsigned min = (Extra_CountMintermsSimple(cuddE(bFunc), max) >> 1) + (Extra_CountMintermsSimple(cuddT(bFunc), max) >> 1);
...

{
    traventry* p;
    char** slot;
    if (st__find_or_add(Visited, (char*)aFunc, &slot)) { // the entry already exists
        p = (traventry*)*slot;
...
void CollectNodesAndComputePaths_rec(DdManager* dd, DdNode* aFunc, DdNode* bCube, st__table* Visited, st__table* CutNodes) {
    ...
    traventry* p;
    char** slot;
    if (st__find_or_add(Visited, (char*)aFunc, &slot)) { // the node is found
        ...
        p->bSum = Cudd_bddOr(dd, bTemp = p->bSum, bCube);
        Cudd_Ref(p->bSum);
        Cudd_RecursiveDeref(dd, bTemp);
        ...
        {    // this is the last visit - propagate the cube
            ...
            if (cuddI(dd, aFunc->index) < s_CutLevel) { // the node is above the cut
                ...
                bCube0 = Cudd_bddAnd(dd, p->bSum, Cudd_Not(bVarTop));
                Cudd_Ref(bCube0);
                bCube1 = Cudd_bddAnd(dd, p->bSum, bVarTop);
                Cudd_Ref(bCube1);
                ...
                return;
            } else { // the node is below the cut
                // add this node to the cut node table, if it is not yet there

                //              DdNode * bNode;
                //              bNode = Cudd_addBddPattern( dd, aFunc );  Cudd_Ref( bNode );
                if (st__find_or_add(CutNodes, (char*)aFunc, &slot)) { // the node exists - should never happen
                    assert(0);
                }
                *slot = (char*)p->bSum;
                Cudd_Ref(p->bSum);
