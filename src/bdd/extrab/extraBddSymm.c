    ...
    DdNode* bFunc) /* the function whose symmetries are computed */
{
    ...
    Cudd_Ref(bSupp);
    zRes = Extra_zddSymmPairsCompute(dd, bFunc, bSupp);
    Cudd_Ref(zRes);
...
Extra_SymmInfo_t* Extra_SymmPairsCreateFromZdd(DdManager* dd, DdNode* zPairs, DdNode* bSupp) {
    ...
    while (zSet != z0) {
        // get the next cube
        zCube = Extra_zddSelectOneSubset(dd, zSet);
        Cudd_Ref(zCube);
        ...
        zSet = Cudd_zddDiff(dd, zTemp = zSet, zCube);
        Cudd_Ref(zSet);
...
int Extra_bddCheckVarsSymmetric(
    ...
    bVars = Cudd_bddAnd(dd, dd->vars[iVar1], dd->vars[iVar2]);
    Cudd_Ref(bVars);
...
int Extra_bddCheckVarsSymmetricNaive(
    ...
    bCube1 = Cudd_bddAnd(dd, Cudd_Not(dd->vars[iVar1]), dd->vars[iVar2]);
    Cudd_Ref(bCube1);
    bCube2 = Cudd_bddAnd(dd, Cudd_Not(dd->vars[iVar2]), dd->vars[iVar1]);
    Cudd_Ref(bCube2);

    bCof01 = Cudd_Cofactor(dd, bF, bCube1);
    Cudd_Ref(bCof01);
    bCof10 = Cudd_Cofactor(dd, bF, bCube2);
    Cudd_Ref(bCof10);
