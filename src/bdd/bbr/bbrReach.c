...
extern Abc_Cex_t* Aig_ManVerifyUsingBddsCountExample(Aig_Man_t* p, DdManager* dd, DdNode** pbParts, Vec_Ptr_t* vOnionRings, DdNode* bCubeFirst, int iOutput, int fVerbose, int fSilent);
...
DdNode* Aig_ManInitStateVarMap(DdManager* dd, Aig_Man_t* p, int fVerbose) {
    ...
    Saig_ManForEachLo(p, pLatch, i) {
        ...
        bProd = Cudd_bddAnd(dd, bTemp = bProd, Cudd_Not(pbVarsX[i]));
        Cudd_Ref(bProd);
        Cudd_RecursiveDeref(dd, bTemp);
    }
...
DdNode** Aig_ManCreateOutputs(DdManager* dd, Aig_Man_t* p) {
    ...
    Saig_ManForEachPo(p, pNode, i) {
        pbOutputs[i] = Aig_ObjGlobalBdd(pNode);
        Cudd_Ref(pbOutputs[i]);
    }
    return pbOutputs;
}
...
DdNode** Aig_ManCreatePartitions(DdManager* dd, Aig_Man_t* p, int fReorder, int fVerbose) {
    ...
    Saig_ManForEachLi(p, pNode, i) {
        ...
        pbParts[i] = Cudd_bddXnor(dd, bVar, Aig_ObjGlobalBdd(pNode));
        Cudd_Ref(pbParts[i]);
    }
    // free global BDDs
...
int Aig_ManComputeReachable(DdManager* dd, Aig_Man_t* p, DdNode** pbParts, DdNode* bInitial, DdNode** pbOutputs, Saig_ParBbr_t* pPars, int fCheckOutputs) {
    ...
    bCubeCs = Bbr_bddComputeRangeCube(dd, Saig_ManPiNum(p), Saig_ManCiNum(p));
    Cudd_Ref(bCubeCs);
    if (pPars->fPartition)
    ...
    bReached = bInitial;
    Cudd_Ref(bReached);
    Vec_PtrPush(vOnionRings, bCurrent);
    Cudd_Ref(bCurrent);
    for (nIters = 0; nIters < pPars->nIterMax; nIters++) {
        ...
        bNext = Cudd_bddVarMap(dd, bTemp = bNext);
        Cudd_Ref(bNext);
        Cudd_RecursiveDeref(dd, bTemp);
        ...
        for (i = 0; i < Saig_ManPoNum(p); i++) {
            if (fCheckOutputs && !Cudd_bddLeq(dd, bNext, Cudd_Not(pbOutputs[i]))) {
                DdNode* bIntersect;
                bIntersect = Cudd_bddIntersect(dd, bNext, pbOutputs[i]);
                Cudd_Ref(bIntersect);
        ...
        if (i < Saig_ManPoNum(p))
            break;
        // get the new states
        bCurrent = Cudd_bddAnd(dd, bNext, Cudd_Not(bReached));
        Cudd_Ref(bCurrent);
        Vec_PtrPush(vOnionRings, bCurrent);
        Cudd_Ref(bCurrent);
        ...
        bReached = Cudd_bddOr(dd, bTemp = bReached, bNext);
        Cudd_Ref(bReached);
        Cudd_RecursiveDeref(dd, bTemp);
...
int Aig_ManVerifyUsingBdds_int(Aig_Man_t* p, Saig_ParBbr_t* pPars) {
    ...
    bInitial = Aig_ManInitStateVarMap(dd, p, pPars->fVerbose);
    Cudd_Ref(bInitial);
    ...
    for (i = 0; i < Saig_ManPoNum(p); i++) {
        if (fCheckOutputs && !Cudd_bddLeq(dd, bInitial, Cudd_Not(pbOutputs[i]))) {
            DdNode* bIntersect;
            bIntersect = Cudd_bddIntersect(dd, bInitial, pbOutputs[i]);
            Cudd_Ref(bIntersect);
            assert(p->pSeqModel == NULL);
            p->pSeqModel = Aig_ManVerifyUsingBddsCountExample(p, dd, pbParts,
                                                              vOnionRings, bIntersect, i, pPars->fVerbose, pPars->fSilent);
...
int Aig_ManVerifyUsingBdds(Aig_Man_t* pInit, Saig_ParBbr_t* pPars) {
    ...
    Saig_ManForEachPi(pInit, pObj, i) if (Aig_ObjRefs(pObj) == 0) break;
    ...
    Saig_ManForEachPi(pInit, pObj, i) if (pObj->pData != NULL)
        Vec_IntPush(vInputMap, Aig_ObjCioId((Aig_Obj_t*)pObj->pData));
    else Vec_IntPush(vInputMap, -1);
    // create new pattern
