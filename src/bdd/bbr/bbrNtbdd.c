...
static inline void Aig_ObjCleanGlobalBdd(DdManager* dd, Aig_Obj_t* pObj) {
...
void Aig_ManFreeGlobalBdds(Aig_Man_t* p, DdManager* dd) {
    Aig_Obj_t* pObj;
    int i;
    Aig_ManForEachObj(p, pObj, i) if (Aig_ObjGlobalBdd(pObj))
...
DdManager* Aig_ManComputeGlobalBdds(Aig_Man_t* p, int nBddSizeMax, int fDropInternal, int fReorder, int fVerbose) {
    ...
    Aig_ObjSetGlobalBdd(Aig_ManConst1(p), dd->one);
    Cudd_Ref(dd->one);
    // set the elementary variables
    Aig_ManForEachCi(p, pObj, i) {
        Aig_ObjSetGlobalBdd(pObj, dd->vars[i]);
        Cudd_Ref(dd->vars[i]);
    }
    ...
    Aig_ManForEachCo(p, pObj, i) {
        ...
        bFunc = Cudd_NotCond(bFunc, Aig_ObjFaninC0(pObj));
        Cudd_Ref(bFunc);
