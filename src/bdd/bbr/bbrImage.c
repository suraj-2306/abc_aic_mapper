...
Bbr_ImageTree_t* Bbr_bddImageStart(
    DdManager* dd,
    DdNode* bCare, // the care set
    int nParts,
    DdNode** pbParts, // the partitions for image computation
    int nVars,
    DdNode** pbVars,
    int nBddMax,
    int fVerbose) // the NS and parameter variables (not quantified!)
{
    ...
    pTree->bCareSupp = Cudd_Support(dd, bCare);
    Cudd_Ref(pTree->bCareSupp);
...
DdNode* Bbr_bddImageCompute(Bbr_ImageTree_t* pTree, DdNode* bCare) {
    ...
    bSupp = Cudd_Support(dd, bCare);
    Cudd_Ref(bSupp);
    if (bSupp != pTree->bCareSupp) {
        bRem = Cudd_bddExistAbstract(dd, bSupp, pTree->bCareSupp);
        Cudd_Ref(bRem);
        if (bRem != b1) {
...
Bbr_ImagePart_t** Bbr_CreateParts(DdManager* dd,
    ...
    for (i = 0; i < nParts; i++) {
        ...
        pParts[i]->bFunc = pbParts[i];
        Cudd_Ref(pParts[i]->bFunc);
        pParts[i]->bSupp = Cudd_Support(dd, pParts[i]->bFunc);
    ...
    pParts[nParts]->bFunc = bCare;
    Cudd_Ref(pParts[nParts]->bFunc);
    pParts[nParts]->bSupp = Cudd_Support(dd, pParts[nParts]->bFunc);
    Cudd_Ref(pParts[nParts]->bSupp);
...
Bbr_ImageVar_t** Bbr_CreateVars(DdManager* dd,
    ...
    bSupp = Cudd_VectorSupport(dd, pbFuncs, nParts);
    Cudd_Ref(bSupp);
    ABC_FREE(pbFuncs);

    // remove the NS vars
    bCubeNs = Cudd_bddComputeCube(dd, pbVars, NULL, nVars);
    Cudd_Ref(bCubeNs);
    bSupp = Cudd_bddExistAbstract(dd, bTemp = bSupp, bCubeNs);
    Cudd_Ref(bSupp);
    ...
    for (bSuppTemp = bSupp; bSuppTemp != b1; bSuppTemp = cuddT(bSuppTemp)) {
        ...
        for (p = 0; p < nParts; p++)
            if (Cudd_bddLeq(dd, pParts[p]->bSupp, dd->vars[bSuppTemp->index])) {
                bParts = Cudd_bddAnd(dd, bTemp = bParts, dd->vars[p]);
                Cudd_Ref(bParts);
                Cudd_RecursiveDeref(dd, bTemp);
                Counter++;
            }
...
*/
    ...
    for (bSuppTemp = pNode2->pPart->bSupp; bSuppTemp != b1; bSuppTemp = cuddT(bSuppTemp)) {
        ...
        pVar->bParts = Cudd_bddExistAbstract(dd, bTemp = pVar->bParts, dd->vars[iNode2]);
        Cudd_Ref(pVar->bParts);
        Cudd_RecursiveDeref(dd, bTemp);
        // add the new var
        pVar->bParts = Cudd_bddAnd(dd, bTemp = pVar->bParts, dd->vars[iNode1]);
        Cudd_Ref(pVar->bParts);
        Cudd_RecursiveDeref(dd, bTemp);
        // update the score
...
***********************************************************************/
Bbr_ImageNode_t* Bbr_CombineTwoNodes(DdManager* dd, DdNode* bCube, Bbr_ImageNode_t* pNode1, Bbr_ImageNode_t* pNode2) {
    Bbr_ImageNode_t* pNode;
    Bbr_ImagePart_t* pPart;
    ...
    for (v = 0; v < nVars; v++)
        if (pVars[v] && pVars[v]->nParts == 2) {
            ...
            for (bTemp = pVars[v]->bParts; bTemp != b1; bTemp = cuddT(bTemp))
                CostCur += pNodes[bTemp->index]->pPart->nNodes * pNodes[bTemp->index]->pPart->nNodes;
            if (CostBest > CostCur) {
                CostBest = CostCur;
                iVarBest = v;
            }
...
***********************************************************************/
void Bbr_FindBestPartitions(DdManager* dd, DdNode* bParts, int nNodes, Bbr_ImageNode_t** pNodes, int* piNode1, int* piNode2) {
    DdNode* bTemp;
    int iPart1, iPart2;
...
void Bbr_bddImagePrintLatchDependency(
    ...
    DdNode** pbParts, // the partitions for image computation
    int nVars,
    DdNode** pbVars) // the NS and parameter variables (not quantified!)
{
    int i;
    DdNode *bVarsCs, *bVarsNs;

    bVarsCs = Cudd_Support(dd, bCare);
    Cudd_Ref(bVarsCs);
    bVarsNs = Cudd_bddComputeCube(dd, pbVars, NULL, nVars);
    Cudd_Ref(bVarsNs);
...
void Bbr_bddImagePrintLatchDependencyOne(
    DdManager* dd,
    DdNode* bFunc, // the function
    DdNode* bVarsCs,
    DdNode* bVarsNs, // the current/next state vars
    int iPart) {
    DdNode* bSupport;
    int v;
    bSupport = Cudd_Support(dd, bFunc);
    Cudd_Ref(bSupport);
    printf(" %3d : ", iPart);
    for (v = 0; v < dd->size; v++) {
...
DdNode* Bbr_bddComputeCube(DdManager* dd, DdNode** bXVars, int nVars) {
    ...
    Cudd_Ref(bRes);
    for (i = 0; i < nVars; i++) {
        bRes = Cudd_bddAnd(dd, bTemp = bRes, bXVars[i]);
        Cudd_Ref(bRes);
        Cudd_RecursiveDeref(dd, bTemp);
    }
...
Bbr_ImageTree2_t* Bbr_bddImageStart2(
    ...
    bCubeAll = Bbr_bddComputeCube(dd, dd->vars, dd->size);
    Cudd_Ref(bCubeAll);
    bCubeNot = Bbr_bddComputeCube(dd, pbVars, nVars);
    Cudd_Ref(bCubeNot);
    pTree->bCube = Cudd_bddExistAbstract(dd, bCubeAll, bCubeNot);
    Cudd_Ref(pTree->bCube);
    ...
    Cudd_Ref(pTree->bRel);
    for (i = 0; i < nParts; i++) {
        pTree->bRel = Cudd_bddAnd(dd, bTemp = pTree->bRel, pbParts[i]);
        Cudd_Ref(pTree->bRel);
        Cudd_RecursiveDeref(dd, bTemp);
    }
